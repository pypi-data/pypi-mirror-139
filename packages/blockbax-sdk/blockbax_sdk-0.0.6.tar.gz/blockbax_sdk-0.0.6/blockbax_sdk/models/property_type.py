...
class PropertyType:
    ...
    def __post_init__(self):
        if self.created_date:
            self.created_date = convertions.convert_any_date_to_datetime(self.created_date)
        if self.updated_date:
            self.updated_date = convertions.convert_any_date_to_datetime(self.updated_date)
        
        if self.predefined_values:
            self.values = [
                property_value.from_dict({types.PropertyDataTypes(self.data_type).value: value}) 
    ...
    def from_api_response(cls, api_response: dict):
        ...
        for property_value_response in api_response.get("values"):
            new_property_value = property_value.from_dict(property_value_response, skip_unexpected = True)
