...
def new(
    ...
    **kwargs,
    ...
    values: dict = {"number": number, "location": location,"text": text, "map_layer": map_layer, "image": image, "area": area}

    if not validation.list_contains_single_value(values.values()) and not inherit:
        to_many_arguments_error = f"PropertyValue takes exactly a single data type, values given: {values}"
        raise errors.ValidationError(to_many_arguments_error)

    if number is not None:
        return NumberPropertyValue(id,caption,inherit,validation.check_number_and_convert_to_decimal(number))
    elif location is not None:
        return LocationPropertyValue(id,caption,inherit,validation.check_location_and_convert(location))
    elif text is not None:
        return TextPropertyValue(id,caption,inherit,validation.check_text(text))
    elif map_layer is not None:
        return MapLayerPropertyValue(id,caption,inherit,validation.check_map_layer_and_convert(map_layer))
    elif image is not None:
        return ImagePropertyValue(id,caption,inherit,validation.check_image(image))
    elif area is not None:
        return AreaPropertyValue(id,caption,inherit,validation.check_area_and_convert(area))
    else:
