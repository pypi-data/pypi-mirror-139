...
def checkInputs(problem_labels, weights, fuzzyNumbers):
    """
    ...
    :param weights: the weights applied for the specific problem
    :param fuzzyNumbers: a set of fuzzy numbers (4 points each)
    """
    # Checks the correspondence of the weights and the labels (must have the same length)
    if (len(problem_labels) != len(weights)): raise WeightsError(
        "The input label vector must have the same number of items that the weights' vector")
    # Checks if the boundaries coincide
    i = 0
    while i < 3:
        if (fuzzyNumbers[i][2] != fuzzyNumbers[i + 1][0]): raise FuzzyNumberError(
            "The bounds of the fuzzy set must coincide")
        if (fuzzyNumbers[i][3] != fuzzyNumbers[i + 1][1]): raise FuzzyNumberError(
            "The bounds of the fuzzy set must coincide")
        # Check whether the previous values are smaller than the next values
        for j in range(3):
            if (fuzzyNumbers[i][j] > fuzzyNumbers[i][j + 1]): raise FuzzyNumberError(
                "The previous point must be smaller than the next")
        i = i + 1


def ulowaOperation(problem_labels, weights, fuzzyNumbers, labels):
    """
    ...
    :param weights: the weights applied for the specific problem
    :param fuzzyNumbers: a set of fuzzy numbers (4 points each)
    :param labels: an array of the different possible labels
    :return: the result label
    """
    # First we check if the inputs given are well-format inputs
    checkInputs(problem_labels, weights, fuzzyNumbers)
    # Sort the labels
    problem_labels = sort(labels, problem_labels)
    # Calculate a and b values
    a = fuzzyNumbers[0][0]
    b = fuzzyNumbers[len(fuzzyNumbers) - 1][3]
    # Start the ULOWA calculation
    if (len(problem_labels) >= 2):
        weight = recalculateWeights(weights, len(problem_labels))
        delta = calculateDelta(problem_labels, weight, fuzzyNumbers, labels)
        deltaList = (delta, delta, delta, delta)
        res = calculateSimilarities(problem_labels, deltaList, labels, fuzzyNumbers, a, b)
        ...
        aux = weights[len(weights) - 1]
        weights.pop(len(weights) - 1)
        weights[len(weights) - 1] = weights[len(weights) - 1] + aux
        ulowaOperation(problem_labels, weights, fuzzyNumbers, labels)
    res = problem_labels[0]
    return res


def calculateSimilarities(problem_labels, deltaList, labels, fuzzyNumbers, a, b):
    """
    ...
    consecutive_var = consecutive(tag1, tag2, labels)
    similarities = []
    # If tags are consecutive only 2 similarities must be calculated
    if (consecutive_var == 1):
        sim1 = [similarity(fuzzyNumbers[labels.index(tag1)], deltaList, a, b), tag1]
        sim2 = [similarity(fuzzyNumbers[labels.index(tag2)], deltaList, a, b), tag2]
        similarities.append(sim1)
        similarities.append(sim2)
    # If tags aren't consecutive, more similarities must be calculated
    else:
        index = labels.index(tag1)
        while (tag1 != tag2):
            aux = [similarity(fuzzyNumbers[labels.index(tag1)], deltaList, a, b), tag1]
...
def consecutive(tag1, tag2, labels):
    """
    ...
    :param labels: reference label vector
    :return: the number of positions of their distance
    """
    return (abs(labels.index(tag1) - labels.index(tag2)))


def recalculateWeights(weights, length):
    """
    ...
    """
    denominator = (weights[length - 2] + weights[length - 1])
    if denominator == 0.0:
        return 0.0
    else:
        return (weights[length - 2] / denominator)


def calculateDelta(problem_labels, weight, fuzzyNumbers, labels):
    """
    ...
    """
    xsi = getCOG(fuzzyNumbers[labels.index(problem_labels[len(problem_labels) - 1])])
    xsj = getCOG(fuzzyNumbers[labels.index(problem_labels[len(problem_labels) - 2])])
...
def getCOG(fuzzyN):
    ...
    :param fuzzyN: a fuzzy set given with 4 points, F = (a1, a2, a3, a4)
...
def getCOM(fuzzyN):
    ...
    :param fuzzyN: the fuzzy set we want to calculate the COM from
    :return: its COM value (Center of Maximum)
    """
    return (fuzzyN[1] + fuzzyN[2]) / 2


def getOrdinal(scale, tag):
    """
