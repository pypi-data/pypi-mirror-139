#coding: utf-8
from flask import request, url_for, session, make_response
...
from esentity.models import AuthModel, SignupModel, SignupModelInvite, UserModel, ManagerModel, CasinoModel, CasinoProxyModel, DomainModel, CampaignModel, ActivityFeedbackModel, ActivitySubscribeModel, ActivityTicketModel , TelegramBotModel, TelegramChatModel, TelegramMessageModel
...
def process_casino(item, actors={}, check_commit=False):
    _doc = item['_source']
    _doc['id'] = item['_id']
    _doc['full_path'] = f"{current_app.config['PREFERRED_URL_SCHEME']}://{current_app.config['DOMAIN']}{get_page_url(_doc)}"
...
class ApiResourceSearch(Resource):
    ...
    def post(self):
        ...
        objs, total = Page.get(path=data['key'], locale=data.get('locale', current_app.config['BABEL_DEFAULT_LOCALE']))
        if total:
            obj = objs.pop()
                        ...
                        if entity.get('is_send_notify') and obj.owner and obj.category == 'provider' and obj.is_active and not obj.is_draft and obj.status == 'published':
                            ...
                            if found:
                                ...
                                if actor.actor_is_active:
                                    url = f"{current_app.config['PREFERRED_URL_SCHEME']}://{current_app.config['DOMAIN']}{get_page_url(obj.to_dict())}"
...
class ApiContentFeedback(Resource):
    ...
    def post(self):
        ...
        def process_approved(_doc):
            ...
            _doc['country'] = _doc['country'].upper()
            return _doc
        ...
        for item in casinos:
            if len(item['_source']['comments']):
                _d2 = {
                    ...
                    'full_path': f"{current_app.config['PREFERRED_URL_SCHEME']}://{current_app.config['DOMAIN']}{get_page_url(item['_source'])}"
                }
        ...
        if 'key' in data:
            ...
            if found == 1:
                ...
                if page.is_active and page.is_searchable and not page.is_redirect:
                    ...
                    url = f"{current_app.config['PREFERRED_URL_SCHEME']}://{current_app.config['DOMAIN']}{get_page_url(page.to_dict())}"
    ...
    def post(self):
        ...
        try:
            ...
            if core == 'Ticket':
                ...
                _res = 'Your review has been sent'
                _id = Activity.generate_id(*_doc.values())
