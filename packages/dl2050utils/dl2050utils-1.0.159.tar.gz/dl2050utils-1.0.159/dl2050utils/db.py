...
class DB():
    def __init__(self, cfg=None, log=None, dbname=None):
        ...
        self.cfg, self.LOG = cfg, log or base_log()
    ...
    async def startup(self, min_size=5, max_size=20, loop=None):
        try:
            ...
            return True
        self.LOG.log(2, 0, label='DBPG', label2='startup', msg=f'CONNECTED POOL to {self.dbname}')
        return False
    ...
    async def execute(self, q):
        ...
        try:
            ...
            self.LOG.log(4, 0, label='DBPG', label2='execute', msg={'error_msg': str(err), 'query': q})
            return None
        finally:
        ...
        try:
            if(one):
                ...
                if nrows:
                    ...
                    if nr is None:
                        self.LOG.log(4, 0, label='DB', label2='calc_query_rows', msg=q)
                        await self.pool.release(con)
                        return None
    ...
    def sync_startup(self, *args, **kwargs): return asyncio.get_running_loop().run_until_complete(self.startup(*args, **kwargs))
    def sync_execute(self, q): return asyncio.get_running_loop().run_until_complete(self.execute(q))
    def sync_query(self, *args, **kwargs): return asyncio.get_running_loop().run_until_complete(self.query(*args, **kwargs))
    def sync_select(self, *args, **kwargs): return asyncio.get_running_loop().run_until_complete(self.select(*args, **kwargs))
    def sync_insert(self, *args, **kwargs): return asyncio.get_running_loop().run_until_complete(self.insert(*args, **kwargs))
    def sync_delete(self, *args, **kwargs): return asyncio.get_running_loop().run_until_complete(self.delete(*args, **kwargs))
    def sync_delete_all(self, *args, **kwargs): return asyncio.get_running_loop().run_until_complete(self.delete_all(*args, **kwargs))
    def sync_update(self,  *args, **kwargs): return asyncio.get_running_loop().run_until_complete(self.update(*args, **kwargs))
