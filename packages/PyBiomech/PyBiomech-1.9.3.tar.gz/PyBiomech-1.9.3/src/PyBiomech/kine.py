...
def pelvisBellPose(mkrs):
    ...
    Z = getVersor(RASI - LASI)
    Y = getVersor(np.cross(Z, RASI - 0.5 * (LPSI + RPSI)))
    X = getVersor(np.cross(Y, Z))
...
def createBellHJCsToMarkers(mkrs):
    R, T = pelvisBellPose(mkrs)
    RT = composeRotoTranslMatrix(R, T)
    RTi = inv2(RT)
    mkrsPelvis = {m: mkrs[m] for m in ['RASI','LASI','RPSI','LPSI']}
    mkrsPelvisLoc = changeMarkersReferenceFrame(mkrsPelvis, RTi)
    interASIdist = np.linalg.norm(mkrsPelvisLoc['LASI'] - mkrsPelvisLoc['RASI'], axis=1)
    D = interASIdist[:,np.newaxis]
    LHJC = np.hstack((-0.19*D, -0.30*D, -0.86*D))
    RHJC = np.hstack((-0.19*D, -0.30*D, -0.14*D))
    mkrsHJCsLoc = {}
    mkrsHJCsLoc['LHJC_bell'] = LHJC
    mkrsHJCsLoc['RHJC_bell'] = RHJC
    mkrsHJCs = changeMarkersReferenceFrame(mkrsHJCsLoc, RT)
    return mkrsHJCs
...
def getJointAngles(R1, R2, R2anglesFun=R2zxy, funInput='jointR', **kwargs):
    ...
    R2anglesFun : func, str
        Function converting from joint rotation matrix, or segment matrices, to angles (see ``R2zxy()``, ``ges()``).
        When a string, it is an axis sequence (eg. 'xyz') for conversion to Euler angles.
    ...
    **kwargs
    ...
    elif funInput == 'jointRMatrix':
        Rj = dot3(inv2(R1), R2)
        for i in range(N):
            a1, q2, a3 = rot.R_to_euler(Rj[i,:,:], R2anglesFun)
            angles[i,0] = a1
            angles[i,1] = a2
            angles[i,2] = a3
