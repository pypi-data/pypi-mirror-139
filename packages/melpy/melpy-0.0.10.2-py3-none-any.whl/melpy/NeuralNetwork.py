...
class Sequential():
    ...
    def fit(self, cost_fn, epochs = 1000, lr = 0.1, momentum = None, verbose = 1, live_stats = None):
        if live_stats == None:
        ...
        for n in [-1,0,1,2]:
            if live_stats.type == n:
                ...
                raise ValueError("unknown value for live_stats.type")

        if live_stats.row_select != "full" and live_stats.row_select != "limited":
            raise ValueError("unknown value for live_stats.row_select")
        
        if type(live_stats.f1) != int:
            raise ValueError("unknown value for live_stats.f1")
        
        if type(live_stats.f2) != int:
            raise ValueError("unknown value for live_stats.f2")
