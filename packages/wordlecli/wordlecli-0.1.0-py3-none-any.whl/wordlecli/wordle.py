...
def compare_words(guess, target):
    ...
    if guess == target:
        text =  f"[green]{guess}[/green]"
    else:
        for i in range(len(guess)):
            
            if guess[i] == target[i]:
                
                text += f"[green]{guess[i]}[/green]"
                target_freq[guess[i]] -= 1
            
            elif (guess[i] != target[i]) and (guess[i] in target):
                
                if target_freq[guess[i]] > 0:
                    text += f"[yellow]{guess[i]}[/yellow]"
                    target_freq[guess[i]] -= 1
                else:
                    text += f"[bright_black]{guess[i]}[/bright_black]"
...
def main(target_num = get_wordle_num()):
    ...
    while turn_count <= 6 and not(win):    
        guess_word = console.input(f"[blue]Please enter a word:[/blue] ")
