...
class _Polygon:
    ...
    def __plot__(self):
        ...
        patch = patches.PathPatch(path, facecolor="lightgreen", lw=0)
        return patch
    ...
    @property
    def segments(self):
        return [sp - ep for i in range(len(self.points)) if (sp := self.points[i - 1]) and (ep := self.points[i])]
...
class LineLike(metaclass=ABCMeta):
    ...
    def __and__(self, other):
        ...
        b = np.array(
            [
                np.dot(self.center, self.A),
                np.dot(other.center, other.A),
            ]
        )
        try:
...
class Line(LineLike):
    ...
    def __init__(self, point, angle_info) -> None:
        ...
        if isinstance(angle_info, Iterable):
            ...
            self.angle_info = np.array([np.sin(angle_info), -np.cos(angle_info)]).astype(np.float64)
