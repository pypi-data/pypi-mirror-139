...
def build(path, name="goldilox", image=None, dockerfile=None, platform=None):
    ...
    if os.path.isdir(path) and os.path.isfile(os.path.join(path, 'MLmodel')):
        ...
        if image is None:
            image = 'condaforge/mambaforge' if venv_type is not None else f"python:{python_version}-slim-bullseye"
        target_args = ["--target", "conda-image"] if venv_type is not None else ["--target", "venv-image"]
