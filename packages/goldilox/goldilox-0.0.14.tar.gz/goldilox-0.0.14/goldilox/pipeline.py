...
class Pipeline(TransformerMixin):
    ...
    def _get_meta_dict(self, requirements=None, conda_clean=True):
        ...
        return {
            ...
            REQUIREMEMTS: get_requirements(environment_type, requirements=requirements, conda_clean=conda_clean),
            VARIABLES: self.variables.copy(),
            DESCRIPTION: self.description,
            RAW: self.raw,
        }

    def _get_meta(self, requirements=None, conda_clean=True):
        return cloudpickle.dumps(_copy(self._get_meta_dict(requirements, conda_clean)))
    ...
    def save(self, path, requirements=None, conda_clean=True):
        """
        @param path: str : output path
        @param requirements: list[str]: a list of requirements. if None - takes from pip
        @param conda_clean: str: If True, clean some packages which are not available for conda in docker (ignored if not on conda)
        ...
        state_to_write = Pipeline.BYTES_SIGNETURE + self._get_meta(
            requirements, conda_clean) + Pipeline.BYTE_DELIMITER + self._dumps()
