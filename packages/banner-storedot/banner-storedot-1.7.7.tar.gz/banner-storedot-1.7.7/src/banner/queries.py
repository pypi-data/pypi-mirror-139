...
class Queries(object):
    ...
    def __cache_query(query, value, cache: Storage, ttl=None):
        try:
            ...
            print(f'Failed Caching {query} into {cache}', e)
    
    def __cached_query(query, cache: Storage):
        try:
            assert(isinstance(cache, Storage))
            
            cached = cache.retrieve(
                query, resp_type=pd.DataFrame
            )
    ...
    def neware_query(
        ...
        __data_columns, __aux_columns = NW_DATA_SIGNIFICANT_COLUMNS, NW_AUX_SIGNIFICANT_COLUMNS
        ...
        if columns != '*':
            ...
            __data_columns = list((set(columns) & set(__data_columns)))
            __aux_columns = list((set(columns) & set(__aux_columns)))
        
        __data_columns, __aux_columns = ', '.join(__data_columns), ', '.join(__aux_columns)
        ...
        if temperature and __aux_columns:
            aux_data = pd.concat([
                Queries.__query(
        ...
        if not raw:
            ...
            try:
                data = calculate_neware_columns(data, cache_df=cache_data)
    ...
    def neware_cache_query(
        ...
        _ips = [str(ip) for ip in ips] if is_non_string_iterable(ips) else [str(ips)]
        _devices = [str(device) for device in devices] if is_non_string_iterable(devices) else [str(devices)]
        _units = [str(unit) for unit in units] if is_non_string_iterable(units) else [str(units)]
        _channels = [str(channel) for channel in channels] if is_non_string_iterable(channels) else [str(channels)]
        _tests = [str(test) for test in tests] if is_non_string_iterable(tests) else [str(tests)]
        ...
        for cache_table in ('neware_cache', 'neware_pulses_cache', 'neware_cache_anode'):
            cache_data = cache_data.append(Queries.__query(f"""
                    SELECT {columns} FROM {cache_table} 
                    WHERE ip IN ({','.join([ip for ip in _ips])})
                    AND dev_uid IN ({','.join([device for device in _devices])})
                    AND unit_id IN ({','.join([unit for unit in _units])})
                    AND chl_id IN ({','.join([channel for channel in _channels])})
                    AND test_id IN ({','.join([test for test in _tests])})
                    AND {condition}
                """,
                connection=connection
            ))
    ...
    def __neware_tests_query(
        ...
        __cached_data = Queries.neware_cache_query(
            __tests['ip'].unique(), 
            __tests['device'].unique(), 
            __tests['unit'].unique(), 
            __tests['channel'].unique(), 
            __tests['test_id'].unique(),
            connection=connection,
            cache=cache,
            ttl=ttl
        )
