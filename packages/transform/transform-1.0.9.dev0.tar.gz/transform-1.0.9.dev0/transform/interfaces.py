...
from .constants import DEFAULT_QUERY_TIMEOUT, MAX_PAGE_SIZE
...
class DWValidationType(Enum):  # noqa: D
    ALL = "allIssues"
    DATA_SOURCE = "dataSourceIssues"
    DIMENSION = "dimensionIssues"
    METRIC = "metricIssues"
...
class BaseInterface:
    ...
    def __init__(self, auth_header: Dict[str, str], rest_api_url: str) -> None:  # noqa: D
        self.rest_api_url = rest_api_url
        self.auth_header = auth_header

        self.gql_client = Client(
            transport=RequestsHTTPTransport(url=self.graphql_api_url, retries=2, headers=auth_header),
            fetch_schema_from_transport=False,
            execute_timeout=90,  # 1.5 minute
        )

    def execute(self, query: DocumentNode, verbose: bool = False, variable_values: Dict[str, Any] = None) -> Dict[str, Any]:  # type: ignore[misc]
...
class BackendInterface(BaseInterface):  # noqa: D
    def __init__(self, auth_header: Dict[str, str], rest_api_url: str = TRANSFORM_PROD_API) -> None:  # noqa: D
        super(BackendInterface, self).__init__(auth_header, rest_api_url)

    def validate_configs(self, config_dir: str) -> ModelKey:
        ...
        try:
            ...
            return ModelKey.from_snake_dict(response.json()["model"])
        except Exception as e:
    ...
    def commit_configs(self, config_dir: str, is_validation: bool = False) -> ModelKey:
    ...
    def get_metrics_metadata(self) -> List[Dict[str, Any]]:  # type: ignore[misc]
        ...
        return self.execute(query, variable_values={"pageSize": MAX_PAGE_SIZE})["myOrganization"]["metrics"]

    def get_metric_metadata_by_name(self, metric_name: str) -> Dict[str, Any]:  # type: ignore[misc]
        ...
        return self.execute(query, variable_values={"metricName": metric_name})["myOrganization"]["metric"]
