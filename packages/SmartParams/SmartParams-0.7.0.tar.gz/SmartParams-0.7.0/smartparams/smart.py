...
class Smart(Generic[_T]):
    ...
    def map(
        ...
        dictionary[key] = value = function(dictionary[key]) if key in dictionary else default
        return value
    ...
    def update_from(
        ...
        if name is None:
            _flatten_keys = tuple(self.keys(flatten=True))
            for key in smart.keys(flatten=True):
                if override or not check_key_is_in(key, _flatten_keys):
