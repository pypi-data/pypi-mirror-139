...
def создать_слой(данные, **kwargs):
  ...
  elif параметры[0] == 'MaxPooling1D':
    if any(i in '()' for i in параметры[1]):
      return MaxPooling1D(int(параметры[1]))
    else:
      return MaxPooling1D(int(параметры[1]))
      
  elif параметры[0] == 'Dropout':
...
def создать_UNET(**kwargs):
  параметры_модели = 'Ошибка:categorical_crossentropy\
      ...
      метрика:dice_coef'
  def isConv2D(layer):
    return layer.split('-')[0]=='Conv2D'
      
  def точность(y_true, y_pred):
    return (2. * K.sum(y_true * y_pred) + 1.) / (K.sum(y_true) + K.sum(y_pred) + 1.)
  
  block1 = kwargs['down_block'].split()
  block2 = kwargs['up_block'].split()
  
  if 'out_classes' in kwargs:
    n_classes = kwargs['out_classes']
  else:
    n_classes = 2
    
  img_input = Input(kwargs['input_size'])
  фильтры_вниз = kwargs['down_filters'].split()
  фильтры_вверх = kwargs['up_filters'].split()
  ...
  layer = block1[0]
  if isConv2D(layer):
    layer = layer[:6]+'-'+фильтры_вниз[текущий_блок]+layer[6:]
  x = создать_слой(layer, входной_размер=kwargs['input_size']) (img_input)
  ...
  for i in range(nBlock-1):
    ...
    for j in range(len(block2)):
      ...
      if layer=='Concatenate':
        ...
        if isConv2D(layer):
          layer = layer[:6]+'-'+фильтры_вверх[текущий_блок]+layer[6:]      
        x = создать_слой(layer) (x)
...
def создать_PSP(**kwargs):
  ...
  final_block = kwargs['final_block']+'-same-softmax'
