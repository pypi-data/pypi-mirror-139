...
def get_images(**kwargs):
  if kwargs['оригиналы'] == 'Самолеты' or kwargs['оригиналы'] == 'Губы/оригинал' or kwargs['оригиналы'] == 'airplane_max' or kwargs['оригиналы'] == 'original':
...
def show_sample(**kwargs):
  if 'quantity' in kwargs:
    n = kwargs['quantity'] # Количество выводимых случайных картинок
  else:
     n = 5
  fig, axs = plt.subplots(2, n, figsize=(25, 7)) #Создаем полотно из n графиков
  # Выводим в цикле n случайных изображений
  for i in range(n):
    idx = np.random.randint(len(kwargs['original']))
    img = kwargs['original'][idx] # Выбираем случайное фото для отображения
    axs[0,i].imshow(img) # Отображаем фото
    axs[0,i].axis('off')  # Отключаем оси
    img2 = kwargs['segmented_images'][idx] # Выбираем случайное фото для отображения
