...
def get_images(**kwargs):
  data_list = ['самолеты', 'самолеты_макс', 'губы', 'люди']
  if 'размер' not in kwargs:
      kwargs['размер'] = (54, 96)
  if kwargs['путь'] in data_list:
    all_paths = ['оригинал', 'сегментация']
    all_airs = [[],[]]
    for idx,name in enumerate(all_paths):
        curr_path = os.path.join('/content',kwargs['путь'], name) 
        all_files = sorted(os.listdir(curr_path))
        for filename in tqdm_(all_files, desc=f'{kwargs["путь"].capitalize()} {name}', ncols=len(all_files)):
        # Читаем очередную картинку и добавляем ее в список с указанным target_size                                                      
            all_airs[idx].append(image.load_img(os.path.join(curr_path,filename),
            target_size=kwargs['размер']))                           
    return all_airs[0], all_airs[1] 

  elif kwargs['путь'] == 'diseases/origin':
    ...
    for d in kwargs['классы']:    
      for filename in sorted(os.listdir(kwargs['путь'] + '/' + d)): 
        # Читаем очередную картинку и добавляем ее в список с указанным target_size                                                      
        images_airplane.append(image.load_img(os.path.join(kwargs['путь'] +'/' + d,filename), 
                                                target_size=kwargs['размер']))     
...
def yt_prep(data, num_classes):
  ...
  return yTrain # Возвращаем сформированный yTrain
...
def create_xy(images_airplane, segments_airplane):
  ...
  xTrain = np.asarray(xTrain) # Переводим в numpy
...
def тест_модели(мод, тестовые_изображения, **kwargs):
  ...
  # indexes = [7, 1, 2, 3, 8] самолеты макс
  # indexes = [85, 789, 362, 142, 11]
