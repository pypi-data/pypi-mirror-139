...
def создать_выборки(путь, MWC, xLen, step):
  ...
  np.save(f'/content/test_sets/{путь}0.npy', x_val[0])
  np.save(f'/content/test_sets/{путь}1.npy', x_val[1])
  np.save(f'/content/test_sets/{путь}2.npy', x_val[2])
  np.save(f'/content/test_sets/{путь}_метки.npy', y_val)
...
def gastro_10_predict(нейронка, xLen, step, text_input = None, классы = None):
    if text_input is None:
        ...
        diagnos =  'Нейросеть думает, что это: ' + str(classes[int(sum(currOut)/len(currPred))]) + ' - ' + str(round(meanrec[int(sum(currOut)/len(currPred))]*100, 2)) + '%'
        print(diagnos)
...
def func_true_false(name, type_check = "show_paths"):
    ...
    if name_check:
        if path_check:
            ...
            assert False, f'Загрузите базу для \'{curr}\'.'
    else:
        if path_check:
