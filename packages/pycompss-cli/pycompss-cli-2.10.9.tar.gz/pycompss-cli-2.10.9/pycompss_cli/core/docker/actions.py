...
import pycompss_cli.core.docker.cmd as docker_cmd
...
class DockerActions(Actions):
    ...
    def run(self):
        ...
        if '--base_log_dir' not in app_args:
            app_args.insert(0, f'--base_log_dir={docker_cmd.default_workdir} ')

        command = "runcompss " + ' '.join(app_args)

        self.docker_cmd.docker_exec_in_daemon(command)

        self.docker_cmd.docker_exec_in_daemon(f'cp -a {docker_cmd.default_workdir}.COMPSs/. /root/.COMPSs/')
    ...
    def app(self):
        print("ERROR: Wrong Environment! Try using a `cluster` or `local` environment")
        exit(1)
