...
class DockerCmd(object):
    ...
    def docker_deploy_compss(self, working_dir,
        ...
        if not self.is_running(master_name):
            ...
            inside_docker = utils.is_inside_docker()

            if utils.is_debug():
                print('\t', '******', 'inside_docker: ', inside_docker)

            if inside_docker:
                for vol in self.client.volumes.list():
                    if utils.is_debug():
                        print('\t', '******', 'volume found: ', vol.name)
                    if vol.name not in extra_mounts and vol.name == 'JenkinsTestVol':
                        if utils.is_debug():
                            print('\t', '******', 'mounting JenkinsTestVol')
                        extra_mounts[vol.name] = '/home/jenkins/tests_execution_sandbox'

            mounts = self._get_mounts(user_working_dir=working_dir, extra_mounts=extra_mounts)
    ...
    def _get_mounts(self, user_working_dir: str, extra_mounts: dict={}) -> list:
        ...
        mounts = [user_dir] if not utils.is_inside_docker() else []
        for source, target in extra_mounts.items():
            if utils.is_debug():
                print('\t', '******', 'Adding extra mount', source, target)
