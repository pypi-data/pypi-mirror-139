...
class LocalActions(Actions):
    ...
    def app_deploy(self):
        ...
        if self.arguments.destination_dir:
            destination_dir = self.arguments.destination_dir
        else:
            destination_dir = app_dir

        local_app_deploy(self.arguments.source_dir, destination_dir)

        self.save_app(app_name, self.arguments.source_dir, self.arguments.destination_dir)
    ...
    def app_list(self):
        ...
        app_data = []
        for app in apps:
            dest_dir = app.remote_dir if app.remote_dir else app.app_dir
            app_data.append([ app.name, app.source_dir, dest_dir ])

        utils.table_print(['Name', 'Source Dir', 'Destination Dir'], app_data)
    ...
    def get_apps(self, env_id=None) -> List[App]:
        ...
        if env_id is not None:
            app_file_apth = env_id + '/apps.json'
        else:
            app_file_apth = self.env_conf['env_path'] + '/apps.json'
            
        if os.path.isfile(app_file_apth):
            with open(app_file_apth, 'r') as f:
                apps_dict: List[dict] = json.load(f)
                self.apps = [App.fromdict(app_dict) for app_dict in apps_dict]
                return self.apps
        else:
            return []
        

    def save_app(self, app_name: str, source_directory: str, destination_dir: str = None):
        app_file_apth = self.env_conf['env_path'] + '/apps.json'
        app_info = App(app_name, self.env_conf['name'], source_directory, destination_dir).__dict__

        if os.path.isfile(app_file_apth):
            with open(app_file_apth, 'r+') as f:
                self.apps: List[dict] = json.load(f)
                self.apps.append(app_info)
                f.write(json.dumps(self.apps))
        else:
            self.apps = [app_info]
            with open(app_file_apth, 'w') as f:
