...
def local_app_deploy(local_source: str, dest_dir: str):
    os.makedirs(dest_dir, exist_ok=True)
    for f in os.listdir(local_source):
        if os.path.isfile(os.path.join(local_source, f)):
            shutil.copy(os.path.join(local_source, f), os.path.join(dest_dir, f))
        else:
            shutil.copytree(os.path.join(local_source, f), os.path.join(dest_dir, f))
    print('App deployed from ' + local_source + ' to ' + dest_dir)