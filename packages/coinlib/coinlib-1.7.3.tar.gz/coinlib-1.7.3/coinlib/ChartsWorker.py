...
class ChartsWorker(WorkerJobProcess):
    ...
    def runIndicatorCalculation(self, indicator_element, dataFrameCopy):
        ...
        try:
            ...
            self.logger().info("running indicator finished " + indicator_element.indicator.subfeature, time.time() - start)
    ...
    def run(self):
        ...
        self.logger().info("Finished complete calculation ", time.time() - start)
        ## let ssave all
        ...
        self.logger().info("Saving DataFrame to chipmunk ", end - start)
