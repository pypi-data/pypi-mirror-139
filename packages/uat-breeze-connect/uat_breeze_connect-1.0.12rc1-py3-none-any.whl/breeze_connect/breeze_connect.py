...
class BreezeConnect():
        ...
        if get_exchange_quotes == False and get_market_depth == False:
            ...
            if exchange_code_name == False:
                ...
                if exchange_code.lower() == "bse":
                    ...
                    if expiry_date == "":
                        raise GetStockTokenError(message = "Expiry-Date cannot be empty for given Exchange-Code.")
                    ...
                    if product_type.lower() == "options":
                        ...
                        elif strike_price == "" and product_type.lower() == "options":
    ...
    def subscribe_feeds(self, stock_token="", exchange_code="", stock_code="", product_type="", expiry_date="", strike_price="", right="", get_exchange_quotes=True, get_market_depth=True):
        if self.sio_handler:
            if stock_token != "":
    ...
    def unsubscribe_feeds(self, stock_token="", exchange_code="", stock_code="", product_type="", expiry_date="", strike_price="", right="", get_exchange_quotes=True, get_market_depth=True):
        if self.sio_handler:
            if stock_token != "":
    ...
    def get_historical_data(self, interval="", from_date="", to_date="", stock_code="", exchange_code="", product_type="", expiry_date="", option_type="", strike_price=""):
        if self.api_handler:
            return self.api_handler.get_historical_data(interval, from_date, to_date, stock_code, exchange_code, product_type, expiry_date, option_type, strike_price)

    def add_margin(self, product_type="", stock_code="", exchange_code="", order_settlement="", add_amount="", margin_amount="", open_quantity="", cover_quantity="", category_INDSTK="", add_margin_amount="", expiry_date="", order_optional_exercise_type="", option_type="", order_stock_code=""):
        if self.api_handler:
            return self.api_handler.add_margin(product_type, stock_code, exchange_code, order_settlement, add_amount, margin_amount, open_quantity, cover_quantity, category_INDSTK, add_margin_amount, expiry_date, order_optional_exercise_type, option_type, order_stock_code)
    ...
    def square_off(self, source_flag="", stock_code="", exchange_code="", quantity="", price="", order_flow="", order_type="", validity="", stoploss="", disclosed_quantity="", protection_percentage="", segment_code="", order_settlement="", margin_amount="", open_quantity="", cover_quantity="", product="", expiry_date="", exercise_type="", option_type="", strike_price="", trade_date="", trade_password="", option_exercise_type=""):
        if self.api_handler:
...
class ApificationBreeze():
    ...
    def customer_login(self, user_id="", password="", date_of_birth=""):
        try:
            if user_id == "" or user_id == None or password == "" or password == None or date_of_birth == "" or date_of_birth == None:
                response = {"Success":"","Status":500,"Error":""}
                if user_id == "" or user_id == None:
                    response["Error"] = "User-ID cannot be empty"
                elif password == "" or password == None:
                    response["Error"] = "Password cannot be empty"
                elif date_of_birth == "" or date_of_birth == None:
                    response["Error"] = "Date-Of-Birth cannot be empty"
                return response
            headers = {
                "Content-Type": "application/json"
            }
    ...
    def get_historical_data(self, interval="", from_date="", to_date="", stock_code="", exchange_code="", product_type="", expiry_date="", option_type="", strike_price=""):
        try:
            if interval == "" or interval == None:
                ...
                return response
            # elif exchange_code.lower() == "nse" and product_type.lower() not in ["cash","eatm","btst","margin","marginplus"]:
            #     response = {"Success":"","Status":500,"Error":"Product-type should be either 'cash', 'eatm', 'btst', 'margin', or 'marginplus' for Exchange-Code 'NSE'"}
            #     return response
            elif exchange_code.lower() == "nfo":
                if product_type == "" or product_type == None:
                    response = {"Success":"","Status":500,"Error":"Product-type cannot be empty for Exchange-Code 'nfo'"}
                    return response
                elif product_type.lower() not in ["futures","options","furtureplus","futureplus_sltp","optionplus"]:
                    response = {"Success":"","Status":500,"Error":"Product-type should be either 'futures', 'options', 'furtureplus', 'futureplus_sltp', or 'optionplus' for Exchange-Code 'NFO'"}
                    return response
                    ...
                    response = {"Success":"","Status":500,"Error":"Option-Type cannot be empty for exchange-code 'nfo'"}
                    return response
                elif option_type.lower() not in ["call","put","others"]:
                    response = {"Success":"","Status":500,"Error":"Option-type should be either 'call', 'put', or 'others'"}
                    return response
            if interval == '1minute':
                interval = 'minute'
            elif interval == '1day':
    ...
    def add_margin(self, product_type="", stock_code="", exchange_code="", order_settlement="", add_amount="", margin_amount="", open_quantity="", cover_quantity="", category_INDSTK="", add_margin_amount="", expiry_date="", option_type=""):
        try:
            if exchange_code == "" or exchange_code == None:
                response = {"Success":"","Status":500,"Error":"Exchange-Code cannot be empty"}
                return response
            elif product_type != "" and product_type != None and product_type.lower() not in ["futures","options","furtureplus","futureplus_sltp","optionplus","cash","eatm","btst","margin","marginplus"]:
                response = {"Success":"","Status":500,"Error":"Product-type should be either 'futures', 'options', 'furtureplus', 'futureplus_sltp', 'optionplus', 'cash', 'eatm', 'btst', 'margin', or 'marginplus'"}
                return response
            elif option_type != "" and option_type != None and option_type.lower() not in ["call","put","others"]:
                response = {"Success":"","Status":500,"Error":"Option-type should be either 'call', 'put', or 'others'"}
                return response 
            body = {
                "exchange_code": exchange_code
            ...
            if order_settlement != "" and order_settlement != None:
                body["order_settlement"] = order_settlement
            ...
            if open_quantity != "" and open_quantity != None:
                body["order_open_quantity"] = open_quantity
            if cover_quantity != "" and cover_quantity != None:
                body["cover_quantity"] = cover_quantity
            if category_INDSTK != "" and category_INDSTK != None:
                body["category_INDSTK"] = category_INDSTK
            if add_margin_amount != "" and add_margin_amount != None:
                body["add_margin_amount"] = add_margin_amount
            if expiry_date != "" and expiry_date != None:
                body["expiry_date"] = expiry_date
            if option_type != "" and option_type != None:
                body["option_type"] = option_type
            body = json.dumps(body, separators=(',', ':'))
            headers = self.generate_headers(body)
        ...
        try:
            if stock_code == "" or stock_code == None or exchange_code == "" or exchange_code == None or product == "" or product == None or action == "" or action == None or order_type == "" or order_type == None or quantity == "" or quantity == None or price == "" or price == None or action == "" or action == None:
                if stock_code == "" or stock_code == None:
                    ...
                    response = {"Success":"","Status":500,"Error":"Price cannot be empty"}
                    return response
                elif validity == "" or validity == None:
                    response = {"Success":"","Status":500,"Error":"Validity cannot be empty"}
                    return response
            elif product.lower() not in ["futures","options","furtureplus","futureplus_sltp","optionplus","cash","eatm","btst","margin","marginplus"]:
                response = {"Success":"","Status":500,"Error":"Product should be either 'futures', 'options', 'furtureplus', 'futureplus_sltp', 'optionplus', 'cash', 'eatm', 'btst', 'margin', or 'marginplus'"}
                return response
    ...
    def square_off(self, source_flag, stock_code, exchange_code, quantity, price, order_flow, order_type, validity, stoploss, disclosed_quantity, protection_percentage, segment_code, order_settlement, margin_amount, open_quantity, cover_quantity, product, expiry_date, exercise_type, option_type, strike_price, trade_date, trade_password, option_exercise_type):
        try:
            body = {
                ...
                "order_stop_loss_price": stoploss,
                "order_disclosed_quantity": disclosed_quantity,
                "protection_percentage": protection_percentage,
                "order_segment_code": segment_code,
                "order_settlement": order_settlement,
                "margin_amount": margin_amount,
                "order_open_quantity": open_quantity,
                "order_cover_quantity": cover_quantity,
                "order_product": product,
                "order_exp_date": expiry_date,
                "order_exc_type": exercise_type,
                "order_option_type": option_type,
                "order_strike_price": strike_price,
                "order_trade_date": trade_date,
                "trade_password": trade_password,
                "order_option_exercise_type": option_exercise_type
            }
            body = json.dumps(body, separators=(',', ':'))
            headers = self.generate_headers(body)
