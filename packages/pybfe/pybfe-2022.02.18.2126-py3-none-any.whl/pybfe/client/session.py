...
def _break_extra_args(extra_args: Optional[Dict[str, Any]]) -> Tuple[List[str], bool]:
    """Convert an open-source format extra_args dict into the Batfish Enterprise arguments."""
    if extra_args is None:
        return [], False
    debug_flags: List[str] = extra_args.get("debugflags", "").split(",")
    debug_flags = [
        f for f in debug_flags if f
    ]  # remove any empty flags from user or default
    disable_parse_reuse: bool = not extra_args.get("parsereuse", True)
    return debug_flags, disable_parse_reuse
...
class Session(BaseSession):
    ...
    def fork_snapshot(
        ...
        debug_flags, disable_parse_reuse = _break_extra_args(extra_args)
        ...
        self._api_gw.ForkSnapshot(
            ForkSnapshotRequest(
                ...
                disable_parse_reuse=disable_parse_reuse,
            )
        )
    ...
    def _initialize_snapshot(
        ...
        if isinstance(upload, str):
            ...
            self._init_snapshot_from_file(
                name, upload, debug_flags, disable_parse_reuse
            )
        else:
            # Upload is IO
            if not zipfile.is_zipfile(upload):
                raise ValueError("The provided data is not a valid zip file")
            self._init_snapshot_from_io(name, upload, debug_flags, disable_parse_reuse)
    ...
    def _init_snapshot_from_file(
        self,
        name: str,
        file_to_send: str,
        debug_flags: List[str],
        disable_parse_reuse: bool,
    ) -> None:
        if os.path.isdir(file_to_send):
            data = zip_from_dir(file_to_send)
            self._init_snapshot_from_io(name, data, debug_flags, disable_parse_reuse)
            ...
            with open(file_to_send, "rb") as fd:
                self._init_snapshot_from_io(name, fd, debug_flags, disable_parse_reuse)
        else:
            raise ValueError("Supplied path does not exist: {}".format(file_to_send))

    def _init_snapshot_from_io(
        self, name: str, fd: IO, debug_flags: List[str], disable_parse_reuse: bool
    ) -> None:
    ...
    def _experimental_delete_policy(
        self,
        policy_uid: str,
        network: Optional[str] = None,
    ) -> None:
        """
        Delete a managed policy for the specified network.

        :param policy_uid: the UID of the policy
        :type policy_uid: str
        """
        network = self._get_network_or_raise(network)
        ...
        req = DeletePolicyRequest(
            credentials=self._credentials,
            network_name=network,
            policy_id=PolicyId(uid=policy_uid),
        )
        self._api_gw.DeletePolicy(request=req)
