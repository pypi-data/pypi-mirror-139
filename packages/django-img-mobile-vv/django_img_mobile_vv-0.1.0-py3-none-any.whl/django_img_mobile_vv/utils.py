...
def get_large_or_small_static_img(filename):
    filename_and_type = filename.split('.')
    filename_and_type[0] += settings.STATIC_IMG_PREFIX 
    updated_filename = filename_and_type[0] + '.' + filename_and_type[1]
    if(os.path.exists(f"{settings.STATICFILES_DIRS[0]}/{updated_filename}")):
