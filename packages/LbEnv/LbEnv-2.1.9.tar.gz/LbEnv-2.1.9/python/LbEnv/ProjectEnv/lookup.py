...
try:
    ...
    def lru_cache():
        "dummy no op alternative for Python 2"
        return lambda f: f
...
def findProject(name, version, platform, allow_empty_version=False):
    ...
    if re.match(r"^\d+\.\d+(\.\d+)*$", version):
        # If we are requested to find a X.Y[.Z] version (new style CMake config)
        # we have to look also for directories with vXrY[pZ]
        parts = version.split(".")
        if len(parts) == 2:
            versions.append("v{}r{}".format(*parts))
        elif len(parts) == 3:
            versions.append("v{}r{}p{}".format(*parts))
        else:
    ...
    for v in versions:
        suffixes.extend(
            [
                "{0}_{1}".format(name, v),
                os.path.join(name.upper(), "{0}_{1}".format(name.upper(), v)),
...
def findDataPackage(name, version):
    ...
    if re.match(r"^\d[\d.]*$", version):
        ...
        version = "".join(a + b for a, b in zip("vrp", parts))
