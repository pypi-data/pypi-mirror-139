...
from autoads.gads import get_existing, get_search_term_report
...
df_enabled = df_existing[df_existing['camp_status'] == 'ENABLED']
df_paused = df_existing[df_existing['camp_status'] == 'PAUSED']
...
if len(df) != 0:
    ...
    paused_campaigns = df_paused['camp_name'].unique().tolist()
    for i,row in df[df['camp_id'].isna()].iterrows():
        if row['Keywords2'] in paused_campaigns:
            df.loc[i,'Keywords2'] = row['Keywords2'] + "_" + str(uuid.uuid4())
...
df_search_term_report = df_search_term_report.loc[df_search_term_report['metrics_conversions']>0,['stv_search_term','adgroup_camp']]
...
df = pd.concat([df,df_search_term_report],axis=0,ignore_index=True)
df = df.drop_duplicates('Keywords')
