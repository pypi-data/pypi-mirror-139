...
keywords_to_remove = df_existing[df_existing['camp_status'].isin(['PAUSED','ENABLED'])]['keyword_name'].unique().tolist()
...
print("Getting meta titles from SERP")
seed_keywords = df["Keywords2"].unique().tolist()
for keyword in seed_keywords:
    print(keyword)
    response,_ = make_live_request(keyword,email,api_key)
    results = get_results(response,tasks,task_params)
    if results.get('organic_description') and response['tasks'][0]['result']:
        organic_description = str(results['organic_description'])
...
print("Performing similarity using meta titles")
...
if len(df) == 0:
    ...
    if len(df_existing) == 0:
        ...
        existing_campaign_id = df_existing['camp_id'].tolist()
        existing_campaign_status = df_existing['camp_status'].tolist()
        existing_keywords = df_existing['keyword_name'].tolist()
        ...
        if len(similarity_indexes[0]) != 0:
            keyword_adgroup_id_pair = list()
            for i,j in zip(similarity_indexes[0],similarity_indexes[1]):
                keyword_adgroup_id_pair.append((new_keywords[i],
                                                existing_campaign_id[j],
                                                existing_campaign_status[j],
                                                similarity_matrix[i][j]))

            df_expand = pd.DataFrame(keyword_adgroup_id_pair,columns=['Keywords','camp_id','camp_status','similarity'])
    ...
    already_exist_campaign = df_existing['camp_name'].tolist()
    df.loc[df["Keywords2"].isin(df_existing['camp_name'].tolist()),'camp_id'] = df_existing.loc[df_existing["camp_name"].isin(df['Keywords2'].tolist()),'camp_id']
