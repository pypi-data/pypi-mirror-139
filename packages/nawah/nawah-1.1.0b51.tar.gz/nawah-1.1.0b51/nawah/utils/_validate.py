...
from nawah.classes import (ATTR, ATTRS_TYPES_ARGS, NAWAH_DOC, NAWAH_ENV,
                           NAWAH_EVENTS, NAWAH_QUERY, ConvertAttrException,
                           InvalidAttrException, InvalidAttrTypeException,
                           MissingAttrException, Query)
from nawah.config import Config
from nawah.enums import LOCALE_STRATEGY, NAWAH_VALUES, Event
...
async def _validate_default(
    *,
    ...
    if (
        ...
        if mode == 'update':
            return NAWAH_VALUES.NONE_VALUE
...
async def validate_attr(
    *,
        ...
        if attr_type._type == 'ANY':
            ...
            if type(attr_val) == dict:
                try:
                    if (not attr_type._args['types']) or (
                        attr_type._args['types']
                        and attr_val['type'] in attr_type._args['types']
                    ):
            ...
            for setting_query_var in re.findall(
                ...
                setting_query['var'] = setting_query['var'].replace(
                    setting_query_var[0],
                    str(_extract_attr(scope=doc, attr_path=setting_query_var[1])),
                )
            ...
            if type(attr_val) == dict and '_id' in attr_val:
                return return_valid_attr(
                    attr_val=attr_val['_id'],
                    attr_oper=attr_oper,
                    attr_oper_args=attr_oper_args,
                )
                ...
                try:
                    return return_valid_attr(
                        attr_val=ObjectId(attr_val),
                        attr_oper=attr_oper,
                        attr_oper_args=attr_oper_args,
                    )
                except Exception as e:
                    logger.debug(
                        'Exception occurred while validating type \'ID\'. Exception details:'
                    )
                    logger.debug(e)
            ...
            if type(attr_val) == int:
                if attr_type._args['ranges']:
                    # [TODO] Implement inf range, ref: https://stackoverflow.com/questions/9884213/looping-from-1-to-infinity-in-python
                    for _range in attr_type._args['ranges']:
            ...
            if re.match(
                ...
                if attr_type._args['allowed_domains']:
                    ...
                    for domain in attr_type._args['allowed_domains']:
                        ...
                        elif not attr_type._args['strict'] and attr_val_domain.endswith(
                            domain
                        ):
                            return return_valid_attr(
                                attr_val=attr_val,
    ...
    if mode == 'update':
        ...
        mode in ['create', 'create_draft']
        and attr_type._default != NAWAH_VALUES.NONE_VALUE
    ):
    ...
    *, dynamic_attr: Dict[str, Any]
    ...
    if dynamic_attr['type'] == 'UNION':
        ...
        for i in range(len(dynamic_attr['args']['union'])):
            shadow_arg_list.append(None)
            (
                dynamic_attr['args']['union'][i],
                shadow_arg_union[i],
            ) = generate_dynamic_attr(dynamic_attr=dynamic_attr['args']['union'][i])
...
def _set_none_value(
    *,
    doc: Union[NAWAH_DOC, List[NAWAH_DOC]],
):
    if type(doc) == dict:
        doc_iter = doc.keys()
    elif type(doc) == list:
    ...
    for j in doc_iter:
        if type(doc[j]) in [list, dict]:
            _set_none_value(doc=doc[j])
        elif doc[j] == NAWAH_VALUES.NONE_VALUE:
            doc[j] = None
