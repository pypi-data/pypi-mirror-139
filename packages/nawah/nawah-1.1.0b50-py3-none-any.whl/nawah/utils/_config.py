...
from nawah.classes import (APP_CONFIG, JOB, NAWAH_DOC, PACKAGE_CONFIG, SYS_DOC,
                           USER_SETTING, AIORedisJSON)
...
async def _config_data():
    ...
    if Config.jobs:
        ...
        for job_name in Config.jobs.keys():
            ...
            if not croniter.is_valid(job.schedule):
                logger.error(
                    f'Job with schedule \'{job_name}\' schedule is invalid. Exiting.'
                )
                exit(1)
    ...
    if Config.cache_server:
        Config._sys_cache = AIORedisJSON(
            aioredis.Redis(host=Config.cache_server, db=Config.cache_db)
        )
        if Config.cache_expiry:
            from nawah.utils import reset_expired_cache
            ...
            Config.jobs['__sys.cache_del'] = job = JOB(
                job=reset_expired_cache, schedule='0 * * * *', prevent_disable=True
            )
            job._cron_schedule = croniter(job.schedule, Config._jobs_base)
            job._next_time = datetime.datetime.fromtimestamp(
                job._cron_schedule.get_next(), datetime.timezone.utc
    ...
    if Config.test:
        ...
        for module in Config.modules.keys():
            ...
            if module_collection:
                ...
                module_collection = Config.modules[
                    module
                ].collection = f'test_{module_collection}'
                if Config.test:
                ...
                if (
                    ...
                    and (
                        (
                            ...
                            and Config.admin_doc[attr][Config.locale]
                            == admin_doc[attr][Config.locale]
                        )
    ...
    for index in Config.data_indexes:
        ...
        try:
            Config._sys_conn[Config.data_name][index['collection']].create_index(
                index['index']
            )
    ...
    for module in Config.modules:
        if Config.modules[module].collection:
            ...
            Config._sys_conn[Config.data_name][
    ...
    for doc in Config.docs:
        ...
        if not doc_results['args']['count']:
            ...
            if doc_results['status'] != 200:
                logger.error('Config step failed. Exiting.')
                exit(1)
        Config._sys_docs[ObjectId(doc_results['args']['docs'][0]['_id'])] = SYS_DOC(
            module=doc.module
        )
