...
async def run_app():
    ...
    from nawah.classes import (ATTR, IP_QUOTA, NAWAH_ENV, ConvertAttrException,
                               InvalidAttrException, JSONEncoder)
    from nawah.config import Config
    from nawah.enums import Event
    from nawah.utils import (_compile_anon_session, _compile_anon_user,
                             _config_data, _import_modules, _process_file_obj,
                             validate_doc)
        ...
        for method in module.methods.values():
            if method.get_method:
                for get_args_set in method.query_args:
                    ...
                    get_routes.append(
                        f'/{module.module_name}/{method._callable.method}{get_args}'
                    )
                ...
                for post_args_set in method.query_args:
                    ...
                    post_routes.append(
                        f'/{module.module_name}/{method._callable.method}{post_args}'
                    )
    ...
    async def handle_msg(
        ...
        try:
            ...
            try:
                ...
                try:
                    res = jwt.decode(
                        res['token'], env['prev_session']['token'], algorithms=['HS256']
                    )
                except Exception:
                ...
                for job_name in Config.jobs:
                    ...
                    if current_time >= job._next_time:
                        ...
                        try:
                            ...
                            logger.error(
                                f'Job \'{job_name}\' has failed with exception.'
                            )
