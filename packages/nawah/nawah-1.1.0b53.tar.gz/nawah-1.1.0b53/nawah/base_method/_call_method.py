...
async def _call_cache(
    ...
    try:
        ...
        cache_dict = await Config._sys_cache.get(
            cache.channel,
            f'.{module.module_name}.{method}.{cache_key}.results',
        )
...
async def _set_cache(channel, module, method, cache_key, results):
    ...
    try:
        ...
        if not await Config._sys_cache.get(channel):
            await Config._sys_cache.set(channel, '.', {})
        try:
            await Config._sys_cache.get(channel, f'.{module}')
        except aioredis.exceptions.ResponseError:
            await Config._sys_cache.set(channel, f'.{module}', {})
        try:
            await Config._sys_cache.get(channel, f'.{module}.{method}')
        except aioredis.exceptions.ResponseError:
