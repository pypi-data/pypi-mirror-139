...
class Client:
    ...
    def log(
        ...
        if schema.shap_values_column_names is None and surrogate_explainability == True:
            try:
                from arize.pandas.surrogate_explainer.mimic import Mimic
            except:
                logger.error(
                    "Need to install arize with the MimicExplainer option: pip install 'arize[MimicExplainer]'"
                )
                raise

            if model_type == ModelTypes.NUMERIC:
                dataframe, col_map = Mimic.augment(
                    df=dataframe,
                    prediction_score_column_name=schema.prediction_label_column_name,
                    feature_column_names=schema.feature_column_names,
                    model_type=model_type,
                )
            else:
                if schema.prediction_score_column_name is None:
                    logger.error("prediction_score_column_name must be specified")
                    raise
                dataframe, col_map = Mimic.augment(
                    df=dataframe,
                    prediction_score_column_name=schema.prediction_score_column_name,
                    feature_column_names=schema.feature_column_names,
                    model_type=model_type,
                )
            schema = Schema(
                ...
                feature_column_names=schema.feature_column_names,
                tag_column_names=schema.tag_column_names,
                ...
                actual_score_column_name=schema.actual_score_column_name,
                shap_values_column_names=col_map,
                actual_numeric_sequence_column_name=schema.actual_numeric_sequence_column_name,
            )
