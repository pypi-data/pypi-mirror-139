...
def isQpsRunning(host='127.0.0.1', port=9822, timeout=0):
    ...
    while True:
        try:
            ...
            answer = myQps.sendCmdVerbose(cmd="$echo hello quarchpy",timeout=timeout)
            break
        except:
            pass
    ...
    if answer is None or answer == "":
        logging.debug("QPS did not return expected output from $echo hello")
        logging.debug("$echo hello: " + str(answer))
        return False
    if answer.__contains__("hello quarchpy"):
        return True
    else:
        logging.debug("QPS did not return expected output from $echo hello")
        logging.debug("$echo hello: " + str(answer))
