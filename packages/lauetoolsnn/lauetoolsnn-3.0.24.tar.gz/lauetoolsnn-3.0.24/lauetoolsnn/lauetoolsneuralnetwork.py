...
try:
    from utils_lauenn import Symmetry,Lattice,\
        ...
        run_, rmv_freq_class, array_generator, vali_array, array_generator_verify,\
...
if default_initialization:
    ...
    coeff_overlap1212 = 0.10 ##10% spots overlap to avoid bad orientation detection
...
class Window(QMainWindow):
    ...
    def generate_training_data(self):
        ...
        # =============================================================================
        run_(self.n, self.rules, self.lattice_material, self.symmetry, self.material_, \
        ...
        if self.material_ != self.material1_:
            run_(self.n1, self.rules1, self.lattice_material1, self.symmetry1, self.material1_, \
        ...
        if self.generate_additional_data:
            ...
            # TODO implement with multiprocessing routine       
...
class MyPopup_image_v1(QWidget):
    def __init__(self, ix, iy, file, data, ccd_label, function_predict, image_no):
        ...
        self.reuse_UB = QPushButton("Index with neighbor matricies")
        self.reuse_UB.clicked.connect(self.neighbor_UB)
        ...
        formLayout.addRow(self.refresh_plot, self.reuse_UB)
...
class Window_allmap(QWidget):
    def __init__(self, limx, limy, filenm, ccd_label, predict_single_file_nodialog):
    ...
    def printProgressBar(self, iteration, total, prefix = '', suffix = 'Complete', 
    ...
    def calculate_image_similarity(self):
        #TODO compare two images ; maybe histogram ?
        values = []
        for ix in range(self.diff_data.shape[0]):
            for iy in range(self.diff_data.shape[1]):
                ...
                values.append([path, path1, ix, iy, self.ccd_label])
...
class AnotherWindowLivePrediction(QWidget):#QWidget QScrollArea
    ...
    def optimize_parameters(self,):
        ...
        w = Window_allmap(self.lim_x, self.lim_y, self.filenm, self.ccd_label.currentText(), self.predict_single_file_nodialog)
        w.show()       
        self.popups.append(w)
        ...
        if event123.dblclick or event123.button == 2:
            ...
            try:
                ...
                try:
                    ...
                    print("No IMAGE file could be found for the selected pixel")
                    return
                w = MyPopup_image_v1(ix, iy, path, Data, ccd_label, self.predict_single_file_nodialog, image_no)
                w.show()       
                self.popups.append(w)
                print('chosen pixel coords are x = %d, y = %d'%(ix, iy))
            except:
