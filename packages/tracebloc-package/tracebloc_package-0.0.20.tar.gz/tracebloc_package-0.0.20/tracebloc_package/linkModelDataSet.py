	...
	def checkmodel(self):
		try:
			...
			if re.status_code == 403:
				text = colored(f'There is no dataset with ID: {self.__datasetId} in your dataset table.\n'
							   f'Please check your dataset ID.', 'red')
				print(text)
				return False
				...
				text = colored(f'There is some values missing.', 'red')
				print(text)
				return False
		except:
...
# 			print(f"It will take around {display} to complete {self.__cycles} cycles for given training plan.")
	def checkTrainingPlan(self):
		...
		if re.status_code == 200:
			...
			if content['status']:
				userResponse = input("You already have an experiment with current Training Plan\n want to proceed?")
	...
	def create(self):
		...
		duplication = self.checkTrainingPlan()
		if duplication:
			#Create Experiment
			header = {'Authorization' : f"Token {self.__token}"}
			...
			if re.status_code == 201:
				...
				print(text, "\n")
				explink = "https://dev.tracebloc.io/experiments/" + self.__datasetId + "/" + content['experimentKey'] + "/"
