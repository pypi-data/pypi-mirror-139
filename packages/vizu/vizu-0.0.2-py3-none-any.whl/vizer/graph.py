...
class PlotBase:
    ...
    def generate_figure(self, df: DataFrame, query=None, x=None, y=None, z=None, group_by=None, groups=None,
        ...
        if group_by:
            if groups:
                query = " or ".join([f"(`{group_by}` == '{x}')" for x in groups])
                ...
                d = {idx_col: chart_data.query(f"`{group_by}` == '{x_grp}'")[idx_col].values}
                if x is not None:
                    d[_x] = chart_data.query(f"`{group_by}` == '{x_grp}'")[x].values

                if y is not None:
                    d[_y] = chart_data.query(f"`{group_by}` == '{y_grp}'")[y].values

                if z is not None:
                    d[_z] = chart_data.query(f"`{group_by}` == '{z_grp}'")[z].values
        ...
        if group_by is not None and groups:
            ...
            annotations = []
            for grp in groups:
                sub = chart_data.query(f"`{group_by}` == '{grp.strip()}'")
                data.append(self._create_fig(sub[x], sub[y], sub[z] if z else None, f"{prefix}_" + grp, sub[idx_col]))
