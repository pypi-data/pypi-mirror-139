...
def evaluate(mixdb: dict,
    ...
    for mi in range(num_mixtures):
        ...
        if 'gain' in mixdb['target_augmentations'][target_augmentation_index].keys():
        ...
        truth_index = mixdb['mixtures'][mi]['truth_index'] - 1
        ...
        if mi + 1 in plot_number:
            ...
            logger.debug(
                '{:{iwidth}}: Mix spec: SNR: {:4.1f}, target gain: {:+4.1f}, file: {:>{twidth}}, {:>{nwidth}}'.format(
                    mi, float(target_snr), target_gain, tname, nname, iwidth=miwidth, twidth=tfwidth, nwidth=nfwidth))
            logger.debug(
                '{:{iwidth}}: Measured: SNR: {:4.1f}, target level: {:4.1f}, SegSNR mean, max, peak80%: {:4.1f}, {:4.1f}, {:4.1f}'.format(
                    mi, mixture_snr, target_level, ssnrmean, ssnrmax, ssnrpk80, iwidth=miwidth))
            ...
            if sub_feature.shape[1] < 5:
                ...
                titlestr = 'All {} features of size {}x{}'.format(sub_feature.shape[0], sub_feature.shape[2],
                                                                  sub_feature.shape[1])
            else:
                if len(truth_active) != 0:
                ...
                titlestr = 'Example feature, frame {} of size {}x{}'.format(fti, sub_feature.shape[2],
                                                                            sub_feature.shape[1])
