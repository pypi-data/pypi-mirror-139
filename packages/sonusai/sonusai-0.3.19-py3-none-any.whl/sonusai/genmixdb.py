...
def genmixdb(config: dict, logging: bool = True, show_progress: bool = False) -> dict:
    ...
    if logging:
        logger.info('')
        logger.info('Found {} mixtures to process'.format(total_mixtures))
    ...
    if logging:
        logger.info('')
        logger.info('Estimated duration:   {}'.format(seconds_to_hms(seconds=total_duration)))
        logger.info('Estimated sizes:')
        logger.info(' mixture:             {}'.format(human_readable_size(mixture_bytes, 1)))
        logger.info(' truth_t:             {}'.format(human_readable_size(truth_t_bytes, 1)))
        logger.info(' feature:             {}'.format(human_readable_size(feature_bytes, 1)))
        logger.info(' truth_f:             {}'.format(human_readable_size(truth_f_bytes, 1)))
        logger.info('Feature shape:        {} x {} ({} total params)'.format(stride, num_bands, stride * num_bands))
    ...
    for n_id in range(len(mixtures)):
        augmented_noise_audio = apply_augmentation(
        ...
        desc = 'genmixdb {} of {}'.format(n_id + 1, len(mixtures))
        mixtures[n_id] = p_map(partial(process_mixture,
                                       mixdb=mixdb,
                                       config=config,
                                       targets=targets,
                                       target_augmentations=target_augmentations,
    ...
    if logging:
        logger.info('')
        logger.info('Actual duration: {}'.format(seconds_to_hms(seconds=total_duration)))
...
def append_input_files(files: list, in_record, cfg: dict, tokens=None):
    ...
    names = glob(in_name)
    if not names:
        logger.error('Could not find {}. Make sure path exists'.format(in_name))
        exit()
    for name in names:
            ...
            try:
                if ext == '.txt':
                    ...
                    for key, value in tokens.items():
                        name = name.replace(value, '${}'.format(key))
...
def process_mixture(mixture: dict,
                    mixdb: dict,
                    config: dict,
                    targets: List[dict],
                    target_augmentations: List[dict],
                    raw_target_audio: list,
                    augmented_noise_audio: np.ndarray,
                    feature_step_samples: int,
                    truth_mutex: bool,
                    class_weights_threshold: list) -> dict:
    ...
    target_augmentation = target_augmentations[target_augmentation_index]
    ...
    generate_truth(mixdb=mixdb,
                   mixture_record=mixture,
                   target_audio=gain_adjusted_target_audio,
                   noise_audio=gain_adjusted_noise_audio)
