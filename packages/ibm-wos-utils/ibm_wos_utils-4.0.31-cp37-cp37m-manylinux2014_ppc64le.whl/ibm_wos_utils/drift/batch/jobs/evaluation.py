...
class DriftEvaluation(AIOSBaseJob):
    ...
    def run_job(self):
        ...
        try:
            ...
            logger.info("Number of partitions in payload dataframe: {}".format(payload_spark_df.rdd.getNumPartitions()))
    ...
    def __load_drift_archive(self):
        ...
        with tarfile.open(SparkFiles.get("drift.tar.gz")) as drift_archive_handle:
            ...
            if self.model_drift_enabled:
                drift_model_path = get(self.arguments, "model_drift.drift_model_path")
    ...
    def __get_payload_spark_df(self):
        ...
        if insufficient_df:
            ...
            # Henc, we set start time to None and get handle on entire table
...
class DriftEval(object):
    ...
    def get_annotations(self, violations_df):
        ...
        if self.record_timestamp_column:
            ...
            annotations_df[self.record_timestamp_column] = violations_df[self.record_timestamp_column].apply(lambda row: row.isoformat())
    ...
    def partitionFn(self, index, data):
        ...
        for chunk in chunks:
            ...
            logger.warn(self.annotations_schema)
            logger.warn(annotations_df[columns].dtypes)
            logger.warn(annotations_df.dtypes)

            annotations_df = annotations_df[columns].apply(
                    lambda row: Row(**row.to_dict()), axis=1)
            
            if len(annotations_df) > 0:
                logger.warn(annotations_df.iloc[0])
            else:
