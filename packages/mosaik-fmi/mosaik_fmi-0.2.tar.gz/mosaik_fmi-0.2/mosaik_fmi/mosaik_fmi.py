...
class FmuAdapter(mosaik_api.Simulator):
    ...
    def init(self, sid, work_dir, fmu_name, fmi_type, instance_name='fmu_model', step_size=1, logging_on=False,
        ...
        if var_table is None:
            self.var_table, self.translation_table = mosaik_fmi.parse_xml.get_var_table(xmlfile)
        else:
        ...
        self.fmi_version = mosaik_fmi.parse_xml.get_fmi_version(xmlfile)
        ...
        self.model_identifier = mosaik_fmi.parse_xml.get_model_identifier(xmlfile, self.fmi_type)
