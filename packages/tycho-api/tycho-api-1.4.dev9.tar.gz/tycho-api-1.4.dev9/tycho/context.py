...
class TychoContext:
    ...
    def get_definition(self, app_id):
        ...
        logger.debug(f"----> ENTER GET_DEFINITION . . .")
    ...
    def get_spec (self, app_id):
        ...
        if not spec:
            ...
            try:
                ...
                logger.debug (f"-----> CONTEXT: {context}")
                template = Template(spec_template)
                spec = template.render(**context)
                logger.debug (f"-----> RENDERED SPEC: {spec}")
