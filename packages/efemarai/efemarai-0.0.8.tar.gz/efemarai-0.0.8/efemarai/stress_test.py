...
class StressTest:
    ...
    def create(
        ...
        return StressTest(
            project, response["id"], name, model, domain, dataset, "NotStarted"
        )

    def __init__(self, project, id, name, model, domain, dataset, state):
    ...
    def domain(self):
        ...
        if self._domain is None:
            self._domain = next(
                d for d in self.project.domains if d.id == self._domain__id
            )
        return self._domain
    ...
    def reload(self):
        ...
        new_obj = self.project.stress_test(self.name)
        self.state = new_obj.state
        return self
