...
def compute_quadrature_rules(rules, integral_type, cell):
    ...
    for rule in rules:
        ...
        # Ignore lower order quadrature rules with same number of points
        if num_points in quadrature_rules:
            if (numpy.allclose(quadrature_rules[num_points][0], points) and
                numpy.allclose(quadrature_rules[num_points][1], weights)):
                pass
            else:
                for cached_rule in quadrature_rule_sizes.keys():
                    if quadrature_rule_sizes[cached_rule] == num_points:
                        if cached_rule[1] > degree:
                            continue
                        else:
                            quadrature_rules[num_points] = (points, weights)
        else:
            quadrature_rules[num_points] = (points, weights)
        # NOTE: Caching lower order rule as higher order rule
