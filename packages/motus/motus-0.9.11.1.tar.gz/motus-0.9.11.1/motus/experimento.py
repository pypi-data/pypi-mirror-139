...
class Experimento:
    ...
    def __init__(self, nombre_archivo, dimension_caja=[100, 100], cantidad_filas=10,
                 cantidad_columnas=10, procesar_tiempo_archivo_irregular=False,
                 puntos_importantes=None, genera_matriz_recurrencia=False):
        ...
        if procesar_tiempo_archivo_irregular is False:
            ...
            print("Se empieza la generación de tiempos regulares: ")
    ...
    def calcula_velocidades(self):
        ...
        self.serie_velocidad = m_vel.calcula_serie_velocidad(self.tiempo,
                                                             self.posiciones_x,
                                                             self.posiciones_y)
        self.velocidad_maxima = round(round(self.serie_velocidad.max(), 3))
        ...
        self.serie_aceleracion = m_vel.calcula_serie_aceleracion()
    ...
    def calcula_velocidades_promediadas(self, cantidad_datos_promedio):
        ...
        self.serie_velocidad_promediada = m_vel.obtener_arreglo_promediado(self.serie_velocidad,
                                                                           cantidad_datos_promedio)

        m_hist.agrega_categoria_histograma(diccionario_histograma=self.histogramas,
                                           categoria="velocidad_promediada",
                                           serie=self.serie_velocidad)

        m_ed.agrega_categoria_entropia(diccionario_entropia=self.entropias,
                                       categoria="histograma_velocidad_promediada",
                                       datos=self.histogramas["velocidad_promediada"])

        self.serie_aceleracion_promediada = m_vel.obtener_arreglo_promediado(self.serie_aceleracion,
                                                                             cantidad_datos_promedio)
        m_hist.agrega_categoria_histograma(diccionario_histograma=self.histogramas,
                                           categoria="aceleracion_promediada",
                                           serie=self.serie_velocidad)

        m_ed.agrega_categoria_entropia(diccionario_entropia=self.entropias,
                                       categoria="histograma_aceleracion_promediada",
                                       datos=self.histogramas["aceleracion_promediada"])

    def calcula_regiones_cuadriculadas(self):

        self.arreglo_region_cuadriculada, self.matriz_cuadriculada = \
            m_reg_cuad.calcula_regiones_cuadriculadas(tiempos=self.tiempos,
    ...
    def calcula_matrices_solo_regiones_preferidas(self):
    ...
    def calcula_arreglos_rutas_vectoriales(self, tamanio_vectorial, brinco_temporal, indice_consecuencia):
        ...
        self.rutas_vectoriaes.obtener_posiciones_vectores()
    ...
    def graficar_figura(self, nombre_grafica, config_grafica):
        print("-----------------------------------------------------------")
        print("Tipo de gráfica", nombre_grafica)
        ...
        if nombre_grafica == 'ruta':
            ...
            brinco_temporal, tamanio_vectores, consecuencia_elegida = \
                config_grafica.regresa_config_grafica_consecuencias()
            ...
            x_pos_antes_consecuencia = x_pos_antes_consecuencia[int(consecuencia_elegida - tamanio_vectores):
                                                                int(consecuencia_elegida):
                                                                int(brinco_temporal)]

            y_pos_antes_consecuencia = y_pos_antes_consecuencia[int(consecuencia_elegida - tamanio_vectores):
                                                                int(consecuencia_elegida):
                                                                int(brinco_temporal)]

            x_pos_despues_consecuencia = x_pos_despues_consecuencia[int(consecuencia_elegida):
                                                                    int(consecuencia_elegida + tamanio_vectores):
                                                                    int(brinco_temporal)]
            y_pos_despues_consecuencia = y_pos_despues_consecuencia[int(consecuencia_elegida):
                                                                    int(consecuencia_elegida + tamanio_vectores):
                                                                    int(brinco_temporal)]

            new_x_pos_antes = x_pos_antes_consecuencia.copy()
            new_x_pos_antes.append(self.posiciones_x[consecuencia_elegida])
            new_y_pos_antes = y_pos_antes_consecuencia.copy()
            new_y_pos_antes.append(self.posiciones_y[consecuencia_elegida])

            x_direct_antes, y_direct_antes = m_rv.obtener_vectores_posicion_flechas(new_x_pos_antes,
                                                                                    new_y_pos_antes)
            ...
            x_direct_despues, y_direct_despues = m_rv.obtener_vectores_posicion_flechas(x_pos_despues_consecuencia,
        ...
        #    plt.legend()
            ...
            print("Matriz de recurrencia región cuadriculada:", self.matriz_recurrencia_region_cuadriculada)
        ...
        #        plt.imshow(np.flipud(self.matriz_cuadriculada)))
            ...
            print("Entro a regresión lineal")

            tipo_regresion_lineal, brinco_temporal, longitud_serie_tiempo = \
                config_grafica.regresa_config_grafica_regresion_lineal()

            print("Tipo regresion lineal: ", tipo_regresion_lineal)
            print("Brinco temporal: ", brinco_temporal)
            print("Longitud Serie Tiempo: ", longitud_serie_tiempo)

            if tipo_regresion_lineal == "Velocidad":
                print("Caso Velocidad")
                arreglo_series_tiempo = np.array(self.serie_velocidad)
            elif tipo_regresion_lineal == "Aceleración":
                print("Caso aceleración")
                arreglo_series_tiempo = np.array(self.serie_aceleracion)

            print("Arreglo series tiempo: ", arreglo_series_tiempo)

            indices_tiempo, series_tiempo = m_arl.generar_series_tiempo(lista_a_procesar=arreglo_series_tiempo,
                                                                      brinco_temporal=brinco_temporal,
                                                                      longitud_ventana_tiempo=longitud_serie_tiempo)

            resultados =m_arl.generar_regresion_lineal_para_series_tiempo(indices_tiempo=indices_tiempo,
            ...
            tamanio_volumenes, div_volumenes, tipo_grafica_volumen, rot_grafica_3d = \
