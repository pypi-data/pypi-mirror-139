...
class PhasingCheck(BaseTest):
    ...
    def vary_current_and_check_position(self, current_sign, max_current):
        init_pos = self.mc.motion.get_actual_position(
        ...
        for value in self.mc.motion.ramp_generator(
            ...
            self.mc.motion.set_current_direct(
                value, servo=self.servo, axis=self.axis
            )
    ...
    def check_position(self, init_pos, resolution):
        current_pos = self.mc.motion.get_actual_position(
            servo=self.servo, axis=self.axis
        )
    ...
    def not_forced_phasing(self):
        self.logger.info("Increasing slowly current quadrature "
                         "set-point until phasing bit is latched")
        phasing_bit_latched = False
        i = 0
        delta_i = 0.1
    ...
    def check_motor_commutation(self):
        ...
        self.logger.info("{} {} {}".format(
            "Slowly increasing Current Direct Set-point until",
            max_test_current, 'A')
        )
        ...
        if result != self.ResultType.SUCCESS:
            return result
        self.logger.info("{} {} {}".format(
            "Slowly decreasing Current Direct Set-point until",
            -max_test_current, 'A')
        )
