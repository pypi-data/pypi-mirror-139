...
from streamlit_gradio import encryptor, queue
from streamlit_gradio.tunneling import create_tunnel
from streamlit_gradio.process_examples import load_from_cache, process_example
...
STATIC_TEMPLATE_LIB = pkg_resources.resource_filename("streamlit_gradio", "templates/")
STATIC_PATH_LIB = pkg_resources.resource_filename("streamlit_gradio", "templates/frontend/static")
VERSION_FILE = pkg_resources.resource_filename("streamlit_gradio", "version.txt")
...
def start_server(interface, server_name=None, server_port=None, auth=None, ssl=None):
    ...
    if server_port is None:  # if port is not specified, start at 7860 and search for first available port        
        ...
        try:
            ...
            raise OSError("Port {} is in use. If a gradio.Interface is running on the port, you can close() it or streamlit_gradio.close_all().".format(server_port))
        port = server_port
