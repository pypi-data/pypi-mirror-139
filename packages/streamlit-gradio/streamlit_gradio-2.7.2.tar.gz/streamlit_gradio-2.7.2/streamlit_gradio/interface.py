...
from streamlit_gradio import encryptor, interpretation, networking, queue, strings, utils  # type: ignore
...
class Interface:
    ...
    def get_streamlit_component(self, key = 'default'):            
        ...
        if key not in st.session_state.gradio_data:
            st.session_state.gradio_data[key] = {
                'input_data': None,
                'output_data': None,
                'interface_config': utils.get_config_file(self)
            }
        ...
        with container:
            ...
            with col1:
                state = gradio_streamlit_input(st.session_state.gradio_data[key]['interface_config'])
                if state:
                    st.session_state.gradio_data[key]['input_data'] = state
                    output, raw_in, raw_out = self.predict_fn(state, st.session_state.gradio_data[key]['interface_config'])
                    st.session_state.gradio_data[key]['output_data'] = output
            with col2:
                gradio_streamlit_output(st.session_state.gradio_data[key]['interface_config'], st.session_state.gradio_data[key]['output_data'])
            container.col1 = col1
            container.col2 = col2
    ...
    def launch(
        ...
        server_port (int): will start streamlit_gradio app on this port (if available). Can be set by environment variable GRADIO_SERVER_PORT.
