...
from . import cal_WGSdist


def extractDomainOSM(bbox,res,bldH):
    from BF2PALM.selectRegion import selectRegion
    from BF2PALM.centroid import centroid
    from BF2PALM.cal_WGSdist import cal_WGSdist
    ...
    resy = res
    bbox = bbox
    bb = selectRegion.selectRegion(bbox)
    ...
    for i in range(np.shape(aa)[0]):
        bb[i][0] = int(np.round((selectRegion.selectRegion(aa)[i][0] - x_min)*x_mul))
        bb[i][1] = int(np.round((selectRegion.selectRegion(aa)[i][1] - y_min)*y_mul))
        xxx.append(int(np.round((selectRegion.selectRegion(aa)[i][0] - x_min)*x_mul)))
        yyy.append(int(np.round((selectRegion.selectRegion(aa)[i][1] - y_min)*y_mul)))
    ...
    for idxP in range(qq.shape[0]):
        if Point(qq['cent'][idxP][0][0],qq['cent'][idxP][1][0]).within(Polygon(selectRegion.selectRegion(aa))): 
            # poly.bounds
            nx = []
            ny = []
            xy = []
    ...
    return(domain,angle,weighted,phi)