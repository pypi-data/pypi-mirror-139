...
class Price(GraphenePrice):
	""" This class deals with all sorts of prices of any pair of assets to
		...
		each being an instance of :class:`crowdwiz.amount.Amount`. The
		...
		:param crowdwiz.asset.Asset base: Base asset
		:param crowdwiz.asset.Asset quote: Quote asset
		:param crowdwiz.crowdwiz.CrowdWiz blockchain_instance: CrowdWiz instance
		...
		Way to obtain a proper instance:
			...
			* ``args`` can be a floating number and ``base`` and ``quote`` being instances of :class:`crowdwiz.asset.Asset`
			...
			* ``args`` being a list of ``[quote, base]`` both being instances of :class:`crowdwiz.amount.Amount`
			* ``args`` being a list of ``[quote, base]`` both being instances of ``str`` (``amount symbol``)
			* ``base`` and ``quote`` being instances of :class:`crowdwiz.asset.Amount`

		This allows instanciations like:

		* ``Price("0.315 USD/CWD")``
		* ``Price(0.315, base="USD", quote="CWD")``
		* ``Price(0.315, base=Asset("USD"), quote=Asset("CWD"))``
		...
		* ``Price(1.0, "USD/GOLD")``
		...
		.. code-block:: python
			...
			>>> from crowdwiz.price import Price
			>>> Price("0.3314 USD/CWD") * 2
			0.662600000 USD/CWD
	...
	def market(self):
		""" Open the corresponding market
			...
			:returns: Instance of :class:`crowdwiz.market.Market` for the
...
class Order(Price):
	""" This class inherits :class:`crowdwiz.price.Price` but has the ``base``
