...
class _Client:
    ...
    def open_connection(self, port=None):
        ...
        try:
            ...
            print(
                "Connection to AlphAI impossible, you might be already "
                "connected or trying on the wrong port. Try closing all "
                "instances of Python or use another another port when calling "
    ...
    def _send(self, command, val=None, res=False):
        ...
        """
        try:
            self.sock.send(pickle.dumps((command, val)))
            if res:
                # blocking call
                pickle_size = pickle.loads(self.sock.recv(28))  # a pickle this
                # small is size 28 bytes
                return pickle.loads(self.sock.recv(pickle_size))
    ...
    def get_distance(self) -> float:
        return self._send("get_distance", res=True)

    def get_camera(self) -> np.ndarray:
        return self._send("get_camera", res=True)

    def get_infra_red(self) -> np.ndarray:
        return self._send("get_ir", res=True)
