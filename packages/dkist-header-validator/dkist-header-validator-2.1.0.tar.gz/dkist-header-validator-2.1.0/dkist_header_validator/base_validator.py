...
class SpecValidator:
    ...
    def _return_hdulist(validated_headers, fits_cards) -> HDUList:
        ...
        if "HISTORY" in fits_cards:
            for line in fits_cards["HISTORY"].splitlines():
                new_hdu.header.add_history(line)
            fits_cards.pop("HISTORY")
        if "COMMENT" in fits_cards:
            for line in fits_cards["COMMENT"].splitlines():
                new_hdu.header.add_comment(line)
            fits_cards.pop("COMMENT")
        if fits_cards:
            raise ValueError(f"Invalid keys found in fits_cards. {fits_cards.keys()=}")
