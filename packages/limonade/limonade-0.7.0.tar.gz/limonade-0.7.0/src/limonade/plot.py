...
class Plot:
    ...
    def __init__(self, canvas_cfg: dict, det_cfg: SimpleNamespace, metadata: dat.Metadata,
                 time_slice: Optional[Sequence] = None, plot_idx: int = 0):
        """
        ...
        :param det_cfg:     Detector config object (a SimpleNamespace containing detector, channel and board
                            configurations as dictionary, as well as the calibration data.
        :param metadata:    The metadata object of the Data object that is plotted.
        ...
        self.labels, self.legend, self.title = self._set_labels(det_cfg, canvas_cfg)
...
class SimplePlot(Plot):
    ...
    def __init__(self, canvas_cfg: dict, det_cfg: SimpleNamespace, metadata: list,
                 time_slice: Optional[Sequence] = None, plot_idx: int = 0):
        """
        On initialization the plot class needs to have both plot and detector configurations. These will be parsed
        to set up the axes, filters and plot.
        ...
        :param plot_idx:    The index of the plot if several are given. Should be deprecated by now.
        """
        #super().__init__(canvas_cfg, det_cfg, time_slice, plot_idx)
        self.metadata = metadata
        self.canvas_cfg = deepcopy(canvas_cfg)
        plot_cfg = canvas_cfg['plot_cfg']
    ...
    def update(self, data_dict: dict) -> None:
        ...
        :param data_dict:   a dict with histo and bins as keywords. The "histo" should contain the histogram array and
                            "bins" should contain a list of the uncalibrated bins (edges) of the axes as numpy arrays.
                            If the dictionary contains also time_slice, it will be updated.
        :return: None
        """

        if 'histo' not in data_dict or 'bins' not in data_dict:
            raise ValueError('Input to OnlinePlot update methods needs "histo" and "bins" to be present in data_dict!')
...
class Gate:
    ...
    def __init__(self, gate_info, type_info, det_cfg, cal, bitmask=False):
        """
        Type info is for simplicity, so we dont need to test the data type again.
        ...
        self.ch_list = det_cfg['ch_list']
        self.ch_map = np.cumsum(self.type_info['ch_mask']) - 1  # Should return 0 for first channel in mask etc.
        ...
        if not bitmask:
            ...
            # for idx, roi in enumerate(gate_info['range']):
            #    print()
            #    print('original range [ch]:', roi)
            #    print('gate range [ch]:', self.range)
            #    print('gate range:[cal]:', dat.poly2(self.range[idx], *cal[self.dtype][self.data_ch, :]))
            #    print()
...
class Filter1d(Filter):
    ...
    def update(self, datas: list):
        ...
        if not self.two_d:
            ...
            print('FILTAH')
            print(histo_tuple.shape, _x.shape)
