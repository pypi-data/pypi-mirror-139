    ...
    def loader(self, data_path, base_name):
        ...
        for channel, ch_idx in enumerate(channel_list):
            ...
            E_vecs[ch_idx] = np.zeros((chunk_num,),
                                      dtype='int16')  # negative is empty so that Caen 0 really corresponds
            ...
            sample_ns = self.parent.config.det['index_variable']['cal']
                ...
                if len(buffer) != 26656:
                    ...
                    if np.any(chunk_counter > 0):
                        ...
                        time_streamer.write([(time_vecs[x][:chunk_counter[x]] - first_time_of_file) *
                                         self.parent.config.det['index_variable']['cal'] for x in ch_list])
...
class G4Loader:
    ...
    def loader(self, data_path, base_name):
        ...
        for ch in range(len(self.ch_list)):
            metadata.total_time = np.uint64(data_dict[ch]['time'][-1] * self.config.det['index_variable']['cal'])
