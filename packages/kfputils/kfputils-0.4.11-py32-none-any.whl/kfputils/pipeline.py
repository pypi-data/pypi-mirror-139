...
def upload_pipeline(
    client, pipeline_name: str, package_file: str, pipeline_version: str = ""
):
    # ) -> (Dict, bool):
...
def res_to_json(res):
    return json.dumps(res.to_dict(), indent=4, sort_keys=True, default=str)


def get_pipeline_url(res):
    resource = res.resource_references[0].key.id
    version = res.id
    return f"#/pipelines/details/{resource}/version/{version}"


# /pipelines/details/b250cbff-e291-4653-b661-2066fcbcce89/version/b127e125-dceb-49c7-ac90-88aecae587e0


def display_upload_message(is_first_version, version, pipeline_name, res):
    if is_first_version:
        print(f"[OK] created first version of pipeline: {pipeline_name}")
    else:
        print(
            f"[OK] created updated version of pipeline: {pipeline_name}, version: {version}"
        )
    pprint(res)
