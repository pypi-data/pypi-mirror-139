...
def thread():
    conn = psycopg2Crate.connect({dsn!r})
    curs = conn.cursor()
    for i in range(10):
...
class TestEncryptPassword(ConnectingTestCase):
    ...
    def test_encrypt_password_post_9_6(self):
        # MD5 algorithm
        self.assertEqual(
            ext.encrypt_password('psycopg2Crate', 'ashesh', self.conn, 'md5'),
            'md594839d658c28a357126f105b9cb14cfc')

        # keywords
        self.assertEqual(
            ext.encrypt_password(
                password='psycopg2Crate', user='ashesh',
    ...
    def test_encrypt_server(self):
        ...
        enc_password = ext.encrypt_password(
            'psycopg2Crate', 'ashesh', self.conn)
        ...
        self.assertEqual(
            ext.encrypt_password(
                'psycopg2Crate', 'ashesh', self.conn, 'scram-sha-256'
            )[:14], 'SCRAM-SHA-256$')

        self.assertRaises(psycopg2.ProgrammingError,
            ext.encrypt_password, 'psycopg2Crate', 'ashesh', self.conn, 'abc')

    def test_encrypt_md5(self):
        self.assertEqual(
            ext.encrypt_password('psycopg2Crate', 'ashesh', algorithm='md5'),
            'md594839d658c28a357126f105b9cb14cfc')
