...
on psycopg2Crate.extension, so I can't create the default JSON typecasters in
...
from psycopg2Crate._psycopg import ISQLQuote, QuotedString
from psycopg2Crate._psycopg import new_type, new_array_type, register_type
...
class Json:
    """
    An `~psycopg2Crate.extensions.ISQLQuote` wrapper to adapt a Python object to
    :sql:`json` data type.
...
def register_json(conn_or_curs=None, globally=False, loads=None,
    ...
    are provided.  Raise `~psycopg2Crate.ProgrammingError` if the type is not found.
...
def _get_json_oids(conn_or_curs, name='json'):
    # lazy imports
    from psycopg2Crate.extensions import STATUS_IN_TRANSACTION
    from psycopg2Crate.extras import _solve_conn_curs
