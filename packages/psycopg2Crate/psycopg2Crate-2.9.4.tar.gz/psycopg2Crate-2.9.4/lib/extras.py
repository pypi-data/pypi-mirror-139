"""Miscellaneous goodies for psycopg2Crate
...
#
# psycopg2Crate is free software: you can redistribute it and/or modify it
...
#
# psycopg2Crate is distributed in the hope that it will be useful, but WITHOUT
...
from psycopg2Crate import extensions as _ext
...
from psycopg2Crate._psycopg import (                             # noqa
...
from psycopg2Crate._json import (                                # noqa
    json, Json, register_json, register_default_json, register_default_jsonb)


# Expose range-related objects
from psycopg2Crate._range import (                               # noqa
...
from psycopg2Crate._ipaddress import register_ipaddress          # noqa
...
class NamedTupleCursor(_cursor):
    ...
    attributes.
        ...
        >>> nt_cur = conn.cursor(cursor_factory=psycopg2Crate.extras.NamedTupleCursor)
        >>> rec = nt_cur.fetchone()
        >>> rec
...
class StopReplication(Exception):
    ...
    `~psycopg2Crate.Error` as occurrence of this exception does not indicate an
    error.
    """
    pass
    ...
    def create_replication_slot(self, slot_name, slot_type=None, output_plugin=None):
        ...
        if slot_type == REPLICATION_LOGICAL:
            if output_plugin is None:
                raise psycopg2Crate.ProgrammingError(
...
def wait_select(conn):
    ...
    `~psycopg2Crate.extensions.set_wait_callback()`. This function uses
    ...
    """
    import select
    from psycopg2Crate.extensions import POLL_OK, POLL_READ, POLL_WRITE
...
def _solve_conn_curs(conn_or_curs):
    ...
    if conn_or_curs is None:
        raise psycopg2Crate.ProgrammingError("no connection or cursor provided")
    ...
    def parse(self, s, cur, _bsdec=_re.compile(r"\\(.)")):
        ...
        for m in self._re_hstore.finditer(s):
            if m is None or m.start() != start:
                raise psycopg2Crate.InterfaceError(
...
def register_hstore(conn_or_curs, globally=False, unicode=False,
    ...
    *oid* parameter, which can be found using a query such as :sql:`SELECT
    ...
    Raise `~psycopg2Crate.ProgrammingError` if the type is not found.
    """
    if oid is None:
...
| \(? ([^",)]+) [,)]                # or an unquoted string
    ...
    def tokenize(self, s):
        ...
        for m in self._re_tokenize.finditer(s):
            if m is None:
                raise psycopg2Crate.InterfaceError(f"can't parse type: {s!r}")
...
def execute_values(cur, sql, argslist, template=None, page_size=100, fetch=False):
    ...
    **not** contain a total result.
    ...
    '''
    from psycopg2Crate.sql import Composable
