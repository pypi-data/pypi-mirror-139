...
You will also need to set a valid API server. To view a list of our servers,
check out our `documentation <https://rockset.com/docs/rest-api>`_.
...
class RocksetApiClient(ApiClient):
    """Wrap the Swagger Client to close connection pools properly.

    There is a bug in the swagger client that is still unmerged.
    https://github.com/swagger-api/swagger-codegen/issues/9991

    The way that python 3.8+ cleans up threadpools doesn't use a
    sleep maintenance loop and instead cleans thing up more consistently.
    This introduces a behavior problem where certain ways of cleaning
    up threadpools relied on this behavior.
    https://bugs.python.org/issue39360

    We'll use atexit to register and cleanup the pool
    """

    def __init__(self, *args, **kwargs):
        atexit.register(self.close)
        super(RocksetApiClient, self).__init__(*args, **kwargs)
...
class Client(object):
    ...
    def config_dir(cls):
        ...
        if "ROCKSET_CONFIG_HOME" in os.environ:
            homedir = "%s" % os.path.expanduser(os.environ["ROCKSET_CONFIG_HOME"])
    ...
    # Constructor
    def __init__(
        self, api_key=None, api_server=None, profile=None, driver=None, **kwargs
    ):
        ...
        if self.api_server[:7] != "http://" and self.api_server[:8] != "https://":
        ...
        # init swagger client
        self.api_client = RocksetApiClient(
    ...
    def _process_exception(self, message, errtype, status_code, start_time, e):
        if status_code == 401:
            raise AuthError(code=status_code, message=message) from None
        elif status_code == 403 or status_code == 413 or status_code == 429:
            raise LimitReached(code=status_code, message=message, type=type) from None
        elif status_code == 400:
            raise InputError(code=status_code, message=message, type=errtype) from None
            ...
            raise RequestTimeout(
                code=status_code, message=message, type=errtype, timeout=elapsed
            ) from None
        elif status_code > 400 and status_code < 500:
            raise InputError(code=status_code, message=message, type=errtype) from None
        elif status_code >= 500 and status_code < 530:
            raise ServerError(code=status_code, message=message, type=errtype) from None
    ...
    def call_api(self, resource, method, *args, **kwargs):
        try:
            ...
            self._process_exception(message, errtype, e.status, start_time, e)
        except ReadTimeoutError as e:
            raise RequestTimeout(message=str(e), timeout=kwargs.get("_request_timeout"))
        return result
    ...
    def sql(self, q, **kwargs):
        ...
        if not isinstance(q, Query):
            raise NotImplementedError("query of type {} not supported".format(type(q)))
        return Cursor(
            q=q,
            client=self,
...
class CollectionClient(object):
    ...
    def retrieve(self, name, workspace="commons"):
        return Collection.retrieve(name=name, workspace=workspace, client=self.client)

    def add_docs(self, name, docs, **kwargs):
        c = Collection(name=name, client=self.client, **kwargs)
        return c.add_docs(docs, timeout=kwargs.get("timeout"))

    def patch_docs(self, name, docpatches, **kwargs):
        c = Collection(name=name, client=self.client, **kwargs)
        return c.patch_docs(docpatches, timeout=kwargs.get("timeout"))

    def remove_docs(self, name, docs, **kwargs):
        c = Collection(name=name, client=self.client, **kwargs)
        return c.remove_docs(docs, timeout=kwargs.get("timeout"))
...
class SourceClient(object):
    ...
    def azure_blob_storage(self, *args, **kwargs):
        return Source.azure_blob_storage(*args, **kwargs)
