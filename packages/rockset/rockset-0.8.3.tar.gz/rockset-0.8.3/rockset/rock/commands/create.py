...
Usage:
    ...
    rock create integration azure_blob_storage <name> [--description=TEXT] --connection_string=CONNECTION_STRING
...
Data sources:
    ...
    <data_source_url>                           specify the data source to auto ingest in order to
                                                populate the collection, e.g.:
                                                    ...
                                                    azure_blob_storage://blob-container
                                                    azure_blob_storage://blob-container/path/prefix
...
"""
        ...
        if parsed["workspace"]:
            ...
            for source in parsed["<data_source_url>"]:
                if source.startswith("s3://"):
                    sources.append(self._source_s3(source, integration, format_params))
                    ...
                    sources.append(self._source_gcs(source, integration, format_params))

                elif source.startswith("azure_blob_storage://"):
                    sources.append(self._source_azure_blob_storage(source, integration))
    ...
    def go_collection(self, resource):
        ...
        self.lprint(
            0,
            'Collection "%s" was created successfully in workspace "%s".'
            % (c.name, c.workspace),
        )
        return 0
    ...
    def go_integration(self, resource):
        ...
        try:
            ...
            self.lprint(
                0, "Integration {} was created successfully.".format(resource["name"])
            )
    ...
    def _source_dynamo(self, url, integration):
        ...
        return self.client.Source.dynamo(table_name=table_name, integration=integration)
    ...
    def _source_mongo(self, url, integration):
        ...
        return self.client.Source.mongo(
            database_name=database, collection_name=collection, integration=integration
        )
    ...
    def _source_azure_blob_storage(
        self, azure_blob_storage_uri, integration, format_params
    ):
        parts = azure_blob_storage_uri[21:].split("/")
        container = parts[0]
        prefix = None
        if len(parts) > 1:
            path = "/".join(parts[1:])
        ...
        return self.client.Source.azure_blob_storage(
            container=container,
            prefix=prefix,
            integration=integration,
            format_params=format_params,
