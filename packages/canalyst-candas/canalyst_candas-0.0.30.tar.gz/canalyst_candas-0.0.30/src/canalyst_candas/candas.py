...
class Search:
    ...
    def kpi_match(self, ticker1, ticker2, is_driver):
        """
        ...
        for i, row in df1.iterrows():
            ...
            df["rank"] = 0.75 * df["word_score_rank"] + 0.25 * df["diff_rank"]
    ...
    def get_ticker_list(self, ticker=""):
        """
        ...
        if self.ticker_list is None:
            ...
            df = self.ticker_list
        if ticker != "":
            df = df.loc[df["Bloomberg Ticker"] == ticker]
        return df
    ...
    def search_time_series_excel(
        self, ticker="", time_series_name="", time_series_description=""
    ):
        """
...
class ModelSet:
    ...
    historical_only : boolean, default False
        Filter to only historical data on instantiation of the class.
    company_info : boolean, default True
        Include additional meta data calls
    file_type : str, default "parquet"
        {"":csv from mds,"pickle":pickle from s3,"parquet":parquet from s3,"csv":csv from s3}
    """
    ...
    def get_featurelibrary(self, col="time_series_name"):
        ...
        for ticker in self.ticker_list:
            try:
                ...
                print("Error on ticker " + str(ticker))
...
class Model:
    ...
    def guidance(self):
        df = self.gt.get_file(self.ticker, "guidance", file_type=self.file_type)
        df = df.dropna()
        self._guidance = df
        return df

    def get_drivers(self, force=False):
        """
        Gets the associated list of drivers for the model.

        Supply with force=False to use cache.
        """
        df = self.gt.get_file(self.ticker, "model_drivers", file_type=self.file_type)
        ...
        drivers = get_drivers_from_api(
            self.config.mds_host, self.csin, self.latest_version, self.api_headers
        )
        ...
        self.get_name_index(force=self.force)
    ...
    def apply_drivers(self, force=False):
        ...
        if df is not None and force == False:
            ...
            self.get_drivers(force=self.force)
            df = self._model_drivers
    ...
    def historical_data_frame(self):  # if cache = true save locally?
        ...
        if self.force == False:
            if self.file_type in ["pickle", "parquet", "csv"]:
                df = self.gt.get_file(self.ticker, "historical_data", self.file_type)
                if df is not None:
                    return df
        ...
        # Converts the "value" column from an object type to a float64 type
    ...
    def forward_data_frame(self):
        if self.force == False:
            ...
            if (
                ...
                or self.file_type == "parquet"
                or self.file_type == "csv"
            ):
                df = self.gt.get_file(self.ticker, "forecast_data", self.file_type)
                if df is not None:
                    self.default_df = df
                    return df

            df = get_data_set_from_mds(
                CsvDataKeys.FORECAST_DATA,
                self.csin,
                self.latest_version,
                self.api_headers,
                self.log,
