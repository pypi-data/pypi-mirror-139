...
def namesplit(inpTitle):
    ...
    for psub1 in prt1:
        if psub1.strip() in differs:
            searchTerm1 = ' '.join(prt1[:prt1.index(psub1)])
            break

    for psub2 in prt2:
        if psub2.strip() in differs:
            searchTerm2 = ' '.join(prt2[:prt1.index(psub2)])
            break
        ...
        for eDif in differs:
            if eDif in list(p1j):
                for eachBigData in dataSet:
                    ctrlLeft1 = (str(eDif) + str(eachBigData)).strip()
                    ctrlRight1 = (str(eachBigData) + str(eDif)).strip()
