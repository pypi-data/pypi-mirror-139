...
from grid.sdk.utils.datastore_uploads import (
    list_incomplete_datastore_uploads,
...
class Datastore:
    ...
    def upload(self):
        ...
        for work in list_incomplete_datastore_uploads(Path(env.GRID_DIR)):
            if work.datastore_id == self._id:
                resume_datastore_upload(client=self._client, grid_dir=Path(env.GRID_DIR), work=work)
                dstore_resp = get_datastore_from_id(c=self._client, datastore_id=self._id, cluster_id=self._cluster_id)
                self.__dict__ = self._setup_from_response(dstore_resp).__dict__
                return
