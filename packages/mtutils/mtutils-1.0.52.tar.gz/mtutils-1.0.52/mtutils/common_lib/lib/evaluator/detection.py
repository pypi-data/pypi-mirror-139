...
from ....utils_vvd import compute_box_box_iou
from ....utils_vvd import dir_check
from ....utils_vvd import cv_rgb_imread
from ....utils_vvd import encode_path
from ....utils_vvd import boxes_painter
...
class DetectionEvaluator(EvaluatorBase):
    ...
    def _get_pred_result_dict(self, dm_pred):
        ...
        for rec in dm_pred:
            ...
            bboxes = np.reshape(self.am.get_xyxy(rec),  [-1,4])
            scores = np.reshape(self.am.get_score(rec), [-1,1])
            labels = np.reshape(self.am.get_classid(rec), [-1])
