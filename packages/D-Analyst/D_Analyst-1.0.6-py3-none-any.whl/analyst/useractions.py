...
class UserActionGenerator(object):
    ...
    def pinchEvent(self, e):
        if e.state() == QtCore.GestureStarted:
            self.action = 'Pinch'
            self.pinch_start_position = (0, 0)
        elif e.state() == QtCore.GestureUpdated:
            self.action = 'Pinch'
        ...
        elif e.state() == QtCore.GestureFinished:
            self.action = None
            self.pinch_position = (0, 0)
            self.pinch_rotation = 0.
            self.pinch_scale = 1.
    ...
    def mouseReleaseEvent(self, e):
        if get_maximum_norm(self.mouse_position,
            ...
            if self.mouse_button == QtCore.LeftButton:
                self.action = 'LeftClick'
            elif self.mouse_button == QtCore.MiddleButton:
                self.action = 'MiddleClick'
            elif self.mouse_button == QtCore.RightButton:
    ...
    def keyPressEvent(self, e):
        key = e.key()
        if key in (QtCore.Key_Control, QtCore.Key_Shift, QtCore.Key_Alt, QtCore.Key_AltGr):
    ...
    def keyReleaseEvent(self, e):
        if e.key() in (QtCore.Key_Control, QtCore.Key_Shift, QtCore.Key_Alt, QtCore.Key_AltGr):
            self.key_modifier = None
        else:
            self.key = None
