...
from expressmoney.api.loans import OrderAPI
...
class CacheMixin:
    ...
    def __init__(self, user: Union[User, int], lookup_field_value: str = None):
    ...
    def upload_file(self, file, file_name, type_):
        self.flush_cache()
        super().upload_file(file, file_name, type_)
...
class BaseAPI:
    ...
    def filter(self, **kwargs) -> tuple:
        ...
        result = [item for item in self._sorted_data if item.get(key) in find_value]
        return tuple(result)
