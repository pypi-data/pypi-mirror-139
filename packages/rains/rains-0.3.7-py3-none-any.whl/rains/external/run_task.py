...
def function_run_task():
    ...
    if len(sys.argv) >= 3:
        ...
        for task_arg in sys.argv[2:]:
            ...
            # importlib 解析路径
            imp_analysis_str = f'rains.external.{_TASK_CACHE_NAMING}'

            # 拼接任务文件缓存目录路径
            # 因为 importlib 导入 py文件 的特殊方式, 需要在当前空间中创建任务文件缓存目录.
            task_cache_path: Path = Path(os.path.split(os.path.realpath(__file__))[0]).joinpath(_TASK_CACHE_NAMING)
            task_cache_path.mkdir(0o777, parents=True, exist_ok=True)
