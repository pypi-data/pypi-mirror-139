    ...
    def _detect_hits(self,frame_in,edge_hit=True):
        ...
        for layer_id in range(1,9):
            ...
            for seg_id,row_id,col_id in zip(seg_id_list,row_id_list,col_id_list):
                layer_segments[seg_id,0+row_id*4,j] = 1
            ...
            for seg_id,up_lo_id,col_id in zip(seg_id_list,up_lo_id,col_id_list):
                if(up_lo_id == 0):
                    if(col_id == 0):
                        ...
                        layer_segments[seg_id,0,j-1] = 1
                        layer_segments[seg_id,0,j] = 1
                        layer_segments[seg_id,0,j+1] = 1
                        layer_segments[seg_id,1,j-1] = 1
                        layer_segments[seg_id,1,j] = 1
                else:
                    if(col_id == 0):
                        ...
                        layer_segments[seg_id,4,j-1] = 1
                        layer_segments[seg_id,4,j] = 1
                        layer_segments[seg_id,4,j+1] = 1
                        layer_segments[seg_id,3,j-1] = 1
                        layer_segments[seg_id,3,j] = 1
        return self._frame_out
