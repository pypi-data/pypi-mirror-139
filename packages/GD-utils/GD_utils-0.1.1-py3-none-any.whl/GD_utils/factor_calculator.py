...
class FactorAnalysis(PortfolioAnalysis):
    ...
    def get_table(self, cum_rnt_ts):
        ...
        PortAnalysis_Calc = PortfolioAnalysis(return_daily)
        Active_Calc = PortfolioAnalysis(alpha_daily)
    ...
    def factor_report(self, col_name, drtion, outputname='./UnnamedReport', display=True):
        ...
        top_n_df = pd.concat([top5, top10, top20, self.BM], axis=1).dropna()
        ...
        return
    def factor_result(self, col_name, drtion, outputname='./UnnamedReport', display=True):
        cum_rnt_decile, metric_table_decile = self.get_Decile_result(col_name, drtion)
