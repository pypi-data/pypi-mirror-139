...
from .exception import MetaflowUnknownUser, MetaflowInternalError
from .metaflow_config import METADATA_SERVICE_AUTH_KEY
...
def get_username():
    ...
    """
    # If local, get service auth key
    # Aero Technology: Altering how we obtain Users
    metadata_key = METADATA_SERVICE_AUTH_KEY
    if metadata_key is None:

        if 'METAFLOW_OWNER' in os.environ:
            batch_user = os.environ.get('METAFLOW_OWNER', None)
        elif 'METAFLOW_USER' in os.environ:
            batch_user = os.environ.get('METAFLOW_USER', None)
        else:
            batch_user = None

        if batch_user is None:
            raise MetaflowUnknownUser()
        return batch_user
    return metadata_key.split(":")[0]
    ## Aero Technology
...
def resolve_identity():
    ...
    if prod_token:
        ## Aero Technology
        return 'production:%s' % "-".join(prod_token.split("-")[:-2])
        ## Aero Technology
    user = get_username()
