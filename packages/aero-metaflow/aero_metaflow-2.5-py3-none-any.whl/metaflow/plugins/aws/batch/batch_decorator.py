...
class BatchDecorator(StepDecorator):
    ...
    apply this decorator automatically to all steps using the ```--with aero``` argument
    ...
    def step_init(self, flow, graph, step, decos, environment, flow_datastore, logger):
        if flow_datastore.TYPE != 's3':
            raise BatchException('The *@aero* decorator requires --datastore=s3.')
        ...
        for deco in decos:
            if isinstance(deco, ResourcesDecorator):
                for k, v in deco.attributes.items():
                    # we use the larger of @resources and @aero attributes
