...
# Added basic Tracking for Jobs on new Flow submission
...
TRACKING_URL = "https://api.aeroplatform.co.uk/tracking?vcpus={vcpus}&memory={memory}"
...
class Batch(object):
    ...
    def kill_jobs(self, flow_name, run_id, user, echo):
        ...
        for job in jobs:
            ...
            try:
                ...
                echo(
                    'Killing Aero job: {name} [{id}] ({status})'.format(
                        name=job['jobName'], id=job['jobId'], status=job['status']
                    )
                )
            except Exception as e:
                echo(
                    'Failed to terminate Aero job %s [%s]'
                    % (job['jobId'], repr(e))
                )
        if not found:
            echo('No running Aero jobs found.')
    ...
    # Aero Technology: Adding Job Tracking
    def launch_job(
        self,
        step_name,
        step_cli,
    ...
    def wait(self, stdout_location, stderr_location, echo=None):
        ...
        # Aero Technology: Updated slot tracking for Aero
        def wait_for_slot(pre_job):

            def progress_bar(val, tot):
                percent = math.ceil((val/tot)*10)

                return f"| " + ("#" * percent) + "" + (" " * (10-percent)) + " | ", (val/tot)*100

            job_blocked = True
            requested_cpu = int(pre_job.cpu_val)
            requested_memory = int(pre_job.memory_val)

            while job_blocked:
                
                resp = requests.get(TRACKING_URL.format(vcpus=requested_cpu, memory=requested_memory), headers=BATCH_METADATA_SERVICE_HEADERS)

                if resp.status_code == 200:
                    job_blocked = False
                    echo(f"New job of {requested_cpu} vcpus has been submitted")

                    parsed_body = resp.json()

                    bar, percent = progress_bar(parsed_body['vcpus'], parsed_body['vcpus_limit'])
                    echo(f"Currently used vCPUs: {bar} {str(percent)}% Usage")

                    bar, percent = progress_bar(parsed_body['vcpus'] + parsed_body['requested_vcpus'], parsed_body['vcpus_limit'])
                    echo(f"Requested vCPUs:      {bar} {str(percent)}% Usage")

                else:
                    parsed_body = resp.json()
                    print(parsed_body)
                    current_vcpus = int(parsed_body['vcpus'])
                    cpu_limit = int(parsed_body['vcpus_limit'])

                    # If limit smaller than requested
                    if parsed_body['vcpus_limit'] < parsed_body['requested_vcpus']:
                        echo(f"You have requested {parsed_body['requested_vcpus']} VCPUs, but you only have access to {parsed_body['vcpus_limit']}")
                        raise BatchException("Task failed!")

                    bar, percent = progress_bar(parsed_body['vcpus'], parsed_body['vcpus_limit'])
                    echo(f"Currently used vCPUs: {bar} {str(percent)}% Usage")

                    echo(f"New job of {requested_cpu} is currently over the limit of utilised vCPUs")

                    time.sleep(60)
                
                    if requested_cpu + current_vcpus > cpu_limit:
                        echo(f"New job of {requested_cpu} would push CPU usage to {requested_cpu + current_vcpus}, which is higher than the limit of {cpu_limit}")
                    else:
                        echo(f"Requesting {requested_cpu} vcpus, current usage now {requested_cpu + current_vcpus}")
                        job_blocked = False
        ...
        def wait_for_launch(job, child_jobs):
            status = job.status
            echo(
                "Aero: Submitting task",
                "stderr",
                batch_id=job.id,
            )
            t = time.time()
            while True:
                if status != job.status or (time.time() - t) > 60:
                    status = job.status
                    echo(
                        "Aero: Allocating resources",
                        "stderr",
                        batch_id=job.id,
                    )
                    t = time.time()
        ...
        wait_for_slot(self.pre_job)
        self.job = self.pre_job.execute()
