...
class StepFunctionsException(MetaflowException):
    headline = 'Aero scheduler error'

class StepFunctionsSchedulingException(MetaflowException):
    headline = 'Aero scheduler scheduling error'
...
class StepFunctions(object):
    ...
    def trigger_explanation(self):
        if self._cron:
            ...
            return 'This workflow triggers automatically '\
                'via a cron schedule defined in Aero.'
        else:
    ...
    def deploy(self, log_execution_history):
        if SFN_IAM_ROLE is None:
            raise StepFunctionsException("No IAM role found for Aero "
                                         "scheduler. You can create one "
    ...
    def schedule(self):
        ...
        if EVENTS_SFN_ACCESS_IAM_ROLE is None:
            raise StepFunctionsSchedulingException("No IAM role found for Aero "
                                                   "scheduler. You can "
    ...
    def _step_cli(self, node, paths, code_package_url, user_code_retries):
        ...
        step = [
            ...
            '--input-paths %s' % paths,
            # Set decorator to aero to execute `task_*` hooks for aero
            # decorator.
            "--with=batch",
        ]
        if any(self.graph[n].type == "foreach" for n in node.in_funcs):
