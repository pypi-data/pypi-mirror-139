...
from metaflow.metaflow_config import AWS_SANDBOX_ENABLED, METADATA_SERVICE_AUTH_KEY
...
class BatchJob(object):
    ...
    def _register_job_definition(
        ...
        def_name = aero_user + '_metaflow_%s' % \
        ...
        try:
            ...
            if type(ex).__name__ == "ParamValidationError" and (
                ...
                raise BatchJobException(
                    ...
                    'you intend to launch Aero tasks' % ex)
            else:
                raise ex
    ...
    def timeout_in_secs(self, timeout_in_secs):
        self.payload["timeout"]["attemptDurationSeconds"] = 60 * 60 * 2 # Timeout of 2 hourspropagateTags
        return self
