			...
			if os.path.isdir(os.path.join(gamepath.replace('common','ingame'),'PLAYER','TEXTURES','PLYRKITS')):
				...
				def on_closing(event, pWbuttons):
					...
					if event.widget == root:
						if new_value is None: new_value = c or jersey_types.index(vals_already[f'{kit[1]} shirt'])
						if parentW:
							...
							vals_already[f'{kit[1]} shirt'] = jersey_types[new_value]
...
def show_all_kits(*args):
	if os.path.isdir(os.path.join(gamepath.replace('common','ingame'),'PLAYER','TEXTURES','PLYRKITS')):
		...
		mainRoot.geometry("800x600+0+0")
		#Code from Codemy
		# Create A Main Frame
		main_frame = Frame(mainRoot)
		main_frame.pack(fill="both", expand=1)
		...
		def callback(n,h,tm):
			...
			queue[n['id']] = edit_team(n,field='fs' if wh == 'first' else 'ss',asChild=mainRoot,buttons = buttons[e])
			print('\033[2A\033[J')
		def on_closing(event):
		...
		for e,t in enumerate(sorted(teams, key=lambda x: x['names'][-1])):
			j = jersey_types.index(t[f'{wh} shirt'])
			...
			kd['command'] = lambda b=kd: callback(b,wh,t)
			buttons[kd['text']] = kd
