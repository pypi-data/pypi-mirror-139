...
from eea.comments.restapi.services.discussion.utils import can_view
from eea.comments.restapi.services.discussion.utils import can_reply
...
class ConversationSerializer(object):
    ...
    def __call__(self):
        ...
        results["items_total"] = batch.items_total if canView else 0
        results["permissions"] = {
            "view_comments": canView,
        ...
        results["items"] = [
            getMultiAdapter(
                (thread["comment"], self.request), ISerializeToJson
            )()
            for thread in batch
        ] if canView else []
