...
def sanitized_run_data(hub, name):
    return {k: v for k, v in hub.idem.RUNS[name].items() if k != "acct_data"}
...
async def create(
    ...
    # Fire an event with the run data for all states
    await hub.idem.event.put(
        routing_key=name,
        profile="idem-run",
        body=hub.idem.state.sanitized_run_data(name),
...
async def apply(
    ...
    if hub.idem.RUNS[name]["errors"]:
        ...
        await hub.idem.event.put(
            ...
            profile="idem-run",
            body=hub.idem.state.sanitized_run_data(name),
            tags={"ref": "idem.state.apply", "type": "state-run"},
        )
        return
