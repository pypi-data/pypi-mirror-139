...
from wisdoms_dapr.consts import (
    ENV_NAME_SERVICE_RUN_MODE,
    ENV_NAME_SERVICE_VERSION,
)
        ...
        for v in allow_verbs:
            result.append(
                ServiceRouteInfo(
                    ...
                    if deps.ScheduleTaskRegister.find_route_schedule_dependency(
                        r
                    )
...
class FastAPIServiceRegister(object):
    ...
    service_register_subscribe_route = (
        consts.EVENT_SUBSCRIBE_ROUTE_SERVICE_REGISTER
    )
    ...
    def __init__(
        ...
        desc: typing.Optional[str] = '',
        *,
        subscribe_infos: typing.Optional[
            list[schemas.DaprSubscribeInfoItem]
        ] = None,
        ...
        self.schedule_tasks_register_method_name = (
            schedule_tasks_register_method_name
        )
        self.schedule_tasks_remove_method_name = (
            schedule_tasks_remove_method_name
        )
        self.remove_schedule_tasks_on_shutdown = (
            remove_schedule_tasks_on_shutdown
        )
        ...
        if enable_route_auth:
            ...
            self.app.middleware('http')(
                self.enforce_route_permission_middleware
            )
    ...
    async def enforce_route_permission_middleware(
        ...
        try:
            ...
            # 授权失败
            return JSONResponse(
                content={'detail': 'Permission denied'}, status_code=403
            )
    ...
    def add_dapr_subscribe_route(self):
        ...
        if infos:
            self.app.get(
                '/dapr/subscribe', tags=[consts.ROUTE_TAG_NAME_AUTO_REGISTER]
            )(lambda: infos)
    ...
    def extract_register_info(self) -> ServiceRegisterInfo:
        ...
        route_infos = extract_fastapi_route_info(
            app=self.app, exclude_verbs=self.exclude_http_verbs
        )
