...
@cachier()
def read_daily(path=None,open=0,close=0,high=0,low=0,tr=0):
...
def comment_on_rets_and_nets(rets,nets,name):
    ...
    comments=pd.DataFrame({'年化收益率':ret_yearly,'年化波动率':vol,'信息比率':info_rate,'月度胜率':win_rate,'最大回撤率':max_draw},index=[name]).T
    return comments
...
class pure_moon():
    ...
    def get_log_cap(cls,boxcox=False):
        ...
        if boxcox:
            cls.cap.cap_size=ss.boxcox(cls.cap.cap_size)[0]
        else:
    ...
    def get_limit_ups_downs(cls):
        ...
        cls.opens_monthly_shift.index = cls.opens_monthly_shift.index.shift(-1)
        ...
        cls.limit_downs = cls.find_limit(cls.rets_monthly_begin, up=-1)
    ...
    # @lru_cache(maxsize=None)
    def run(self, groups_num, neutralize=False, boxcox=False, y2=False, plt_plot=True, plotly_plot=False, filename='分组净值图',
...
class pure_fall():
    def __init__(
        ...
        #沪深300和中证500成分股
        self.hs300=pd.read_excel(self.homeplace.daily_data_file+'300和500成分股.xlsx',sheet_name='沪深300').stack().reset_index(level=1)
        self.zz500=pd.read_excel(self.homeplace.daily_data_file+'300和500成分股.xlsx',sheet_name='中证500').stack().reset_index(level=1)
...
class pure_moonnight():
    '''封装选股框架'''
    __slots__ = ['shen']
    def __init__(self, factors, groups_num, neutralize=False, boxcox=False,by10=False, y2=False, plt_plot=True, plotly_plot=False,
                 filename='分组净值图', time_start=None, time_end=None, print_comments=True):
        '''直接输入因子数据'''
        self.shen=pure_moon()
        self.shen.set_factor_df_date_as_index(factors)
        self.shen.prerpare()
        self.shen.run(groups_num=groups_num,neutralize=neutralize,boxcox=boxcox,y2=y2,plt_plot=plt_plot,plotly_plot=plotly_plot,filename=filename,time_start=time_start,time_end=time_end,print_comments=print_comments)
