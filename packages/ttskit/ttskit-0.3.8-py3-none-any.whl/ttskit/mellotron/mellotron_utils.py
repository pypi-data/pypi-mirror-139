...
def adjust_words(events):
    ...
    out = "|".join(["".join([k[0] for k in w]) for w in new_events])
    print(out)
    return new_events
...
def get_data_from_musicxml(filepath, bpm, phoneme_durations=None, convert_stress=False):
    ...
    for k, v in score.items():
        ...
        events_arpabet = fix_arpabet(events_arpabet)  # add
