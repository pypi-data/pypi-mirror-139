...
class TextMelLoader(torch.utils.data.Dataset):
    ...
    def get_data_inference_v2(self, data_dir):
        (text_data, mel_data, speaker_data, f0_data) = self.get_data_inference(data_dir)
        # assert mel_data.shape[1] < self.max_decoder_steps
        embed_data = np.zeros(256)  # 临时
        out = transform_data_train(
            hparams=self.hparams,
    ...
    def get_data_inference(self, audiopath_and_text):
        audiopath, text, speaker = audiopath_and_text
