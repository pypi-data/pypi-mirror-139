...
async def present(
    ...
    try:
        before_ret = (
            await hub.exec.aws.iam.role_policy_attachment.is_role_policy_attached(
    ...
    if ctx.get("test", False):
        if before_ret["result"]:
            ...
            result["comment"] = f"Would attach aws.iam.role_policy_attachment {name}"
        return result
...
async def absent(
    ...
    if not before_ret["result"]:
        ...
        result["comment"] = f"Would detach aws.iam.role_policy_attachment {name}"
        return result
    else:
        try:
            result["old_state"] = before_ret["ret"]
    ...
    for role_name in role_name_list:
        ...
        if not ret_attached_policies["result"]:
            ...
            for resource in resources:
                resource_name = f"{role_name}-{resource.get('PolicyArn')}"
                resource_translated = list()
                resource_translated.append({"role_name": role_name})
                resource_translated.append({"policy_arn": resource.get("PolicyArn")})
                result[resource_name] = {
                    "aws.iam.role_policy_attachment.present": resource_translated
                }
    return result
