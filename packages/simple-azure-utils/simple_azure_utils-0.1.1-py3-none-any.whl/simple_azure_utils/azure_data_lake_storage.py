...
class AzureStorageAccount:
    ...
    def upload_data(self, data, upload_path, metadata=None, overwrite=False, storage_account_name=None, settings_path=None):
        ...
        if metadata is not None:
            set_metadata_results = blob_client.set_blob_metadata(metadata=metadata)
            return {'upload_results': upload_results, 'set_metadata_results': set_metadata_results}
        return upload_results

    def upload_file(self, file_path, upload_path, metadata=None, overwrite=False, storage_account_name=None, settings_path=None):
        data_lake_client = self._get_storage_account_client(storage_account_name, settings_path)
        container_name = self._get_container_name(upload_path)
    ...
    def upload_files(self, files_to_upload, storage_account_name=None, metadata=None, settings_path=None, overwrite=False):
        """
        :param files_to_upload: [{'localPath': 'downloads/test.txt', 'uploadPath': 'containername/test-uploads/test.txt'}..]
        :param storage_account_name: mystorageaccount
        :param metadata: {'owner': 'john smith'}
        :param settings_path:
        :param overwrite:
        :return:
        """
        ...
        for file_to_upload in files_to_upload:
            ...
            with open(local_path, 'rb') as f:
                ...
                if metadata is not None:
                    logging.debug(f'Setting metadata for uploaded file: {metadata}')
                    set_metadata_results = blob_client.set_blob_metadata(metadata=metadata)
                    upload_result['set_metadata_results'] = set_metadata_results
    ...
    def _create_download_dir(blob_path, container_name, download_folder, remove_spaces_from_download_path=False):
        ...
        logging.debug(f'Created download path {download_location}')
    ...
    def _get_storage_account_client(self, storage_account_name, settings_path):
        try:
            logging.debug(f'Getting Storage Account Client for {storage_account_name}')
