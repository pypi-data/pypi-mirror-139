...
def main():
    ...
    for key,value in inventory['hosts']['webserver'].items():
        ...
        if key=="key_file" and value ==0: 
            ...
            logging.warning("ok- simple ssh")
        if key =="key_file" and value ==1:
            ssh.connect(ip, username=username, password=password, key_filename=os.path.join(os.path.expanduser('~'), ".ssh", "id_ed25519"))
            logging.warning("ok- shh with keyfile")
        ...
        for key,value in todo[i].items():
            ...
            if key == "module" and value =="service":
                ...
                if state=="enabled":
                    ...
                    session.get_pty()
                    session.exec_command("sudo bash -c \"" + "systemctl enable " + service + "\"")
                    ...
                    session.set_combine_stderr(True)
                    session.get_pty()
                    session.exec_command("sudo bash -c \"" + "systemctl start " + service + "\"")
                    stdin = session.makefile('wb', -1)
                    stdout = session.makefile('rb', -1)
                    stderr = session.makefile('rb',-1)
                    stdin.write(password + '\n')
                    ...
                    session.set_combine_stderr(True)
                    session.get_pty()
                    session.exec_command("sudo bash -c \"" + "systemctl restart " + service + "\"")
                    stdin = session.makefile('wb', -1)
                    stdout = session.makefile('rb', -1)
                    stderr = session.makefile('rb',-1)
                    stdin.write(password + '\n')
                    ...
                    session.set_combine_stderr(True)
                    session.get_pty()
                    session.exec_command("sudo bash -c \"" + "systemctl stop " + service + "\"")
                    stdin = session.makefile('wb', -1)
                    stdout = session.makefile('rb', -1)
                    stderr = session.makefile('rb',-1)
                    stdin.write(password + '\n')
                    ...
                    session.set_combine_stderr(True)
                    session.get_pty()
                    session.exec_command("sudo bash -c \"" + "systemctl disable " + service + "\"")
                    stdin = session.makefile('wb', -1)
                    stdout = session.makefile('rb', -1)
                    stderr = session.makefile('rb',-1)
                    stdin.write(password + '\n')
            ...
            if key =="module" and value =="apt":
                ...
                if applicationstate == "present":
                    ...
                    session.set_combine_stderr(True)
                    session.get_pty()
                    session.exec_command("yes | sudo bash -c \"" + "apt-get install " + application + "\"")
                    stdin = session.makefile('wb', -1)
                    stdout = session.makefile('rb', -1)
                    stderr = session.makefile('rb',-1)
                    stdin.write(password + '\n')
                    ...
                    session.set_combine_stderr(True)
                    session.get_pty()
                    session.exec_command("yes | sudo \"" + "apt remove " + application + "\"")
                    stdin = session.makefile('wb', -1)
                    stdout = session.makefile('rb', -1)
                    stderr = session.makefile('rb',-1)
                    stdin.write(password + '\n')
