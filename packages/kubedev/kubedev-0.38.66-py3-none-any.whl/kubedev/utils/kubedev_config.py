...
class KubedevConfig:
  ...
  @staticmethod
  def get_all_envs(kubedev, apps, build, container, generics):
    envs = KubedevConfig.load_envs(
        kubedev, build=build, container=container)
    if generics:
      appTypes = ['deployments', 'cronjobs', 'generic']
    else:
      appTypes = ['deployments', 'cronjobs']
    for appType in appTypes:
      if appType in kubedev:
        for (appName, app) in kubedev[appType].items():
          if apps is None or appName in apps:
            appEnvs= KubedevConfig.load_envs(
                app, build=build, container=container)
            envs = {**envs, **appEnvs}
    return envs
  ...
  def get_all_app_definitions(kubedev: dict) -> dict:
    ...
    return {
      key: definition for key, definition in {**if_exists(kubedev, 'deployments'), **if_exists(kubedev, 'cronjobs')}.items()
    }
  ...
  def prepare_envs(envs: dict, env_accessor: object) -> tuple:
    ...
    return (
      {name: {**attribs, **{'rawTargetName': env_name(name, attribs), 'targetName': f'${{{env_name(name, attribs)}}}'}} for name, attribs in sortedEnvs.items()},
  ...
  @staticmethod
  def get_helm_set_env_args(kubedev: dict, env_accessor: object, variable_overrides: dict = dict(), add_unknown_variable_overrides: bool = False, cmdline_as_list: bool = False) -> dict:
    '''
    Returns shell parameters for helm commands in the form of ``--set <variable>="${<variable>}" ...''
    from a kubedev config.
    '''
    (envs, extraEnvs) = KubedevConfig.prepare_envs(KubedevConfig.get_all_envs(kubedev, None, False, True, True), env_accessor=env_accessor)
    for var, value in variable_overrides.items():
      if (var in envs) or add_unknown_variable_overrides:
        envs[var] = {
          'targetName': value
        }
  ...
  def get_kubeconfig_path(env_accessor, file_accessor):
    ...
    if isinstance(cfg, type(None)):
      ...
      file_accessor.save_file(kubeconfig_temp_path, cfg, True, True)
  ...
  def get_docker_run_envs(image: dict, env_accessor: object) -> tuple:
    ...
    [0]: A list with all ["--env", "<name>=<value>"] parameters to the "docker run ..." call.
    [1]: A dict of extra environment variables and their values that must be passed to the container.

    :param image: One entry returned from KubedevConfig.get_images()
    """
    (envs, extraEnvs) = KubedevConfig.prepare_envs(image['containerEnvs'], env_accessor)
    return (
      [arg for arglist in [['--env', f'{name}="{attribs["targetName"]}"'] for name, attribs in sorted(envs.items())] for arg in arglist],
      extraEnvs
    )
  ...
  @staticmethod
  def get_docker_run_volumes_list(volumes: dict, file_accessor, shell_executor) -> list:
    """
    Returns a string with all "--volume ..." parameters to the "docker run ..." call.

    :param volumes: A dict with volume definitions.
    """
      ...
      if type(containerPathSpec) is dict:
        if 'path' in containerPathSpec:
          ...
          if 'content' in containerPathSpec:
            ...
            file_accessor.save_file(tempFilePath, content=containerPathSpec['content'], overwrite=True, private=False)
            ...
            tempFilePath = os.path.join('.kubedev', f'temp_{hostPath}')
            file_accessor.save_file(tempFilePath, content=b64decode(containerPathSpec['base64']).decode('utf-8'), overwrite=True, private=False)
  ...
  def get_docker_run_ports(image):
    """
    Returns a list with all ["--publish", "<dev>:<container>"] port mappings parameters to the "docker run ..." call.

    :param image: One entry returned from KubedevConfig.get_images()
    """
    return [arg for argslist in [
      ["--publish", f"{port['dev']}:{port['container']}"] for port in image['ports'].values() if "container" in port and "dev" in port
    ] for arg in argslist]
