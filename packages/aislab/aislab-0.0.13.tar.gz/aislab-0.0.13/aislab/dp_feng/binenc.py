...
def mrgbns(bns, i, j=np.nan):
    ...
    if type(bns[i]['lb']) is list:
        ...
        print('ERROR: Unknown type of x in mrgbns()...')  # todo: make this error
...
###################################################################################
def sbng(bng, md=5, crt=None, skip_mos=['m', 'o', 'sv'], met_dist='manh', met='co1y', nmin_uy=[50, 50], mar=np.inf, pt=0.05, ba=0, maxprm=20000, dGBInd=5, tolmindGBInd=5, mt=np.nan, minB=50, tolminB=20, stats=True, dsp=False):
    bng = new(bng)
    ...
    for i in range(m):
        ...
        if met == 'hclust':
            ...
            gd = nmin_uy[1] 
            nd = nmin_uy[2]
            bns, map, D = spih_sb_co1y(bns, crt, xtp1, ytp, md, gd, bd, nd, mar, pt, ba, dGBInd, tolmindGBInd, mt, minB, tolminB, maxprm, dsp)
        flg = 0
        if bns is None:
            bns = bng[i]['bns']
...
###################################################################################
def spih_sb_co1y(bns, crt, xtp, ytp, md, gd, bd, nd, mar, pt, ba, dGBInd, tolmindGBInd, mt, minB, tolminB, maxprm, dsp):
    if crt is None:
    ...
    map = nans((len(bns), 2))
    if len(ytp) == 1:
        if ytp[0] == 'bin':     bns = reduce_bns1(bns, xtp, ytp, gd=gd, bd=bd)
        elif ytp[0] == 'num':   bns = reduce_bns1(bns, xtp, ytp, nd=nd)
    ...
    for md in mm:
        ...
        bns1, __, g, g2, nw = reduce_bns2(bns1, ytp, md, mar)
        m = len(bns1)
        ...
        if s2.shape[0] != 0:
            ...
            if dsp: print('Run SB: ', i + 1, '/', m, cname, ',      time: ', toc5(dsp=False))
    ...
    if not bns_b is None:
        ...
        for i in bns_b.keys():
            ...
            n1 = len(lb)
            i2 = i1 + n1
            map[i1:i2, :] = np.hstack([c_(lb), c_([i]*n1)])
            i1 = i2
    else:
        map = []

    return bns_b, map, crit_b
