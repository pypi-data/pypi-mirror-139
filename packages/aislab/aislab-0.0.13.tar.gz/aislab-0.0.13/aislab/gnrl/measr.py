...
"""
import numpy as np
from aislab.md_reg import *# needed for vif()
...
def betai(a, b, x):
    ...
    return bi
##############################################################################
def chi2(g, b): # for binary dependent
    if g.ndim == 1: g = g.reshape(len(g), 1); b = b.reshape(len(b), 1)
    G = g.sum(axis=0)
    B = b.sum(axis=0)
    egi = (g + b)*G/(G + B)
    ...
    return chi
###################################################################################
def confm(x1, x2, w=None, ux1=None, ux2=None):
    # confusion matrix
    if ux1 is None: ux1 = np.unique(x1)
    if ux2 is None: ux2 = np.unique(x2)
...
def ks(g, b):
    ...
    return ks
###################################################################################
def mape(y, ym, w=None):
    if w is None: w = ones((len(y), 1))
    y[np.abs(y) <= 1e-6] = 1e-6
    return np.mean(w*np.abs((y - ym)/y))*100
###################################################################################
def mse(y, ym, w=None):
    if w is None: w = ones((len(y), 1))
    return np.sum(w*(y - ym)^2)/np.sum(w)*100
###################################################################################
def pvalC(v, C2):
...
def pvalF(F, v1, v2, tp='ot'):
    ...
    # v2 = c_(v2)
    # F = c_(F)
    if np.any(v1) <= 0 or np.any(v2 <= 0):  raise Exception('In PVALF(): Degrees(s) of freedom must be positive.')
    if np.any(F < 0):                       raise Exception('In PVALF(): F value(s) must be non negative...'); return []
    if tp == 'ot':
        if isinstance(F, (int, float)):  F = m_(F)
        if isinstance(v1, (int, float)):  v1 = ones(F.shape)*v1
        if v1.size == 1 and F.size > 1:  v1 = ones(F.shape)*v1.flatten()[0]
        if isinstance(v2, (int, float)):  v2 = ones(F.shape)*v2
        if v2.size == 1 and F.size > 1:  v2 = ones(F.shape)*v2.flatten()[0]
    ...
    return p
###################################################################################
def vaf(Y, Ym, w=np.empty([0, 0]), p=0, floor_to_0=True):
    Nw, r = Y.shape
    if w.size > 0:
        if w.size < Y.size: w = np.tile(w, (1, r))
        Nw = np.sum(w, axis=0).T
        mY = np.sum(Y*w, axis=0)/Nw
        SSE = sum((Y - Ym)**2*w)
        SST = sum((Y - mY)**2*w)
    else:
        mY = np.mean(Y, axis=0)
        SSE = np.sum((Y - Ym)**2, axis=0)
        SST = np.sum((Y - mY)**2, axis=0)
    VAF = 1 - (SSE/SST*(Nw - 1)/(Nw - p))
    if floor_to_0: VAF[VAF < 0] = 0
    return VAF*100
###################################################################################
def vif(x):
    m = x.shape[1]
