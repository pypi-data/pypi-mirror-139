...
class XGBoostGBTreeClassifierPrimitive(ProbabilisticCompositionalityMixin[Inputs, Outputs, Params, Hyperparams],
    ...
    """
    A XGBoost classifier using ``xgb.XGBoostClassifier`` with GBTree Boosting type.
    ...
    def fit(self, *, timeout: float = None, iterations: int = None) -> CallResult[None]:
        ...
        # convert data to np.array to avoid column name issue
    ...
    def produce_feature_importances(self, *, timeout: float = None, iterations: int = None) -> CallResult[Outputs]:
        ...
        feature_importances_array = feature_importances_array.reshape((1, len(self._attribute_columns_names)))
