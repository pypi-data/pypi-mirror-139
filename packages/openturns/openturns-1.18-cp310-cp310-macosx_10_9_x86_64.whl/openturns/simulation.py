...
def SobolIndicesAlgorithmImplementation_DrawSobolIndices(*args):
    r"""
    Draw the Sobol' indices.
...
def SobolIndicesAlgorithm_DrawSobolIndices(*args):
    r"""
    Draw the Sobol' indices.
...
class MauntzKucherenkoSensitivityAlgorithm(SobolIndicesAlgorithmImplementation):
    ...
    .. math::
       ...
       \hat{VT}_{i} & = & \frac{1}{N-1} \sum_{k=1}^{N} \tilde{\vect{g}}(\vect{A}_k) \left( \tilde{\vect{g}}(\vect{A}_k) -  \tilde{\vect{g}}(\vect{E}_k) \right)
       \end{array}
...
class SubsetSampling(EventSimulation):
    ...
    def getEventOutputSample(self):
        ...
        -------
        outputSample : :class:`~openturns.Sample`
            Ouput sample that realized the event.
        """
