...
# BASE_URL="https://20a5-14-98-21-242.ngrok.io"
...
def run_pipreqs(args, savepath):
	# print('pipreqs {}'.format(args))
	# print(args, savepath)
	#  2> /dev/null
	os.system('pipreqs {} --savepath {} 2> /dev/null'.format(args, savepath))
...
def pipreqsnb_main(input_path):
	...
	try:
		...
		run_pipreqs(temp_path, input_path+"requirements.txt")
...
def create_serving_model(file_name,s3Path,servingproject,servingprojectid,pipPackages,auth_token,training,model_type,subtype,image_type,preprocess=None,postprocess=None,predict=None,retry=0,is_lambda_deploy="false",startup_command_list=[]):
	# print("Inside")
	...
	model= {
		...
		"startupCommands" : startup_command_list,
	...
	if(model_resp.status_code==200):
		...
		sleep(5*retry)
		return create_serving_model(file_name,s3Path,servingproject,servingprojectid,pipPackages,auth_token,training,model_type,subtype,image_type,preprocess,postprocess,predict,retry=retry,is_lambda_deploy=is_lambda_deploy,startup_command_list=startup_command_list)
...
def get_from_pipreqs(notebook_path,pyFile=False):
	...
	if not pyFile:
		os.system("jupyter nbconvert --Application.log_level=50 {NOTEBOOK_PATH} --to python --output-dir={LOCAL_DIR}/req".format(NOTEBOOK_PATH=notebook_path,LOCAL_DIR=LOCAL_DIR))
		# os.system("jupyter nbconvert --to python {NOTEBOOK_PATH} --output-dir={LOCAL_DIR}/req".format(NOTEBOOK_PATH=notebook_path,LOCAL_DIR=LOCAL_DIR))
	else:
...
def invoke_publishproject(servingprojectid, infra, auth_token, invokepublishproject=True, linux_user="app"):
	...
	auth_headers={"Authorization": "Bearer "+auth_token}
	publishproject=requests.post(invoke_url,headers=auth_headers,data={"projectId":servingprojectid,"infratype":infra,"privileged_mode":linux_user})
...
def parse_startup_scripts(startup_script_path):
	try:
		fd = open(startup_script_path, 'r')
		command_list = fd.readlines()
		command_list = [command.replace("\n","") for command in command_list]
		print("Startup commands "  + str(command_list))
		fd.close()
		return command_list
	except Exception as e:
		print("Could not parse startup scripts file")
		exit(0)
...
class Clouderizer(object):
	...
	def start(self, name,infra="standard",image="standard",user="app"):
		global loadingDisplay
		if not name:
			print("Choose a specific project!")
			return
		...
		if user not in LINUX_USERS:
			print("--user is not an accepted value")
			print("Available linux users : app, root (default: app)")
			return
		...
		publishproject=invoke_publishproject(serving_project[0]["id"],infra,creds["token"],linux_user=user)
	...
	def quickdeploy(self,file_path=None,requirements=None,predict=None,preprocess=None,postprocess=None,model_type=None,notebook_type=None,infra="standard",image="standard",schema=None,user="app",startup_scripts=None):
	# def deploy(self,first_path=None,second_path=None,third_path=None,model_type=None,notebook_type=None,enable_gpu=None):
		...
		if execution_type=="notebook":
			...
			checkPathExists(requirements_path)
			checkPathExists(startup_scripts)
			# if (requirements_path!=None and not os.path.exists(requirements_path)):
			# 	print("Notebook path does not exist "+ requirements_path)
			# 	return
			...
			startup_command_list=[]
			if startup_scripts!=None:
				startup_command_list=parse_startup_scripts(startup_scripts)

			print("TYPE : NOTEBOOK")
			print("NOTEBOOK FILE PATH : " + notebook_path)
			print("FETCHED PIP PACKAGES : ", pipPackages)		
			print("FETCHED STARTUP COMMANDS LIST : ", startup_command_list)
			...
			if type(servingproject)==dict and "id" in servingproject:
				...
				if uploadmodel_resp=="Success":
					try:
						servingmodel=create_serving_model(file_name,s3Path,servingproject,servingproject["id"],pipPackages,auth_token,"true","regular",subtype,image,startup_command_list=startup_command_list)
					except Exception as e:
						print(e)
						print("Some exception occurred while preparing your notebook, please try again!")
						return
					...
					if type(servingmodel)==dict and "id" in servingmodel:    
						print("Project created!")
						publishproject=invoke_publishproject(servingproject["id"],infra,auth_token,linux_user=user)
						# print(publishproject)
						print("Publishing the project...this might take a while")
						if type(publishproject)==dict and "success" in publishproject and publishproject["success"]==True:
							signal.signal(signal.SIGINT, signal_handler)
			...
			if type(servingproject)==dict and "id" in servingproject:
				...
				if uploadmodel_resp=="Success":
					...
					try:
						...
						if subtype in ["python","onnx","pythonscore"]:
							deployment_model_type="regular"
						servingmodel=create_serving_model(file_name,s3Path,servingproject,servingproject["id"],pipPackages,auth_token,"false",deployment_model_type,subtype,image,preprocess=preprocess_path,postprocess=postprocess_path,predict=predict_path,startup_command_list=startup_command_list)
					except Exception as e:
						print(e)
						print("Some exception occurred while preparing your model, please try again!")						
						return
		...
		if execution_type=="notebook":
			...
			if type(servingproject)==dict and "id" in servingproject:
				...
				if uploadmodel_resp=="Success":
					...
					publishproject=invoke_publishproject(servingproject["id"],infra,auth_token,invokepublishproject=False)
