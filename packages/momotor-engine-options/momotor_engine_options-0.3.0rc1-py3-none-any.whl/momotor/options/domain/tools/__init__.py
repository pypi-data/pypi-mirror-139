...
class ToolOptionDefinition(OptionDefinition):
    ...
    Provides appropriate defaults for :py:attr:`domain` and :py:attr:`default` fields:
    the default for :py:attr:`domain` is ``{TOOLS_DOMAIN}``, and the default for :py:attr:`default` is the option name,
    i.e. if no option for the given name is in any bundle, the default version as defined in the registry
    will be used.
    ...
    * auto-generates suitable :py:attr:`doc` if none is provided
    ...
    def __post_init__(self):
        ...
        if '/' in name:
            raise ValueError('ToolOptionDefinition.name cannot contain /')
