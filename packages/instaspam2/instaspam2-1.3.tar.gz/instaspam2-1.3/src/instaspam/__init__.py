...
def login():
    # Login using User Id
    system("rm rockyvbg_uuid_and_cookie.json")
    system("rm rockyvbg.checkpoint")
    bot.login(username="rockyvbg", password="9773Rajni")

def logindotenv():
    load_dotenv()
    username1 = os.getenv('username')
    passsword1 = os.getenv('password')
    system("rm "+username1+"_uuid_and_cookie.json")
    system("rm "+username1+".checkpoint")
...
def love_spam():
    ...
    if user0 in readfile:
        ...
        for kathakali  in range(10):
            timer()
            bot.send_messages(costom , user1)
