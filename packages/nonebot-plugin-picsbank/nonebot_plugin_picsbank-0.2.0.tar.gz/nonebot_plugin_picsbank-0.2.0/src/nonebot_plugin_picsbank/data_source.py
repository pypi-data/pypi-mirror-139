...
class PicBank(object):
    ...
    def match(self, image_bytes: bytes, group_id: str = '0') -> str:
        """
        匹配词条
        :param group_id: 群号 不填为全局
        :param image_bytes: bytes格式的图片
        :return: 首先匹配成功的消息列表
        """
        # 优先检测群内
    ...
    def set(
        ...
        if sid == '':  # 默认special_key
            sid = return_str
        self._data[group_id].append({'limit': limit, 'hash_str': hash_str, 'return': return_str, 'sid': sid})
        self.__save()
        return '设置成功'
