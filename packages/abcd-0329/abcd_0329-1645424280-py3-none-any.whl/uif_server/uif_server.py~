...
class Domain(object):
    ...
    def DispachPath(self):
        ...
        while True:
            self.using_count += 1
            res = "node%s" % str(self.using_count)
...
class FreeUser(object):
    ...
    def GetUserInfo(self):
        ...
        if self.is_online and self.using_client is not None:
            res['client_info'] = self.using_client.GetClientInfo()
        return res
...
async def GetOP(req):
    ...
    if kind == 'user':
        for item in all_user:
            res.append(all_user[item].GetUserInfo())
    else:
        for item in all_client:
            res.append(all_user[item].GetClientInfo())
