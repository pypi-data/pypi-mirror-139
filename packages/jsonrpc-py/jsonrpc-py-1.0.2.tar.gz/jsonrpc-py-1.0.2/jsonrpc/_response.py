...
class BatchResponse(BaseBatchResponse[Response]):
    ...
    def json(self) -> list[dict[str, Any]]:
        ...
        """
        sorter: Final[Callable[[dict[str, Any]], bool]] = lambda obj: "result" in obj and "error" not in obj
        return sorted(map(lambda response: response.json, self), key=sorter, reverse=True)
