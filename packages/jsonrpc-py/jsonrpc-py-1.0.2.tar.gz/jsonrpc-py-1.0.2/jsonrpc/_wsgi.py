...
class BaseWSGIHandler(MutableMapping[str, Any], metaclass=ABCMeta):
    ...
    def __call__(self, environ: WSGIEnvironment, start_response: StartResponse, /) -> Iterator[bytes]:
        ...
        wsgi_response: Callable[..., Iterator[bytes]] = partial(self._get_response, start_response)
        ...
        try:
            ...
            print_error(error, file=environ["wsgi.errors"])
            exc_info: Final[ExcInfo] = type(error), error, getattr(error, "__traceback__", None)
    ...
    def _get_response(self,
        ...
        *,
        ...
        if (response_body := b"" if response_body is None else response_body):
            ...
            headers.append(("Content-MD5", self._md5_base64_digest(response_body)))
