...
class BaseLabelme(object):
            ...
            if json_path:
                ...
                if data_info['labelme_info']['shapes']:
                    for shape in data_info['labelme_info']['shapes']:
                        # print(shape)
                        # print(shape['label'] + '=' + shape['flags'])
                        # a = shape['label'] + '=' + shape['flags']
                        if shape['label'] not in self.name_classes:
                            self.name_classes.append(shape['label'])
                            self.name_classes_flags.append(str(shape['label']) + '=' + str(shape['flags']))
                            # isinstance(self.flags, dict)
                            # print(shape['flags'] ,dict)
                        # for i in shape['flags']:
                        # print(i)
                        # if shape['flags'] not in self.flags:
    ...
    def __repr__(self):
        ...
        """
        tb = pt.PrettyTable()
        tb.field_names = ['num_images', 'num_type', 'num_classes', 'num_labelme', 'num_background', 'flags']

        tb.add_row(
            [self.num_images, self.num_type, self.num_classes, self.num_labelme, self.num_background, self.num_flags])
