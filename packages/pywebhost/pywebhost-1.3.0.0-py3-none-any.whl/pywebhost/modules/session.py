...
class UniqueSessions(type):
    # Creates unique session dict per class in class (not instance) creation
    # Sessions are still non-singleton. But their associated session stores now are.
    def __new__(cls,*args, **kwargs):        
        obj = super().__new__(cls,*args,**kwargs)        
        obj._sessions = dict()
        return obj            
class Session(dict,metaclass=UniqueSessions):    
    def mapUri(self,url):
    ...
    def new_uid(self):
        '''Generates new uid'''
        str_ = '%s-not-%s-secure-%s' % (SESSION_KEY,time.time() * 1e6,randrange(1e7,1e8-1))
    ...
    def get_session(self):
        '''Gets session dictionary by own uid,may be overridden
        
        Returns:
            dict : The such dict
        '''
        if self.session_id:
            if not self.session_id in self._sessions.keys():self._sessions[self.session_id] = {}
    ...
    def __setitem__(self, __k, v):
        rv = super().__setitem__(__k, v) # write to session dict
        self.set_session()
        return rv
