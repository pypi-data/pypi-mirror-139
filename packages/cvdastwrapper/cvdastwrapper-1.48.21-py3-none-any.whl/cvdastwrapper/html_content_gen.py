...
def generate_test_report(api_test_report, spec, test):
    ...
    for each_failed_test in failed_test:
        ...
        req_resp_file_name = dlogrespdname + '/request_response_'+ api.replace('/', '-') + '.csv'
        with open(req_resp_file_name, 'a+') as csvfile:
            csvwriter = csv.writer(csvfile, delimiter='|', lineterminator='\n')
            csvwriter.writerow([sr_no, testinput.strip(), status_code, response])
        api_details[attack]['testinput'] = '<a href=\'' + req_resp_file_name + '\'>Request_Response</a>'
