...
class fitderiv:
    ...
    def __init__(
        ...
        esterrs: boolean, optional
            If True, measurement errors are empirically estimated by the variance
            across replicates at each time point.
            If False, the variance of the measurement error is assumed to be the
            same for all time points and its magnitude is a hyperparameter that
            is optimised.
        optmethod: string, optional
            The algorithm used to optimise the hyperparameters, either 'l_bfgs_b'
            or 'tnc'.
        ...
        statnames: list of strings
            ...
            The default names are:
                    ...
                    crosses a line parallel to the time-axis that passes through
        ...
        try:
            ...
            raise (
                SystemExit(
                    "Gaussian process not recognised."
                )
            )
        self.bds = bds
    ...
    def calculatestats(self, nosamples=100, statnames=None, showerrors=True):
        ...
        # lag time
        lagtime = (
            tmgr + (fs[0, np.arange(nosamples)]
                    - fs[im, np.arange(nosamples)]) / mgr
        )
        # store stats
        ds = {}
