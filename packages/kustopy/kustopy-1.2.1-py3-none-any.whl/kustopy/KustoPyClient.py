...
class Client:
    ...
    def get_schema(self, table_name):
        response = self.query(f'{table_name} | getschema')
        return response
    ...
    def ingestion_properties(self, dataframe):
        ...
        input_datatype = self.get_input_type(dataframe)
        columns_list = []
        csv_mapping_list = []
        ...
        if input_datatype == 'pandas':
            ...
            # Loop through each column and create respective stings
            for i, col in enumerate(dataframe.columns):
                kusto_type = type_map.get(dataframe.select(col).dtypes[0][1])
                columns_list.append(f'["{col}"]: {kusto_type}')
                # Merge them to one string
            columns_string = ', '.join(columns_list)
    ...
    def get_table_folder(self, table_name):
        ...
        if table_exists:
            return self.query(f'.show table {table_name} details | project Folder', dataframe=True)['Folder'][0]
        else:
    ...
    def write_table(self, dataframe, table_name, folder='KustoPyTables'):
        # Get the datatype of the input dataframe
        input_datatype = self.get_input_type(dataframe)
        ...
        if table_exists:
            raise Exception(
                f"Table '{table_name}' already exists in database '{self.database}'. If you want to replace the table use write_replace_table(). If you want to append to the table use write_append_table().")
        else:
            try:
                # Create the table
    ...
    def write_replace_table(self, dataframe, table_name, folder='KustoPyTables'):
        # First drop the table (and the mapping table)
        try:
            self.drop_table(table_name)
            self.write_table(dataframe, table_name, folder)
    ...
    def write_append_table(self, dataframe, table_name, folder='KustoPyTables'):
        # Get the datatype of the input dataframe
        input_datatype = self.get_input_type(dataframe)
        ...
        if table_exists:
            if input_datatype == 'pandas':
                ...
                type_map = {'float64': FloatType(),
                            ...
                            'int32': IntegerType(),
                            'int': IntegerType()}
                for idx, type in zip(dataframe.dtypes.index, dataframe.dtypes):
                    schema_list.append(StructField(f'{idx}', type_map.get(str(type)), True))
                schema = StructType(schema_list)
                sdf = spark.createDataFrame(dataframe, schema)
                sdf.write. \
                    format("com.microsoft.kusto.spark.datasource"). \
                    option("kustoCluster", self.cluster). \
                    option("kustoDatabase", self.database). \
