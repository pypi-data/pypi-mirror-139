...
class buggy(object):
    ...
    def __call__(self, fct):
        ...
        def wrapper(*args, **kwargs):
            logger.warning("%s is buggy, use at your own risk. Expected fix "
                           ": %s" % (fct.__name__, self.date))
            try:
                return_value = fct(*args, **kwargs)
            except Exception:
                print("%s Crashed : Didn't i told you it was buggy ?"
                      % (fct.__name__))
                raise
            return return_value
        return wrapper
...
class memoizehd(object):
    ...
    def __call__(self, fct):
        cachebase = os.path.join(self.basepath, str(hash(fct)) + "_" +
                                 str(os.getpid()))
        @wraps(fct)
        def wrapper(*args, **kwargs):
            cachefile = cachebase + ''.join([str(i) for i in map(hash, args)])
            cachefile += '_'.join([str(hash(kwargs[i])) for i in kwargs]) \
                         + ".npy"
            try:
                os.stat(cachefile)
                logger.info("Parameters hash matched calling -- %s --, "
                            "reading cached return from file " % \
                (fct.__name__))
