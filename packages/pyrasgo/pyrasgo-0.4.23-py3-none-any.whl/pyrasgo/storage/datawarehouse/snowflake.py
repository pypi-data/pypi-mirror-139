...
class SnowflakeDataWarehouse(DataWarehouse, Session, metaclass=DataWarehouseSession):
    ...
    def get_source_table(self, table_name: str, database: Optional[str] = None, schema: Optional[str] = None,
        ...
        result_set = self.execute_query(f"SELECT * FROM {database}.{schema}.{naming._quote_restricted_keywords(table_name)} {f'LIMIT {record_limit}' if record_limit else ''}")
    ...
    def grant_table_ownership(self, table: str, role: str, database: Optional[str] = None, schema: Optional[str] = None):
        ...
        database = database or self.database
        schema = schema or self.schema
        self.execute_query(f"GRANT OWNERSHIP ON {database}.{schema}.{naming._quote_restricted_keywords(table)} TO ROLE {role} REVOKE CURRENT GRANTS;")
    ...
    def grant_table_access(self, table: str, role: str, database: Optional[str] = None, schema: Optional[str] = None):
        ...
        database = database or self.database
        schema = schema or self.schema
        self.execute_query(f"GRANT SELECT ON {database}.{schema}.{naming._quote_restricted_keywords(table)} TO ROLE {role};")
    ...
    def clone_table(self, existing_table: str, new_table: str, overwrite: bool = False):
        ...
        """
        if overwrite:
            self.execute_query(f"CREATE OR REPLACE TABLE {naming._quote_restricted_keywords(new_table)} CLONE {naming._quote_restricted_keywords(existing_table)};")
        else:
            self.execute_query(f"CREATE TABLE {naming._quote_restricted_keywords(new_table)} CLONE {naming._quote_restricted_keywords(existing_table)};")
    ...
    def append_to_table(self, from_table: str, into_table: str):
        ...
        """
        self.execute_query(f"INSERT INTO {naming._quote_restricted_keywords(into_table)} SELECT * FROM {naming._quote_restricted_keywords(from_table)};")
    ...
    def _write_dataframe_to_table(self, df: pd.DataFrame, table_name: str, append: bool = False):
        ...
        dfutils._cleanse_sql_dataframe(df)
        write_pandas(conn=self.user_connection, df=df, table_name=naming._cleanse_sql_name(table_name), quote_identifiers=False)