...
class Datalake(Dao):
    ...
    def get_resource_url_on_platform(self,) -> str:
        ...
        return "{}/datalake/{}".format(self.connexion.base_url, self.id)
    ...
    def _init_upload_data(self, length, counter):
    ...
    def _upload_data(self, paths: List[str], tags: List[str], source: str = "sdk"):
        global shared_data_list
        global total_length
        global upload_counter
        ...
        for path in paths:
            try:
                ...
                r = self.connexion.post('/datalake/{}/data/add'.format(self.id), data=data).json()
                data_list.append(self._convert_response_to_data(r["data"]))
            except Exception as e:
                logger.error('\nCould not upload {} because {}'.format(path, str(e)))
                continue
        with upload_counter.get_lock():
            upload_counter.value += len(paths)
            print_next_bar(upload_counter.value, total_length)
    ...
    def upload_data(self, filepaths: Union[str, List[str]],
        ...
        nb_threads = min(len(filepaths), nb_threads)
        infos_split = list(mlt.chunks(filepaths, nb_threads))
        f = partial(self._upload_data, tags=tags, source=source)
        counter = Value('i', 0)
        print_next_bar(0, len(filepaths))
        with Pool(nb_threads, initializer=self._init_upload_data,
                initargs=(len(filepaths), counter,)) as pool:
            result = pool.map(f, infos_split)
        
        shared_data_list = list(chain.from_iterable(result))
    ...
    def list_data(self) -> MultiData:
        ...
        Returns:
            ...
            You can manipulate this object returned and add some tags or feeding a dataset.
        """
        r = self.connexion.get('/datalake/{}/data'.format(self.id)).json()
    ...
    def fetch_data(self, quantity: int = -1, tags: Union[str, List[str]] = [], no_tags: bool=False) -> MultiData:
        ...
        r = self.connexion.get('/datalake/{}/data'.format(self.id), params=params).json()
        list_data = list(map(self._convert_response_to_data, r["data"]))
