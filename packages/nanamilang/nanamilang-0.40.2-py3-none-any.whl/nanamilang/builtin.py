...
class BuiltinMacros:
    ...
    def require_macro(tree: list, local_env: dict, module_env, eval_function) -> list or Token:
        ...
        refer_names = []  # <---- 'refer: ['foo 'bar]; it means: create aliases only for _these_ names from the module
        refer_all = False  # <------- ':refer :all' toggle; it means: create aliases for all the names from the module
            ...
            if not evaluated.empty().truthy():  # <------------------------ if user form was like: (require ['stdlib])
                ...
                if require_opts:  # <--------------------------- if user form was like: (require ['stdlib ...opts...])
                    ...
                    if isinstance(require_refer_option_or_nil, datatypes.Vector):  # <-------- if option was :refer []
                        ...
                        refer_names = [symbol.reference() for symbol in require_refer_option_or_nil.items()]  # <-grab
                        ...
                        refer_all = True  # <------------------------------------------ set 'refer all' marker to true
                    else:
        ...
        loader.Loader.load(module_name, module_env, refer_names, refer_all)  # <-------- load requested module by name
