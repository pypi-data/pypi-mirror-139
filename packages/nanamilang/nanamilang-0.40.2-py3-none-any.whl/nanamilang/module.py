...
class Module:
    ...
    class EnvironStorage:
        ...
        def grab(self, m_ref: 'Module',
                 refer_names: list, refer_all: bool) -> None:
            """Module::Environ take another module, then grab environ from it"""

            global_names = m_ref.environ().storage().keys()  # <--- global names

            if refer_all:  # <--- it means basically: create alias for each name
                refer_names = global_names  # <----- will create aliases for all
            ...
            for g_name, data_type in m_ref.environ().storage().items():
                if g_name in refer_names:  # <- if we're supposed to create alias
                    self.set(f'{self._m_ref.name}/{g_name}',
                             data_type)  # <------------ actually create an alias
