...
def handle(exceptions: tuple, m_name: str, cs: list):
    ...
    :param cs: nanamilang macros/functions call stack
    """
    ...
    def evaluate(self, module_env) -> tuple:
        ...
        def recursive(local_env: dict,
            ...
            if isinstance(identifier_token_or_form, list):
                ...
                result_or_nexception = recursive(local_env, identifier_token_or_form)
                if isinstance(result_or_nexception, datatypes.NException):
                    return result_or_nexception
                # If expression form has been evaluated to Function, call it with all collected args
                ...
                if isinstance(result_or_nexception, datatypes.Keyword):
                    return BuiltinFunctions.get_func(
                        args + [result_or_nexception]
                        if len(args) == 1 else [args[0], result_or_nexception, args[1]])
            ...
            if identifier_token_or_form.identifier():
                ...
                # If final data type is Function, handle Function call with all collected arguments
                if isinstance(final, datatypes.Function):
                    return final.reference()(args)
