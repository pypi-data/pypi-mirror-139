...
class HashMap(Collection):
    ...
    @staticmethod
    def unpack(h) -> tuple:
        """NanamiLang HashMap, unpack reference"""

        if h.empty().truthy():
            return tuple()
        return shortcuts.partitioned2plain(
            (k, v) for k, v in h.reference().values())
    ...
    def conj(self, items: List['HashMap']) -> 'HashMap':
        ...
        return HashMap(tuple(
            list(HashMap.unpack(self))
            + list(shortcuts.partitioned2plain(map(HashMap.unpack, items)))))
