...
class Module:
    ...
    class EnvironStorage:
        ...
        def grab(self, m_ref: 'Module',
                 require_symbols: list, require_all: bool) -> None:
            """Module::Environ take another module and grab environ from there"""

            module_symbols = m_ref.environ().storage().keys()  # <----- all names

            if require_all:
                require_symbols = module_symbols

            assert set(require_symbols) <= set(module_symbols), (
                'Module::Environ::grab(): required symbols not in module symbols'
            )

            # TODO: this implementation maps __all__ possible global symbols amd
            #       if require_symbols/all is present, makes appropriate aliases

            self._m_mapping.update({m_ref.name(): m_ref})
            for g_name, data_type in m_ref.environ().storage().items():
                if g_name in require_symbols:
                    self.set(f'{self._m_ref.name}/{g_name}',
                             data_type)  # <----- assign global to current module
                self.set(f'{m_ref.name()}/{g_name}', data_type)  # update globals
