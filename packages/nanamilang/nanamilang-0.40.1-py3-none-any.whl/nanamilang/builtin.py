...
class BuiltinMacros:
    ...
    @meta({'name': 'comment',
           ...
           'docstring': 'Allows to replace all given forms with a Nil'})
    ...
    @meta({'spec': [[Spec.ArityIs, 2]],
           ...
           'docstring': 'Allows to globally bind a value to access it from everywhere'})
    ...
    def require_macro(tree: list, local_env: dict, module_env, eval_function) -> list or Token:
        ...
        if isinstance(evaluated, datatypes.Symbol):  # <--------------------- if user form was like: (require 'stdlib)
            ...
            if not evaluated.empty().truthy():  # <------------------------ if user form was like: (require ['stdlib])
                ...
                if require_opts:  # <--------------------------- if user form was like: (require ['stdlib ...opts...])
                    ...
                    require_opts_hash_map = datatypes.HashMap(tuple(require_opts))
                    ...
                    if isinstance(require_refer_option_or_nil, datatypes.Vector):  # <-------- if option was :refer []
                        ...
                        require_symbols = [data_type.reference() for data_type in require_refer_option_or_nil.items()]
                        ...
                        require_all = True  # <-------------------------------------- set 'require all' marker to true
                    else:
                        if not isinstance(require_refer_option_or_nil, datatypes.Nil):
        ...
        loader.Loader.load(module_name, module_env, require_symbols, require_all)  # <-- load requested module by name
    ...
    @meta({'spec': [[Spec.ArityVariants, [1, 2]]],
           'name': 'let',
           'forms': ['(let [b1 v2 ... bN vN])',
                     '(let [b1 v2 ... bN vN] body)'],
           'docstring': 'Allows to locally bind values to access them in block'})
...
class BuiltinFunctions:
    ...
    @meta({'spec': [[Spec.ArityIs, 2],
           ...
           'name': 'apply',
           'forms': ['(apply function arguments-vector)'],
    ...
    def apply_func(args: List[datatypes.Base]) -> datatypes.Base:
        ...
        arguments: datatypes.Vector
        function, arguments = args

        return function.reference()(arguments.items())  # <--- return result of the Function call with the arguments
    ...
    @meta({'name': 'conj',
           ...
           'docstring': 'Returns empty Vector; collection (maybe with the the items appended)'})
    ...
    def wrapped_func(args: List[datatypes.Function]) -> datatypes.Py3Object:
        ...
        def _adapted(_args: list, kwargs: dict):

            return (datatypes.HashMap((datatypes.Keyword('args'), datatypes.Py3Inst(_args),
                                       datatypes.Keyword('kwargs'), datatypes.Py3Inst(kwargs))),)

        return datatypes.Py3Object(lambda *_args, **kwargs: function.reference()(_adapted(args, kwargs)).reference())
    ...
    @meta({'spec': [[Spec.ArityIs, 1],
           ...
           'docstring': 'Returns imported Python 3 module wrapped as a Py3Object'})
