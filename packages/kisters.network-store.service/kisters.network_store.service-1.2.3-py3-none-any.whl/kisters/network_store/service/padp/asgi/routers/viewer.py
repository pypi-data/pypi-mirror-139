...
def get_viewer_router(network_store: NetworkStore):
    ...
    async def embedable_viewer(network_uid: str):
        """Create a bokeh network viewer widget that can be embedded"""
        try:
            return await _build_embedable_viewer(network_uid)
        except Exception as e:
            raise HTTPException(status.HTTP_418_IM_A_TEAPOT, str(e))

    async def _build_embedable_viewer(network_uid: str):
        fig = figure(
        ...
        for link in raw_links:
            ...
            if link["element_class"] in LINK_AS_NODE_TYPES:
                ...
                if len(vertices) >= 1:
                    ...
                    center = _centroid(source_node, target_node)
                nodes[link["uid"]] = {
                    "uid": link["uid"],
        ...
        for link in links:
            ...
            x = [
                nodes[link["source_uid"]]["location"]["x"],
                *(v["x"] for v in link.get("vertices", [])),
                nodes[link["target_uid"]]["location"]["x"],
            ]

            y = [
                nodes[link["source_uid"]]["location"]["y"],
                *(v["y"] for v in link.get("vertices", [])),
                nodes[link["target_uid"]]["location"]["y"],
            ]
        ...
        # Set viewport
        x = [node["location"]["x"] for node in nodes_list] + [
            v["x"] for link in links for v in link.get("vertices", [])
        ]
        y = [node["location"]["y"] for node in nodes_list] + [
