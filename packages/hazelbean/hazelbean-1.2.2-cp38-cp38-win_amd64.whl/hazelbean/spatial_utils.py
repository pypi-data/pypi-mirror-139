...
def create_gdal_virtual_raster_using_file(file_paths_list, output_tif_path, write_vrt_to_tif=True, bands='all', vrt_extent_shift_match_path=None, extent_shift_match_path=None, remove_generator_files=True,
...
def warp_raster_to_match(input_path,
    ...
    srs_sr_wkt = hb.get_raster_info(input_path)['projection']
...
def get_rank_array_and_keys(input_array, nan_mask=None, highest_first=True, ndv=None, verbose=True):
    ...
    if ndv is not None and nan_mask is None:
        ...
        valid_mask = np.where(input_array!=ndv, 1, 0)
    ...
    if nan_mask is not None and valid_mask is not None:
...
def reclassify_array(input_array, rules, not_in_dict_behavior='zero'):
    # Potentially faster than reclassify_flex but is NOT memory safe.
    raise NameError('TODOO Deprecated for MUCH faster reclassify_flex, thoug hperhaps I should extend that to also work from straight arrays?')


def reclassify_flex(input_flex, rules, output_path=None, output_data_type=None, array_threshold=10000, match_path=None, verbose=False):
    ...
    if output_data_type is None:
        ...
        if type(rules) is np.ndarray:
            ...
            output_data_type = 1
        elif type(rules[list(rules)[0]]) == np.int:
            output_data_type = 5
        ...
        #     np.ndarray:
        #      output_data_type = hb.numpy_type_to_gdal_number[rules[list(rules)[0]].dtype]
        # elif type()
    ...
    if isinstance(rules, dict):
        ...
        if max_key < array_threshold and max_value < array_threshold:
            ...
            if max_key <= 255 and max_value <= 255:
                ...
                rules = np.zeros(int(math.ceil(max_key)) + 1, dtype=hb.gdal_number_to_numpy_type[output_data_type])
        ...
        if isinstance(rules, dict):
            if isinstance(rules[list(rules)[0]], int):
                ...
                if rules[list(rules)[0]].dtype == np.uint8:
                    output_data_type = 1
                elif rules[list(rules)[0]].dtype == np.int:
                    output_data_type = 5
    ...
    if treat_input_as_arrayframe: # Then it has a path and needs to be done with raster calculation (but is memory safe)
        if isinstance(rules, dict):
            ...
            if input_flex.data_type == 1:
                if output_data_type == 1:
                    ...
                    hb.raster_calculator_hb(base_raster_path_band, hb.reclassify_uint8_to_uint8_by_dict, output_path,
                                            output_data_type, input_flex.ndv, read_datatype=1,
                                             gtiff_creation_options=hb.DEFAULT_GTIFF_CREATION_OPTIONS,
                                             calc_raster_stats=False)
                elif 2 <= output_data_type <= 5:
                    rules = {np.uint8(k): np.int(v) for k, v in rules.items()}
                    base_raster_path_band = [(input_flex.path, 1), (rules, 'raw')]
                    # TODOO This doesn't work because the iterblocks component automatically recasts it to the output type before running.
                    hb.raster_calculator_hb(base_raster_path_band, hb.reclassify_uint8_to_int_by_dict, output_path,
                                            output_data_type, input_flex.ndv, read_datatype=1,
                                            gtiff_creation_options=hb.DEFAULT_GTIFF_CREATION_OPTIONS,
                                            calc_raster_stats=False)
                    ...
                    hb.raster_calculator_hb(base_raster_path_band, hb.reclassify_uint8_to_float32_by_dict, output_path,
                                            output_data_type, input_flex.ndv, read_datatype=1,
                                            gtiff_creation_options=hb.DEFAULT_GTIFF_CREATION_OPTIONS,
                                            calc_raster_stats=False)
                    ...
                    hb.raster_calculator_hb(base_raster_path_band, hb.reclassify_uint8_to_float64_by_dict, output_path,
                                            output_data_type, input_flex.ndv, read_datatype=1,
                                            gtiff_creation_options=hb.DEFAULT_GTIFF_CREATION_OPTIONS,
                                            calc_raster_stats=False)
            elif 2 <= input_flex.data_type <= 5:
                if output_data_type == 1:
                    rules = {np.int(k): np.uint8(v) for k, v in rules.items()}
                    base_raster_path_band = [(input_flex.path, 1), (rules, 'raw')]

                    hb.raster_calculator_hb(base_raster_path_band, hb.reclassify_int_to_uint8_by_dict, output_path,
                                            output_data_type, input_flex.ndv, read_datatype=5,
                                            gtiff_creation_options=hb.DEFAULT_GTIFF_CREATION_OPTIONS,
                                            calc_raster_stats=False)
                elif 2 <= output_data_type <= 5:
                    rules = {np.int(k): np.int(v) for k, v in rules.items()}
                    base_raster_path_band = [(input_flex.path, 1), (rules, 'raw')]

                    hb.raster_calculator_hb(base_raster_path_band, hb.reclassify_int_to_int_by_dict, output_path,
                                            output_data_type, input_flex.ndv, read_datatype=5,
                                            gtiff_creation_options=hb.DEFAULT_GTIFF_CREATION_OPTIONS,
                                            calc_raster_stats=False)
                elif output_data_type == 6:

                    rules = {np.int(k): np.float32(v) for k, v in rules.items()}
                    base_raster_path_band = [(input_flex.path, 1), (rules, 'raw')]

                    hb.raster_calculator_hb(base_raster_path_band, hb.reclassify_int_to_float32_by_dict, output_path,
                                            output_data_type, input_flex.ndv, read_datatype=5,
                                            gtiff_creation_options=hb.DEFAULT_GTIFF_CREATION_OPTIONS,
                                            calc_raster_stats=False)
                elif output_data_type == 7:
                    rules = {np.int(k): np.float64(v) for k, v in rules.items()}
                    base_raster_path_band = [(input_flex.path, 1), (rules, 'raw')]
            ...
            if input_flex.data_type == 1:
                if output_data_type == 1:
                    ...
                    hb.raster_calculator_hb(base_raster_path_band, hb.reclassify_uint8_to_uint8_by_array, output_path,
                                            output_data_type, input_flex.ndv, read_datatype=1,
                                            gtiff_creation_options=hb.DEFAULT_GTIFF_CREATION_OPTIONS,
                                            calc_raster_stats=False)
                if 2 <= output_data_type <= 5:
                    base_raster_path_band = [(input_flex.path, 1), (rules.astype(np.int), 'raw')]
                    hb.raster_calculator_hb(base_raster_path_band, hb.reclassify_uint8_to_int_by_array, output_path,
                                            output_data_type, input_flex.ndv, read_datatype=1,
                                            gtiff_creation_options=hb.DEFAULT_GTIFF_CREATION_OPTIONS,
                                            calc_raster_stats=False)
                if output_data_type == 6:
                    base_raster_path_band = [(input_flex.path, 1), (rules.astype(np.float32), 'raw')]
                    hb.raster_calculator_hb(base_raster_path_band, hb.reclassify_uint8_to_float32_by_array, output_path,
                                            output_data_type, input_flex.ndv, read_datatype=1,
                                            gtiff_creation_options=hb.DEFAULT_GTIFF_CREATION_OPTIONS,
                                            calc_raster_stats=False)
                if output_data_type == 6:
                    base_raster_path_band = [(input_flex.path, 1), (rules.astype(np.float64), 'raw')]
                    hb.raster_calculator_hb(base_raster_path_band, hb.reclassify_uint8_to_float64_by_array, output_path,
                                            output_data_type, input_flex.ndv, read_datatype=1,
                                            gtiff_creation_options=hb.DEFAULT_GTIFF_CREATION_OPTIONS,
                                            calc_raster_stats=False)
            elif 2 <= input_flex.data_type <= 5:
                if output_data_type == 1:
                    ...
                    hb.raster_calculator_hb(base_raster_path_band, hb.reclassify_int_to_uint8_by_array, output_path,
                                            output_data_type, input_flex.ndv, read_datatype=5,
                                            gtiff_creation_options=hb.DEFAULT_GTIFF_CREATION_OPTIONS,
                                            calc_raster_stats=False)
                elif 2 <= output_data_type <= 5:
                    base_raster_path_band = [(input_flex.path, 1), (rules.astype(np.int8), 'raw')]
                    hb.raster_calculator_hb(base_raster_path_band, hb.reclassify_int_to_int_by_array, output_path,
                                            output_data_type, input_flex.ndv, read_datatype=5,
                                            gtiff_creation_options=hb.DEFAULT_GTIFF_CREATION_OPTIONS,
                                            calc_raster_stats=False)
                elif output_data_type == 6:
                    ...
                    hb.raster_calculator_hb(base_raster_path_band, hb.reclassify_int_to_float32_by_array, output_path,
                                            output_data_type, input_flex.ndv, read_datatype=5,
                                            gtiff_creation_options=hb.DEFAULT_GTIFF_CREATION_OPTIONS,
                                            calc_raster_stats=False)
                elif output_data_type == 7:
                    base_raster_path_band = [(input_flex.path, 1), (rules.astype(np.float64), 'raw')]
                    hb.raster_calculator_hb(base_raster_path_band, hb.reclassify_int_to_float64_by_array, output_path,
                                            output_data_type, input_flex.ndv, read_datatype=5,
                                            gtiff_creation_options=hb.DEFAULT_GTIFF_CREATION_OPTIONS,
                                            calc_raster_stats=False)
            elif input_flex.data_type == 6:
        ...
        if isinstance(rules, dict):
            ...
            if input_flex.data_type == 1:
                if output_data_type == 1:
                    rules = {np.uint8(k): np.uint8(v) for k, v in rules.items()}
                    output_array = np.asarray(hb.reclassify_uint8_to_uint8_by_dict(input_flex.data, rules))
                elif 2 <= output_data_type <= 5:
                    rules = {np.uint8(k): np.int(v) for k, v in rules.items()}
                    output_array = np.asarray(hb.reclassify_uint8_to_int_by_dict(input_flex.data, rules))
                elif output_data_type == 6:
                    rules = {np.uint8(k): np.float32(v) for k, v in rules.items()}
                    output_array = np.asarray(hb.reclassify_uint8_to_float32_by_dict(input_flex.data, rules))
                elif output_data_type == 7:
                    rules = {np.uint8(k): np.float64(v) for k, v in rules.items()}
                    output_array = np.asarray(hb.reclassify_uint8_to_float64_by_dict(input_flex.data, rules))
            elif 2 <= input_flex.data_type <= 5:
                if output_data_type == 1:
                    rules = {np.int(k): np.uint8(v) for k, v in rules.items()}
                    output_array = np.asarray(hb.reclassify_int_to_uint8_by_dict(input_flex.data, rules))
                elif 2 <= output_data_type <= 5:
                    rules = {np.int(k): np.int(v) for k, v in rules.items()}
                    output_array = np.asarray(hb.reclassify_int_to_int_by_dict(input_flex.data, rules))
                elif output_data_type == 6:
                    rules = {np.int(k): np.float32(v) for k, v in rules.items()}
                    output_array = np.asarray(hb.reclassify_int_to_float32_by_dict(input_flex.data, rules))
                elif output_data_type == 7:
                    rules = {np.int(k): np.float64(v) for k, v in rules.items()}
                    output_array = np.asarray(hb.reclassify_int_to_float64_by_dict(input_flex.data, rules))
            elif input_flex.data_type == 6:
                raise NameError('reclassify unable to handle floats yet. Unimplemented. ALSO NOTE, if you attempt to fix this later, the rules as array method DOES NOT WORK FOR FLOATS BECAUSE OF THE INDICES NEED TO BE FLOATS, so have to use the dict() method.. ' + str(input_flex))
            elif input_flex.data_type == 7:
                raise NameError('reclassify unable to handle floats yet. Unimplemented. ALSO NOTE, if you attempt to fix this later, the rules as array method DOES NOT WORK FOR FLOATS BECAUSE OF THE INDICES NEED TO BE FLOATS, so have to use the dict() method.. ' + str(input_flex))
            ...
            if input_flex.data_type == 1:
                if output_data_type == 1:
                    output_array = np.asarray(hb.reclassify_uint8_to_uint8_by_array(input_flex.data, rules.astype(np.uint8)))
                if 2 <= output_data_type <= 5:
                    output_array = np.asarray(hb.reclassify_uint8_to_int_by_array(input_flex.data, rules.astype(np.int)))
                if output_data_type == 6:
                    output_array = np.asarray(hb.reclassify_uint8_to_float32_by_array(input_flex.data, rules.astype(np.float32)))
                if output_data_type == 7:
                    output_array = np.asarray(hb.reclassify_uint8_to_float64_by_array(input_flex.data, rules.astype(np.float64)))
            elif 2 <= input_flex.data_type <= 5:
                if output_data_type == 1:
                    output_array = np.asarray(hb.reclassify_int_to_uint8_by_array(input_flex.data, rules.astype(np.uint8)))
                if 2 <= output_data_type <= 5:
                    output_array = np.asarray(hb.reclassify_int_to_int_by_array(input_flex.data, rules.astype(np.int)))
                if output_data_type == 6:
                    output_array = np.asarray(hb.reclassify_int_to_float32_by_array(input_flex.data, rules.astype(np.float32)))
                if output_data_type == 7:
                    output_array = np.asarray(hb.reclassify_int_to_float64_by_array(input_flex.data, rules.astype(np.float64)))
            elif input_flex.data_type == 6:
                raise NameError('reclassify unable to handle floats yet. Unimplemented. ALSO NOTE, if you attempt to fix this later, the rules as array method DOES NOT WORK FOR FLOATS BECAUSE OF THE INDICES NEED TO BE FLOATS, so have to use the dict() method.. ' + str(input_flex))
            elif input_flex.data_type == 7:
                raise NameError('reclassify unable to handle floats yet. Unimplemented. ALSO NOTE, if you attempt to fix this later, the rules as array method DOES NOT WORK FOR FLOATS BECAUSE OF THE INDICES NEED TO BE FLOATS, so have to use the dict() method.. ' + str(input_flex))
...
def extract_global_netcdf_to_geotiff(input_nc_path,
                                     output_dir,
...
def describe_netcdf(input_nc_path, return_as_string=True):
    # Like original funciton but intended to fail gracefully and informatively so can work through the complex index structure.
    ds = netCDF4.Dataset(input_nc_path, 'r')

    return_string = '\nDescription of netcdf file at ' + str(input_nc_path)
    return_string += '\n'
    ds_dict = ds.__dict__
    return_string += '  NC metadata dict:\n'
    for k, v in ds_dict.items():
        return_string += '    ' + str(k) + ': ' + str(v).replace('\n', '') + '\n'

    return_string += '  NC Dimensions:\n'
    for k, v in dict(ds.dimensions).items():
        return_string += '    ' + str(k) + ': ' + str(v).replace('\n', '') + '\n'

    vars_dict = dict(zip(ds.variables.keys(), ds.variables.values()))
    return_string += '  NC variables:\n'
...
def extract_global_netcdf_to_geotiff_interactive(input_nc_path, output_dir, vars_to_extract=None, time_indices_to_extract=None, ndv=None):
    # Like original funciton but intended to fail gracefully and informatively so can work through the complex index structure.
    ds = netCDF4.Dataset(input_nc_path, 'r')
    L.info('Loaded nc file ', input_nc_path)

    ds_dict = ds.__dict__
    L.info('  NC metadata dict:\n', hb.pp(ds_dict, return_as_string=True))
    ...
    L.info('  NC dimensions', hb.pp(dims_dict, return_as_string=True))

    vars_dict = dict(zip(ds.variables.keys(), ds.variables.values()))
    L.info('  NC variables', hb.pp(vars_dict, return_as_string=True))
    ...
    # START HERE, the time years were stored as DATA... use this to label the rasters? Consider how to make this generalized.
    ...
    for var_name, var in ds.variables.items():
        ...
        for time_index in time_indices_to_extract:
            ...
            if var_name not in dim_names:
                ...
                output_geotiff_path = os.path.join(output_dir, var_name + '_' + str(time_index) + '.tif')
                hb.save_array_as_geotiff(array, output_geotiff_path, ndv=ndv, data_type=7, compress=True, n_cols=array.shape[1], n_rows=array.shape[0],
                                         geotransform_override=geotransform, projection_override=projection)


def netcdf_to_geotiffs(input_nc_path, output_dir, vars_to_extract=None, var_rename_dict=None, time_indices_to_extract=None, time_rename_op=None, ndv=None, data_type=None, return_only_proposed_filenames=False, verbose=True):
    # Like original funciton but intended to fail gracefully and informatively so can work through the complex index structure.
    ds = netCDF4.Dataset(input_nc_path, 'r')
    L.info('Loaded nc file ', input_nc_path)
    if verbose:
