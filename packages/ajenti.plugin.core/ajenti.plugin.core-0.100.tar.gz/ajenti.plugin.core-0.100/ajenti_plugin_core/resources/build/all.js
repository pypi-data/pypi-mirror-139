...
angular.module('core').controller('CorePWResetMailController', function ($scope, $log, $rootScope, $routeParams, $http, $window, identity, notify, gettext, customization) {
    ...
    $scope.send_email_reset = function () {
...
angular.module('core').controller('CorePWResetController', function ($scope, $log, $rootScope, $routeParams, $window, $http, identity, notify, gettext, customization) {
    ...
    $http.post('/api/master/check_password_serial', { serial: $scope.serial }).then(function (resp) {
        $scope.ready = true;
    ...
    $scope.save_password = function () {
        $http.post('/api/core/check_password_complexity', { password: $scope.password2 }).then(function (resp) {
            $http.post('/api/master/update_password', { serial: $scope.serial, password: $scope.password2 }).then(function (resp) {
...
angular.module('core').service('config', function ($http, $q, initialConfigContent) {
    ...
    this.getSmtpConfig = function () {
        return $http.get("/api/core/smtp-config").then(function (response) {
            return response.data;
        });
    };

    this.setSmtpConfig = function (config) {
        return $http.post("/api/core/smtp-config", config).then(function (response) {
            return response.data;
        });
    };
