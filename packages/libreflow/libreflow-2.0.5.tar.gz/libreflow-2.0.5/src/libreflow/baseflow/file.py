...
class FileSystemMap(EntityView):
        ...
        if (
            ...
            cursor = (
                ...
                .aggregate([
                    ...
                    {
                        '$lookup': {
                            'from': self.statutes_collection_name(),
                            'let': { 'pattern': { '$concat': ['^', '$name', '/history/revisions/v\d\d\d/sync_statutes/', self.root().project().admin.multisites.current_site_name.get()]}},
                            'pipeline': [
                                {
                                    '$match': {
                                        '$expr': {
            ...
            for n, d in name_and_doc:
                ...
                if d['last_revision']:
                    d['last_revision'] = d['last_revision'][0]
                
                    for ss in d['sync_statutes']:
                        if ss['name'].startswith(d['last_revision']['name']):
                            d['last_revision']['current_sync_status'] = ss['status']
                        else:
                            d['last_revision']['current_sync_status'] = 'NotAvailable'
    ...
    def columns(self):
        return ['Name', 'Last revision', 'Comment']
    ...
    def _fill_row_cells(self, row, item):
        ...
        if last_revision_data:
            row['Last revision'] = last_revision_data['name'].rsplit('/', maxsplit=1)[-1]
            row['Comment'] = last_revision_data['comment']

    def _fill_row_style(self, style, item, row):
        style['Name_icon'] = item.get_icon(extension=self._document_cache[item.oid()]['format'])
        style['Name_activate_oid'] = item.open.oid()
        style['Last revision_icon'] = self._get_status_icon(item)

        last_revision_data = self._document_cache[item.oid()]['last_revision']

        if last_revision_data:
            style['Last revision_foreground-color'] = item.get_revisions().STYLE_BY_STATUS[
                last_revision_data['current_sync_status']
            ][0]
        
        style['Comment_activate_oid'] = item.show_history.oid()
