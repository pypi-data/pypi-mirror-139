...
def generate_nmf_reconstruction(n_comp: int, n_feat: int, n_obs: int, h_icorr_min: float = 0.5,
                                ...
                                random_norms: bool = True, epsilon: float = 0.10, n_clust_w: int = 1,
                                n_clust_h: int = 1) -> Tuple[pd.DataFrame]:
    """
    ...
    n_clust_w: int = 1
        The number of clusters in W. Must be less than or equal to n_comp
    n_clust_h: int = 1
        The number of clusters in H. Must be less than or equal to n_comp.
        n_clust_w == ncomp or n_clust_h == ncomp must hold.
    Returns
    -------
    4 dataframes representing respectively M, W, H, E
    """
    n_clust_h = min(max(n_clust_h, 1), n_comp)
    n_clust_w = min(max(n_clust_w, 1), n_comp)
    assert (n_clust_h == n_comp or n_clust_w == n_comp)
