...
export default class ChunkedUploadStorage extends BaseStorage {
  constructor(newUploadEndpoint, getUploadsEndpoint) {
    super();
  ...
  static initiateUpload(upload, endpoint = null) {
    ...
    // Initial request to retrieve the upload infos.
    if (!_endpoint) {
  ...
  async save(upload) {
    ...
    if (!upload.chunkCount) {
      try {
        await ChunkedUploadStorage.initiateUpload(upload);
      } catch (error) {
        ...
        if (error.request.status === 409) {
          try {
            // Restart the upload.
            await ChunkedUploadStorage.initiateUpload(upload, error.response.data.file._actions.edit_data);
    ...
    try {
      ...
      if (error.request.status === 413) {
        ...
        kadi.alert($t('Error finishing upload.'), {request: error.request});
      }
      this.pauseCallback(true, upload);
      return;
    }
  ...
  getUploadProgress(upload, additionalSize = 0) {
    ...
    return ((ChunkedUploadStorage.getTotalChunkSize(upload) + additionalSize) / upload.size) * 100;
  }
  ...
  isResumable(upload) {
    ...
    return upload.state === 'paused'
      && (upload.file !== null || ChunkedUploadStorage.getTotalChunkSize(upload) === upload.size);
  }
    ...
    return axios.get(this.getUploadsEndpoint)
      .then((response) => {
        ...
        response.data.items.forEach((upload) => {
          upload.storageType = upload.storage_type;
  ...
  finalizeUpload(upload) {
    ...
    const _updateStatus = () => {
      ...
      axios.get(upload.getStatusEndpoint)
        ...
        .catch((error) => {
          kadi.alert($t('Error updating upload status.'), {request: error.request});
        });
    };
  ...
  async uploadChunks(upload) {
    ...
    while (true) {
      ...
      const chunkIndex = ChunkedUploadStorage.findNextChunkIndex(upload);
      ...
      while (true) {
        ...
        try {
          ...
          // Check if the request was canceled from outside.
          ...
          if (error.request.status === 413) {
            // Use the error message from the backend.
            kadi.alert(error.response.data.description, {type: 'warning'});
            this.pauseCallback(false, upload);
            return false;
          }

          timeout += 5000;

          const timeoutMsg = $t('Retrying in {{timeout}} seconds.', {timeout: timeout / 1000});
          kadi.alert(`${errorMsg} ${timeoutMsg}`, {request: error.request, timeout});
