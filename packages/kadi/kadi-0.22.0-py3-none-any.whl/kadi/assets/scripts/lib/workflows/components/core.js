...
class BaseComponent extends Rete.Component {
  ...
  builder(node) {
    ...
    // Retrieve any potential properties from flow nodes and delete them from the data property.
    if (node.data._meta) {
  ...
  fromFlow(flowNode) {
    const node = {
      ...
      // Only some properties are passed to the created node and therefore the builder function, so we use the data
