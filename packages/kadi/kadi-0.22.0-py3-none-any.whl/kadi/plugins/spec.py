...
@hookspec
def kadi_get_resource_nav_items(resource):
    """Hook for collecting templates for navigation menu items of resources.

    For resource types where no navigation items should be collected, ``None`` can be
    returned instead.

    :param resource: The resource which the navigation menu belongs to, either a
        :class:`.Record`, :class:`.File`, :class:`.Collection`, :class:`.Group` or
        :class:`.Template`.
    """
