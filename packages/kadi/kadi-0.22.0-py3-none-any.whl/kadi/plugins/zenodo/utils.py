...
class BaseZenodoForm(KadiForm):
    ...
    export_filter = ExportFilterField()
...
class RecordZenodoForm(BaseZenodoForm):
    ...
    package_data = BooleanField(
        ...
        description=_l(
            "Upload all record data in a single ZIP archive rather than invidiually."
        ),
    )
...
class CollectionZenodoForm(BaseZenodoForm):
    ...
    package_data = BooleanField(
        ...
        description=_l(
            ...
            " invidiual archive for each record."
        ),
    )
...
class ZipStreamGenerator(BaseStreamGenerator):
    ...
    def __init__(
        ...
        # The ZIP stream needs to keep track of its size, since we need to specify the
        # "Content-Length" header for the Zenodo upload to work correctly (see also
        # https://github.com/zenodo/zenodo/issues/1986). Unfortunately, the archive
        # cannot be compressed when doing this.
