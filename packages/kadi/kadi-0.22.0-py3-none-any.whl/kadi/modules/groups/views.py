...
from kadi.modules.permissions.tasks import start_apply_role_rules_task
from kadi.modules.permissions.utils import create_username_role_rule
...
def manage_members(id, qparams):
    ...
    members_form = AddMembersForm(_suffix="members")
    rules_form = AddRulesForm(_suffix="rules")

    if qparams["tab"] == "members" and members_form.validate_on_submit():
        add_roles(User, members_form.users.data, group, members_form.role.data)
        db.session.commit()

        flash(_("Changes saved successfully."), "success")
        return redirect(url_for("groups.manage_members", id=group.id))

    if rules_form.validate_on_submit():
        ...
        if role_rule:
            ...
            if rules_form.retroactive.data:
                if not start_apply_role_rules_task(role_rule=role_rule):
                    flash(_("Could not apply rule retroactively."), "danger")
...
def view_revision(group_id, revision_id):
    ...
    revision = Group.revision_class.query.get_or_404(revision_id)
