...
def download_file(file):
    ...
    # support range responses, which will probably require special handling for
    # different storages in the future. Furthermore, the "conditional" flag seems to be
    # only necessary when using the dev server to support range responses, otherwise the
    # web server handles that via "X-Sendfile".
    return send_file(
        filepath,
...
def stream_files(record):
    """Stream all files of a record to a client for downloading.

    All files will be compressed in a ZIP archive on the fly. Note that due to this, the
    response will not include a "Content-Length" header, as the final size of the
    archive is not known at this point in time, and "X-Sendfile" cannot be used either.

    :param record: The record the files belong to.
    :return: The response object.
    """
    zip_stream = ZipStream(compress_type=zipfile.ZIP_DEFLATED)
