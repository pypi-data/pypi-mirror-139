            ...
            if isinstance(obj, cls) and session.is_modified(obj):
                ...
                if getattr(obj, "state", "active") != "active":
                    ...
                    if not history.deleted or history.deleted[0] != "active":
                        # Do not return here, as that seems to break the event chain.
                        update_timestamp = False

                if update_timestamp:
                    # Do not update the timestamp if only changes in relationships
                    # occured and none of the related objects' state (if present) is
                    # active.
...
class NestedTransaction:
    ...
    savepoint yet. If the release produces an error, the savepoint will be rolled back
    automatically. The ``success`` attribute of the transaction can be used to check the
...
def aquire_lock(obj):
    ...
    UPDATE`` until the session is committed or rolled back. Once the lock is aquired,
...
def has_extension(extension_name):
    ...
    :return: ``True`` if the extension is installed, ``False`` if the extension is not
        installed, the database is not reachable or the database is not PostgreSQL.
    """
