...
def compact_json(data):
    ...
    Note that unicode characters contained in the given data will be output as-is rather
    than them being escaped.
...
def signal_resource_change(resource, user=None, created=False):
    ...
    Generally, it is supposed to be run after a resource was created or changed and the
    change triggered the creation of a new revision.

    Note that this function may issue a database rollback.

    :param resource: The resource that was created or changed.
    :param user: (optional) The user that triggered the creation or change. Defaults to
        the current user.
...
def flatten_list(values):
    ...
    Flattens a list containing single values or nested lists. A nested list in a nested
