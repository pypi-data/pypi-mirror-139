...
#            self.vertact[(L[g2]*h,oldvertno+g1*len(Gr.gamma.genera)+j)]=oldvertno+di[(L[g2]*h,g1)]*len(Gr.gamma.genera)+j
    ...
    # returns a list of all possible codimension 1 degenerations of the Gstgraph self occurring at the vertex v (and the elements of the orbit of v under G
                        ...
                        for n1_add in range(1, floor(QQ(n) / n1_min + 1)):
                            for n2_add in range(1, floor(QQ(n) / n2_min + 1)):
                                ...
                                if n1.divides(n) and n2.divides(n) and lcm(n1, n2) == n:
                                    ...
                                    g1 = floor((n1 * (2 * dege.genera(0) - 2) + n1 *
                                               (sum([1 - QQ(1) / e[alpha] for alpha in I1]) + 1 - QQ(1) / e_new)) / QQ(2) + 1)
                                    g2 = floor((n2 * (2 * dege.genera(1) - 2) + n2 *
                                               (sum([1 - QQ(1) / e[alpha] for alpha in I2]) + 1 - QQ(1) / e_new)) / QQ(2) + 1)
                                    ...
                                    for md in mark_dist:
                                        ...
                                        for alpha in range(len(I1)):
                                            for j in range(QQ(n) / e[I[alpha]]):
                                                new_legs_plus[(md[alpha] + j) % (QQ(n) / n1)
                                                              ] += [self.legact[(NumtoG[j], I[alpha])]]
                                        for alpha in range(len(I1), len(I)):
                                            for j in range(QQ(n) / e[I[alpha]]):
                                                new_legs_plus[(md[alpha] + j) % (QQ(n) / n2) + (QQ(n) / n1)
                                                              ] += [self.legact[(NumtoG[j], I[alpha])]]
                                ...
                                for e_new in n0.divisors():
                                    for g0 in [x for x in range(floor(QQ(n) / (2 * n0) + 1)) if gcd(x, QQ(n) / n0) == 1]:
                                        for nu1 in [j for j in range(e_new) if gcd(j, e_new) == 1]:
                                            ...
                                            # first: use Riemann-Hurwitz to compute the genera of the vertices above 0,1
                                            gen0 = floor(
                                                (n0 * (2 * dege.genera(0) - 2) + n0 * (sum([1 - QQ(1) / e[alpha] for alpha in I]) + 2 - QQ(2) / e_new)) / QQ(2) + 1)
                                            ...
                                            for md in mark_dist:
                                                ...
                                                for alpha in range(len(I)):
                                                    for j in range(QQ(n) / e[I[alpha]]):
                                                        new_legs_plus[(md[alpha] + j) % (QQ(n) / n0)
                                                                      ] += [self.legact[(NumtoG[j], I[alpha])]]
...
class KappaPsiPolynomial(object):
    ...
    def _monomial_str(self, kappa, psi):
        r"""
        ...
        for v, kappa_v in enumerate(kappa):
            ...
            for deg, power in enumerate(kappa_v):
                if power:
                    deg += 1
                    kappa_str.append('kappa_%d^%d' % (deg, power) if power != 1 else 'kappa_%d' % deg)
            if kappa_str:
                result.append('({})_{}'.format('*'.join(kappa_str), v))

        # psi part
        for i, j in psi.items():
            result.append('psi_%d^%d' % (i, j) if j != 1 else 'psi_%d' % i)
...
@cached_function
def generating_indices(g, n, r, FZ=True, moduli='st'):
    r"""
    Returns the indices of a basis of `R^r(\bar M_{g,n})` in terms of the standard list of generators.
    This assumes that the generalized Faber-Zagier relations [PPZ15]_ are a complete set of relations.
    ...
    if FZ or r <= (3 * g - 3 + n) / QQ(2) or not (g <= 1 or (g == 2 and n <= 19)):
        rel = matrix(DR.list_all_FZ(g, r, tuple(range(1, n + 1)), moduli_type=get_moduli(moduli, DRpy=True)))
...
def Hidentify(g, dat, method='pull', vecout=False, redundancy=False, markings=None):
    ...
    TESTS::
      ...
      Hidentify(2, H, markings=[2], vecout=True)
    ...
    if method == 'pull' and not found:
        # indices of generators in rank in which forgetful_* \bar H sits
        ...
        if kpinters:
            b += [(bar_H * kppo).evaluate() for kppo in kppolynomials]
        ...
        result = R.from_basis_vector(x, 3 * g - 3 + N - r)
...
class prodtautclass(object):
    ...
    def partial_pushforward(self, gamma0, dicv, dicl):
        ...
        for v in range(gamma0.num_verts()):
            preimgr[v].rename_legs(rndics[v], shift=preimgr[v]._maxleg)
    ...
    def factor_reconstruct(self, m, otherfactors):
        ...
        result = R.zero()
        for t in self.terms:
            result += prod([(t[oi] * othertestclass[oi]).evaluate() for oi in otherindices]) * R([t[m]])
        result.simplify()
...
def FZresulttable(output='betti'):
    ...
    if output == 'betti':
        ...
        bettidict = {(a[0], a[1], a[2]): len(gicache[(a, b)]) for a, b in dict(gicache)}
    if output == 'FZconfirm':
        gicache = FZ_conjecture_holds.cache
        bettidict = {(a[0], a[1], a[2]): gicache[(a, b)] for a, b in dict(gicache)}
...
PICKLED_FUNCTIONS = [generating_indices, genstobasis, FZ_conjecture_holds]
    ...
    try:
        with open('new_geninddb.pkl', 'rb') as file:
            ...
            for k, cache in dumped.items():
                # add some lines to account for additional argument in 'generating_indices' and 'genstobasis'
                if (k == 0 and any(len(c[0]) == 4 for c in cache) or k == 1 and any(len(c[0]) == 3 for c in cache)):
                    from sage.misc.cachefunc import NonpicklingDict
                    d = NonpicklingDict()
                    for u1, u2 in cache:
                        d[(u1 + ('st',), u2)] = cache[(u1, u2)]
                    cache = d
...
def kappaclass(a, g=None, n=None):
    ...
    TESTS::
      ...
      See https://gitlab.com/jo314schmitt/admcycles/-/merge_requests/109 for details.
      sage: a = kappaclass(2)
