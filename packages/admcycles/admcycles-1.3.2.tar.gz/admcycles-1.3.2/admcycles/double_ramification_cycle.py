...
def DR_cycle(g, Avector, d=None, k=None, rpoly=False, tautout=True, basis=False, chiodo_coeff=False, r_coeff=None, moduli='st', base_ring=QQ):
    r"""Returns the k-twisted Double ramification cycle in genus g and codimension d
    ...
    if any(isinstance(ai, MPolynomial) for ai in Avector):
        ...
        pol, gens = DRpoly(g, d, n, tautout=tautout, basis=basis, gensout=True,
                           chiodo_coeff=chiodo_coeff, moduli=moduli)
        subsdict = {gens[i]: Avector[i] for i in range(n)}

        if tautout:
            # TODO: with merge requests 109! the substitution do not modify in place anymore
            pol = pol.subs(subsdict)
            pol.simplify()
            return pol
    ...
    v = vector([DR_coeff_new(decs, g, n, d, Avector, k, rpoly, chiodo_coeff, r_coeff) for decs in gens])
    if not chiodo_coeff:
        v *= 1 / ZZ(2)**d
    # v1=vector([QQ(a) for a in DR.convert_vector_to_monomial_basis(v,g,d,tuple(range(1, n+1)),DR.MODULI_ST)])
...
def DR_coeff_new(G, g, n, d, Avector, k, rpoly, chiodo_coeff, r_coeff):
    ...
    mpoly = mpoly_special(exp_list, h0, u, Vbasis, m0 + 1, deg, chiodo_coeff, r_coeff, scalar_factor, exp_list_fine, d)
...
def mpoly_special(exp_list, h0, u, Vbasis, start, degr, chiodo_coeff, r_coeff, scalar_factor, exp_list_fine, d):
    r"""
...
def multivariate_interpolate(f, d, n, gridwidth=1, R=None, generator=None):
    ...
    for p in points:
        ...
        lagr = prod([prod([generator[i] - (j * gridwidth - shift)
...
def DRpoly(g, d, n, dplus=0, tautout=True, basis=False, ring=None, gens=None, gensout=False, chiodo_coeff=False, moduli='st'):
    r"""
    ...
    """
    def f(*Avector):
        return DR_cycle(g, Avector, d, tautout=False, basis=basis, moduli=moduli)
