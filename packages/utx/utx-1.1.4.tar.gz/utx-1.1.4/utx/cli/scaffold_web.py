...
def create_scaffold_web(project_name):
    ...
    create_folder(os.path.join(project_name, "driver"))
...
from run import cli_headless, cli_driver_path

ST.PROJECT_ROOT = BASE_DIR
is_headless, browser_driver_path = web_info(cli_headless=cli_headless,
                                            cli_driver=cli_driver_path,
                                            ini_headless=ini.getvalue('web_info', 'headless'),
                                            ini_driver=DRIVER_PATH)


@pytest.fixture(scope="session", autouse=True)
def web_init(request):
    driver = web_fixture(request, driver_path=browser_driver_path, headless=is_headless)
    return driver
"""
...
cli_headless, cli_driver_path = cli_web_env()
...
from utx.core.utils.tools import decryption, selector_v2
...
class TestWeb:
    ...
    def test_ui_web(self, case, web_init):
        ...
        result, dev = run_web_air(case=case, log_path=LOG_PATH, case_path=CASE_PATH,
                                  base_dir=BASE_DIR,
                                  static_dir=decryption('aHR0cHM6Ly9jZG4uanNkZWxpdnIubmV0L2doL29wZW51dHgvc3RhdGljLw=='))
