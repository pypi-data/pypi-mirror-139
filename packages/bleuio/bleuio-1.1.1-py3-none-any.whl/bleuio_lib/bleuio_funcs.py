...
class BleuIo(object):
    ...
    def __poll_serial(self):
        ...
        if not self.rx_buffer.isspace():
            if self.rx_buffer.__contains__(str.encode("\r\n")):
                ...
                if self._scanning:
                    self.rx_scanning_results.append(self.rx_buffer.decode("utf-8", "ignore"))
                if self._spsstream:
                    self.rx_sps_results.append(self.rx_buffer.decode("utf-8", "ignore"))
                if not self.rx_state == "rx_ready" and not self._scanning:
    ...
    def __ata(self, isOn):
        ...
        while not self.rx_state.__eq__("rx_ready"):
            if self.rx_response.__contains__("Show Ascii Off!\r\n") or "Show Ascii On!\r\n":
                # self.__threadLock.acquire()
                self.__set_rx_state("rx_ready")
                # self.__threadLock.release()
    ...
    def __atds(self, isOn):
        ...
        while not self.rx_state.__eq__("rx_ready"):
            if self.rx_response.__contains__("Auto discover Services On!\r\n") or "Auto discover Services Off!\r\n":
                self.__set_rx_state("rx_ready")
                response = self.rx_response
            if self.rx_buffer.decode("utf-8", "ignore").__contains__("\r\nERROR\r\n"):
                self.__set_rx_state("rx_ready")
    ...
    def __at_gapdisconnectall(self):
        ...
        while not self.rx_state.__eq__("rx_ready"):
            if self.rx_buffer.decode("utf-8", "ignore").__contains__("All connections terminated."):
                self.__set_rx_state("rx_ready")
                response = self.rx_response
            if self.rx_buffer.decode("utf-8", "ignore").__contains__("\r\nERROR\r\n"):
                self.__set_rx_state("rx_ready")
    ...
    def __at_gapiocap(self, io_cap):
        ...
        while not self.rx_state.__eq__("rx_ready"):
            if self.rx_buffer.decode("utf-8", "ignore").__contains__("\r\nI/O CAP ="):
                self.__set_rx_state("rx_ready")
                response = self.rx_response
            if self.rx_buffer.decode("utf-8", "ignore").__contains__("\r\nERROR\r\n") or self.rx_buffer.decode("utf-8",
                                                                                                               "ignore").__contains__(
    ...
    def __at_gappair(self, bonded):
        ...
        while not self.rx_state.__eq__("rx_ready"):
            if bonded:
                if self.rx_buffer.decode("utf-8", "ignore").__contains__(
                        "\r\nBONDING SUCCESS") or self.rx_buffer.decode("utf-8", "ignore").__contains__(
                        "\r\nBLE_EVT_GAP_NUMERIC_REQUEST") or self.rx_buffer.decode("utf-8", "ignore").__contains__(
                        "\r\nBLE_EVT_GAP_PASSKEY_REQUEST"):
                    self.__set_rx_state("rx_ready")
                    response = self.rx_response
            else:
                if self.rx_buffer.decode("utf-8", "ignore").__contains__(
                        "\r\nPAIRING SUCCESS") or self.rx_buffer.decode("utf-8", "ignore").__contains__(
                        "\r\nBLE_EVT_GAP_NUMERIC_REQUEST") or self.rx_buffer.decode("utf-8", "ignore").__contains__(
                        "\r\nBLE_EVT_GAP_PASSKEY_REQUEST"):
                    self.__set_rx_state("rx_ready")
                    response = self.rx_response
    ...
    def __at_get_conn(self):
        ...
        while not self.rx_state.__eq__("rx_ready"):
            if self.rx_buffer.decode("utf-8", "ignore").__contains__("Our Role:") or self.rx_buffer.decode("utf-8",
                                                                                                           "ignore").__contains__(
    ...
    def __at_sec_lvl(self, sec_lvl):
        ...
        while not self.rx_state.__eq__("rx_ready"):
            if self.rx_buffer.decode("utf-8", "ignore").__contains__("SECURITY LEVEL =") or self.rx_buffer.decode(
                    "utf-8", "ignore").__contains__("\r\nOK\r\n"):
                self.__set_rx_state("rx_ready")
                response = self.rx_response
            if self.rx_buffer.decode("utf-8", "ignore").__contains__("\r\nERROR\r\n"):
                self.__set_rx_state("rx_ready")
    ...
    def __help(self):
        ...
        self.send_command("--H")
        while not self.rx_state.__eq__("rx_ready"):
            if self.rx_buffer.decode("utf-8", "ignore").__contains__("[A] = Usable in All Roles"):
                self.__set_rx_state("rx_ready")
                response = self.rx_response
            if self.rx_buffer.decode("utf-8", "ignore").__contains__("\r\nERROR\r\n"):
                self.__set_rx_state("rx_ready")
