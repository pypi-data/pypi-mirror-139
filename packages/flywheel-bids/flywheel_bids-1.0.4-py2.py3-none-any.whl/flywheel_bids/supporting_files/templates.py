...
def test_where_clause(conditions, context):
    ...
    for field, match in conditions.items():
        if "$" in field:
            ...
            logger.debug(f"Checking if <{value}> in field <{field}> matches <{match}>")
...
def processValueMatch(value, match, context, condition=None):
    ...
    if isinstance(match, dict):
        ...
        if isinstance(value, list):
            for item in value:
                if item == match:
                    logger.debug(f"Success: <{item}> matches <{match}>")
                    return True

            logger.debug(f"value <{value}> does not match <{match}> ")
            return False

        elif value == match or value in match:
            logger.debug(f"Success: <{value}> matches <{match}>")
            return True

        elif value != match or not (value in match):
