...
from flywheel_bids.supporting_files import utils, errors
...
def download_bids_dir(
    ...
    if all_acqs:
        for ses_acq in all_acqs:
            ...
            for f in acq.get("files", []):
                ...
                # Get the fw acquisition label, i.e., Series + Series Description
                try:
                    fw_hierarchy[path] = os.path.join(
                        acq["files"][0]["info"][namespace]["Path"],
                        acq["files"][0]["info"][namespace]["Filename"],
                    )
                except TypeError:
                    logger.error("Trouble with %s", acq["files"])
    else:
        msg = (
            container_type
            + ", with subjects="
    ...
    write_bidsignore("fw_hierarchy.json", outdir)

    download_bids_files(fw, filepath_downloads, dry_run)

    save_fw_hierarchy(fw_hierarchy, outdir)
...
def save_fw_hierarchy(fw_hierarchy, outdir):
    """
    Export the matching BIDS and non-BIDS filepaths as a json.
    Args:
        fw_hierarchy (dict): The new, BIDS path as the key; the fw, non-BIDS path as the value.
    """
    new_path = os.path.join(outdir, "fw_hierarchy.json")
    # Write out contents to JSON file
    with open(new_path, "w+") as outfile:
        json.dump(fw_hierarchy, outfile, sort_keys=True, indent=4)
