...
from _assembly.lib.util.time import isoformat_to_milliseconds
from _assembly.lib.util.time import nanoseconds_to_isoformat, _comparison_error
from dateutil.parser import isoparse
from _assembly.api.types.date import Date, DateDelta
from _assembly.api.types.time import Time, TimeDelta
...
class DateTime:
    ...
    """
    def __init__(self, year, month, day, hour, minute, second, millisecond = 0, timestamp = None,
    ...
    def set_year(self, year, check_date_fields=True):
        return DateTime.from_date_and_time(self.date.set_year(year, check_fields=check_date_fields),
                                           self.time, check_date_fields = check_date_fields)

    def set_month(self, month, check_date_fields=True):
        return DateTime.from_date_and_time(self.date.set_month(month, check_fields=check_date_fields),
                                           self.time, check_date_fields = check_date_fields)

    def set_day(self, day, check_date_fields=True):
        return DateTime.from_date_and_time(self.date.set_day(day, check_fields=check_date_fields),
                                           self.time, check_date_fields = check_date_fields)
    ...
    def set_millisecond(self, millisecond):
        return DateTime.from_date_and_time(self.date, self.time.set_millisecond(millisecond))
    ...
    def from_datetime_dict(cls, dct):
        return DateTime(int(dct['year']), int(dct['month']), int(dct['day']), int(dct['hour']), int(dct['minute']),
                        int(dct['second']), int(dct['millisecond']))
    ...
    def __repr__(self):
        return "%s.%s(%d, %d, %d, %d, %d, %d, %d)" % (self.__class__.__module__, self.__class__.__qualname__,
    ...
    def __add__(self, other):
        if isinstance(other, DateTimeDelta):
            ...
            return DateTime(d.year, d.month, d.day, t.hour, t.minute, t.second, t.millisecond)
    ...
    def __sub__(self, other):
        ...
        raise ValueError('can only subtract a DateTime, a DateTimeDelta, a DateDelta, '
...
class DateTimeDelta:
    ...
    def from_nanos(cls, nanos):
        return (DateTime.fromisotimestamp(nanoseconds_to_isoformat(nanos))
                - DateTime.fromisotimestamp(nanoseconds_to_isoformat(0)))

    def to_nanos(self):
        return 10**6 * isoformat_to_milliseconds(
            (DateTime.fromisotimestamp('1970-01-01T00:00:00.000Z') + self).toisotimestamp())

    def __repr__(self):
        return "%s.%s(%s, %s)" % (self.__class__.__module__, self.__class__.__qualname__, repr(
            self._date_delta), repr(self._time_delta))
