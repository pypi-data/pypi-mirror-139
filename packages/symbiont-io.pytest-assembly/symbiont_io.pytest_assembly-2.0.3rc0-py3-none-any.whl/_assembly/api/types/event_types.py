...
from _assembly.api.types.function_type import SymbiontFunctionType
from _assembly.api.util.time import utc_time_iso
...
class BaseEvent():
    ...
    def __init__(self, call_op, data=None):
        ...
        self.channel = call_op.tx.write_channel if call_op.function_type == SymbiontFunctionType.executable else None
...
class ContractEvent(BaseEvent):
    def __init__(self, call_op, type_suffix, data=None):
        self.type = path.join(call_op.contract_ref.name, call_op.contract_ref.version, type_suffix)
