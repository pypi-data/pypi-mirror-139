...
from _assembly.sdk.pytest_plugin.honeycomb_reporting import (report_benchmark_to_honeycomb, report_pytest_to_honeycomb,
                                                             HONEYCOMB_TESTDATA_DATASET)
...
def pytest_runtest_logreport(report):
    if report.when == 'call':
        for m in report.markers:
            if m.name == "benchmark" or (m.name == "usefixtures" and ("benchmark" in m.args)):
                return
        report_pytest_to_honeycomb(report, PytestUtil.config)
...
def pytest_configure(config):
    ...
    config.addinivalue_line('markers', 'proptest: mark test as an expensive property test that only runs with --proptests')
    config.addinivalue_line('markers', 'incremental: mark test as a depenent of a previous one')
    config.addinivalue_line('markers', 'requires_async: mark test as a test which requires async cals')


def pytest_benchmark_generate_json(config, benchmarks, include_data, machine_info, commit_info):
    for benchmark in benchmarks:
        report_benchmark_to_honeycomb(benchmark, PytestUtil.config)
    libhoney.flush()  # Ensure we send all data
...
@pytest.fixture(scope='class')
def network(nodes, sdk_logger, sdk_args):
    """a network client that allows invoking contract functions, either locally or remote"""
    network = nodes_networks.network_fixture(nodes, sdk_args)
    yield network
...
@pytest.fixture(scope='class')
def nodes(sdk_args, tracer):
    """direct access to the nodes for an associated network, safe for any remote network type"""
    return nodes_networks.nodes_fixture(sdk_args, tracer)
...
def tracer(sdk_args):
    tracer = None
    span = None
    if sdk_args.honeycomb_api_key is not None and sdk_args.honeycomb_dataset is not None and sdk_args.tracing:
        ...
        beeline.init(writekey=writekey, dataset=dataset, service_name=service_name, api_host=honeycomb_host)
...
def tracer_function(sdk_args, tracer, tracer_class, request):
    if tracer is not None:
        ...
        span = beeline.start_span(context={'name': request.node.name}, parent_id=parent_id)
...
def tracer_class(sdk_args, tracer, request):
    if tracer is not None:
        cls = request.cls
        if cls is not None:
            span = beeline.start_span(context={'name': request.cls.__name__}, parent_id=tracer.id)
            yield span
            beeline.finish_span(span)
        else:
            yield None
...
def _is_local(sdk_args):
    return sdk_args.pool_debug or (sdk_args.network_config is None and sdk_args.network_name is None
