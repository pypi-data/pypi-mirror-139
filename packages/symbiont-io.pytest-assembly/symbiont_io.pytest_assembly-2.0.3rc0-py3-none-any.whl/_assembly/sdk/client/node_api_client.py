...
import beeline
from _assembly.sdk.client.node_client import query_node
from _assembly.lib.util.contracts import local_code_from_ref
from _assembly.sdk.client.job_management import Job
from _assembly.sdk.client.neo_transactions import build_neo_publish_contracts, build_neo_upgrade_protocol
from _assembly.lib.util.time import nanoseconds_to_isoformat

from _assembly.api.types.date_time import DateTime, DateTimeDelta
...
def register_key_alias(node_session, sync=False):
    """
...
def deregister_key_alias(node_session, key_alias, sync=False):
    """
    ...
    :return: if sync is false, a `Job`, else a string for the created key_alias
    """
    maybe_job = query_node(node_session, 'POST',
                           '/key_aliases/deregister/{}?sync={}'.format(key_alias, str(sync).lower()),
                           {}, role='admin')
    if sync:
        return None
...
def publish(node_session, contract_path, contract_refs, sync=False):
    """
    publishes contracts to a mock-network instance
    :param node_session: node to target
    :param contract_refs: list of contracts to publish
    :param sync: if the underlying sync api should be used
    :return: if sync is false, a `Job`, else None after contracts finish publishing
    """
...
@beeline.traced(name="node_api.call")
def call(node_session, key_alias, contract_ref, function, kwargs, sync=False, retries=5, query_tx_index=None):
    """
...
def restart_at(node_session, tx_index):
    ...
    """
    data = query_node(node_session, 'POST', '/config/restart_at', {'tx_index': tx_index}, role='admin')
    return data
...
def add_timeshift(node_session, timeshift):
    ...
    :return: None
    """
    query_node(node_session, 'POST', '/config/timeshift', {'timeshift': timeshift}, role='admin')
...
def add_datetime_shift(node_session, timeshift):
    ...
    query_node(node_session, 'POST', '/config/timeshift', {'timeshift': ts}, role='admin')
...
def set_datetime(node_session, timestamp):
    ...
    add_datetime_shift(
        node_session,
        timestamp - DateTime.fromisotimestamp(nanoseconds_to_isoformat(int(server_time)))
    )
...
@beeline.traced(name="node_api.list_contracts")
...
@beeline.traced(name="node_api.list_key_aliases")
...
@beeline.traced(name="node_api.list_deregistered_key_aliases")
def list_deregistered_key_aliases(node_session):
    ...
    """
    results = query_node(node_session, 'GET', '/key_aliases/deregistered', None, role='admin')
    return {result['key_alias']: result['tx_index'] for result in results}


@beeline.traced(name="node_api.contract_info")
def contract_info(node_session, contract_ref):
    ...
    """
    return query_node(node_session, 'GET', '/contracts/{}/{}'.format(
        contract_ref.name,
        contract_ref.version_to_use_on_api()
    ), None, language_version=contract_ref.language, role='admin')


@beeline.traced(name="node_api.storage")
def storage(node_session, key_alias, contract_ref):
    ...
    data = query_node(
        node_session, 'GET', path, None, language_version=contract_ref.language, key_alias=key_alias, role='admin')
...
@beeline.traced(name="node_api.channels")
...
@beeline.traced(name="node_api.digest")
def digest(node_session, tx_index=None, sync=False):
    """
    Provides a digest of the node state
    :param as_of_index: state up to the provided tx_index, default to latest
    :param sync: when sync=true the digest is returned otherwise a job_id is provided
    """
    path = f'/digest?sync={str(sync).lower()}&'
...
def current_datetime(node_session):
    ...
    """
    return DateTime.fromisotimestamp(nanoseconds_to_isoformat(statusz(node_session)['current_time']))
...
def append_smartlog_transaction(node_session, topics, data):
    ...
    result = query_node(node_session, 'POST', '/smartlog/transactions', transactions, role='admin')
...
def get_transactions(node_session, log_target, start_index, topics=None, max_count=100,
    ...
    return query_node(node_session, 'GET', f'/{log_target}/transactions/{start_index}', params, role='admin')
...
def neo_publish_contracts(node_session, contract_path, contract_refs, neo_key, neo_crt):
    (topics, byte_data) = build_neo_publish_contracts(contract_path, contract_refs, neo_key, neo_crt)
...
def neo_upgrade_protocol(node_session, txe_protocol, sympl_version, neo_key, neo_crt):
    (topics, byte_data) = build_neo_upgrade_protocol(txe_protocol, sympl_version, neo_key, neo_crt)
