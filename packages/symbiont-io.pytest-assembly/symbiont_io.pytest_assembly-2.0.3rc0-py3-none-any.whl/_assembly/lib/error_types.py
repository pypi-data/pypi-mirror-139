...
class BaseError(Exception):
    ...
    def to_json(self):
        return {
            'code': self.code, 'type': self.error_type, 'message': self.message,
            'data': str(self.inner) if isinstance(self.inner, Exception) else self.inner
        }
...
class DuplicatePublishError(VmError):
    ...
    def __init__(self, contract_ref):
        super().__init__(404,
                         type(self).__name__,
                         "attempted publish but contract already published: '{}'".format(contract_ref))
...
class BaseContractError(VmError):
    ...
    def __init__(self, message, inner=None):
        ...
        super().__init__(status_code, CONTRACT_REQUEST_SERVER_ERROR, f"{message}", inner=inner)
