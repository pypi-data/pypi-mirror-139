...
class LogJsonFormatter(logging.Formatter):
    def __init__(self, component, *args, **kwargs):
        ...
        self.ignore_fields = (
            'msg', 'args', 'span', 'event', 'msecs', 'process', 'levelno', 'tracer', 'funcName', 'processName',
            'created', 'relativeCreated', 'name', 'stack_info', 'threadName')
...
def set_up(logger, component, log_level, async_logging=True, console=True, file=None):
    ...
    # disable async logging as it messes up sanic multiprocessing
    # if async_logging:
    #     que = queue.Queue(-1)
    #     queue_handler = logging.handlers.QueueHandler(que)
    #     logger.addHandler(queue_handler)
    #     for handler in handlers:
    #         queue_listener = logging.handlers.QueueListener(que, handler)
    ...
    def handle_exception(exc_type, exc_value, exc_traceback):
        logger.error('Unhandled Exception: {}'.format(str(exc_value)), exc_info=(exc_type, exc_value, exc_traceback))
