...
def unparse_unsupported_types(o):
    ...
    elif getattr(getattr(o, '__class__', None), '__name__', None) in ['Decimal', 'CronExpression',
                                                                      'Date', 'DateDelta',
                                                                      'DateTime', 'DateTimeDelta',
                                                                      'Time', 'TimeDelta', 'KeyAlias',
                                                                      'ChannelName']:
        return o.serialize()
    else:
...
def parse_unsupported_types(o):
    if '__is_custom__' in o:
        if o['__is_custom__'] == 'contract_ref':
            ...
            raise ValueError("Unsupported type in parse_unsupported_types: {}".format(o['__is_custom__']))
    else:
        return dict(sorted(o.items())) if type(o) is dict else o


def dumps(x, sort_keys=False):
    return simplejson.dumps(x, default=unparse_unsupported_types, sort_keys=sort_keys, tuple_as_array=False)
