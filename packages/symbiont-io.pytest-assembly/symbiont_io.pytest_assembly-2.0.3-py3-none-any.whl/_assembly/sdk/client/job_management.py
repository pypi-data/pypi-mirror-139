...
from _assembly.api.types.event_types import JobFailEvent, JobCompleteEvent
...
class Job():
    ...
    @beeline.traced(name="job.start_waiting")
    def start_waiting(self, timeout=60):
        from _assembly.sdk.client import node_api_client
        ...
        while True:
            events = node_api_client.events(self.node_session, job_ids=job_ids)

            if job_done_event is None:
                # find if there is any job fail event for the root job_id
                job_fail_event = next((e for e in events if is_fail_event(e) and e['job_id'] == self.job_id
                                       and e['signing_key_alias'] == self.submitter),
                                      job_fail_event)

                # find if there are any job completed event for the root job_id
                job_done_event = next((e for e in events if is_completion_event(e) and e['job_id'] == self.job_id),
                                      None)

            # remove the job ids of the completed jobs
            job_ids_done = [e['job_id'] for e in events if is_known_completion_event(e, job_ids)]
            job_ids = deduplicate([job_id for job_id in job_ids if job_id not in job_ids_done])
        ...
        if not job_done_event:
            raise Exception(f'a JobComplete event was expected for job_id {self.job_id} '
        ...
        if job_fail_event is not None:
            if 'DuplicatePublishError' in str(job_done_event['data']):
                ...
                error = job_fail_event['data']['error'] if 'error' in job_fail_event['data'] else None
                ...
                raise ContractErrorInJob(node_fqdn, self.job_id, error_msg, error_data)

        return CompleteJob(self, job_done_event, tx_index, [])

    def sync_with(self, *key_aliases, timeout=120):
        """
        ...
        return completed_job.sync_with(*key_aliases, timeout=timeout)
...
class CompleteJob():
    def __init__(self, job, event, tx_index, synced_with):
        ...
        self.key_alias = self.event['signing_key_alias'] if 'signing_key_alias' in self.event else None
    ...
    def sync_with(self, *key_aliases, timeout=120):
        ...
        """
        # targets = [ka for ka in key_aliases if ka not in self.synced_with]
        # sessions = {self.network_client.node_sessions[self.network_client.alias_locations[ka]] for ka in targets}
        ...
        self.network_client.sync_sessions(self.node_session, self.tx_index, sessions, timeout=timeout)
        # self.synced_with.extend(targets)
        return self
...
def is_known_completion_event(event, job_ids):
    return is_completion_event(event) and \
        event['job_id'] in job_ids
