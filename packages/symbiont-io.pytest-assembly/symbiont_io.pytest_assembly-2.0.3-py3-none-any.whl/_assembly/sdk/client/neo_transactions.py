...
def _make_neo_signature(neo_private_key_pem_bytes, neo_cert_pem_bytes, data):
    ...
    if len(neo_private_key_pem_bytes) == 0 or len(neo_cert_pem_bytes) == 0:
        ...
        private_key = serialization.load_pem_private_key(
            neo_private_key_pem_bytes,
            password=None,
            backend=default_backend()
        )

        signature = {
            'signature': base64.b16encode(private_key.sign(data, ec.ECDSA(hashes.SHA384()))).decode('utf-8'),
...
def build_neo_publish_contracts(contract_path, contract_refs, neo_key, neo_crt):
    ...
    contracts = [{
        ...
        'code': base64.b64encode(gzip.compress(local_code_from_ref(contract_path, contract_ref)
...
def build_neo_upgrade_protocol(txe_protocol: Optional[int], sympl_version: Optional[int], neo_key, neo_crt):

    payload = {
        'command': 'upgrade protocol',
        'version': 1
    }
