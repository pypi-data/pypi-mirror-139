...
def run_marker_setup(item):
    ...
    if 'requires_async' in item.keywords:
        if sdk_args.sync:
            pytest.skip('requires async api but underlying client in sync mode: {}'.format(item.name))
    ...
    if 'requires_network' in item.keywords:
        if sdk_args.skip_requires_network or not \
            ...
            pytest.skip('requires multi node network but currently running in local mode')
    ...
    if benchmark_marker:
        if not sdk_args.benchmarks:
            ...
            if sdk_args.baseline:
                ...
                baseline_param_string = str(baseline_parameters).replace(', ', '-').replace('(', '[').replace(')', ']')
