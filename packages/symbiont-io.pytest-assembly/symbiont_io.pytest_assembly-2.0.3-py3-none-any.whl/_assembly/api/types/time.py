...
from _assembly.lib.util.time import TIME_FORMAT, TIME_FORMAT_WITHOUT_TZ, _check_int_field, _comparison_error
...
class Time:
    ...
    def toisotimestamp(self):
        return time(hour=self.hour, minute=self.minute, second=self.second,
                    microsecond=self.millisecond * 1000).strftime(TIME_FORMAT_WITHOUT_TZ)[:-3] + 'Z'
    ...
    def __repr__(self):
        return "%s.%s(%d, %d, %d, %d)" % (self.__class__.__module__, self.__class__.__qualname__, self._hour,
                                          self._minute, self._second, self._millisecond)
    ...
    def serialize(self):
        return {
            'hour': str(self._hour), 'minute': str(self._minute), 'second': str(self._second),
            'millisecond': str(self._millisecond), 'timestamp': self.toisotimestamp()
        }
    ...
    def __lt__(self, other):
        ...
        return (self.hour, self.minute, self.second, self.millisecond) < (other.hour, other.minute, other.second,
                                                                          other.millisecond)
    ...
    def to_milliseconds(self):
        return self._hour * 60 * 60 * 1000 + self._minute * 60 * 1000 + self._second * 1000 + self._millisecond

    @classmethod
    def from_millisecond(cls, millis):
        if millis < 0:
            return Time.from_millisecond(24 * 60 * 60 * 1000 - (abs(millis) % (24 * 60 * 60 * 1000)))
    ...
    def __sub__(self, other):
        if isinstance(other, Time):
            return TimeDelta(milliseconds = self.to_milliseconds() - other.to_milliseconds())
...
class TimeDelta:
    def __init__(self, hours = 0, minutes = 0, seconds = 0, milliseconds = 0, str = None):
        ...
        if milliseconds > 86400000:
            raise ValueError('a TimeDelta can only represent the difference of two Times of the day.'
...
def _check_time_fields(hour, minute, second, millisecond):
    ...
    if not 0 <= hour <= 23:
        raise ValueError('hour must be in %d..%d' % (0, 23), f'Time({hour}, {minute}, {second}, {millisecond})')
    if not 0 <= minute <= 59:
        raise ValueError('minute must be in %d..%d' % (0, 59), f'Time({hour}, {minute}, {second}, {millisecond})')
    if not 0 <= second <= 59:
        raise ValueError('second must be in %d..%d' % (0, 59), f'Time({hour}, {minute}, {second}, {millisecond})')
    if not 0 <= millisecond <= 999:
        raise ValueError('millisecond must be in %d..%d' % (0, 999), f'Time({hour}, {minute}, {second}, {millisecond})')
