...
from _assembly.lib.util.time import DATETIME_FORMAT, _check_int_field, _comparison_error
...
class Date:
    ...
    def check_date_fields(self):
        ...
        if not MINYEAR <= year <= MAXYEAR:
            raise ValueError('year must be in %d..%d' % (MINYEAR, MAXYEAR), f'Date({year}, {month}, {day})')
        if not 1 <= month <= MONTHS_IN_YEAR:
            raise ValueError(f'month must be in 1..{MONTHS_IN_YEAR}', f'Date({year}, {month}, {day})')
        dim = _days_in_month(month, year)
        if not 1 <= day <= dim:
            raise ValueError('day must be in 1..%d' % dim, f'Date({year}, {month}, {day})')
    ...
    def __repr__(self):
        return "%s.%s(%d, %d, %d)" % (self.__class__.__module__, self.__class__.__qualname__, self._year,
                                      self._month_int, self._day_int)
    ...
    def serialize(self):
        return {
            'year': str(self._year), 'month': str(self._month_int), 'day': str(self._day_int),
            'timestamp': self.toisotimestamp()
        }

    # convert the Date into an ISO8601 timestamp "YYYY-MM-DDT00:00:00.000Z"
    def toisotimestamp(self):
        return datetime(self._year, self._month_int, self._day_int).isoformat() + '.000Z'
    ...
    def add_months(self, num_months, date_rolling=False):
        ...
        if new_month > MONTHS_IN_YEAR:
            new_year += int(new_month / MONTHS_IN_YEAR) if num_months >= MONTHS_IN_YEAR else 1

            # if the new month value is divisible by 12, we actually want the previous year
            if new_month % MONTHS_IN_YEAR == 0:
                new_year -= 1

            new_month = new_month % MONTHS_IN_YEAR if (new_month % MONTHS_IN_YEAR) > 0 else MONTHS_IN_YEAR
    ...
    def subtract_months(self, num_months, date_rolling=False):
        ...
        # are we out of months?
        if new_month <= 0:
            new_year -= int(abs(new_month) / MONTHS_IN_YEAR) + 1 if num_months > MONTHS_IN_YEAR else 1
            new_month = new_month % MONTHS_IN_YEAR if (new_month % MONTHS_IN_YEAR) > 0 else MONTHS_IN_YEAR
...
class DateDelta:
    ...
    def as_str(self):
        ...
        return "%010d/%010d/%010d" % (if_neg(self.years), if_neg(self.months), if_neg(self.days))

    def serialize(self):
        return {'years': str(self._years), 'months': str(self._months), 'days': str(self._days),
                'as_str': self.as_str()}
    ...
    def __add__(self, other):
        return DateDelta(self._days + other._days, self._months + other._months, self._years + other._years)
