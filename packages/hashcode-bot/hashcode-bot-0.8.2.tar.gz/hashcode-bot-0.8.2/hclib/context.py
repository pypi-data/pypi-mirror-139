...
class Run:
    ...
    def run_dataset(self, dataset, *, capture_output=True):
        ...
        best_dataset = self.context.dataset_highscore(dataset)
        if best_dataset and best_dataset.solution_path:
            best_solution = best_dataset.solution_path
        else:
            best_solution = ""
...
class DatasetRunResult:
    ...
    def solution_path(self):
        path = (self.run.root / self.dataset.stem).with_suffix(".out")
    ...
    def submit(self, sources_blob_key=None):
        ...
        solution_blob_key = self.run.context.judge.upload(self.solution_path)
