...
class Payments(Resource):
    ...
    def response(self):
        ...
        data["payload"] = response_data.get("payload", {})

        if super().get_bank_id() == super().COOP:

            data["message"] = response_data.get("MessageDescription", "")
            data["code"] = int(response_data.get("MessageCode", -1111111))
            data["date"] = response_data.get("MessageDateTime", date.today().strttime('%d/%m/%y %H:%M:%S'))
            data["transaction_id"] = response_data.get("Destinations", {}).get("TransactionID", "")
            ...
            data["date"] = response_data.get("date", date.today().strftime('%d/%m/%y %H:%M:%S'))
