...
class SubProcess(sp.Popen):
    ...
    def __init__(self, command_line, **kargs):
        ...
        signal_value = kargs.pop('signal', signal.SIGTERM)
        if sys.version_info > (3, 5):
            self.signal = signal.Signals(signal_value)
        else:
    ...
    def wait(self, timeout=None, expected=None):
        ...
        if expected is not None:
            assert self.returncode == expected, \
                '{} returns {}, but expected was {}'.format(self, self.returncode, expected)
    ...
    def terminate(self, signum=None, assure=True):
        ...
        while 1:
            ...
            if difftime > 5:
                self.log.warning('{} is still alive after {:.2f} seconds!'.format(self, difftime))
