...
class ControlIOBuffer:
    ...
    def info(self, info_str: str, *args: Any) -> None:  # noqa: D102
        ...
        if args:
            msg = ctrl_msg.new_ctrl_out_info_msg(info_str.format(*args))
        else:
            msg = ctrl_msg.new_ctrl_out_info_msg(info_str)
    ...
    def error(self, error_msg: str, *args: Any) -> None:  # noqa: D102
        if args:
            msg = ctrl_msg.new_ctrl_out_error_msg(error_msg.format(*args))
        else:
            msg = ctrl_msg.new_ctrl_out_error_msg(error_msg)
        self.push_ctrl_out(msg)

    def warn(self, warn_msg: str, *args: Any) -> None:  # noqa: D102
        if args:
            msg = ctrl_msg.new_ctrl_out_warn_msg(warn_msg.format(*args))
        else:
            msg = ctrl_msg.new_ctrl_out_warn_msg(warn_msg)
        self.push_ctrl_out(msg)

    def update_progress(self, percent: float) -> None:  # noqa: D102
        # Not currently implemented in any version
        pass
