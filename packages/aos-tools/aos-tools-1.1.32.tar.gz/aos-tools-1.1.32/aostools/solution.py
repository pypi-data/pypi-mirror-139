...
class Variables:
    ...
    def check_variable(self, values=[]):
        # check and condition: [v1, v2, v3...], AND operation
        new_values = []
        for v in values:
            v = v.strip()
            if v:
...
class Solution:
    def __init__(self, components, compiler='gcc'):
        ...
        def cpu_set_variable(cpu, arch):
            ...
            if arch == '' or arch.upper() == 'CSKY':
                ...
                self.variables.set('ARCH', arch, True)
            elif arch.upper() == 'XTENSA':
                self.variables.set('cpu_num', 'esp32')
                self.variables.set('ARCH', arch, True)
            else:
        ...
        def update_defines_from_cmdline(component):
            for k, v in SCons.ARGLIST:
                if k != 'V' and k != 'BOARD':
                    if str.isdigit(v):
                        v = int(v)
                        if not SCons.GetOption('verbose'):
                            put_string("%s is number %d" % (k, v))
                    else:
                        if not SCons.GetOption('verbose'):
                            put_string("%s is str %s" % (k, v))
                    self.variables.set(k, v)
                    self.defines[k] = v
                    self.org_defines[k] = v

        def check_duplicate_value(array, value):
            check_num = 0
            for c in array:
                if( value != c ):
        ...
        def update_variables():
            ...
            for component in self.components:
                if component.type == 'dynamic':
                    add_defines(component)
                    for c in component.build_config.shldflag.split():
                        component.private_shldflag.append(c)
            ...
            for component in self.components:
                if component.type == 'solution':
                    add_defines(component)
                    update_defines_from_cmdline(component)
            ...
            self.dylib_path = os.path.join(self.yoc_path, 'dylib')
            self.libpath.append(self.dylib_path)
            self.app_path = os.path.join(self.yoc_path, 'app')
            ...
            for component in self.components:
                ...
                if (len(component.source_files) > 0 and component.name not in self.libs
                   and component.type == "dynamic" and component.name != "separate_app"):
                        self.dylibs.append(component.name)
                        self.depend_dylibs.append(os.path.join(self.dylib_path,
                                                'lib' + component.name +'.dylib'))
        ...
        update_variables()
        if self.solution_component.separate_app:
            add_define('AOS_SEPARATE_APP', 1)
        ...
        def check_variables(values=[]):
            # check and condition: [['aa'], ['bb', 'cc'], ['dd', 'ee']] or [['ff']], OR operation
            for v in values:
                if check_variable(v):
                    return True
    ...
    def show(self, show_component=True, show_libs=True, show_include=True, show_define=True, show_veriable=True, show_flags=True):
        ...
        if show_flags:
            ...
            put_string("SHLINKFLAGS :" + ' '.join(self.SHLINKFLAGS))
...
def riscv_cpu_id(cpu):
    ...
    if a == 'rv32emc':
        ...
        return 'rv64_32gpr'
    elif a == 'c906fd' or a == 'c906v' or a == 'c910' or a == 'c910v' or a == 'c906fdv':
        return 'rv64fd_32gpr'
    elif a == 'esp32-c3':
        return 'rv32imc'
    else:
