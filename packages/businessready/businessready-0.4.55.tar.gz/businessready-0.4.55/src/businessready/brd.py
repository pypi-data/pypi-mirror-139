...
def DNAC_all(url, username, password):
    ...
    DNAC_site_vlan(url, username, password)
        ...
        if vlansJSON is not None:
            ...
            for vlan in vlansJSON['response']:
                ...
                if vlanTopologyJSON is not None:
                    vlanToplogy_template = env.get_template('DNA_vlan_topology.j2')
                # Render Templates
                    for filetype in filetype_loop:
                        parsed_output = vlanToplogy_template.render(site = site['name'],siteMembers = sitesMembersJSON['device'],filetype_loop=loop_counter)
                        loop_counter = loop_counter + 1
