...
def DNAC_site_member(url, username, password):
    try:
        ...
        if sitesJSON is not None:
            for site in sitesJSON['response']:
                ...
                # Render Templates
                    for filetype in filetype_loop:
                        parsed_output = site_member_template.render(site = site,siteMember = sitesMembersJSON['device'],filetype_loop=loop_counter)
                        loop_counter = loop_counter + 1
    ...
    # -------------------------
                    if loop_counter <= 3:
                        with open(f"DNAC Site Member.{ filetype }", "w") as fh:
                            fh.write(parsed_output)               
                            fh.close()
                    else:
                        with open("DNAC Site Member Mind Map.md", "w") as fh:
                            fh.write(parsed_output)               
                            fh.close()
                    with open(f"DNAC Site Member.json", "w") as fh:
