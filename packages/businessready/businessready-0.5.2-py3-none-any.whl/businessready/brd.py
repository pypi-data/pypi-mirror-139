...
def DNAC_templates(url, username, password):
    try:
        ...
        # create folders to hold files
        # -------------------------
                    if not os.path.exists(f"{ template['id'] }"):
                        os.mkdir(f"{ template['id'] }")
                    else:
                        print("Directory already exists")
    ...
    # -------------------------
                            if loop_counter <= 3:
                                with open(f"{ template['id'] }\DNAC { template['name'] } Template.{ filetype }", "w") as fh:
                                    fh.write(parsed_output)               
                                    fh.close()                       
                            else:
                                with open(f"{ template['id'] }\DNAC { template['name'] } Template Mind Map.md", "w") as fh:
                                    fh.write(parsed_output)               
                                    fh.close()
                            with open(f"{ template['id'] }\DNAC { template['name'] } Template.json", "w") as fh:
