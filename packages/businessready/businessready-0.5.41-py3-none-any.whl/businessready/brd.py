...
def Meraki_organization_devices(url, token):
    ...
    # -------------------------
                    if loop_counter <= 3:
                        with open(f"Meraki { org['name'] } Devices.{ filetype }", "w") as fh:
                            fh.write(parsed_output)               
                            fh.close()
                    else:
                        with open("Meraki { org['name'] } Devices Mind Map.md", "w") as fh:
                            fh.write(parsed_output)               
                            fh.close()
                    with open(f"Meraki { org['name'] } Devices.json", "w") as fh:
