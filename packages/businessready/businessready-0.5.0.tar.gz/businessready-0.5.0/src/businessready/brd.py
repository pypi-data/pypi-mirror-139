...
def DNAC_projects(url, username, password):
    ...
    # -------------------------
                            if loop_counter <= 3:
                                with open(f"DNAC { template['name'] } Template.{ filetype }", "w") as fh:
                                    fh.write(parsed_output)               
                                    fh.close()                       
                            else:
                                with open(f"DNAC { template['name'] } Template Mind Map.md", "w") as fh:
                                    fh.write(parsed_output)               
                                    fh.close()
                            with open(f"DNAC { template['name'] } Template.json", "w") as fh:
