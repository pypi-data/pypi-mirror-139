...
def Meraki_organization_devices(url, token):
    try:
        ...
        # -------------------------
                if not os.path.exists(f"{ org['name'] }"):
                    ...
                    print("Directory already exists")                
                organizationDevicesRAW = requests.request("GET", f"{ url }/api/v1/organizations/{ org['id'] }/inventoryDevices", headers=headers)
                organizationDevicesJSON = organizationDevicesRAW.json()

                if organizationDevicesJSON is not None:
                    organization_device_template = env.get_template('Meraki_organization_devices.j2')
                    loop_counter = 0
        # Render Templates
                    for filetype in filetype_loop:
                        parsed_output = organization_device_template.render(device = organizationDevicesJSON,filetype_loop=loop_counter)
                        loop_counter = loop_counter + 1
    ...
    # -------------------------
                        if loop_counter <= 3:
                            with open(f"{ org['name'] }/Meraki { org['name'] } Devices.{ filetype }", "w") as fh:
                                fh.write(parsed_output)               
                                fh.close()
                        else:
                            with open(f"{ org['name'] }/Meraki { org['name'] } Devices Mind Map.md", "w") as fh:
                                fh.write(parsed_output)               
                                fh.close()
                        with open(f"{ org['name'] }/Meraki { org['name'] } Devices.json", "w") as fh:
                            json.dump(organizationDevicesJSON, fh, indent=4, sort_keys=True)
                            fh.close()                            
        return(organizationDevicesJSON)
    except Exception as e:
        logging.exception(e)
