...
def _ax_quantiles(ax, quantiles, twin='x'):
    ...
    if twin == 'x':
        ...
        ax_top.set_xticklabels(
            ["{1:0.{0}f}%".format(int(i / (len(quantiles) - 1) * 100 % 1 > 0), i / (len(quantiles) - 1) * 100) for i in
             range(len(quantiles))], color="#545454", fontsize=7)
...
def _second_order_quant_plot(ax, quantiles, ALE, **kwargs):
    ...
    CF = ax.contourf(X, Y, ALE_interp(x, y),alpha=0.5)
    plt.colorbar(CF)
...
def _first_order_ale_cat(predictor, train_set, feature, features_classes, feature_encoder=None):
    ...
    for i in range(num_cat):
        ...
        if len(subset) != 0:
            ...
            z_up = subset.copy()
            # The main ALE idea that compute prediction difference between same data except feature's one
            z_low[feature] = np.quantiles[i - 1]
            z_up[feature] = np.quantiles[i]
...
def ale_plot(model, train_set, features, plot=False,bins=40, monte_carlo=False, predictor=None, features_classes=None, **kwargs):
    ...
    if plot:
        ...
        if len(features) == 1:
            ...
            if monte_carlo:
                ...
                mc_ratio = kwargs.get('monte_carlo_ratio', 0.1)
                mc_replicates = np.asarray(
                    [[np.random.choice(range(train_set.shape[0])) for _ in range(int(mc_ratio * train_set.shape[0]))]
                     for _
                     in range(mc_rep)])
                for k, rep in enumerate(mc_replicates):
                    train_set_rep = train_set.iloc[rep, :]
