...
from ..utils import  LightLabyrinthVerbosityLevel, LearningProcess3D
...
from .._tools import _MultilabelScaler
import numpy as np
...
class LightLabyrinth3DRandomMultilabelClassifier(RandomLightLabyrinth3D):
    """A random multilabel Light Labyrinth model.
        ...
        and `light_labyrinth.multioutput.LightLabyrinth3DMultilabelClassifier`.
        ...
        activation : `light_labyrinth.hyperparams.activation.ReflectiveIndexCalculator3DRandom`, default=`light_labyrinth.hyperparams.activation.ReflectiveIndexCalculator3DRandom.random_3d_softmax_dotproduct`
            Activation function applied to each node's output.

            -`random_3d_softmax_dotproduct` - softmax function over product of weights and input light, for a given node.
        ...
        error : `light_labyrinth.hyperparams.error_function.ErrorCalculator`, default=`light_labyrinth.hyperparams.error_function.ErrorCalculator.mean_squared_error`
            ...
            -`scaled_mean_squared_error` - Adaptation of MSE meant primarily for multilabel classifiaction.
                For each pair of outputs the only thing that matters is whether yi+ is higher than yi- or not,
                rather than the exact values. Therefore it may be benefitial to alter the loss function so that it punishes
        ...
        depth : int
            Depth of the LightLabyrinth given by the nubmer of unique classes.
        ...
        See Also
        --------
        light_labyrinth.dim3.LightLabyrinth3DClassifier : 3-dimensional Light Labyrinth classifier for multiclass classifiaction.
        light_labyrinth.multioutput.LightLabyrinth3DMultilabelClassifier : 3-dimensional Light Labyrinth classifier for multilabel classification.
        light_labyrinth.multioutput.LightLabyrinth3DRandomMultioutputRegressor : random 3-dimensional Light Labyrinth regressor for multioutput regression.
    ...
    def __init__(self, height, width, features, bias=True, indices=None,
                 activation=ReflectiveIndexCalculator3DRandom.random_3d_softmax_dotproduct,
    ...
    def fit(self, X, y, epochs, batch_size=1.0, stop_change=1e-4, n_iter_check=0, epoch_check=1, X_val=None, y_val=None, verbosity=LightLabyrinthVerbosityLevel.Nothing):
        ...
        y : ndarray of shape (n_samples, n_lables)
            The target labels (binary values indicationg whether a given sample belongs to
        ...
        batch_size : int or float, default=1.0
            Size of minibatches for stochastic optimizers given either as portion of 
        ...
        self._encoder = _MultilabelScaler()
    ...
    def predict_proba(self, X):
        ...
        """
        y_pred = super().predict(X)
        untransformed = self._encoder(y_pred)
