...
def get_file(filename):
    with open(filename) as f:
        proxies = [Proxy(p) for p in f.read().split() if p.strip()]
    return proxies
...
def get_openproxy():
    ...
    r = requests.get(url, headers=headers, timeout=PROXINE_TIMEOUT)
    proxies = [Proxy(p) for d in r.json()["data"] for p in d["items"]]
    return proxies
...
def get_proxy_scrape():
    ...
    r = requests.get(url, headers=headers, timeout=PROXINE_TIMEOUT)
    proxies = list(map(Proxy, r.text.split()))
    return proxies
...
def get_spys_one():
    ...
    proxies = [Proxy(":".join(t)) for t in zip(ips, ports)]
