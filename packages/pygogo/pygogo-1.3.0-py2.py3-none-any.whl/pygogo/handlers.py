...
def email_hdlr(subject="You've got mail", host="localhost", port=587, **kwargs):
    ...
    Kwargs:
        ...
        credentials (Tuple[str]): The email sever username & password (default: None).
    ...
    def_credentials = (username, password) if username or password else None
    kwargs.setdefault("credentials", def_credentials)

    args = (address, sender, recipients, subject)
    credentials = kwargs["credentials"]
    secure = () if credentials else None
    return hdlrs.SMTPHandler(*args, credentials=credentials, secure=secure)
