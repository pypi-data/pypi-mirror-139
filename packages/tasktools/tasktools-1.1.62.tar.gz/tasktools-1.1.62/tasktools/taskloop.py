...
class TaskLoop:
    ...
    def finish(self, finish=print):
        self.active_task.remove_done_callback(self.done_callback)
        self.add_done_callback(finish)
    ...
    def renew(self, task, coro, fargs, *args, **kwargs):
        ...
        if not task.cancelled():
            try:
                    ...
                    if VERSION.major == 3 and VERSION.minor >= 8:
                        task = loop.create_task(
                            coromask(pause, pause_args,
                                     pause_kwargs, fargs), name=self._name)
                    if VERSION.major == 3 and VERSION.minor == 7:
                        task = loop.create_task(
                            coromask(pause, pause_args,
                                     pause_kwargs, fargs))
                ...
                rprint(f"Error de cancelación fargs {fargs}")
                raise ce
