...
class TaskScheduler:
    ...
    def assigned_ids(self):
        ...
        codes = [self.stations.get(ids, {}).get("code") for ids in assigned_tasks]
        assigned_ids = set(filter(lambda e:e, codes))
        return assigned_ids

    @property
    def availabe_slots(self):
        print(now(), "Assigned tasks", self.assigned_tasks)
        available = {}
        for ipt, tasks in self.assigned_tasks.items():
            assigned_tasks = {t for t, v in tasks.items()
                              if v}
            print(now(), "Available slots assigned task", assigned_tasks)
            if len(assigned_tasks) == self.lnproc:
                available[ipt] = False 
            else:
                available[ipt] = True 
        return available
    ...
    def pick_one(self):
        for ipt, val in self.availabe_slots.items():
            icos = self.assigned_tasks[ipt]
            if val:
                return ipt, True
        return "", False
