...
def get_latest_execution_id(con):
    ...
    cursorObj.execute("SELECT eid FROM robothistoric2.execution ORDER BY eid DESC LIMIT 1;")
...
def update_execution_table(con, eid, pid):
    ...
    cursorObj.execute("SELECT COUNT(*) FROM robothistoric2.test WHERE eid=%s AND status='PASS';" % (eid))
    execution_rows = cursorObj.fetchone()
    tests_passed = execution_rows[0]

    cursorObj.execute("SELECT COUNT(*) FROM robothistoric2.test WHERE eid=%s AND status='FAIL';" % (eid))
    execution_rows = cursorObj.fetchone()
    tests_failed = execution_rows[0]

    cursorObj.execute("SELECT COUNT(*) FROM robothistoric2.test WHERE eid=%s AND status='SKIP';" % (eid))
    execution_rows = cursorObj.fetchone()
    tests_skipped = execution_rows[0]

    tests_total = tests_passed + tests_failed + tests_skipped

    sql = "UPDATE robothistoric2.execution SET total=%s, pass=%s, fail=%s, skip=%s WHERE eid=%s;" % (tests_total, tests_passed, tests_failed, tests_skipped, eid)
    cursorObj.execute(sql)
    con.commit()

    cursorObj.execute("SELECT eid, pass, total FROM robothistoric2.execution ORDER BY eid DESC LIMIT 1;")
    rows = cursorObj.fetchone()
    # update robothistoric.TB_PROJECT table
    cursorObj.execute("UPDATE robothistoric2.project SET Last_Updated = now(), total = %s, percentage =%s WHERE pid='%s';" % (rows[2], float("{0:.2f}".format((rows[1]/rows[2]*100))), pid))
    con.commit()
    return str(rows[0])

def update_test_table(con, eid, test, status, duration, msg):
    cursorObj = con.cursor()
    sql = """UPDATE robothistoric2.test SET status='%s', time='%s' WHERE name="%s" AND eid=%s""" % (str(status), str(duration), str(test), eid)
    cursorObj.execute(sql)
