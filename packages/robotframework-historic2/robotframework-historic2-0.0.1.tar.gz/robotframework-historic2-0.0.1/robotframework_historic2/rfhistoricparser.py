...
def rfhistoric_parser(opts):
    ...
    # connect to database
    rootdb = connect_to_mysql_db(opts.host, opts.username, opts.password, 'robothistoric2')
...
def insert_into_execution_table(ocon, pid, description, total, passed, failed, skipped, etime):
    ...
    sql = "INSERT INTO robothistoric2.execution (eid, pid, description, time, total, pass, fail, skip, etime) VALUES (%s, %s, %s, now(), %s, %s, %s, %s, %s);"
    val = (0, pid, description, total, passed, failed, skipped, etime)
    rootCursorObj.execute(sql, val)
    ocon.commit()

    rootCursorObj.execute("SELECT eid, pass, total FROM robothistoric2.execution ORDER BY eid DESC LIMIT 1;")
    rows = rootCursorObj.fetchone()

    # update robothistoric.TB_PROJECT table
    rootCursorObj.execute("UPDATE robothistoric2.project SET updated = now(), total = %s, percentage =%s WHERE pid='%s';" % (rows[2], float("{0:.2f}".format((rows[1]/rows[2]*100))), pid))
    ocon.commit()
    return str(rows[0])

def insert_into_test_table(con, eid, pid, test, status, duration, msg, tags):
    cursorObj = con.cursor()
    sql = "INSERT INTO robothistoric2.test (tid, eid, pid, name, status, time, error, tag) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)"
