...
def randomBoyName():
    with open('./data/nameboy.txt',encoding="utf8") as f:
        words = f.read().split()
...
def randomGirlName():
    with open('./data/namegirl.txt',encoding="utf8") as f:
        words = f.read().split()
        girl_name = random.choice(words)
        return girl_name

#generate lastname

def randomLastName():
    with open('./data/lastname.txt',encoding="utf8") as f:
        words = f.read().split()
...
#this generator return a list:
#a===number of code or name or ....

def codemelliGenerator(a):
    x=[]
    for i in range(a):
        x.append(generator())
    return x  

def boyNameGenerator(a):
    x=[]
    for i in range(a):
        x.append(randomBoyName())
    return x

def girlNameGenerator(a):
    x=[]
    for i in range(a):
        x.append(randomGirlName())
    return x

def lastnameGenerator(a):
    x=[]
    for i in range(a):
        x.append(randomLastName())
    return x

def fullBoyNameGenerator(a):
    x=[]
    for i in range(a):
        x.append(randomFullBoyName())
    return x

def fullGirlNameGenerator(a):
    x=[]
    for i in range(a):
        x.append(randomFullGirlName())
    return x

def fullRandomNameGenerator(a):
    x=[]
    for i in range(a):
        x.append(randomName())
        return x
