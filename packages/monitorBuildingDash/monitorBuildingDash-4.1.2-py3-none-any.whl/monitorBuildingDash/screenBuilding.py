...
####### VMUC
ip,port = df_devices.loc['vmuc',['IP','port']]
FILEPLC_VMUC = CONFFOLDER + 'plc_vmuc.pkl'
FILE_XML_VMUC  = glob.glob(CONFFOLDER+ 'vmuc*ModbusTCP*.xml')[0]
FILE_INSTR_VMUC = CONFFOLDER + 'dfinstr_vmuc.pkl'
vmucClient = comUtils.ModeBusDeviceXML('vmuc',ip,port,
            COMPTEURS,VARIABLES,FILEPLC_VMUC,FREQ_VMUC,
            file_xml=FILE_XML_VMUC,file_dfInstr=FILE_INSTR_VMUC,
            build_dfInstr=False,generatePLC=False)

####### SMARTLOGGER
def build_dfInstr_smartLogger():
    ...
    return dfInstr
ip,port=df_devices.loc['smartlogger',['IP','port']]
FILEPLC_SL = CONFFOLDER + 'plc_smartlogger.pkl'
smartLoggerClient = comUtils.ModeBusDeviceSingleRegisters('smartlogger',ip,port,
        COMPTEURS,VARIABLES,FILEPLC_SL,freq=FREQ_SMARTLOGGER,
        dfInstr=build_dfInstr_smartLogger(),generatePLC=False)

####### ENGIE SLS
def build_dfInstr_engieSLS():
    dfInstr=pd.DataFrame()
    dfInstr['adresse']     = [4198,4200,4246,4300]
    dfInstr['intAddress']  = dfInstr['adresse']
    dfInstr['type']        = ['UINT32']*len(dfInstr)
    dfInstr['size(mots)']  = [2]*len(dfInstr)
    dfInstr['size(bytes)'] = [4]*len(dfInstr)
    dfInstr['unit']        = ['kW','kW','kWh','kWh']
    dfInstr['description'] = ['puissance_soutirage','puissance_injection','energie_soutirage','energie_injection']
    dfInstr['scale']       = [1]*len(dfInstr)
    dfInstr['addrTCP']     = [20]*len(dfInstr)
    dfInstr['point de comptage'] = ['siteSLS']*len(dfInstr)
    dfInstr.index = dfInstr['point de comptage']+'-'+dfInstr['description']
    return dfInstr
ip,port=df_devices.loc['SLS_Engie',['IP','port']]
fileplc_slsengie = CONFFOLDER + 'plc_slsEngie.pkl'
engieSlSclient = comUtils.ModeBusDeviceSingleRegisters('SLS_Engie',ip,port,
            COMPTEURS,VARIABLES,fileplc_slsengie,dfInstr=build_dfInstr_engieSLS(),
            generatePLC=False,freq=FREQ_ENGIESLS)
# print(engieSlSclient.dfplc)
# print(FREQ_ENGIESLS)
####### METEO
meteoClient = comUtils.Meteo_Client(FREQ_METEO)

DEVICES = {
    'vmuc':vmucClient,
    'smartLogger':smartLoggerClient,
    'SLS_Engie':engieSlSclient,
    'meteo':meteoClient,
        }
# sys.exit()
# ==============================================================================
...
class ScreeningBuilding_dumper(SuperDumper_daily,Config_extender):
    def __init__(self,**kwargs):
        ...
        SuperDumper_daily.__init__(self,FOLDERPKL,DB_PARAMETERS,DEVICES,PARKING_TIME,**kwargs)
...
class ScreenBuildingComputer(VisualisationMaster_daily,Config_extender):
    def __init__(self,**kwargs):
        Config_extender.__init__(self)
        VisualisationMaster_daily.__init__(self,FOLDERPKL,DB_PARAMETERS,DEVICES,PARKING_TIME,**kwargs)
    ...
    def get_last_days_month_since_start(self,t1):
        ...
        list_lastDays=[]
        t=t1-pd.DateOffset(hours=t1.hour,minutes=t1.minute)
        while t>first_day:
    ...
    def get_conso_prod_sls_lastValue(self,formula='g+f-e+pv'):
        t1 = pd.Timestamp.now(tz='CET')
        t0 = t1-pd.Timedelta(minutes=1)
        return self.getTags_sls_conso(t0,t1,rs='10s',units='kWh',formula=formula).iloc[-1,-2:]
    ...
    def get_compteurs_sls_since_start(self,t1,level='all'):
        ...
        dfs=[]
        for t in listdays:
            t0,t1=t,t+pd.Timedelta(hours=23,minutes=58,seconds=0)
            # print(t0,t1)
            df = self.loadtags_period(t0,t1,tags,rs='10s',rsMethod='forwardfill')
            if df.empty:
                t0 = self.get_nearest_available_day(t)
                t1 = t0 + pd.Timedelta(hours=23,minutes=58,seconds=0)
                df = self.loadtags_period(t0,t1,tags,rs='10s',rsMethod='forwardfill')
            df=df.iloc[[-1],:]
            dfs.append(df)
        df = pd.concat(dfs)
    ...
    def track_energy_compteurs_months(self,t1):
        ...
        nameFile = 'conso_elec_' + t1.strftime('%B_%Y') + '.xlsx'
    ...
    def fig_conso2days_sls(self,t1,tfsize=17,**kwargs):
        ...
        fig=px.area()
        fig.add_trace(go.Scatter(x=df.index,y=df['part PV'],name='Electricité solaire',
                        line_color='black',stackgroup='1',fillcolor=colPV))
        df['part Grid']=df['part Grid']-df['part PV']
        fig.add_trace(go.Scatter(x=df.index,y=df['part Grid']+df['part PV'],name='Electricité du réseau',
                        line_color='black',stackgroup='1',fillcolor=colgrid))
        ...
        fig.add_trace(go.Scatter(x=df.index,y=df['Consommation du site'],name='Consommation du site',line_color='black'))

        dfEngie = self.get_slsTotalEngie(t1,'roll')
        fig.add_trace(go.Scatter(x=dfEngie.index,y=dfEngie.squeeze(),name='Compteur Engie(soutirage glissant)',line_color='red'))
        dfEngiem = self.get_slsTotalEngie(t1,'mean')
        fig.add_trace(go.Scatter(x=dfEngiem.index,y=dfEngiem.squeeze(),name='Compteur Engie(soutirage original)',line_color='magenta'))
    ...
    def fig_repartitioncompteurs_sls(self,t1,tfsize=17,**kwargs):
        ...
        fig.update_layout(xaxis = dict(tickmode='array',tickvals = ti,ticktext = ti.strftime('%A %d %b %Y')))
        fig = px.area(df)
        fig = self.utils.updateColorMap(fig,'gist_ncar')
        for l in ti[1:] :
            fig.add_vline(x=l,xref='x', line_width=3, line_dash="solid", line_color="black")
