...
class FormatterPayloadAzure:
    ...
    def format_payload_update_testspoints(testpoints, outcomes):
        testpoints = FormatterPayloadAzure.update_status_testcases(
            testpoints, outcomes)
        ...
        for key, value in groupby(tests, key=itemgetter('planId', 'suiteId')):
            outcomes[key] = [
                FormatterPayloadAzure.format_testpoint_payload(k) for k in value]
        return outcomes
    ...
    def format_payload_create_runs(suites):
        for suite in suites:
            testpoints = FormatterPayloadAzure.update_status_testcases(
                testpoints, outcomes)
            testpoints = FormatterPayloadAzure.format_testresult_payload(
                testpoints)
            tests = sorted(testpoints, key=itemgetter('planId', 'suiteId'))
            outcomes = {}
            for key, value in groupby(tests, key=itemgetter('planId', 'suiteId')):
