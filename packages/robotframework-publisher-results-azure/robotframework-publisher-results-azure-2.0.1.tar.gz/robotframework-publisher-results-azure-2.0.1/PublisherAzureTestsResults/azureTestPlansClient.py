...
class AzureTestPlansClient:
    ...
    def get_tests_points(self, testcases):
        ...
        response = requests.request("POST", url, headers=self.headers,auth=self.authbasic, data=payload)
    ...
    def update_testspoints_outcomes(self, testplanId, suiteId, testspoints):
        ...
        response = requests.request("PATCH", url, headers=self.headers,auth=self.authbasic, data=payload)
    ...
    def test_accessibility_project(self):
        response=requests.request("GET",self.azure_url_project,auth=self.authbasic)
