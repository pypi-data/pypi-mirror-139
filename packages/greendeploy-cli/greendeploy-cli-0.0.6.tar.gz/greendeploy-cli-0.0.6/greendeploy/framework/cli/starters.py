...
This module implements commands available from the greendeploy CLI for creating
projects.
"""
import os
import re
...
def new(
    ...
    """Create a new greendeploy project."""
    if checkout and not starter_name:
        raise GreenDeployCliError("Cannot use the --checkout flag without a --starter value.")
...
def _fetch_config_from_file(config_path: str) -> Dict[str, str]:
    """Obtains configuration for a new greendeploy project non-interactively from a file.
    Args:
    ...
    Raises:
        GreenDeployCliError: If the file cannot be parsed.
    """
    try:
...
def _make_cookiecutter_args(
    ...
    """
    config.setdefault("greendeploy_version", version)
...
def _create_project(template_path: str, cookiecutter_args: Dict[str, str]):
    """Creates a new greendeploy project using cookiecutter.
    Args:
    ...
    Raises:
        GreenDeployCliError: If it fails to generate a project.
    """
...
def _get_cookiecutter_dir(
    ...
    try:
        ...
        error_message = f"Dockerized Django project template not found at {template_path}."
        ...
        raise GreenDeployCliError(error_message) from exc
...
class _Prompt:
    """Represent a single CLI prompt for `greendeploy new`"""
...
def _validate_config_file(config: Dict[str, str], prompts: Dict[str, Any]):
    ...
    Args:
        ...
        prompts: Prompts from prompts.yml.
    Raises:
        GreenDeployCliError: If the config file is empty or does not contain all the keys
            required in prompts, or if the output_dir specified does not exist.
    """
    if config is None:
        raise GreenDeployCliError("Config file is empty.")
    if missing_keys := set(prompts) - set(config):
        click.echo(yaml.dump(config, default_flow_style=False))
        raise GreenDeployCliError(f"{', '.join(missing_keys)} not found in config file.")
