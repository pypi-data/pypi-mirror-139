...
class JVCProjector:
    ...
    def info(self) -> tuple[str, bool]:
        ...
        return self._send_command(
            send_command=Commands.info.value, ack=ACKs.menu_ack.value
        )
    ...
    def _get_power_state(self) -> str:
        ...
        if not success:
            if "Connection Refused" in msg:
                # JVC Projector Connection Refused when getting power state. Try polling less often.
                # Not erroring because its not really an issue. JVC just has a stupid interface
