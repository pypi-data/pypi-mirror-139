...
class JVCProjector:
    ...
    async def _async_do_command(
        ...
        async with self._lock:
            ...
            if received_ack == ack_value and command_type == Header.operation.value:
                ...
                writer.close()
                success = False
                result = f"Unexpected ack received from PJ after sending command: {ack}. Perhaps a command got cancelled because a new connection was made."
                self.logger.error(result)
    ...
    def power_on(
        ...
        Turns on PJ
        """
        return asyncio.run(self.power_on())
    ...
    def power_off(
        ...
        Turns off PJ
        """
        return asyncio.run(self.power_off())
