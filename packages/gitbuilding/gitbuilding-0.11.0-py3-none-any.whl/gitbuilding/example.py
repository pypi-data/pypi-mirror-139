...
def output_example_project():
    ...
    # IMAGE DIRECTORY
    make_local_dir(newdir+"/images")

    # CONFIG FILE
    write_local_file(newdir+"/buildconf.yaml", example_config())

    # OVERVIEW FILE
    write_local_file(newdir+"/index.md", example_landing())

    # TESTPAGES
    for i, page in enumerate(["testpage1.md", "testpage2.md"]):
        write_local_file(newdir+"/"+page, testpage(f"Test Page {i+1}"))

    write_local_file(newdir+"/widget.md", example_part_page())

    # PARTS LIST
    write_local_file(newdir+"/Parts.yaml", example_partlib())

    # README
    write_local_file(newdir+"/README.md", example_readme())

    # PARTS LIST
    write_local_file(newdir+"/.gitignore", example_gitignore())
