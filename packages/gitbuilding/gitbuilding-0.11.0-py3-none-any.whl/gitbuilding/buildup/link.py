...
class LinkData:
    ...
    def _non_part_link_types(self):
        return [self.step, self.output, self.bom, self.previewpage, self.zip]
...
class LinkDataSchema(Schema):
    ...
    pattern = fields.Str(missing=None, allow_none=True)
    step = fields.Bool(missing=False)
    output = fields.Bool(missing=False)
    bom = fields.Bool(missing=False)
    previewpage = fields.Bool(missing=False)
    zip = fields.Bool(missing=False)
    hidden = fields.Bool(missing=False)
    _variables = fields.Dict(missing=None, allow_none=True)
...
class BaseLink():
    ...
    * Image
    ...
    def weblink(self):
        """
        Wether the link is a web link or a local link
        """
    ...
    def is_buildup(self):
        """
        Return whether this is a buildup link. Note that a link to a
    ...
    def as_output_file(self, files_to_zip=None):
        ...
        if files_to_zip is not None:
            ValueError('files_to_zip should only be set for zip links')
    ...
    def link_md(self, url_translator=None, text_override=None):
        ...
        if self.is_output:
            ...
            text = '' if self.buildup_data.hidden else self.linktext
        ...
        if text_override is None:
            ...
            text = text_override
        kramdown = '{:target="_blank"}' if self._weblink else ''
        return f'[{text}]({location} "{self.alttext}"){kramdown}'
...
class Link(BaseLink):
    ...
    def as_output_file(self, files_to_zip=None):
        ...
        if self._weblink or self.internal_link:
            return None
        return super().as_output_file(files_to_zip)
