...
def replace_regex_group(string, replacement, match_obj, group_no):
    ...
    rep = match_obj.string[:match_obj.start(group_no)]+replacement+match_obj.string[match_obj.end(group_no):]
...
def _check_if_condition(if_condition, variables):
    ...
    cond_regex = (r'^((var_[a-zA-z0-0_\-]+) +(is(?: +not)?) +([a-zA-z0-0_\-]+))'
                  r'(?: +(?:and|or) +var_[a-zA-z0-0_\-]+ +is(?: +not)? +[a-zA-z0-0_\-]+)*$')
...
def _parse_include_dict(inc_dict, variables):
    ...
    for key in inc_dict:
        if key.startswith('var_'):
            #Creeate dictionary if currently None
...
def parse_inline_data(data):
    ...
    empty_is_true = ['step', 'output', 'bom', 'previewpage', 'zip', 'hidden']
    if data in [None, '']:
        return None
...
def separate_yml_md(raw_md):
    ...
    """
    try:
        md_metadata = frontmatter.loads(raw_md, handler=YAMLHandler()).to_dict()
...
class BuildUpParser():
    ...
    def _preprocessed_md_inject_details(self, details_md):
        ...
        return self._preprocessed_md.replace(self.get_title_match(), self.get_title_match()+details_md)
    ...
    def _preprocess_md(self, variables):
        ...
        for inc_statement in self._get_include_statements(updated_md):
            updated_md = self._process_include_statment(updated_md,
                                                        inc_statement,
                                                        variables,
                                                        url_translator)
    ...
    def _process_include_statment(self, updated_md, inc_statement, variables, url_translator):
        ...
        return self._execute_include_statment(updated_md, inc_statement, page_obj, url_translator)

    def _execute_include_statment(self, updated_md, inc_statement, page_obj, url_translator):
        """
        Replace incldue stament with contents of page
        """
        try:
...
class Details:
    ...
    def as_markdown(self):
        """
        Return a markdown interpretation of the details header. This is not used when generating HTML
        due to the serious limitations of markdown tables.
        """
        md = "\n\n"
    ...
    def as_output_dict(self, page_path, url_translator):
        ...
        for key in self._dict:
            md = proccess_buildup_snippet(self._dict[key], page_path, url_translator)
            out_dict[key] = md
        return out_dict
