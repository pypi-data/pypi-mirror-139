...
def findBricks():
    ...
    '''
    print(f'With following Bricks:')
    for node in connect.unit_id:
        for i in range(local.info[node]['number of connected bricks']):
            id = slave.id[node, i]
            ...
            if found == "None":
                ...
                error.close()
                brick_info += f'{i + 1}: ID: {slave.id[node, i] // 1000}-{slave.id[node, i] % 1000} Instance: None Hardware Revision: {slave.hard_rev[node, i]}\n'
            else:
                globals()[f"Brick{i + 1}"] = found()

                found = "Brick"+str(i+1)
                globals()[f"Brick{i+1}"].node = node
                globals()[f"Brick{i+1}"].name = bricks[id]
                globals()[f"Brick{i+1}"].brick_id = slave.id[node, i]
                globals()[f"Brick{i+1}"].brick_no = i + 1
                globals()[f"Brick{i+1}"].data_length_miso = slave.data_length_miso[node, i]
                globals()[f"Brick{i+1}"].data_length_mosi = slave.data_length_mosi[node, i]
                globals()[f"Brick{i+1}"].offset_miso = slave.offset_miso[node, i]
                globals()[f"Brick{i+1}"].offset_mosi = slave.offset_mosi[node, i]
                brick_info += f'{i + 1}: ID: {slave.id[node, i] // 1000}-{slave.id[node, i] % 1000} Instance: {found} Hardware Revision: {slave.hard_rev[node, i]}\n'
    print(brick_info)
    print("To abort the program please press STRG + C\n\n")
    global MODE
    MODE = input("To Start press 'y' or to exit the application press 'e'")
    while MODE != 'y' and MODE != 'e':
        MODE = input("To Start press 'y' or to exit the application press 'e'")
    if MODE == 'e':
        bB_update.stopThread = True
        exit()
...
class CAE_B3U4I:
    ...
    def calcEnergy(self):
        ...
        if self.result_cnt != self.old_result_cnt:
            ...
            if self.energy[0] > sys.maxsize:
                self.energy[0] = 0
            self.energy[1] += energy_val * self.power[1]
            if self.energy[1] > sys.maxsize:
                self.energy[1] = 0
            self.energy[2] += energy_val * self.power[2]
            if self.energy[2] > sys.maxsize:
                self.energy[2] = 0
        return self.energy
...
class CAE_G2Mi2Ao:
    ...
    def pulse_input_sum1(self, ):
        ...
        lock.release()
        if self.pulse_sum1 >= sys.maxsize:
            self.pulse_sum1 = 0
    ...
    def pulse_input_sum2(self, ):
        ...
        lock.release()
        if self.pulse_sum2 >= sys.maxsize:
            self.pulse_sum2 = 0
