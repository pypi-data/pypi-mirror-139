...
from meili_sdk.websockets.models.slow_down_message import SlowDownMessage
from meili_sdk.websockets.models.task import Task, Batched_Task
    ...
    def run_in_thread(self, wait_to_connect=True):
        try:
            self.thread = threading.Thread(
                target=self.run, args=(), kwargs={"loop": True}
            )
            self.thread.start()
