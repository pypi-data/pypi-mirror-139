...
class MssqlToOracleOperator(BaseOperator):
    ...
    def __init__(
            ...
            *args,
            **kwargs):
        super(MssqlToOracleOperator, self).__init__(*args, **kwargs)
    ...
    def execute(self, context):
        try:
            ...
            self.log.info('Executing: %s', self.from_sql)
            ms_data = hook.get_records(self.from_sql)
            self.log.info('Executing ms_data: %s', ms_data)
            if self.insert_table!=None:
                to_hook.insert_rows(self.insert_table, ms_data)
            else:
                for d in ms_data:
                    self.log.info('Executing ms_data: %s', d)
