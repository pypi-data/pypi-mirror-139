...
def buildDecisionTree(config):
    ...
    if geneticProgramming:
        ...
        chromosome_list, forest = GPFE(cntnAttr, attribute_new, config)
    ...
    winner_attribute, train_data = findGains(train_data, attribute_new, 1, max_depth)
    ...
    for _leaf, _index in enumerate(
            tqdm(range(len(classes)), desc=Fore.GREEN + Style.BRIGHT + "Creating Root...           ", mininterval=0.1,
                 ncols=150)):
    ...
    while len(set(filter(lambda x: x.terminateBuilding is False, leaf_list))) >= 1:
        ...
        for _leaf in tqdm(notTerminated,
                          desc=Fore.GREEN + Style.BRIGHT + "Creating Tree...(Depth = " + str(depth) + ')',
                          mininterval=0.1, ncols=150):
            attribute_new = copy.deepcopy(attribute)
            if geneticProgramming:
                cntnAttr = list(filter(lambda x: x.type is 'Continuous', attribute_new))
            ...
            winner_attribute, _leaf.dataset = findGains(_leaf.dataset, attribute_new, depth, max_depth)
...
def findGains(data, attribute, depth, max_depth):
    gains = []
    ...
    for _index in range(len(attribute)):
        if attribute[_index].name != 'Decision':
            if attribute[_index].type is 'Continuous':
                data = processContinuousFeatures(data, attribute[_index], entropy, depth, max_depth)
    ...
    if winner_attribute.type is 'Continuous':
        data = processContinuousFeatures(data, winner_attribute, entropy, depth, max_depth)
