...
def read_list_data():
    ...
    if time_slice is not None:
        if time_slice[1] is None:
            time_slice = (time_slice[0], data.get_end_time())
    else:
        time_slice = [0, data.get_end_time()]

    if arg_dict.plotstyle is not None:
        # style should be a style dict or path to matplotlib stylesheet. In practise it is a stylesheet saved in
        # config/stylecfg dir
    ...
    if arg_dict.save:
        ...
        for aplot in plot_list:
            ...
            hut.write_ascii(aplot, out_path=data_path, out_name=legend, calibrate=arg_dict.calibrate, saveall=arg_dict.save_full)
...
def parse_inputs():
    ...
    :param data_names:  Base names of the data, one for each path.
    :param plot:        Name of the plot configuration file without the _plotcfg.json.
    :param time:        Time interval in given
    :param reset:       Flag to read data again from raw files
    :param calibrate:   Plot data with calibrated or raw axes
    :param save:        Plotted data as .csv files
    :param saveall:     Plotted data as .csv files, but including empty channels (for math operations)
    :param style:       Name of the style configuration file without the _stylecfg.json.
    '''
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Read and plot data.')
    parser.add_argument('detname', metavar='detname', type=str,
                        help='detector configuration file (without the _cfg.json)')
    parser.add_argument('directory', metavar='datadir', type=str, nargs='?',
                        help='Data directory. Either an absolute path, or path relative to data directory.')
    parser.add_argument('plot', metavar='plot', type=str, nargs='?',
                        help='plot macro name')
    parser.add_argument('-f', '--filename', metavar='file', type=str,
                        help='name of datafile if not the same as datadir. Wildcards can be used, but the file name'
                             'has to be unique.')
    parser.add_argument('-t', '--time', metavar=('t'), type=str, nargs='+',
    ...
    parser.add_argument('-r', '--reset', action='store_true',
                        help='reread the data from binary file')
    parser.add_argument('-u', '--uncalibrate', action='store_false',
                        help='skip energy calibration')
    parser.add_argument('-s', '--save_csv', action='store_true',
                        help='Save a csv file of the plot')
    parser.add_argument('-ss', '--save_fullcsv', action='store_true',
                        help='Save a csv file of the plot including empty bins for math')
    parser.add_argument('-S', '--save_phd', action='store_true',
                        help='Save a phd file of the plot')
    parser.add_argument('-p', '--plotstyle', metavar='style_name', type=str,
    ...
    with open('local_cfg.json') as lc:
        l_cfg = json.load(lc)
    ...
    confd = load_config(args.detname, l_cfg, paths)
    ...
    names = []
    for adir in paths:
        base_name = parse_file(adir, args.filename, raw_ext=confd.det['extension'])
        print('base_name:', base_name)
        if len(base_name) > 1:
            print('too many data files!')
            sys.exit()
    ...
    if args.time is not None:
        time_slice = parse_time(args.time)
    else:
        time_slice = None

    if args.plotstyle is not None:
        plotstyle = args.plotstyle + '.mplstyle'
    else:
        plotstyle = None

    if args.save_fullcsv:  # full csv switch sets csv saves on automatically
        args.save_csv = True

    print('debugprint, names:', paths, names)
    if not args.profile:
        #(cfg, data_dirs, data_names, plot, time_slice=None, reset=False, calibrate=True,
        #      save=False, saveall=False, style=None)
        read_data(confd, paths, names, args.plot, time_slice=time_slice, reset=args.reset, calibrate=args.uncalibrate, 
                  save=args.save_csv, saveall=args.save_fullcsv, phd=args.save_phd, effcal=args.effcal,
                  style=plotstyle)
    else:
        import cProfile
        import pstats, io
        ...
        with open('prof-stats.txt', 'w') as outfile:
            ps = pstats.Stats(pr, stream=outfile).sort_stats(sortby)
            ps.print_stats()
        ps.print_stats()
    '''
    read_list_data()