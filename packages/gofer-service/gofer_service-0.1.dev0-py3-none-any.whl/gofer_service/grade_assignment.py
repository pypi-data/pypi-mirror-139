...
async def grade_assignment(submission, sec='3', assignment='lab01'):
    ...
    try:
        ...
        if "hw" in assignment:
            assign_type = "hw"
        solution_path = '/opt/materials-x22-private/materials/x22/{assign_type}/{sec}/{assignment}/autograder.zip'
        zip_path = solution_path.format(assign_type=assign_type, sec=sec, assignment=assignment)
        command = [
                'otter', 'run',
                '-a',
                zip_path,
