...
class Query:
    ...
    consistent_datatypes = {"numeric":
                                ["bit", "smallmoney", "money", "int",
    ...
    def _field_spell_out(self, field):
        ...
        consistent_vec = self.consistent_datatypes[field.internalDataType.lower()]
        if (
    ...
    def _where_spell_out(self, where):
        ...
        for where_condition in where:
            if isinstance(where_condition, Condition):
                ...
                consistent_vec = self.consistent_datatypes[where_condition.field.internalDataType.lower()]
                if (
    ...
    def _groupby_spell_out(self, clause):
        consistent_vec = self.consistent_datatypes[clause.field.internalDataType.lower()]
...
class SmartQuery:
    ...
    def spell_out_untyped(self):
        spell_array = []
        for query in self.queries:
            spell = query.spell_out_untyped()
            spell_array.append(spell)
        return spell_array
