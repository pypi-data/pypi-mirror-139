...
class MeshCtrl():
    ...
    async def get_mesh_groups(self) -> dict:
        data = {
            "action": "meshes"
        }

        return await self._websocket_call(json.dumps(data))


    # created a group with the specified name
    async def create_mesh_group(self, name: str) -> dict:
        data =  {
            "action": "createmesh",
            "meshname": name,
            "meshtype": 2,
    ...
    async def run_command(self, node_id: str, command: str, runAsUser: int = 0) -> dict:
        data =                     {
            "action": "runcommands",
            "cmds": command,
