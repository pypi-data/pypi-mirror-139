...
def create_phantom(sqlshell, time_in, time_out):
    command = "INSERT INTO records (car_number, brutto, tara, cargo, time_in, " \
              ...
              "(SELECT id FROM clients WHERE short_name='Физлицо')," \
              "(SELECT id FROM trash_cats WHERE cat_name='ТКО'), " \
...
def check_can_create_phantom(sql_shell, ar_instance, time_out):
    if (not check_last_record_time_in(sql_shell) and check_in_ar(ar_instance)
            and not check_last_record_time_out(sql_shell, time_out)):
        return True


@wsqluse.wsqluse.tryExecuteGetStripper
def check_last_record_time_in(sql_shell, wait_minutes=3):
    timenow = datetime.datetime.now() - datetime.timedelta(minutes=wait_minutes)
...
def check_in_ar(ar_instance):
    if ar_instance.is_status_ready:
        return True
