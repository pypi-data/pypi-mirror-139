...
class CacheState(abc.ABC, Generic[StateType]):
    ...
    def __enter__(self) -> StateType:
        ...
        logger_no_user_data.debug('Acquired state lock')
        logger_no_user_data.debug(f'Does state path {self._state_path} exist? {os.path.exists(self._state_path)}')
        if os.path.exists(self._state_path):
            logger_no_user_data.debug('Opening and reading state path')
            with open(self._state_path, mode='r') as file:
                logger_no_user_data.debug('Opened state path. Loading json')
                self._state = json.loads(file.read())
        else:
            logger_no_user_data.debug('State path does not exist, getting default state')
            self._state = self._get_default_state()
            logger_no_user_data.debug('Opening and reading state path')
            with open(self._state_path, mode='w') as file:
                logger_no_user_data.debug('Opened state path. Dumping json')
        ...
        logger_no_user_data.debug(f'Got LFS state {self._state}')
    ...
    def _acquire_state_lock(self) -> None:
        logger_no_user_data.debug('Acquiring state lock...')
        timeout_seconds = 5.0
        seconds_to_sleep = 0.5
        logger_no_user_data.debug(
            f'Does lock path {self._state_lock_path} exist? {os.path.exists(self._state_lock_path)}'
        )
        while os.path.exists(self._state_lock_path):
            logger_no_user_data.debug('Sleeping until lock path no longer exists')
            time.sleep(seconds_to_sleep)
            timeout_seconds -= seconds_to_sleep
            if timeout_seconds < 0:
                logger_no_user_data.debug('Timed out while waiting for lock file')
                raise CacheStateError('Cache state timed out waiting for lock file')

        logger_no_user_data.debug('Making cache dir')
        os.makedirs(self._cache_dir, exist_ok=True)
        logger_no_user_data.debug('Opening lock file')
        lock_file = open(self._state_lock_path, mode='x')
        logger_no_user_data.debug('Closing log file')
        lock_file.close()
        logger_no_user_data.debug('Closed log file')
