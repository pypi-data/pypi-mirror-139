...
def orders_list(options):
    ...
    # connections_data, errors = remote.api.get("/connections")
    # connections = {k: v["name"] for k, v in connections_data.items()}

    # if connection not in connections:
    #     click.echo(f"Connection `{connection}` doesn't exist or was deleted.")
    #     return
    endpoint = "/orders"
    payload = {}

    if connection:
        endpoint = "/connection/{connection}/orders".format(connection=connection)
    ...
    if not connection:
        # display count
        for conn, orders in data.items():
            table_data.append({
                "connection": conn,  # connection.get(conn, conn),
                "orders": len(orders)
            })
        ...
        # display  order list
        if not data:
            click.echo("\nNo orders found for connection {}".format(connection))
            return
...
def order_create(options):
    ...
    connections = {}
    supported_connections, errors = remote.api.get("/connections")
    for key, connection in supported_connections.items():
        connections[connection['name']] = connection["connection_id"]
    selected_connection = connections[inputs.option_selector(
        "Execute via connection", list(connections.keys()))]

    payload["strategy_id"] = selected_strategy
    payload["connection_id"] = selected_connection
...
def order_update(options):
    ...
    click.echo(f"Modify order for {order.get('ticker')} on {order.get('connection_id')}:\n")
