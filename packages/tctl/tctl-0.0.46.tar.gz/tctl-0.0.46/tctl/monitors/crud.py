...
def monitor_create(options):
    ...
    if kind == "price":
        ...
        connection = connections[inputs.option_selector(
            "Monitor position on broker connection", list(connections.keys()))]
...
def monitor_delete(options):
    ...
    utils.success_response(
        f"The monitor `{monitor}` was removed from your connection")
