...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(
        ...
        self.menus = utils.struct(
            ...
            allModels=QtWidgets.QMenu(self.tr("Select &Model")),
        ...
        self.minMaxNeuronValues = [25,700]
...
#        self.currentModel = self.settings.value("currentModel", str) or None
    ...
    def autosegmentation(self):
        ...
        if self.imagePath != None:
            ...
            if not self.imagePath.endswith((".png", ".tiff", "tif")):
                ...
                if self.imagePath.endswith(".png"):
                    im_output.save(osp.splitext(self.imagePath)[0] + "_curr.png")
                    self.imagePath = osp.splitext(self.imagePath)[0] + "_curr.png"
                    self.loadFile(self.imagePath)
                elif self.imagePath.endswith(".tiff"):
                    im_output.save(osp.splitext(self.imagePath)[0] + "_curr.tiff", quality=100)
                    self.imagePath = osp.splitext(self.imagePath)[0] + "_curr.tiff"
                    self.loadFile(self.imagePath)
                elif self.imagePath.endswith(".tif"):
                    im_output.save(osp.splitext(self.imagePath)[0] + "_curr.tif", quality=100)
                    self.imagePath = osp.splitext(self.imagePath)[0] + "_curr.tif"
                    self.loadFile(self.imagePath)
        ...
        if self.imagePath != None:
            ...
            if self.imagePath.endswith((".png", ".tiff", "tif")):
                ...
                minArea, maxArea = self.minMaxNeuronValues
                pred_main(self.imagePath.replace("\\", "/"), model=self.currentModel, minimal_area_neuron_in_pixels=minArea, maximal_area_neuron_in_pixels=maxArea)

                self.loadPolygons()
                self.setClean()
                self.imagePath = im_path
    ...
    def adjustMinNeuronSize(self, value):
        minArea, ok = QtWidgets.QInputDialog().getInt(self, "Choose minimum Neuron Area for Neuron Labeling (default: 25)",
                                                      "Minimum Area (px)",
    ...
    def adjustMaxNeuronSize(self, value):
        maxArea, ok = QtWidgets.QInputDialog().getInt(self,
                                                      "Choose maximum Neuron Area for Neuron Labeling (default: 700)",
                                                      "Maximum Area (px)",
    ...
    def getJSONFile(self):
        ...
        # one_dir_up = label_file.split("/")[-1]
        # label_file = label_file.replace(one_dir_up, "predictions/img")
    ...
    def refresh(self):
        ...
        imlist = [filename for filename in allfiles if filename[-4:] in [".tif", "tiff"]]

        # Assuming all images are the same size, get dimensions of first image
        w, h = Image.open(self.lastOpenDir + "/" + imlist[0]).size
        N = len(imlist)
        ...
        out.save(self.lastOpenDir + "/1-" + str(N) + "_mean.png")
        ...
        self.imagePath = self.lastOpenDir + "/1-" + str(N) + "_mean.png"
        self.loadFile(self.imagePath)
