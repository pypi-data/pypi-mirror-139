...
#****
PLOT_BOUNDS_MARKERS = '*#&='
def plotmesx(mes, symbattr: str, nlx: int = 6, ncx: int = 4, cmt: str = '',
            fgname: str = None, inter: bool = True, orient: Literal['h', 'v', ''] = '') -> None:
...
def plotmes(mes, symbattr: str, nl: int = 3, nc: int = 2, cmt: str = '', fgname: str = None,
            inter: bool = True, orient: Literal['h', 'v', ''] = '') -> None:
    ...
    for i, item in enumerate(mes[0:nl*nc], start=1):
        ...
        if isinstance(item, tuple):
            ...
            if len(item) > 3:
                ...
                ax.plot(talt/xscale, alt, 'm-', label='Alternate')
                ymin, ymax = min(ymin, min(alt)), max(ymax, max(alt))
        if not bresidu:
        ...
        if nbm > 0:
            ...
            if bresidu:
                tm, r, nm = tsadd(tm, -np.array(m), nm, tv, v, nv)
...
def pageplot(mes, cmt: str, nl: int = 3, nc: int = 2, fgname: str = None,
            inter: bool = True, lang: str = 'FR', orient: Literal['h', 'v', ''] = '') -> None:
...
def pageplotx(mes, cmt: str, nlx: int = 6, ncx: int = 4, fgname: str = None,
            inter: bool = True, lang: str = 'FR', orient: Literal['h', 'v', ''] = '') -> None:
    '''Multipage plot'''
    if fgname:
        fnam, fext = splitext(fgname)
