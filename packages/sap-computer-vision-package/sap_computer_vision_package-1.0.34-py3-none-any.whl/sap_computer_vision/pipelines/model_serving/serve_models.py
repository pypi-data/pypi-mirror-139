    ...
    def predict(self, input_dict, **kwargs):
        ...
        for img in images:
            ...
            if self.task == 'OBJECT_DETECTION':
                ...
                if (self.iou_threshold, float) and self.iou_threshold > 0.0:
                    indices = nms(res['pred_boxes'].tensor, res['scores'], self.iou_threshold).detach().numpy()
                else:
                    indices = torch.arange(len(res['scores']))
                res['scores'] = res['scores'][indices].tolist()
                res['pred_boxes'] = res['pred_boxes'].tensor[indices].tolist()
                res['pred_classes'] = res['pred_classes'][indices].tolist()
