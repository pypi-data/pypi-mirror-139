...
def factor_analysis(full_factor_value,
    ...
    ic_func = stats.spearmanr if rank_ic else stats.pearsonr
    ic_func = wrapped_ic_func(ic_func)
    ...
    for index, date in enumerate(target_value.index):
        ...
        if neutralization in {'style', 'industry_style'}:
            ...
            data = None
        neutralized = neutralize(normalized, neutralization, industry_tag, data)

        f_s = neutralized.reindex(stocks)
        ...
        total_returns[date] = (np.sum(weight.values * (ret.values + 1)) + 1) / (np.sum(weight.values) + 1) - 1
    ...
    # 多空收益率
    total_returns_df = pd.Series(total_returns)
