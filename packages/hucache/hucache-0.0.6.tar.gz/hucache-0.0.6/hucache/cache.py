...
class Cache(object):
    ...
    def __prune_arguments(self, *args, **kwargs):
        arg_spec = inspect.getargspec(self._func)
...
class CacheFactory(object):
    ...
    @staticmethod
    def from_store_url(store_type, store_url, prefix=None, default_timeout=DEFAULT_TIMEOUT):
        if store_type == "redis":
    ...
    def cache(self, ruler, timeout=None):
        def wrapper(func):
            return Cache(self._store, self._prefix, func, ruler, timeout or self._default_timeout)
        return wrapper
