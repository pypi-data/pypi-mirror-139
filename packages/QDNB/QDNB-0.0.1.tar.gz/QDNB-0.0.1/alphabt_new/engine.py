...
from .lib import (mypivot, neutralize, standardize, winsorize, mkt_neutral, weight_control, update_weight, cal_group_return, Nancorr,
                  RankNancorr, equalweight, weightby)
...
class BackTestingEngine:
    ...
    def cal_alpha_pnl(self, tgt_weight, shift=1, longonly=False, cap_invest_on_each_side=0.5, get_group_contri=True):
        ...
        self.turnover, self.returns, self.g_returns, self.na_log = [], [], [], []
        ...
        """ result preprocess """
        result = pd.DataFrame({'turnover': self.turnover, "gross_return": self.returns},
    ...
    def perf_eval(self, result, perf_cal_return="gross_return"):
        ...
        assert condition, "Do not find {perf_cal_return} in result"
    ...
    def multidim_backtest(self, shift=None, benchmark=None, groups=None, plotit=False,
                          perf_cal_return="gross_return", *args):
        ...
        if plotit:
            self.plot_pnl(ls_pnl, lsidx_pnl, group_pnl, *args)
        return ls_pnl, lsidx_pnl, group_pnl, perf_ls, perf_lsidx

    def combo_backtest(self, shift=None, benchmark=None, plotit=False, perf_cal_return="gross_return", *args):
        if shift is None:
            shift = self.ctx.shift
        if benchmark is None:
            benchmark = self.ctx.benchmark
        ...
        lsidx_pnl[f"prop_return_over_benchmark"] = lsidx_pnl["return"] - indexrtr
        ...
        if plotit:
            self.plot_comb_pnl(l_pnl, lsidx_pnl, *args)
        return l_pnl, lsidx_pnl, perf_l, perf_lsidx

    @staticmethod
    def plot_pnl(ls, lsidx, gprtr, size=(12, 12)):
        plt.figure(figsize=size)
        ax1 = plt.subplot(3, 1, 1)
        ax1.plot(ls[["return", "gross_return"]].cumsum(axis=0))
        ax1.legend(["prop pnl", "gross pnl"])
        ax1.set_title("ls")

        ax2 = plt.subplot(3, 1, 2)
        ax2.plot(lsidx[["return", "gross_return"]].cumsum(axis=0))
        ax2.legend(["prop pnl", "gross pnl"])
        ax2.set_title("lsidx")

        ax3 = plt.subplot(3, 1, 3)
        ax3.plot(gprtr.cumsum(axis=0))
        ax3.legend(gprtr.columns)
        ax3.set_title("group")
        plt.show()

    @staticmethod
    def plot_comb_pnl(l_pnl, lsidx_pnl, size=(12, 12)):
        plt.figure(figsize=size)
        ax1 = plt.subplot(2, 1, 1)
        ax1.plot(l_pnl[["return", "gross_return"]].cumsum(axis=0))
        ax1.legend(["prop pnl", "gross pnl"])
        ax1.set_title("long pnl")

        ax2 = plt.subplot(2, 1, 2)
        ax2.plot(lsidx_pnl[["prop_return_over_benchmark", "gross_return_over_benchmark"]].cumsum(axis=0))
        ax2.legend(["prop pnl", "gross pnl"])
