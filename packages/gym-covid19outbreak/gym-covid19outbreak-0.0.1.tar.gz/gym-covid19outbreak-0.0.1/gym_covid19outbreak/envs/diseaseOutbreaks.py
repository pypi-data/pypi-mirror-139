...
class DiseaseOutbreaks:
	...
	def reset_game(self):
		...
		for _ in range(ENEMY_COUNT):
			sars = SarsCovII()
			sars.scale = 0.5
			sars.rect.centerx = random.randrange(400, SCREEN_WIDTH - sars.image.get_width())
	...
	def __view_update(self, mode='human'):		
		...
		if not self.__game_over:
			...
			for flower in plant_hit:
				...
				self.score += 2
				self.collect_plant += 1
				self.reward = 1 + self.collect_plant
