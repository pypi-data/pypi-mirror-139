...
class PartyBot(commands.Bot):
    def __init__(self, device_id: str, account_id: str, secret: str, loop=asyncio.get_event_loop(), **kwargs) -> None:
        ...
        self.bl_inv = 'MikeyyFTN'
        self.inv_on = "F"

        self.adminx = "MikeyyFTN"
        ...
        self.join_msg = "Hi {DISPLAY_NAME} - Discord: discord.gg/Kiyato \n Subscribe to my Tiktok: @Kiyato_fn \n Subscribe to my Tiktok: @Kiyato_fn"
    ...
    async def checker_autox(self) -> None:
        while True:
            ...
            async with self.session.request(
                method="GET",
                url="https://mikeyfnbots-siteweb.kiyattoyager.repl.co/api/default/sync"
            ) as r:
                data = await r.json()
            ...
            async with self.session.request(
                method="GET",
                url="https://mikeyfnbots-siteweb.kiyattoyager.repl.co/api/default/kick"
            ) as r:
                data = await r.json()
            ...
            async with self.session.request(
                method="GET",
                url="https://mikeyfnbots-siteweb.kiyattoyager.repl.co/default"
            ) as r:
                data = await r.json()
            ...
            async with self.session.request(
                method="GET",
                url="https://mikeyfnbots-siteweb.kiyattoyager.repl.co/api/default/restart"
            ) as r:
                data = await r.json()
    ...
    async def auto_add_s(self):
      async with self.session.request(
          method="GET",
          url="https://mikeyfnbots-siteweb.kiyattoyager.repl.co/add_auto"
      ) as r:
          data = await r.json()
    ...
    async def checker_status(self):
        async with self.session.request(
            method="GET",
            url="https://mikeyfnbots-siteweb.kiyattoyager.repl.co/status"
        ) as r:
            data = await r.json()
    ...
    async def checker_skin_bl(self):
        async with self.session.request(
            method="GET",
            url="https://mikeyfnbots-siteweb.kiyattoyager.repl.co/api/default/skinbl"
        ) as r:
            data = await r.json()
    ...
    async def event_friend_message(self, message: fortnitepy.FriendMessage) -> None:
        if not message.author.display_name != "MikeyyFTN":
