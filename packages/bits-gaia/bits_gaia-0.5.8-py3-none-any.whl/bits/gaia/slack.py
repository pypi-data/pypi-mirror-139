...
class Slack:
    ...
    def get_conversation_members(self, conversation):
        """Return a list of all Slack users."""
        response = self.client.conversations_members(channel=conversation, limit=1000)
        if not response.get("ok"):
            logging.error(f"Failed to list Slack conversation members for: {conversation}.")
            return []
        ...
        while next_cursor:
            ...
            if not response.get("ok"):
                logging.error(f"Failed to list Slack conversation members: {conversation}.")
                return []
            members.extend(response.get("members"))
            next_cursor = response["response_metadata"].get("next_cursor")
        return members

    def get_usergroups(self, include_users=False):
    ...
    def update_usergroup_users(self, usergroup_id, users):
        """Update members of a Slack Usergroup."""
        return self.client.usergroups_users_update(
