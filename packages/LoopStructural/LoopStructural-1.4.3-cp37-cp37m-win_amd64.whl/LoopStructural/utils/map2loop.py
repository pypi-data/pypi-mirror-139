...
def process_map2loop(m2l_directory, flags={}):
    ...
    fault_fault_relations = pd.read_csv(
        m2l_directory + "/output/fault-fault-relationships.csv"
    )
    fault_strat_relations = pd.read_csv(
        m2l_directory + "/output/group-fault-relationships.csv"
    )
    ...
    fault_orientation_probabilities = flags.get(
        "fault_orientation_probabilities", False
    )
    ...
    with open(m2l_directory + "/graph/fault-fault-intersection.txt", "r") as f:
        for l in f:
            ...
            while i < len(intersection_data):
                fault_intersection_angles["Fault_{}".format(fault_id)].append(
                    [
                        "Fault_{}".format(int(intersection_data[i])),
                        intersection_data[i + 1],
                        float(intersection_data[i + 2]),
                    ]
                )
                i += 3
    ...
    thickness = {}
    max_thickness = 0
    thickness_file = flags.get(
        "thickness", m2l_directory + "/output/formation_summary_thicknesses.csv"
    )
        ...
        for l in file:
            if i >= 1:
                ...
                if thickness_probabilities:
                    ...
                    a = (0 - mean) / std
                    b = 100.0
                    thickness[linesplit[0]] = (truncnorm.rvs(a, b, size=1) * std)[
                        0
                    ] + mean
                else:
    ...
    if (
        np.sum(contact_orientations["polarity"] == 0) > 0
        and np.sum(contact_orientations["polarity"] == -1) == 0
    ):
    ...
    for i in groups["group number"].unique():
        ...
        for c, colour in zip(
            groups.loc[groups["group number"] == i, "code"],
            groups.loc[groups["group number"] == i, "colour"],
        ):
            strat_val[c] = np.nan
            if c in thickness:
                stratigraphic_column[g][c] = {
                    "max": val[g],
                    "min": val[g] - thickness[c],
                    "id": unit_id,
                    "colour": colour,
                }
                unit_id += 1
    ...
    for g in groups["group"].unique():
        ...
        for c in groups.loc[groups["group"] == g, "code"]:
            contact_orientations.loc[
                contact_orientations["formation"] == c, "feature_name"
            ] = supergroups[g]
    ...
    for fname in fault_orientations["formation"].unique():
        displacements.loc[displacements["fname"] == fname, "dip_dir"] = np.mean(
            fault_orientations.loc[
                fault_orientations["formation"] == fname, "DipDirection"
            ]
        )
    max_displacement = {}
    downthrow_dir = {}
    ...
    for f in displacements["fname"].unique():
        ...
        displacements_numpy = displacements.loc[
            displacements["fname"] == f,
            ["vertical_displacement", "downthrow_dir", "dip_dir", "X", "Y"],
        ].to_numpy()
        # index = np.argmax(np.abs(displacements_numpy[:, 0]), )
        index = np.argsort(np.abs(displacements_numpy[:, 0]))[
            len(np.abs(displacements_numpy[:, 0])) // 2
        ]

        max_displacement[f] = displacements_numpy[index, 0]
        ...
        if np.abs(displacements_numpy[index, 1] - displacements_numpy[index, 2]) > 90:
            ...
            fault_orientations.loc[
                fault_orientations["formation"] == f, "DipDirection"
            ] -= 180  # displacements_numpy[
            # index, 1]
        # find the middle of the fault as the mean of the line, average dip direction and the influence distance
        fault_centers[:3] = np.mean(
            fault_locations.loc[fault_locations["formation"] == f, ["X", "Y", "Z"]],
            axis=0,
        )
        fault_centers[3] = np.mean(
            fault_orientations.loc[
        ...
        fault_centers[4] = fault_dimensions.loc[
            fault_dimensions["Fault"] == f, "InfluenceDistance"
        ]
        fault_centers[5] = fault_dimensions.loc[
            fault_dimensions["Fault"] == f, "HorizontalRadius"
        ]
        if type(fault_slip_vector) == dict:
            ...
            fault_slip = np.array([0.0, 0.0, -1.0])
        stratigraphic_column["faults"][f] = {
            "FaultCenter": fault_centers[:3],
            "FaultDipDirection": fault_centers[3],
            "InfluenceDistance": fault_dimensions.loc[
                fault_dimensions["Fault"] == f, "InfluenceDistance"
            ].to_numpy(),
            "HorizontalRadius": fault_dimensions.loc[
                fault_dimensions["Fault"] == f, "HorizontalRadius"
            ].to_numpy(),
            "VerticalRadius": fault_dimensions.loc[
                fault_dimensions["Fault"] == f, "VerticalRadius"
            ].to_numpy(),
        ...
        if "colour" in fault_dimensions.columns:
            stratigraphic_column["faults"][f]["colour"] = fault_dimensions.loc[
                fault_dimensions["Fault"] == f, "colour"
            ].to_numpy()
    ...
    fault_orientations.drop(
        ["strike", "DipDirection", "dip", "DipPolarity"], inplace=True, axis=1
    )
    fault_orientations["feature_name"] = fault_orientations["formation"]
    fault_locations["feature_name"] = fault_locations["formation"]

    data = pd.concat(
        [tangents, contact_orientations, contacts, fault_orientations, fault_locations]
    )
...
def build_model(
    ...
    **kwargs
    ...
    model = GeologicalModel(
        boundary_points[0, :], boundary_points[1, :], rescale=rescale
    )
    ...
    if not skip_faults:
        ...
        for f in m2l_data["max_displacement"].keys():
            ...
            fault_vertical_radius = m2l_data["stratigraphic_column"]["faults"][f][
                "VerticalRadius"
            ]
    ...
    for i in np.sort(m2l_data["groups"]["group number"].unique()):
        ...
        group_features.append(model.create_and_add_foliation(g, **foliation_params))
