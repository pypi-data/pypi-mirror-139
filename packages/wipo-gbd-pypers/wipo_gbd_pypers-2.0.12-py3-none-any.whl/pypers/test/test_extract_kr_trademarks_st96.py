import unittest
from pypers.steps.fetch.extract.kr.trademarks_st96 import TrademarksST96
...
<Transaction xmlns="http://www.wipo.int/standards/XMLSchema/trademarks">
  <TradeMarkTransactionBody>
    ...
    for i in range(0, 10):
        xml_dest = os.path.join(dest, 'F1111%s.xml' % i)
        with open(xml_dest, 'w') as f:
            f.write(xml_content)
        xml_dest = os.path.join(dest, 'F1111%s.jpg' % i)
        with open(xml_dest, 'w') as f:
            f.write(xml_content)
...
def mock_get_nodevalue(*args, **kwargs):
    ...
    res = 'F1111%s' % mock_get_value_counter
...
class TestMerge(unittest.TestCase):
    ...
    cfg = {
        'step_class': ' pypers.steps.fetch.extract.kr.trademarks_st96.TrademarksST96',
        'sys_path': None,
        'name': 'TrademarksST96',
        'meta': {
            'job': {},
    ...
    def test_process(self):
        ...
        step = TrademarksST96.load_step("test", "test", "step")
        step.process()
