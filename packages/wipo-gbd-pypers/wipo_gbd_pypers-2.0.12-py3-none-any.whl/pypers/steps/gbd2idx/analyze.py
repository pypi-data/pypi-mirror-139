...
class Analyze(EmptyStep):
    ...
    def process(self):
        ...
        self.collection_name = self.collection.replace('gbd2idx_', '')
        ...
        index_collection_config = index_config.get_collection_config()
        ...
        for st13, record in self.files.items():
            ...
            for img_info in record['img_files']:
                ...
                is_analysed = self._is_analysed(img_info['crc'],
                                                latest_img_analysis)
                is_skipped = self._skip_feature(feature, index_collection_config)
    ...
    def _skip_feature(self, feature, config):
        return feature is 'Word' and not config['analyzeVerbalImages']
