...
def configure(set_default: bool = o(prompt='Do you want to set this bucket as default?', default=False)):
...
def get_bucket(bucket):
    ...
    if bucket_obj:
        try:
            typer.echo(typer.style(text='Authenticating Backblaze...', fg=typer.colors.BLUE))
...
def remote_upload(bucket: Optional[str] = None):
    url = None
    while url is None:
        url = str(p('Remote URL', default='https://wordpress.org/latest.zip')).strip()
    ...
    typer.echo(typer.style(text=f'Downloading data from {url}', fg=typer.colors.BLUE))
    ...
    with tempfile.NamedTemporaryFile(mode='w+b') as f:
        ...
        typer.echo(typer.style(text='Uploading file to Backblaze.', fg=typer.colors.BLUE))       
        if do_upload(path=f.name, save_as=save_as, bucket=bucket):
            base_url = bucket_obj.get('url')
            uploaded_url = f'{base_url}/{save_as}'
            typer.echo(typer.style(text=f'File has been uploaded: {uploaded_url}', fg=typer.colors.GREEN))
        else:
