...
class SettingsState(Singleton, SettingsREPMixin):
    ...
    def is_proxy_enabled(self):
        proxy_enable = self.config.get("api.proxy.enable")
        return bool(proxy_enable and str(proxy_enable).lower() in ("true"))

    @cached_property
    def proxy_url(self):
        return self.config.get("api.proxy.url", None)
    ...
    def process_ts_response(self, data):
        # Update settings here and make sure self.last_update is updated when we get new agent settings
        pass
