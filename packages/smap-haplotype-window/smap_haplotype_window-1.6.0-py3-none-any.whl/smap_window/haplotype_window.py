...
class _FilePathWrapper:
    ...
    def recursive_stem(self) -> str:
        """Return the name of the file, without any extensions."""
        previous = self._file.stem
        while True:
            if Path(previous).stem == previous:
                return previous
            else:
                previous = Path(previous).stem
...
class Fastq(_FilePathWrapper):
    ...
    def count_sequences_per_window(
        self, lookup: Dict[str, List[Tuple[Window, str]]], threads=1
    ) -> Dict[Window, Dict[str, int]]:
        opener = partial(xopen, threads=threads)
        counter = HaplotypeCounter(lookup)
            ...
            for read in fq_fh:
                ...
                try:
                    ...
                    for window, orientation in windows:
                        if orientation == "-":
                            read = reverse_complemented_sequence(read)
...
class WindowAdapter(LinkedAdapter):
    ...
    @classmethod
    def from_window(cls, window: Window) -> "WindowAdapter":
        ...
        upstream_adapter = FrontAdapter(
            anchor1.seq, max_errors=0, min_overlap=len(anchor1.seq)
        )
        downstream_adapter = BackAdapter(
            anchor2.seq, max_errors=0, min_overlap=len(anchor2.seq)
        )
        return cls(
            upstream_adapter,
            downstream_adapter,
...
class HaplotypeCounter:
    ...
    def __init__(self, lookup: Dict[str, List[Tuple[Window, str]]]) -> None:
        unqiue_windows: MutableSet = set(
            [window for windows in lookup.values() for window, _ in windows]
        )
        self._read_handlers: Dict[Window, AdapterCutter] = {
            window: AdapterCutter([WindowAdapter.from_window(window)])
            for window in unqiue_windows
        }
...
def match_bam_with_fastq(
    ...
    for bam in bam_files:
        bam_stem = bam.recursive_stem()
        matched = False
        for fastq in fastq_files:
            fastq_stem = fastq.recursive_stem()
            if bam_stem.startswith(fastq_stem):
                matched = True
...
def counting_worker(bam: Bam, fastq: Fastq, windows: Windows) -> pd.DataFrame:
    read_lookup = bam.sort_read_ids_per_window(windows)
    counts = fastq.count_sequences_per_window(read_lookup)
    LOGGER.info("Done sorting %s and %s", bam, fastq)
    return counts_to_dataframe(bam.stem, counts)


def main(args):
    parsed_args = parse_args(args)
    if not parsed_args.anchors.is_file():
        raise FileNotFoundError(
            f"Anchor .gff file {parsed_args.anchors} "
    ...
    with parsed_args.anchors.open("r") as open_gff, parsed_args.genome.open(
        "rb"
    ) as open_genome:
    ...
    if parsed_args.plot >= PLOT_ALL:
        frequency_matrix.plot_frequencies(
            parsed_args.plot_type, parsed_args.frequency_bounds
        )
        ...
        if parsed_args.dosage_filter:
            ...
            if parsed_args.plot >= PLOT_SUMMARY:
                ...
                if parsed_args.locus_correctness_filter:
                    ...
                    with parsed_args.anchors.open("r") as open_gff:
