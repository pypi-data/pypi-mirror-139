...
def parse_args(args):
    parser = ArgumentParser("haplotype-window")
    ...
    input_output_group.add_argument(
        "genome",
        type=Path,
        help="Reference genome to which the " + ".fastq files were mapped to.",
    )
    input_output_group.add_argument(
        "anchors",
        type=Path,
        help=".gff file containing the anchor "
        + "points, must contain NAME=<> in "
        + "column 9.",
    )
    input_output_group.add_argument(
        "alignments_dir",
        type=Path,
        help="Directory containing BAM and BAM "
        + "index files (default current "
        + "directory)",
    )
    input_output_group.add_argument(
        "sample_dir",
        ...
        + " that were mapped to the reference"
        + " genome to create the .bam files."
        + " The .fastq file names must have "
        + " the same prefix as the bam files "
        + " specified in --dir.",
    )
    input_output_group.add_argument(
        "guides",
        ...
        default=None,
        type=Path,
        help="Optional fasta file containing the"
        + "sequences from the guides. "
        + "Useful when primers against the vector "
        + "are used as a positive control. "
        + "In multiplex setting, this list of "
        + "sequences can be used to identify "
        + "exactly which guide(s) were used in "
        + "the CRISPR-Cas9/gRNA delivery vector.",
    )
    ...
    input_output_group.add_argument(
        ...
        action="store_true",
        default=False,
        help="Write .fastq files containing the reads for "
        + "each window in a seperate file per input sample.",
    )
    ...
    discrete_calls_group.add_argument(
        ...
        help=(
            "Create a new .gff file defining only the loci that were "
    ...
    filtering_group.add_argument(
        ...
        type=int,
        help=(
            "Minimal number of distinct haplotypes per locus across all "
    ...
    filtering_group.add_argument(
        ...
        type=float,
        help=(
            "Maximal number of reads per locus per sample, read depth "
    ...
    filtering_group.add_argument(
        ...
        type=float,
        help=(
            "Minimal haplotype frequency (in %%) to retain the haplotype "
    ...
    resources_group.add_argument(
        ...
        action="store_true",
        default=False,
        help="Reduces the memory load significantly, "
        "but increases time to calculate results.",
    )
