import copy
import requests
import competitivetracker
from .exceptions import CompetitiveTrackerAPIException
import base64
...
class RequestsTransport(object):
    ...
    def request(self, method, uri, params, **kwargs):
        ...
        if response.status_code == 204:
            return True
        if not response.ok:
            raise CompetitiveTrackerAPIException(response)
        try:
...
class Resource(object):
    ...
    def request(self, method, uri, params, **kwargs):
        ...
        headers = {
            'User-Agent': 'python-competitive-tracker/' + competitivetracker.__version__,
