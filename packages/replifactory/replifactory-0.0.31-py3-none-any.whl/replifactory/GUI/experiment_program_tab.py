...
def get_culture_options():
    culture_options = [BatchCulture, ChemostatCulture, TurbidostatCulture, MorbidostatCulture,
                       EnduranceStressCulture]  # BlankCulture, PatientCulture
...
class VialConfigWidget:
    def __init__(self, program_viewer_tab, vial_number):
        ...
        self.active_button = widgets.ToggleButton(description="", value=bool(self.culture._is_active), layout=Layout(width="40px"), icon="toggle-off")
        ...
        if self.culture is not None:
            ...
            left_panel = VBox([HBox([title, self.active_button,self.blank, self.inoculate]), algorithm, description_box, copypaste])
