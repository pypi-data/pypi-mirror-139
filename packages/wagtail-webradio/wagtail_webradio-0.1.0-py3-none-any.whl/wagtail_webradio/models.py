...
class Podcast(AutoSlugMixin, index.Indexed, ClusterableModel):
    ...
    sound_url = models.URLField(verbose_name=_("sound URL"))
    ...
    def clean(self):
        """
        Validate the sound URL by opening it and checking for errors. This can
        be disabled by setting `WEBRADIO_VALIDATE_PODCAST_URL` to `False`.
        """
        if self.sound_url and getattr(
            settings, 'WEBRADIO_VALIDATE_PODCAST_URL', True
        ):
            error = None

            try:
                urlopen(self.sound_url, timeout=2)
            except HTTPError as e:
                error = (
                    _("Unable to fetch the file at this URL (Error %d).")
                    % e.code
                )
            except URLError as e:
                error = (
                    _("Unexpected error while fetching the file (%s).")
                    % e.reason
                )

            if error:
                raise ValidationError({'sound_url': error})
