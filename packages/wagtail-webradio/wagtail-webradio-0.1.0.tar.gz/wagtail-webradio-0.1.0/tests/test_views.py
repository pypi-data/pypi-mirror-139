...
class TestRadioShowAdminViews(RadioShowAdminViewsMixin, AdminViewTestMixin):
    def test_index(self):
        ...
        assert html.find(
            'link',
            href='/static/wagtail_webradio/css/radio_show_podcasts_cell.css',
        )
...
class TestPodcastAdminViews(PodcastAdminViewsMixin, AdminViewTestMixin):
    ...
    @patch('wagtail_webradio.models.urlopen')
    @override_settings(WEBRADIO_VALIDATE_PODCAST_URL=True)
    def test_validate_sound_url(self, mock_urlopen):
        podcast = PodcastFactory()
        url = self.get_edit_url(podcast)
        ...
        podcast.refresh_from_db()
        assert podcast.sound_url == 'http://localhost/some-file'

    @patch('wagtail_webradio.models.urlopen')
    @override_settings(WEBRADIO_VALIDATE_PODCAST_URL=True)
    def test_validate_sound_url_empty(self, mock_urlopen):
        podcast = PodcastFactory()
        url = self.get_edit_url(podcast)
        ...
        assert 'sound_url' in response.context['form'].errors

    @patch('wagtail_webradio.models.urlopen')
    @override_settings(WEBRADIO_VALIDATE_PODCAST_URL=True)
    def test_validate_sound_url_http_error(self, mock_urlopen):
        podcast = PodcastFactory()
        url = self.get_edit_url(podcast)
        ...
        mock_urlopen.side_effect = HTTPError('url', 404, '', None, None)

        response = form.submit(status=200)
        assert response.context['form'].errors['sound_url'][0] == (
            "Unable to fetch the file at this URL (Error 404)."
        )

    @patch('wagtail_webradio.models.urlopen')
    @override_settings(WEBRADIO_VALIDATE_PODCAST_URL=True)
    def test_validate_sound_url_url_error(self, mock_urlopen):
        podcast = PodcastFactory()
        url = self.get_edit_url(podcast)
        ...
        mock_urlopen.side_effect = URLError('The reason is here')

        response = form.submit(status=200)
        assert response.context['form'].errors['sound_url'][0] == (
            "Unexpected error while fetching the file (The reason is here)."
        )
...
class TestGroupPermissionsView(AdminViewTestMixin):
    ...
    def test_form_media(self):
        ...
        assert html.find(
            'link',
            href='/static/wagtail_webradio/css/radio_show_permissions.css',
        )
