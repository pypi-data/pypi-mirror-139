...
def mail(subject, context, template, from_email, to_email, connection=None, reply_to=None, attach_files=[], cc=None, bcc=None):
	...
	# email debug mode
	email_mode = getattr(settings, HELPFUL_EMAIL_MODE, 'BLANK')
	email_debug_info = getattr(settings, HELPFUL_EMAIL_DEBUG_INFO, False)
	email_debug_to_email = getattr(settings, HELPFUL_EMAIL_DEBUG_TO_EMAIL, [])

	if email_mode == 'PROD':
		pass
	elif email_mode == 'DEBUG':
		if email_debug_info:
		...
		to_email = email_debug_to_email
		cc = []
		bcc = []
	elif email_mode == 'BLANK':
		return
	else:
		return
