    ...
    def get_item(self, item_id, n_retries=0, store_binary: bool = False) -> Optional[Dict]:
        ...
        if self._mode == LoadMode.ADHOC:
            ...
            if item is not None:
                    ...
                    with open(metainformation_path, 'r') as f:
                        try:
                            ...
                            if str_changed_date_json is not None and str_changed_date_item is not None:
                                changed_date_item = DLDSClient.parse_dlds_dataset_item_date(str_changed_date_item)
                                changed_date_json = DLDSClient.parse_dlds_dataset_item_date(str_changed_date_json)

                            if changed_date_json > changed_date_item:
                                logger.warning('The item\'s cached metainformation are newer than those from DLDS.'
                                               'Expecting an error and downloading the current item again. ')
                ...
                if not item['exists']:
                    item_serialized = self._dlds_client.download_dataset_item(
                    ...
                    for decoder in self._decoders:
                        ...
                        if item_file_extension in extensions:
                            try:
                                ...
                                logger.warning(f'Could not decode image for item {item_id}: {e}')
                                loading_error = True
                            break
