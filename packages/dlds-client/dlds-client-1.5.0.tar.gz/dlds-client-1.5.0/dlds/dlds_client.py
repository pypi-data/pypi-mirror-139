...
class DLDSClient:
    ...
    @staticmethod
    def get_dlds_dataset_item_date_format_string() -> str:
        return '%Y-%m-%dT%H:%M:%S.%fZ'

    @staticmethod
    def get_dlds_dataset_item_date_format_string_short() -> str:
        return '%Y-%m-%dT%H:%M:%SZ'

    @staticmethod
    def parse_dlds_dataset_item_date(date_string: Optional[str]) -> Optional[datetime]:
        ...
        try:
            return datetime.strptime(date_string, DLDSClient.get_dlds_dataset_item_date_format_string())
        except (TypeError, ValueError):
            pass

        try:
            return datetime.strptime(date_string, DLDSClient.get_dlds_dataset_item_date_format_string_short())
        except (TypeError, ValueError):
            pass
    ...
    @staticmethod
    def dlds_temp_dir(*sub_dirs: str) -> str:
        temp_dir = os.path.join(DLDSClient.dlds_dir(), 'temp', *sub_dirs)
    ...
    def get_dataset_items(self, dataset_id: Optional[int] = None,
                          ...
                          created_date_since: Optional[Union[str, datetime]] = None,
                          created_date_until: Optional[Union[str, datetime]] = None,
                          status: Optional[List[str]] = None,
                          query_expression: Optional[Condition] = None,
                          free_query_str: Optional[str] = None) -> Optional[List[Dict]]:
        """
        ...
        :param data_subsets_id:     Identifier of the subset whose content be downloaded or None if you don't want
        ...
        items_per_request: int = 5000

        def format_time(t: Optional[Union[str, datetime]]) -> Optional[str]:
        ...
        if free_query_str is not None:
            if not free_query_str.startswith('&'):
                free_query_str = f'&{free_query_str}'
            condition_str += free_query_str

        # Compile URL
        url: Optional[str] = f'{self.server_url}/dataset-items/?{condition_str}'
    ...
    def delete_dataset(self, dataset_id):
        ...
        try:
            response = self.http.delete(f'{self.server_url}/datasets/{dataset_id}')
            response.raise_for_status()
            return True
        except rq.exceptions.RequestException as e:
            logger.error(e)
    ...
    def create_data_subset(self, subset_name, dataset_id, description=''):
        ...
        try:
            response = self.http.post(f'{self.server_url}/data-subsets/', data=subset_dict)
            response.raise_for_status()
            data = response.json()
            return data
        except rq.exceptions.RequestException as e:
    ...
    def delete_data_subset(self, subset_id):
        ...
        try:
            response = self.http.delete(f'{self.server_url}/data-subsets/{subset_id}')
            response.raise_for_status()
            return True
        except rq.exceptions.RequestException as e:
            logger.error(e)
    ...
    def create_dataset_item(self,
        ...
        if created_date is not None:
            if type(created_date) in (int, float):
                created_date_dt = datetime.fromtimestamp(created_date)
            else:
                created_date_dt = created_date

            if type(created_date_dt) != datetime:
    ...
    def upload_inference_results(self, results, timestamp, iteration, model_id):
        ...
        >>>         'classification': {
        >>>             'classes': [],              # list of DLDS class IDs (for classification)
        ...
        >>>             'classes': [...]            # list of DLDS class IDs (for detection)
        ...
        response = self.http.post(f'{self.server_url}/bulk-data/',
                                  ...
                                  files=dict(data=(f'inference-data-{timestamp}-{model_id}.msgpack', msg_bytes)))
        response.raise_for_status()
