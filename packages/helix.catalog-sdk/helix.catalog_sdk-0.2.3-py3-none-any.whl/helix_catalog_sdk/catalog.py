...
class Catalog:
    ...
    def get_resource_unprocessed_directories(
        ...
        if resource.last_processed:
            ...
            last_processed: str = resource.last_processed.get_last_processed(
                environment
            )
            self.logger.info(
                f"Found last processed path {last_processed} for resource {resource.name}"
            )
            all_paths: List[str] = self.get_all_directories(resource, last_processed)
            self.logger.debug(
                f"---- Found directories for path [{last_processed}] ------"
            )
            ...
            self.logger.info(
                f"---- Sorted directories for path [{last_processed}] ------"
            )
