...
for benchmark in [True, False]:
    ...
    drop = ["n_total", "n_val", "prop_train", "prop_val", "prop_test"]
    ...
    def color_scale_mae(value, min_val, max_val):
        n_colors = 100
        pal = sns.diverging_palette(240, 10, n=n_colors * 1.0)
        diff = value - min_val
        idx = int((n_colors - 1) * (diff) / (max_val - min_val))
        color = pal[idx]
        ...
        for j, value in enumerate(array):
            ...
            if j >= 1 and j < n_columns:
                ...
                if np.isnan(value) or np.isnan(r2_min) or np.isnan(r2_max):
                    ...
                    rect = Rectangle(
                        [j * cell_width, len(data_values) - i - 1],
                        cell_width * cell_w,
                        1 * cell_h,
                        facecolor="gray",
                        edgecolor="k",
                    ...
                    if j in [j * cell_w for j in range(len(array))]:
                        ...
                        if value > r2_max or value < r2_min:
                            ...
                            rect = Rectangle(
                                ...
                                cell_width * cell_w,
                                1 * cell_h,
                                facecolor=color_scale_mae(value, r2_min, r2_max),
                                edgecolor="k",
                            )
                ax.add_patch(rect)
    ...
    if benchmark:
        plt.text(
            -left_bound + (cell_width + left_bound) / 2,
            len(data_values) * cell_h + 0.5,
            "Extended Properties",
            verticalalignment="center",
            horizontalalignment="center",
        )
