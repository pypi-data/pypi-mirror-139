...
def x_cached(name='', hash_key=None, also_parquet=False, outer_level=1, cache_folder=None):
    def decorator(func):
        ...
        def _cached(*args, **kwargs):
            ...
            cache_folder2 = cache_folder or xsettings.CACHE_PATH.joinpath(f_name)

            code_text = f_src + f_name + name + str(hash_key) + str((args, kwargs))
            code_hash = hashlib.md5(code_text.encode('utf-8')).hexdigest()

            cache_subfolder = cache_folder2.joinpath(code_hash)
...
def x_cached_call(func, *args, name='', hash_key=None, also_parquet=False, cache_folder=None, **kwargs):
    dec = x_cached(name=name, also_parquet=also_parquet, outer_level=2, cache_folder=cache_folder)
