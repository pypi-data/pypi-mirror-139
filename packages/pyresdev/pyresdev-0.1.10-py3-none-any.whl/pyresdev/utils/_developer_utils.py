...
def create_class_dictionaries(df,class_to_use:str):
    df['BV']=df['BV'].fillna(0)
    groupby = df[[class_to_use, 'BV', 'IDEmailStatus']].groupby(class_to_use).agg(BV=('BV', 'mean'),
                                                                                 Conversions=(
                                                                                     'IDEmailStatus', 'mean'))
...
def map_conv_prob_using_dictionary(df, bv_dict, prob_dict, column:str):
    ...
    if column not in df.columns:
        raise Exception("MXType column not preset ( Check Caps)")
    logger.info(f'Defining dict for {column}')
    bv_col = column+"_bv"
