...
class MessageProcessor:
    ...
    async def send_app_nav(self, conversation_id, app_nav):
        method = 'POST'
        payload = {
            'appNavs': [app_nav]
        }
        endpoint = f"{FC_ENDPOINT}/Accounts/{FC_ACCOUNT_ID}/Calls/{conversation_id}/AppNavs"
        requests.post(endpoint, json=payload, auth=(FC_ACCOUNT_ID,FC_ACCOUNT_ID))
        return None
    ...
    def log_appNav(self, sender_id: str, message: UserUttered):
        try:
            if (message.text == '/disconnect'):
                ...
                asyncio.create_task(self.send_app_nav(sender_id, app_nav))
