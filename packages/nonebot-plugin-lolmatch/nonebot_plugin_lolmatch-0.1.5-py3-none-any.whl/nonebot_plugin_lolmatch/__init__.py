...
from nonebot.adapters.onebot.v11 import Bot, GroupMessageEvent, Message, NetworkError
...
lol_today = on_command('lol', aliases={'LOL'}, priority=5, block=True)
test = on_command('test', permission=SUPERUSER, block=True)
...
async def _(matcher: Matcher, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if "本周" in msg:
        ...
        try:
            await lol_today.finish(await LoLMatch.show_subbed_tournament(event.group_id))
        except ValueError:
            await lol_today.finish("检查输入赛事ID应为数字")
    elif "详情" in msg:
        ...
        await lol_today.finish(await LoLMatch.sub_tournament_group(int(sub_id), event.group_id))
    elif "赛事" in msg:
        ...
        for tournamentID, tournament_matches in today_matches.items():
            ...
            for group_id in sub_dict[int(tournamentID)]:
                try:
                    ...
                    logger.warning(f"{__plugin_name__} 向群 {group_id} 发送 {match_result} 失败")
