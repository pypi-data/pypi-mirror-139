# perllib module for pythonizer, generated by makelib.py on Mon Feb 21 00:15:50 2022
#
...
import sys,os,re,fileinput,subprocess,collections.abc,warnings,inspect,itertools,signal,traceback,io,tempfile,calendar,types,random,dataclasses,builtins,codecs,struct
...
def binmode(fh,mode='b',encoding=None,errors=None):
    """Handle binmode"""
...
def _create_all_fh_methods(fh):
    ...
    methods=dict(autoflush=autoflush,binmode=binmode, close=close, eof=eof, 
...
def open_(file,mode,encoding=None,errors=None,checked=True):
...
def translate_and_count(table, var, replace=True, complement=False, delete=False):
...
def translate_element(base, index, table, replace=True, complement=False):
...
def translate_global(packname, varname, table, replace=True, complement=False):
    """Perform a tr translate on a global, and also count the # of matches"""
    result = []
    ctr = 0;
    namespace = getattr(builtins, packname)
