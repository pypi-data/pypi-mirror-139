...
from energinetml.core.backend import AbstractBackend, ConfigError
...
class AzureBackend(AbstractBackend):
    ...
    def get_workspace(self, project_meta: dict) -> Workspace:
        """_summary_
        ...
        for keyword in expected_keywords:
            if keyword not in project_meta:
                raise ConfigError(f"Expected '{keyword}' as a key.")
    ...
    def submit_model(
        ...
        with model.temporary_folder() as path:
            config = ScriptRunConfig(
                source_directory=os.path.join(path, model.module_name),
                command=["python", model.module_name, "model", "train", "--cloud-mode"]
                + list(params),
    ...
    def download_model_files(
        ...
        run_id: str,
        folder_path: str,
        # trained_model_relative_path: str,
        # trained_model_absolute_path: str,
    ) -> str:
        """Downloads model files to folder including snapshot (model.py, model.json, etc) as a
        zip-file and trainedmodel filesfrom the Azure ML cloud,
        based on an `experiment_name` and a `run_id`.
