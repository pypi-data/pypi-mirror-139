...
class xtable:
    ...
    def json(self,force_string_typed=False):
        self.datafix()
        import json
        tbl = list()
        for r in self.__data:
            ...
            for ix, val in enumerate(r):
                row[self.__header[ix]] = (str(val) if force_string_typed else val)
            tbl.append(row)
        return json.dumps(tbl, indent=2)

    def yaml(self,force_string_typed=False):
        self.datafix()
        import yaml
        tbl = list()
        for r in self.__data:
    ...
    def __repr__(self):
        ...
        if os.environ.get("force_ansicolor", "").lower() in ["true", "yes", "y", "1"]:
            colored = True
        elif os.environ.get("force_ansicolor", "").lower() in ["false", "no", "n", "0"]:
            colored = False
...
def xtable_main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="force string type when dumping JSON/YAML to skip data type limit",
    )
    parser.add_argument(
        "--tree",
        dest="tree",
    ...
    def showres(t):
        if args.pivot:
            ...
            if args.format == "json":
                print(t.json(force_string_typed=args.forcestring), end="")
            elif args.format == "yaml":
                print(t.yaml(force_string_typed=args.forcestring), end="")
