...
from .utils.connect import connect_to_database, dataframe_to_sql, db_conn_flavor
from .utils.csp_connect import connect_to_csp
from .utils.dbconn import sql_enquote_id
from .utils.df_utils import CloseAfterIteration, load_dataframe_from_sql, LoadStrategy
        ...
        if isinstance(resource_name, str):
            ...
            if open_args.get("sql") or open_args.get("sql_table"):
                ...
                if table:
                    ...
                    sql = f"SELECT * from {sql_enquote_id(table, dotted=True, mode=flavor)}"
                if sniff_only:
                    connect_strategy = db_conn
                    return LoadStrategy(opener=connect_strategy.opener, open_args=connect_strategy.open_args, read_method=load_dataframe_from_sql, use_dask=use_dask, sql=sql)
                return load_dataframe_from_sql(db_conn, sql, use_dask)
            # detect NoSQL
