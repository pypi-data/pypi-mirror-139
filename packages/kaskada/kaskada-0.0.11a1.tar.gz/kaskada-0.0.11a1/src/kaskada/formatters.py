...
from domonic.html import a, b, button, div, p, pre, script, style, table, td, th, tr
import html
import html5lib
import inspect
import io
...
def html_obj_table_rows(obj, props):
    ...
    props.pop(obj_id_key) if obj_id_key in props else None
    ...
    if obj_name is not None:
        rows.append(html_obj_table_row(obj_name_key, obj_name))
    for key in props:
...
def get_shared_generic_response_props_and_schema_dataframe(obj):
    ...
    if hasattr(obj, "result_type"):
        props["result_type"] = html.escape(convert_fenl_datatype(obj.result_type))
...
def generic_response_html_formatter(obj):
    ...
    other = table(_class="kda_table")
    appendChildIfNotNone(other, get_request_details_table_row_if_exists(props))
    ...
    if len(props) == 1 and type(props[list(props)[0]]) not in (int, float, bool, str):
        ...
        appendChildren(nested_table, html_obj_table_rows(actual_obj, actual_props))
        details.appendChild(html_table_row(obj_key, nested_table))
    else:
        appendChildren(details, html_obj_table_rows(obj, props))

    schema = convert_df_to_domonic(schema_df) if schema_df is not None else None 

    return str(tab_panel([("Details", details), ("Schema", schema), ("Other", other), ("Raw", pre(pprint.pformat(obj)))]))
...
def data_type_html_formatter(obj):
    ...
    title_to_set_active = ""
    result_type = html.escape(convert_fenl_datatype(obj))
    schema_df = None
...
def response_list_html_formatter(list_resp):
    ...
    return str(tab_panel([("Details", details), ("Other", other), ("Raw", pre(pprint.pformat(list_resp)))]))
...
def get_shared_query_response_content(resp_obj):
    ...
    appendChildren(other, rows)

    return { "Details": details, "Schema": schema, "Other": other}
...
def query_response_html_formatter(resp_obj):
    ...
    return str(tab_panel([("Details", content["Details"]), ("Schema", content["Schema"]), ("Other", content["Other"]), ("Raw", pre(pprint.pformat(resp_obj)))]))
...
def fenlmagic_query_result_html_formatter(obj):
    ...
    return str(tab_panel([("Dataframe", dataframe), ("Details", content["Details"]), ("Schema", content["Schema"]), ("Other", content["Other"]), ("Raw", pre(pprint.pformat(raw).replace("\\n","")))]))

def entity_filter_html_formatter(obj):
    props = get_properties(obj)
    filter = table(_class="kda_table")
    filter_type = type(obj).__name__
    if filter_type == "EntityPercentFilter":
        filter.appendChild(html_obj_table_row("percent", obj.percent))

    return str(tab_panel([("Filter", filter), ("Raw", pre(pprint.pformat(props).replace("\\n","")))]))
