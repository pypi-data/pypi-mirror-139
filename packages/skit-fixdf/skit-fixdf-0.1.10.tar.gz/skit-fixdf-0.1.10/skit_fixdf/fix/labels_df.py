...
def get_entities_from_duckling(
    ...
    reftime: int,
    type_: str,
    duckling_token: Optional[str] = None,
):
...
def make_entity_label_df(
    df: pd.DataFrame, duckling_url: str, duckling_token: Optional[str] = None
) -> pd.DataFrame:
    """
    ...
    for i, row in tqdm(
        ...
        for col in columns:
            ...
            if isinstance(raw_data, dict):
                ...
                # handle value extraction differently based on the key
                if key == const.DATA:
                    if isinstance(value, (dict, list)):
                        value = json.dumps(value, ensure_ascii=False, indent=0)
                elif key == const.PREDICTION:
                    if isinstance(value, list) and len(value):
                        first_intent_prediction, *_ = value
                        value = first_intent_prediction.get(
                            const.NAME
                        )  # get the name of the first predicted intent.
                    # else prediction is an empty dictionary
            else:
                value = None
...
def prepare_tagged_df_for_training(
    ...
    df = add_columns_from_metadata(
        df,
        [
            (const.INTENTS, const.INTENT)
        ],  # if the prediction dictionary is not empty, `intents` key will be present
    ...
    df = fix.df.rename_columns(
        df, {const.DATA_ID: const.ID, const.ALTERNATIVES: const.UTTERANCES}
    )
