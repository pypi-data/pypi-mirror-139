...
def user_has_subscriptions_in_products_list(user, products):
    ...
    try:
        # TODO: the customer needs to be synced with the right subscriber
        user_customer = Customer.objects.get(subscriber=user)
        user_subscriptions = [s.id for s in Subscription.objects.filter(customer=user_customer.id)]
