        ...
        for error in errors:
            try:
                if(error.original_error.__class__.__name__ != "FriendlyError"):
                    logger.error(str(error.original_error), stack_info=True, extra={'request_info': args, 'request': self.get_context(request)})
            except AttributeError:
                if(error.__class__.__name__ != "FriendlyError"):
                    logger.error(str(error), stack_info=True, extra={'request_info': args, 'request': self.get_context(request)})
