...
from ._typing import Any, Callable, Dict, List, Optional, Tuple
from .messages import ParameterMessage, RemoteAssetMessage
...
class ParametersBatch(object):
    ...
    def __init__(self, base_interval):
        ...
        self.last_time = 0.0
        self.items = {}  # type: Dict[str, Tuple[ParameterMessage, int]]
    ...
    def append(self, message, offset):
        ...
        self.items[name] = (message, offset)
        return True

    def accept(self, callback, unconditional=False):
        # type: (Callable[[ParameterMessage, int], None], bool) -> bool
    ...
    def _accept(self, callback):
        # type: (Callable[[ParameterMessage, int], None]) -> None
        """Accepts the specified callback for all parameters collected so forth"""
        keys = list(self.items.keys())
        for key in keys:
            callback(*self.items[key])  # unpack the tuple with message and offset
            self.items.pop(key)
