...
from models.transformer.bert_modeling import BertModel
from models.transformer.bert_tokenization import BertTokenizer
...
class Config(object):
    model_key_map = {
        # 百度预训练模型ernie-1.0
        'ernie-1.0': {
            'model_url': "https://huggingface.co/nghuyong/ernie-1.0/resolve/main/pytorch_model.bin",
            'config_url': "https://huggingface.co/nghuyong/ernie-1.0/resolve/main/config.json",
            'vocab_url': "https://huggingface.co/nghuyong/ernie-1.0/resolve/main/vocab.txt"
        }
    }

    """配置参数"""
    def __init__(self, dataset, embedding=None, **kwargs):
        model_path_name = kwargs.get('pretrain_name_path', 'ernie-1.0')
        ...
        self.tokenizer = BertTokenizer.from_pretrained(
            os.path.join(self.bert_path, 'vocab.txt') if os.path.isdir(self.bert_path) else self.bert_path)
        self.tokenizer = BertTokenizer.from_pretrained(os.path.join(self.bert_path,'vocab.txt') if os.path.isdir(self.bert_path) else self.bert_path)
