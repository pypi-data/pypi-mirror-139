...
class ShynaIs:
    ...
    s_process_camurl = ShynaLookForCameraUrl.LookForCameraUrl()
    is_this = False
    ...
    def is_pc_device_online(self):
        ...
        task_time_sequence = []
        try:
            self.s_data.query = "SELECT task_time FROM last_run_check where task_date='" + str(self.s_time.now_date) + "' AND process_name='pc_online_check' "
            self.result = self.s_data.select_from_table()
    ...
    def is_cam_online(self):
        ...
        try:
            if self.is_shivam_in_front_of_any_cam():
                ...
                cam_url = self.s_process_camurl.get_cam_url()
                if cam_url is False:
                    self.is_this = False
                else:
                    for item in cam_url:
                        if self.s_process_camurl.open_url(ur=item):
                            self.is_this.append(True)
                        else:
                            self.is_this.append(False)
    ...
    def speak_device_is(self):
        ...
        try:
            if self.is_shivam_at_home() is True and self.is_rasp_device_online() is True:
                self.is_this = "rasp"
...
test = ShynaIs()
print(test.is_termux_device_online())