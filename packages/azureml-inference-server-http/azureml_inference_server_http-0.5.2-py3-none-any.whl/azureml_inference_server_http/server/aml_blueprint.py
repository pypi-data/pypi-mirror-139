...
class AMLBlueprint(Blueprint):
    ...
    def _get_swagger(self):
        try:
            ...
            if gen_swagger:
                ...
                output_schema = (
                    get_output_schema(main.run) if not wrapped else get_output_schema(main.driver_module.run)
                )
    ...
    def _init_appinsights(self, app):
        try:
            ...
            app.wsgi_app = AppInsightsWrapper(app.wsgi_app, self.appinsights_client)
    ...
    def register(self, app, options, first_registration=False):
        ...
        init_debuggability(self.logger, app)
        ...
        if self.scoring_timeout_env_variable in os.environ.keys() and self.is_int(
            os.environ[self.scoring_timeout_env_variable]
        ):
