...
class Monitoring:
    ...
    def initialize_monitoring(  # noqa
        ...
        try:
            ...
            if verbose:
                LOG.exception('The inferred dataset_id was: ')
                LOG.exception(dataset_id)
                LOG.exception('The inferred model_info was: ')
                LOG.exception(model_info)
            raise e
        ...
        overall_result = True
        init_result = self._init_monitoring_api_call(verbose, enable_backend_modifications)
    ...
    def _precompute_api_call(self, verbose):
    ...
    def _init_monitoring_api_call(self, verbose, enable_backend_modifications):
        init_result = None
        try:
            path = [
                'init_monitoring',
    ...
    def update_monitoring_config(
        ...
        path = ['monitoring_update', self.connection.org_id]
        if project_id:
            path.append(project_id)
        if model_id:
            if not project_id:
