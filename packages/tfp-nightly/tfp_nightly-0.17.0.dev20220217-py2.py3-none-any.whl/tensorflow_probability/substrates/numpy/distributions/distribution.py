...
from tensorflow_probability.python.internal.backend.numpy import tf_inspect  # pylint: disable=g-direct-tensorflow-import
  ...
  *not* automatically inherit
  ...
  def experimental_local_measure(self, value, backward_compat=False, **kwargs):
    ...
    if getattr(self, '_experimental_tangent_space'):
      ...
      from tensorflow_probability.substrates.numpy.experimental import tangent_spaces
...
class _AutoCompositeTensorDistributionMeta(_DistributionMeta):
  ...
  def __new__(mcs, classname, baseclasses, attrs):  # pylint: disable=bad-mcs-classmethod-argument
    ...
    cls = super(_AutoCompositeTensorDistributionMeta, mcs).__new__(  # pylint: disable=too-many-function-args
        mcs, classname, baseclasses, attrs)
    if 'tensorflow_probability.substrates.numpy.distributions' in cls.__module__:
      module_name = 'tfp.distributions'
    elif ('tensorflow_probability.substrates.numpy.experimental.distributions'
          in cls.__module__):
...
#   `tensorflow_probability/substrates/numpy/distributions/distribution.py`
