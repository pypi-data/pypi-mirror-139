...
class ExportFileFormat(enum.Enum):
    ...
    On January 4th, 2022, the known maximum number of rows is 1,048,576. Thus, if e.g. 1,1 million containers are
    generated, opening this xlsx file in Excel is not supported by the specifications.
    """

    xls = "xls"
    """The xls format is the precursor of the xlsx format. This should only be used of a software demands this file
    format. Older versions of Excel had more restrictions on the size, e.g. Excel 2003 is known to have only supported
    65,536 rows (see e.g.
    http://web.archive.org/web/20140819001409/http://news.office-watch.com/t/n.aspx?articleid=1408&zoneid=9)
    which is less than what large terminals nowadays handle within a month. Even with a hypothetical TEU
    factor of 2, this only reaches 1,572,864 TEU throughput per year.
    """
