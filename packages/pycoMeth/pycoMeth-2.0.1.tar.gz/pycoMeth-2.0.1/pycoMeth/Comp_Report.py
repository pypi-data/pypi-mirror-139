...
def Comp_Report(
    ...
    * report_non_significant
    ...
    loader = MetH5Loader(h5_read_groups_key=read_groups_key, sample_id_list=sample_id_list, h5_file_list=h5_file_list)
    ...
    if not all_in(req_fields, df.columns):
        raise pycoMethError(
            "Invalid input file type passed. Expecting Meth_Comp TSV file generated from samples processed with Interval_Aggregate"
        )
    ...
    if not all_in(df["chromosome"], tx_df["chromosome"]):
        log.error(
            "Not all the chromosomes found in the data file are present in the GFF3 file. This will lead to missing transcript ids"
        )
    ...
    if not all_in(df["chromosome"], chr_len_d.keys()):
        log.error(
            "Not all the chromosomes found in the data file are present in the Fasta file. This will lead to missing reference sequences in the ideogram"
        )
    ...
    log.info("Iterating over intervals with sufficient difference")
    
    for idx, line in tqdm(
        iter_idx_tuples(valid_df),
        total=len(valid_df),
        unit=" intervals",
        unit_scale=True,
        desc="\tProgress",
        ...
        if idx in top_dict:
            ...
            # Generate figures and tables
                ...
                top_interval_summary.append(
                    get_interval_summary(line=line, close_tx_df=close_tx_df, rank=rank, out_file=link_out_file)
                )
    ...
    if api_mode:
        ...
        if export_static_plots:
            kaleido.export_plotly_svg(
                fig=all_heatmap_fig, fn=os.path.join(outdir, plot_outdir, "all_heatmap.svg"), width=1400
            )
            kaleido.export_plotly_svg(
                fig=all_ridgeplot_fig, fn=os.path.join(outdir, plot_outdir, "all_ridgeplot.svg"), width=1400
            )
            kaleido.export_plotly_svg(
                fig=catplot_fig, fn=os.path.join(outdir, plot_outdir, "all_catplot.svg"), width=1400
            )
            kaleido.export_plotly_svg(
                fig=ideogram_fig, fn=os.path.join(outdir, plot_outdir, "all_ideogram.svg"), width=1400
            )
            kaleido.export_plotly_svg(
                fig=tss_dist_fig, fn=os.path.join(outdir, plot_outdir, "all_tss_distance.svg"), width=1400
            )
...
def get_cpg_df(loader: MetH5Loader, line):
    """"""
    lab_list, llr_list, pos_list, _ = loader.read_raw_llrs(Coord("", line.chromosome, line.start, line.end))
...
def get_close_tx_df(tx_df, chromosome, start, end, max_tss_distance=100000):
    ...
    rdf = tx_df.query(
        "chromosome == '{}' and tss >= {} and tss <= {}".format(
            chromosome, start - max_tss_distance, end + max_tss_distance
        )
    )
    tss_dist = []
