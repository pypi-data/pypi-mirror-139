...
# Sign of Ip, Bt (positive=ccw from above), valid for COCO=17 (CLISTE)
        ...
        jcoco = self.coco%10 - 1
        ebp   = self.coco//10
        psi_sign = -self.ip_ccw*sigma['bp'][jcoco]*sigma['rphiz'][jcoco]
        psi_2pi  = (2.*np.pi)**ebp
        self.psi_fac = psi_sign*psi_2pi
...
# Assuming SI
        ...
        coco_in = self.coco
        logger.info('COCO conversion from %d to %d' %(coco_in, coco_out))
        jc_in   = coco_in %10 - 1
        jc_out  = coco_out%10 - 1
        ebp_in  = coco_in//10
        ebp_out = coco_out//10
