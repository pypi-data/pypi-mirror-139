...
class SMSActivateAPI:
    ...
    def response(self, action, response):
        ...
        if action == "getNumbersStatus":
            ...
            order_id = int(data[0])
            phone = int(data[1])
            result =  {"order_id": order_id, "phone": phone}
            self.__debugLog(result)
            return result
    ...
    def getNumber(self, service = None, forward = None, freePrice = None, maxPrice = None, phoneException = None, operator = None, ref = None, country=None):
