...
def invoke(id: str, workflow: str, params: str, work_dir: str,
    ...
    # Get the command
    command = (
        f'{nextflow} -log {log_file} run {workflow} '
        f'-params-file {params} -w {work_dir} -ansi-log false')

    if wsl:
        logging.info('Running command in WSL.')
        command = 'wsl ' + command
