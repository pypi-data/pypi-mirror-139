...
class Database:
    ...
    def get_hk_stock_ohlc(self, code, start_date, end_date, freq, price_adj=False, vol_adj=False):
        ...
        if False not in list(check_bool_dict.values()):
            ...
            if price_adj == False:
                ohlc_result = json.loads(json.loads(response.content).replace("'", "\""))
            else:
...
def get_hk_holiday_and_expiry_date(start_year, end_year, format='int'):
    holiday_date_list = []
    half_day_mkt_date_list = []
