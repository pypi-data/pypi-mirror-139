...
class SnapshotOperation:
    def __init__(self, instance_service: InstanceService, image_waiter: AMIWaiter):
    ...
    def save(
        ...
        logger.info(
            "Create image from deployed app '{}'. New image id: '{}'".format(
                deployed_app_name, image.id
            )
        )
    ...
    def get_image(self, ec2_client, reservation_id, deployed_app_name):
        response = ec2_client.describe_images(
            Filters=[
                {
                    "Name": "tag:" + self._format_source_key_name(NAME_TAG_KEY),
    ...
    def get_snapshots(self, ec2_session, instance_id, snapshot_name=None):
        ...
        instance_snapshots = list(
            ec2_session.snapshots.filter(Filters=[instance_id_filter])
        )
        result = []
    ...
    def restore_snapshot(
        self, ec2_client, ec2_session, instance_id, snapshot_name, tags
    ):
        instance = self.instance_service.get_instance_by_id(ec2_session, instance_id)
        instance_snapshot = self.get_snapshots(
        ...
        # Create a new volume from the requested snapshot and put it in the same
        # availability zone
        ...
        new_volume.attach_to_instance(
            InstanceId=instance.id, Device=root_device_mapping
        )
    ...
    def save_snapshot(self, ec2_client, ec2_session, instance_id, snapshot_name, tags):
        ...
        create_snapshot_result = ec2_client.create_snapshot(
            VolumeId=root_device_volume_id
        )
