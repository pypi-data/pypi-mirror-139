...
class AbstractDeviceIndexStrategy:
    def __init__(self):
        pass
...
class AllocateMissingValuesDeviceIndexStrategy(AbstractDeviceIndexStrategy):
    ...
    def apply(self, actions):
        ...
        specific_device_index_actions_sorted = sorted(
            specific_device_index_actions, key=lambda x: x.actionParams.vnicName
        )
        ...
        for no_di_action in no_device_index_actions:
            ...
            specific_device_index_actions_sorted.insert(
                device_index_counter, no_di_action
            )

        # make sure device index values are continues
        if (
            specific_device_index_actions_sorted[-1].actionParams.vnicName
    ...
    def _is_vnic_name_valid(self, action):  # vnicName = device_index(AWS)
        return (
            action.actionParams.vnicName is not None
            and int(action.actionParams.vnicName) >= 0
        )
