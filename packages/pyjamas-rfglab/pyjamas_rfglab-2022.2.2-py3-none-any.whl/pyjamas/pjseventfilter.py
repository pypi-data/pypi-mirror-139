            ...
            if self.pjs.slicetracker is not None and event.type() == QtCore.QEvent.MouseMove and event.buttons() == QtCore.Qt.LeftButton:
                ...
                if event.type() == QtCore.QEvent.MouseButtonRelease:
                    ...
                    if self.balloon_timer.isActive():
                        ...
                        # Grow an active contour from current position.
                        invim = rimutils.invert(
                            skif.gaussian(self.pjs.imagedata, sigma=self.INFLATE_GAUSSIAN_SIGMA))

                        # Check if the click was inside an existing polyline and, in that case, delete the polyline
                        # (and use it as seed to start balloon).
                        init_poly = self.pjs.removePolyline(self.x, self.y, self.pjs.curslice, pushundo=True)

                        if not init_poly:
                            init_ls = numpy.zeros(self.pjs.imagedata.shape, dtype=numpy.int8)
                            init_ls[max(0, self.y - 1):min(self.y + 2, self.pjs.imagedata.shape[0]),
                            max(0, self.x - 1):min(self.x + 2, self.pjs.imagedata.shape[1])] = 1

                        else:
                            init_ls = rimutils.mask_from_polylines(self.pjs.imagedata.shape,
                                                                   [rutils.RUtils.qpolygonf2list(init_poly)], brushsz=1)

                        thepoints = skis.morphological_geodesic_active_contour(invim, num_iter=self.BALLOON_ITERS,
                                                                               init_level_set=init_ls, smoothing=1,
                                                                               balloon=self.balloon_force)

                        self.pjs._poly_ = rimutils.extract_contours(thepoints)

                        if len(self.pjs._poly_) > 0:
                            self.pjs._poly_ = self.pjs._poly_[0]  # object 0 is the background.
                        else:
                            self.pjs._poly_ = []
                            self.balloon_force = self.INITIAL_BALLOON_FORCE
                            return False
