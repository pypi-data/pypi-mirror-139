...
friendly_name: str = "Templateco Plugin"
compatible_templates: List[str] = ["templateco"]
selected_by_default = True


def _parse_folder_to_templateco(folder_name: str) -> Tuple[str, ...]:
    """
    ...
    """
    parsed_folder = re.fullmatch(r"^([a-z]+)_([a-z_]*)_plugin$", folder_name)
...
@get_questions_decorator
def get_questions(folder_name: str, **_: Any) -> Iterable[Mapping[str, Any]]:
    """
    Query the user for how the plugin will work.
    """
    (prefix, name) = _parse_folder_to_templateco(folder_name)
...
@post_template_hook_decorator
def post_template_hook(output_folder: Path, data: Dict[str, Any], **_: Any) -> None:
    """
    Run some code after it has been templated.
    """
    if data["package_manager"] == "Poetry" and data["tooling"]:
        (exit_code, output) = command_runner(
            f"""
            cd {output_folder}
            git init
            poetry add --dev black mypy isort flake8 pydocstyle toml pytest \
                pytest-mock types-setuptools
            """,
            shell=True,
        )
