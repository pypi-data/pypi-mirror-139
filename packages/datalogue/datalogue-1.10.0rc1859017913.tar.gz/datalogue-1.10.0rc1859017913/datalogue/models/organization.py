...
class User:
    ...
    def __init__(
        ...
        email: str,
        organization_ids: Union[None, List[str]],
        group_ids: Union[None, List[str]],
    ):
        """
        User constructor
...
class Domain:
    ...
    def __repr__(self):
        return (
            f"{self.__class__.__name__}(org_id= {self.org_id}, domain= {self.domain!r})"
        )
...
class PublicToken:
    def __init__(
        self, id: UUID, user_id: UUID, name: str, organization_id: UUID, checksum: str
    ):
        self.id = id
