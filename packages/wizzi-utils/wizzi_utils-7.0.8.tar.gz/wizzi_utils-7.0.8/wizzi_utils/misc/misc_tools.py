...
def get_linkable_exception() -> str:
    """
    :return: e.g. File "D:\workspace\2021wizzi_utils\wizzi_utils\misc\test\test_misc_tools.py", line 851,
...
def main_wrapper(
    ...
    if tf_v:
        ...
        # except (ImportError, ModuleNotFoundError, NameError, AttributeError) as err:
        #     print(add_color('* tensorflow not Found. error: {}'.format(err), 'r'))
...
def find_files_in_folder(dir_path: str, file_suffix: str, ack: bool = False, tabs: int = 1) -> list:
    """
    :param dir_path:  root dir. will be looked in recursively
    :param file_suffix:  e.g. '.jgp'
    :param ack:
    :param tabs:
    :return:
    see find_files_in_folder_test()

    sort by numbers in basename:
    folder_path = './path' # content [1.jpg, 10.jpg 2.jpg]
    folder_imgs=wu.find_files_in_folder(folder_path, file_suffix='.jpg', ack=True)
    print(folder_imgs)
    # >>> [1.jpg, 10.jpg, 2.jpg]
    def sorter(x):
        return int(os.path.basename(x).split('.')[0])
    folder_imgs.sort(key=sorter)
    print(folder_imgs)
    # >>> [1.jpg, 2.jpg, 10.jpg]

    folder_path = './path' # content [gl_1.jpg, gl_10.jpg gl_2.jpg]
    folder_imgs=wu.find_files_in_folder(folder_path, file_suffix='.jpg', ack=True)
    print(folder_imgs)
    # >>> [gl_1.jpg, gl_10.jpg, gl_2.jpg]
    def sorter(x):
        return int(os.path.basename(x).split('.')[0][3:])
    folder_imgs.sort(key=sorter)
    print(folder_imgs)
    # >>> [gl_1.jpg, gl_2.jpg, gl_10.jpg]
    """
    ...
    if os.path.exists(dir_path):
        ...
        if ack:
            ...
            print('{}found {} files that ends with {} in folder "{}":'.format(tabs * '\t', len(all_files_processed),
                                                                              file_suffix, dir_path))
...
def wizzi_utils_requirements():
    pip = 'pip install {}'
    pack = 'Package {}({}.py, namespace: {}):'
    pack_test = '\ttest_{}.py(namespace: {}.test):'
    sub_pack = 'Sub Package: {}'

    MT_PACK, MT_MOD, MT_SC = 'misc', 'misc_tools', 'mt'
    # ALG_PACK, ALG_MOD, ALG_SC = 'algorithms', 'algorithms', 'alg'
    # COT_PACK, COT_MOD, COT_SC = 'coreset', 'coreset_tools', 'cot'
    GOT_PACK, GOT_MOD, GOT_SC = 'google', 'google_tools', 'got'
    JT_PACK, JT_MOD, JT_SC = 'json', 'json_tools', 'jt'
    CVT_PACK, CVT_MOD, CVT_SC = 'open_cv', 'open_cv_tools', 'cvt'
    PYPLT_PACK, PYPLT_MOD, PYPLT_SC = 'pyplot', 'pyplot_tools', 'pyplt'
    ST_PACK, ST_MOD, ST_SC = 'socket', 'socket_tools', 'st'
    TT_PACK, TT_MOD, TT_SC = 'torch', 'torch_tools', 'tt'
    # TFT_PACK, TFT_MOD, TFT_SC = 'tensorflow', 'tensorflow_tools', 'tft'
    TFLT_PACK, TFLT_MOD, TFLT_SC = 'tflite', 'tflite_tools', 'tflt'
    TTS_PACK, TTS_MOD, TTS_SC = 'tts', 'tts', 'tts'
    MODELS_PACK, MODELS_MOD, MODELS_SC = 'models', 'models', 'models'
    print('Requirements per package:')
    print('\tTo use a test of a package, must install all package requirements')

    print(LINES)
    print(pack.format(MT_PACK, MT_MOD, MT_SC))
    print('\t{} # to use 7z compress and extract'.format(pip.format('py7zr')))
    print(pack_test.format(MT_MOD, MT_SC))
    print('\t\tNone')

    # print(LINES)
    # print(pack.format(ALG_PACK, ALG_MOD, ALG_SC))
    # print('\t{}'.format(pip.format('sklrean')))
    # print(pack_test.format(ALG_MOD, ALG_SC))
    # print('\t\t{}'.format(sub_pack.format(PYPLT_PACK)))

    # print(LINES)
    # print(pack.format(COT_PACK, COT_MOD, COT_SC))
    # print('\tNone')
    # print(pack_test.format(COT_MOD, COT_SC))
    # print('\t\tNone')

    print(LINES)
    print(pack.format(GOT_PACK, GOT_MOD, GOT_SC))
    print('\t*** see instructions in order to use written in class google_handler docstring')
    print('\t{}'.format(pip.format('pydrive')))
    print('\t{}'.format(pip.format('yaml')))
    print(pack_test.format(GOT_MOD, GOT_SC))
    print('\t\t{}'.format(sub_pack.format(ST_PACK)))

    print(LINES)
    print(pack.format(JT_PACK, JT_MOD, JT_SC))
    print('\tNone')
    print(pack_test.format(JT_MOD, JT_SC))
    print('\t\tNone')

    print(LINES)
    print(pack.format(CVT_PACK, CVT_MOD, CVT_SC))
    print('\t{}'.format(pip.format('opencv-python')))
    print('\t{}'.format(sub_pack.format(PYPLT_PACK)))
    print('\t{} # for CameraWu type_cam=="imutils"'.format(pip.format('imutils')))
    print('\t{} # for CameraWu type_cam=="acapture"'.format(pip.format('acapture')))
    print(pack_test.format(CVT_MOD, CVT_SC))
    print('\t\t{}'.format(sub_pack.format(ST_PACK)))

    print(LINES)
    print(pack.format(PYPLT_PACK, PYPLT_MOD, PYPLT_SC))
    print('\t{} # for compare_images_multi_sets_squeezed()'.format(pip.format('torch')))
    print(pack_test.format(PYPLT_MOD, PYPLT_SC))
    print('\t\t{} # for compare_images_multi_sets_squeezed_test()'.format(pip.format('torch')))
    print('\t\t{} # with the tests as well'.format(sub_pack.format(CVT_PACK)))

    print(LINES)
    print(pack.format(ST_PACK, ST_MOD, ST_SC))
    print('\tNone')
    print(pack_test.format(ST_MOD, ST_SC))
    print('\t\t{}'.format(sub_pack.format(JT_PACK)))

    print(LINES)

    print(pack.format(TT_PACK, TT_MOD, TT_SC))
    print('\ttorch:')
    print('\t\t{} # cpu'.format(pip.format('torch')))
    print('\t\tfor gpu support check your cuda and google for the pip command')
    print('\t\tall options: https://pytorch.org/get-started/locally/')
    print('\t\t** may need to uninstall numpy and re-install')
    print('\t{}'.format(pip.format('torchvision')))
    print(pack_test.format(TT_MOD, TT_SC))
    print('\t\tNone')

    # print(LINES)
    # print(pack.format(TFT_PACK, TFT_MOD, TFT_SC))
    # print('\ttensorflow:')
    # print('\t\t{}'.format(pip.format('tensorflow')))
    # url = 'https://stackoverflow.com/questions/50622525/'
    # url += 'which-tensorflow-and-cuda-version-combinations-are-compatible'
    # print('\t\tall options: {}'.format(url))
    # print(pack_test.format(TFT_MOD, TFT_SC))
    # print('\t\tNone')

    print(LINES)
    print(pack.format(TFLT_PACK, TFLT_MOD, TFLT_SC))
    print('\ttflite options:')
    url_win10 = 'https://google-coral.github.io/py-repo/'
    print('\t\t{} # windows 10 and linux'.format(pip.format('--index-url {} tflite_runtime'.format(url_win10))))
    print('\t\t\tmore info on https://www.tensorflow.org/lite/guide/python')

    print(pack_test.format(TFLT_MOD, TFLT_SC))
    print('\t\t{}'.format(None))

    print(LINES)
    print(pack.format(TTS_PACK, TTS_MOD, TTS_SC))
    print('\t{}'.format(pip.format('pyttsx3')))
    print(pack_test.format(TTS_MOD, TTS_SC))
    print('\t\tNone')

    print(LINES)
    print(pack.format(MODELS_PACK, MODELS_MOD, MODELS_SC))
    print('\t{}'.format(sub_pack.format(ST_PACK)))
    print('\t{}'.format(sub_pack.format(PYPLT_PACK)))
    print('\t{}'.format(sub_pack.format(CVT_PACK)))
    print('\t{} # for tracking'.format(pip.format('opencv-contrib-python')))
    print('\tinstall tflite for od and pd models')
    print(LINES)
    return
...
class FPS:
    ...
    def __init__(self, summary_title: str = None) -> None:
    ...
    def _times_elapsed_not_empty(self) -> bool:
        ret = True if len(self.times_elapsed) > 0 else False
        return ret
    ...
    def start(self, ack_progress: bool = False, tabs: int = 1, with_title: bool = False) -> None:
        if ack_progress:
            ...
            string += 'Iter {} Started:'.format(len(self.times_elapsed) + 1)
            print(string)
    ...
    def get_fps(self) -> float:
        fps = -1.0
        if self._times_elapsed_not_empty():
            avg_times = np.mean(self.times_elapsed)
            if avg_times > 0.0001:
                fps = 1 / avg_times
        return fps
    ...
    def so_far(self, tabs: int = 1, with_title: bool = False) -> None:
        if self._times_elapsed_not_empty():
            ...
            if with_title:
                string += '{}:'.format(self.summary_title)
            string += 'Iter {} Done: '.format(len(self.times_elapsed))
            ...
            if len(self.times_elapsed) > 1:
                string += 'totalTime={:.3f}s, '.format(np.sum(self.times_elapsed))
    ...
    def finalize(self, tabs: int = 1) -> None:
        ...
        if self._times_elapsed_not_empty():
            ...
            # add rounds to title
            title += '({} iters)'.format(len(self.times_elapsed))
            print('{}{}'.format(tabs_s, add_color(title, ops='underlined')))

            # total time
            total = '{}\tTotal   Run Time = {:.3f}s'.format(tabs_s, np.sum(self.times_elapsed))
            print(total)

            # average time
            avg_times = np.mean(self.times_elapsed)
            std_times = np.std(self.times_elapsed)
            avg_str = '{}\tAverage Run Time = {:.3f}s (std = {:.3f})'.format(tabs_s, avg_times, std_times)
            print(avg_str)
            # FPS
            if avg_times > 0.0001:
                fps = '{}\t{:.2f} FPS'.format(tabs * '\t', 1 / avg_times)
            else:
                fps = '{}\tFPS very high'.format(tabs * '\t')
            print(add_color(fps, ops='r'))
        else:
