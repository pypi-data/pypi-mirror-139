...
from gopublic import __version__  # noqa, ditto

CONTEXT_SETTINGS = dict(auto_envvar_prefix='GOPUBLIC', help_option_names=['-h', '--help'])
...
class gopublicCLI(click.MultiCommand):
...
@click.command(cls=gopublicCLI, context_settings=CONTEXT_SETTINGS)
...
@click.option(
    "-i",
    "--instance",
    help='Name of instance in %s. This parameter can also be set via the environment variable GOPUBLISH_INSTANCE' % global_config_path(),
    default='__default',
    show_default=True,
    required=True
)
...
@pass_context
def gopublic(ctx, instance, verbose, path=None):
