...
class cmake_new:
    ...
    def _env(self, init_env=None, override_env=None):
        """
        Compute the environment for the project.
        """
        new_env = self.project.environment(envdict=init_env)
        if override_env:
        ...
        if self.project.slot:
            ...
            deployment_dir = str(self.project.slot.get_deployment_directory())
    ...
    def _run(self, cmd, **kwargs):
        ...
        cmd_kwargs["env"] = self._env(
            init_env=kwargs.get("init_env"), override_env=kwargs.get("override_env")
        )
    ...
    def configure(self, **kwargs):
        ...
        elif "LCG_VERSION" in kwargs.get("init_env", {}):
            LCG_VERSION = kwargs["init_env"]["LCG_VERSION"]
        else:
            out = (
        ...
        if "LCG_EXTERNALS_FILE" not in kwargs.get("cache_entries", []):
            toolchain = find_path(
                os.path.join(
                    ...
                    "{}.cmake".format(kwargs["init_env"]["BINARY_TAG"]),
                ),
                search_path=self._env(kwargs.get("init_env"))
        ...
        if not toolchain:
            ...
            ).format(LCG_VERSION, kwargs["init_env"]["BINARY_TAG"])
...
class make:
    ...
    def _make(self, target, **kwargs):
        ...
        env = self.project.environment(envdict=kwargs.get("init_env"))
        env.update(kwargs.get("override_env", {}))
...
class cmake_old(make):
    ...
    def build(self, **kwargs):
        ...
        for target in (
            ...
            self.reports[target] = self._make(target, **kwargs)
        ...
        with open(
            ...
            conf_log.writelines(self.reports["configure"].stdout.splitlines(True)[2:-1])
            conf_log.write(
...
class cmake:
    ...
    def __new__(cls, project: Project):
        return (
            cmake_new(project)
            if is_new_cmake_style(project.get_deployment_directory())
