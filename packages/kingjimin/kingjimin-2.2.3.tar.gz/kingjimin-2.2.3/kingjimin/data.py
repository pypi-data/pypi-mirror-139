...
class Power_Consumption(BaseData):
    def __init__(self, pwr_data):
        ...
        self.cpu_gpu_avg = None
        self.cpu_gpu_max = None
        self.cpu_avg = None
        self.cpu_max = None
        self.gpu_avg = None
        self.gpu_max = None
        if pwr_data != None:
            self.power_consumption = pwr_data.get("power_consumption", None)
            self.cpu_gpu_avg = self.power_consumption.get("cpu-gpu (avg)", None)
            self.cpu_gpu_max = self.power_consumption.get("cpu-gpu (max)", None)
            self.cpu_avg = self.power_consumption.get("cpu (avg)", None)
            self.cpu_max = self.power_consumption.get("cpu (max)", None)
            self.gpu_avg = self.power_consumption.get("gpu (avg)", None)
            self.gpu_max = self.power_consumption.get("gpu (max)", None)