...
def _resolve_azure_access_token(scope, sp, client_id=None, endpoint_type=None):
    """
    ...
    if identity_type == _IdentityType.MANAGED:
        ...
        credential = ClientSecretCredential(
            sp_cred['tenantId'], sp_id, sp_cred['password'])
        access_token = credential.get_token(scope)
        token = access_token.token
        expires_on = access_token.expires_on
...
def register_access_token_resolver(requests_channel):
    def resolve(request, writer, socket):
        try:
            ...
            sp = request.get('sp')
            (token, expires_on) = _resolve_azure_access_token(scope, sp)
