...
if not hasattr(spacy, 'nlp'):
	...
	#spacy.getdoc	= lambda snt: ( bs := spacy.txn.get(snt.encode()), doc := spacy.frombs(bs) if bs else spacy.nlp(snt), spacy.txn.put(snt.encode(), spacy.tobs(doc)) if not bs else None )[1] 
...
def getdoc(snt, fillcache:bool=False): 
	...
	doc = spacy.frombs(bs) if bs else spacy.nlp(snt)
	if not bs and fillcache : 
		with env.begin(write=True) as tw: 
			tw.put(snt.encode(), spacy.tobs(doc))
	return doc
