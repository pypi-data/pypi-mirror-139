...
if eshost: 
	...
	es.config	= config 
	es.newindex = lambda idxname : (es.indices.delete(idxname) if es.indices.exists(idxname) else None, es.indices.create(idxname, es.config))[1] # body=
	es.rows		= lambda query: requests.post(f"http://{eshost}:{esport}/_sql",json={"query": query}).json().get('rows',[]) 
	es.sql		= lambda query: requests.post(f"http://{eshost}:{esport}/_sql",json={"query": query}).json().get('rows',[]) 
	es.sntnum	= lambda cp: es.sql(f"select count(*) cnt from {cp} where type = 'snt'" )[0][0] #75222
	es.lexnum	= lambda w,cp: es.sql(f"select count(*) cnt from {cp} where low = '{w}' and type = 'tok'")[0][0] # opened = 70
	es.lemnum	= lambda w,cp: es.sql(f"select count(*) cnt from {cp} where lem = '{w}' and type = 'tok'")[0][0] 
	es.sqlsi	= lambda query: (si:=Counter(), [si.update({row[0]:1}) for row in es.sql(query)])[0]
	es.warmup   = lambda host, port: requests.put(f"http://{host}:{port}/_cluster/settings", json={"persistent": {"search.max_buckets": 1000000}}).text
