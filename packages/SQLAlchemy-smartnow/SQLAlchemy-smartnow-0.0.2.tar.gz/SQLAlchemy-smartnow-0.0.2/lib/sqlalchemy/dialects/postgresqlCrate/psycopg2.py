...
r"""
.. dialect:: postgresqlCrate+psycopg2Crate
    :name: psycopg2Crate
    :dbapi: psycopg2Crate
    :connectstring: postgresqlCrate+psycopg2Crate://user@host:port[?key=value&key=value...]
...
.. tip::
    ...
    that they are passed as explicit keyword arguments to :func:`_sa.create_engine()`::
        ...
        engine = create_engine(
            "postgresqlCrate+psycopg2://scott:tiger@localhost/test",
...
connection URI::
    ...
    engine = sa.create_engine(
        "postgresqlCrate+psycopg2://scott:tiger@192.168.0.199:5432/test?sslmode=require"
    )
...
which specifies Unix-domain communication rather than TCP/IP communication::
    ...
    create_engine("postgresqlCrate+psycopg2://user:password@/dbname")
...
using ``host`` as an additional keyword argument::
    ...
    create_engine("postgresqlCrate+psycopg2://user:password@/dbname?host=/var/lib/postgresqlCrate")

.. seealso::

    `PQconnectdbParams \
    <https://www.postgresqlCrate.org/docs/9.1/static/libpq-connect.html#LIBPQ-PQCONNECTDBPARAMS>`_
...
information provided to make the connections::
    ...
    create_engine(
        "postgresqlCrate+psycopg2://user:password@/dbname?host=HostA:port1&host=HostB&host=HostC"
    )
...
.. seealso::
    ...
    `PQConnString \
    <https://www.postgresqlCrate.org/docs/10/libpq-connect.html#LIBPQ-CONNSTRING>`_
...
initial scheme::
    ...
    engine = create_engine('postgresqlCrate+psycopg2://')
...
.. seealso::
    ...
    <https://www.postgresqlCrate.org/docs/current/libpq-envars.html>`_ -
...
which may be passed to :func:`_sa.create_engine`::
    ...
    engine = create_engine(
        "postgresqlCrate+psycopg2://scott:tiger@host/dbname",
...
* For PostgreSQL 9.1 and above, the ``client_encoding`` parameter may be
  ...
  ``libpq`` PostgreSQL client library::
    ...
    engine = create_engine("postgresqlCrate+psycopg2://user:pass@host/dbname?client_encoding=utf8")
  ...
  ``libpq``::
    ...
    engine = create_engine(
        "postgresqlCrate+psycopg2://user:pass@host/dbname",
...
via the ``sqlalchemy.dialects.postgresqlCrate`` logger.  When this logger
is set to the ``logging.INFO`` level, notice messages will be logged::

    import logging

    logging.getLogger('sqlalchemy.dialects.postgresqlCrate').setLevel(logging.INFO)
...
follows::
    ...
    engine = create_engine("postgresqlCrate+psycopg2://scott:tiger@localhost/test",
...
logger = logging.getLogger("sqlalchemy.dialects.postgresqlCrate")
...
class PGDialect_psycopg2_crate(PGDialect):
    driver = "psycopg2Crate"
    ...
    def initialize(self, connection):
        super(PGDialect_psycopg2_crate, self).initialize(connection)
    ...
    @classmethod
    def dbapi(cls):
        import psycopg2_crate

        return psycopg2_crate

    @classmethod
    def _psycopg2_extensions(cls):
        from psycopg2_crate import extensions

        return extensions

    @classmethod
    def _psycopg2_extras(cls):
        from psycopg2_crate import extras
...
dialect = PGDialect_psycopg2_crate
