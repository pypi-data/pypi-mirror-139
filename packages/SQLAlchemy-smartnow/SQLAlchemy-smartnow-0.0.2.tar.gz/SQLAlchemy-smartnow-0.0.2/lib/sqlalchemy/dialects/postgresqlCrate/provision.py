...
@create_db.for_db("postgresqlCrate")
...
@drop_db.for_db("postgresqlCrate")
...
@temp_table_keyword_args.for_db("postgresqlCrate")
def _postgresql_temp_table_keyword_args(cfg, eng):
    return {"prefixes": ["TEMPORARY"]}


@set_default_schema_on_connection.for_db("postgresqlCrate")
...
@drop_all_schema_objects_pre_tables.for_db("postgresqlCrate")
...
@drop_all_schema_objects_post_tables.for_db("postgresqlCrate")
def drop_all_schema_objects_post_tables(cfg, eng):
    from sqlalchemy.dialects import postgresqlCrate

    inspector = inspect(eng)
    with eng.begin() as conn:
        for enum in inspector.get_enums("*"):
            conn.execute(
                postgresqlCrate.DropEnumType(
                    postgresqlCrate.ENUM(name=enum["name"], schema=enum["schema"])
                )
            )


@prepare_for_drop_tables.for_db("postgresqlCrate")
