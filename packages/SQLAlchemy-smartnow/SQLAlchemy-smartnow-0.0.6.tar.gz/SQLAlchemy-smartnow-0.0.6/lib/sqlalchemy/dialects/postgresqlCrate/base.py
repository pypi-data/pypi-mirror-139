...
.. note::
   ...
   occurrences of SERIAL with IDENTITY::
       ...
       @compiles(CreateColumn, 'postgresqlCrate')
...
To set isolation level using :func:`_sa.create_engine`::
    ...
    engine = create_engine(
        "postgresqlCrate+pg8000://scott:tiger@localhost/test",
        execution_options={
...
at :ref:`schema_set_default_connections`::
    ...
    engine = create_engine("postgresqlCrate+psycopg2://scott:tiger@host/dbname")
...
<https://www.postgresql-crate.org/docs/current/static/ddl-schemas.html#DDL-SCHEMAS-PATH>`_.
...
reflection process as follows::
    ...
    >>> engine = create_engine("postgresqlCrate://scott:tiger@localhost/test")
    >>> with engine.connect() as conn:
...
.. seealso::
    ...
    <https://www.postgresql-crate.org/docs/9.0/static/ddl-schemas.html#DDL-SCHEMAS-PATH>`_
...
and :meth:`~.postgresqlCrate.Insert.on_conflict_do_nothing`:

.. sourcecode:: pycon+sql

    >>> from sqlalchemy.dialects.postgresqlCrate import insert
...
.. seealso::
    ...
    `INSERT .. ON CONFLICT
    <https://www.postgresql-crate.org/docs/current/static/sql-insert.html#SQL-ON-CONFLICT>`_
...
:attr:`~.postgresqlCrate.Insert.excluded` is available as an attribute on
...
:meth:`~.postgresqlCrate.Insert.on_conflict_do_nothing` method:
...
The :class:`_postgresql.TSVECTOR` type can provide for explicit CAST::
    ...
    from sqlalchemy.dialects.postgresqlCrate import TSVECTOR
...
.. tip::
    ...
    regarding
    `Full Text Search <https://www.postgresql-crate.org/docs/13/textsearch-controls.html>`_.
...
syntaxes. It uses SQLAlchemy's hints mechanism::
    ...
    # SELECT ... FROM ONLY ...
    result = table.select().with_hint(table, 'ONLY', 'postgresqlCrate')
    print(result.fetchall())

    # UPDATE ONLY ...
    table.update(values=dict(foo='bar')).with_hint('ONLY',
                                                   dialect_name='postgresqlCrate')

    # DELETE FROM ONLY ...
    table.delete().with_hint('ONLY', dialect_name='postgresqlCrate')
...
an index (see
https://www.postgresql-crate.org/docs/8.3/interactive/indexes-opclass.html).
...
https://www.postgresql-crate.org/docs/8.3/static/indexes-types.html). These can be
...
of :class:`.PGInspector`, which offers additional methods::
    ...
    engine = create_engine("postgresqlCrate+psycopg2://localhost/test")
...
.. seealso::
    ...
    <https://www.postgresql-crate.org/docs/current/static/sql-createtable.html>`_
...
* ``unnest()`` - in order to generate a PostgreSQL ARRAY literal, the
  :func:`_postgresql.array` construct may be used::
    ...
    >>> from sqlalchemy.dialects.postgresqlCrate import array
    >>> from sqlalchemy import select, func
...
.. seealso::
    ...
    <https://www.postgresql-crate.org/docs/current/sql-expressions.html#SQL-SYNTAX-ROW-CONSTRUCTORS>`_

    `PostgreSQL Row Constructor Comparison
    <https://www.postgresql-crate.org/docs/current/functions-comparisons.html#ROW-WISE-COMPARISON>`_
...
* :meth:`.postgresqlCrate.ENUM.create` , :meth:`.postgresqlCrate.ENUM.drop` - individual
...
class ENUM(sqltypes.NativeForEmulated, sqltypes.Enum):
    ...
    def __init__(self, *enums, **kw):
        ...
        :param create_type: Defaults to True.
         ...
         :meth:`~.postgresqlCrate.ENUM.create`
         or :meth:`~.postgresqlCrate.ENUM.drop`
         are called directly.
         Setting to ``False`` is helpful
         when invoking a creation scheme to a SQL file
         without access to the actual database -
         the :meth:`~.postgresqlCrate.ENUM.create` and
         :meth:`~.postgresqlCrate.ENUM.drop` methods can
        ...
        if native_enum is False:
            util.warn(
                ...
                "sqlalchemy.dialects.postgresqlCrate.ENUM datatype; this type "
...
class PGDDLCompiler(compiler.DDLCompiler):
    ...
    def visit_create_index(self, create):
        ...
        if self.dialect._supports_create_index_concurrently:
            concurrently = index.dialect_options["postgresqlCrate"]["concurrently"]
            if concurrently:
                text += "CONCURRENTLY "
        ...
        using = index.dialect_options["postgresqlCrate"]["using"]
        if using:
            text += (
                "USING %s "
        ...
        ops = index.dialect_options["postgresqlCrate"]["ops"]
        text += "(%s)" % (
            ", ".join(
                [
        ...
        includeclause = index.dialect_options["postgresqlCrate"]["include"]
        if includeclause:
            inclusions = [
                index.table.c[col]
        ...
        withclause = index.dialect_options["postgresqlCrate"]["with"]
        if withclause:
            text += " WITH (%s)" % (
                ", ".join(
                    [
        ...
        tablespace_name = index.dialect_options["postgresqlCrate"]["tablespace"]
        if tablespace_name:
            text += " TABLESPACE %s" % preparer.quote(tablespace_name)

        whereclause = index.dialect_options["postgresqlCrate"]["where"]
    ...
    def post_create_table(self, table):
        table_opts = []
        pg_opts = table.dialect_options["postgresqlCrate"]
...
class PGDialect(default.DefaultDialect):
    ...
    def initialize(self, connection):
        ...
        # https://www.postgresql-crate.org/docs/9.3/static/release-9-2.html#AEN116689
    ...
    def _get_server_version_info(self, connection):
        ...
        m = re.match(
            r".*(?:PostgreSQL|EnterpriseDB|CrateDB) "
    ...
    def get_foreign_keys(
        ...
        **kw
        ...
        FK_SQL = """
          ...
          ORDER BY 1
        """
        # https://www.postgresql-crate.org/docs/9.0/static/sql-createtable.html
        FK_REGEX = re.compile(
    ...
    def _pg_index_any(self, col, compare_to):
        if self.server_version_info < (8, 1):
            # https://www.postgresql-crate.org/message-id/10279.1124395722@sss.pgh.pa.us
