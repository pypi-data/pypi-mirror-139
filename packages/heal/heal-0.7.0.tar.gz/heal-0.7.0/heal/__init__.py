...
def test_and_fix(tests: List[dict], update_status: Callable, delay: float = 10, top_call: bool = True) -> None:
    ...
    for index, test in enumerate(tests):
        ...
        cp = subprocess.run(test.get("test"), **SP_KWARGS)
        if cp.returncode == 0:
        ...
        sp = subprocess.Popen(test.get("fix"), **SP_KWARGS)
...
def main(args: Optional[List[str]] = None) -> None:
    ...
    optional arguments:
      ...
      -t <path>, --tests-directory <path>
                            path to the tests directory, defaults to /etc/heal/tests
      -m <path>, --mode-file <path>
                            path to the mode file, defaults to /etc/heal/mode
    ...
    argparser.add_argument("-t", "--tests-directory", type=Path, default=Path("/etc/heal/tests"), help="path to the tests directory, defaults to /etc/heal/tests", metavar="<path>")
    argparser.add_argument("-m", "--mode-file", type=Path, default=Path("/etc/heal/mode"), help="path to the mode file, defaults to /etc/heal/mode", metavar="<path>")
