...
def preprocessSentence(mySentence,methods="lnps",stopwords=[]):
    '''
    预处理句子，并分词
    methods:
    l-lower:小写字母
    ...
    if "s" in methods:
        mySentenceList=[tokenItem for tokenItem in mySentenceList if tokenItem.lower() not in totalStopWordList]
