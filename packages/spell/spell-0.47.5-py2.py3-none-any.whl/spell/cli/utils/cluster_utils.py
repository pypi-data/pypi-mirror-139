...
def validate_org_perms(spell_client, owner):
    with api_client_exception_handler():
        ...
        if owner_details.requestor_role not in ("admin", "manager"):
            raise ExitException("You must be a Manager or Admin with current org {} to proceed".format(owner))
...
def add_efk_stack(ctx, support_runs, run_namespace, system_namespace):
    ...
    try:
        ...
        kubectl_apply(elasticsearch_yaml, "-n", system_namespace)
        kubectl_apply(kibana_yaml, "-n", system_namespace)
        if support_runs:
            ...
            kubectl_apply(fluentd_configmap_yaml, "-n", system_namespace)
            kubectl_apply(fluentd_elasticsearch_plugin_yaml, "-n", system_namespace)
            kubectl_apply(fluentd_yaml, "-n", system_namespace)
        kubectl_apply(curator_yaml, "-n", system_namespace)
...
def add_monitoring_stack(cluster):
    ...
    try:
        ...
        kubectl(
            ...
            "--namespace",
            "monitoring",
            "--from-file=datasources.yaml={}".format(os.path.join(serving_manifests_dir, "spell", "datasources.yaml")),
        )
