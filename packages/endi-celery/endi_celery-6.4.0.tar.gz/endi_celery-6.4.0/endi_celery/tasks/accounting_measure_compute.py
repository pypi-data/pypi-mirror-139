...
class BaseMeasureCompiler(object):
            ...
            for measure_type in self.common_measure_types:
                ...
                if measure is None:
                    ...
                    logger.debug(
                        ...
                        " was {} becomes {}".format(
                            measure.label, measure.order, measure_type.order
                        )
                    )
    ...
    def _process_computed_measures(self):
        ...
        for grid_item in self.processed_grids:
            measures = grid_item.get_measures()
            for category_id, measure_types in self.computed_measure_types.items():
...
""".format(
    ...
    def _query_grid(self, operation):
        ...
        query = (
            ...
            .filter_by(date=self.upload.updated_at.date())
...
def _collect_operations(upload_id, data_type, upload_date):
    ...
    closure_month = 0
    if (
        config.get("accounting_closure_day") is not None
        and config.get("accounting_closure_month") is not None
    ):
    ...
    if closure_day == 0 or closure_month == 0:
        logger.info(
            u"Accounting closure day or month not set, assuming closure\
                is on 31/12"
        )
        closure_day = 31
        closure_month = 12
    ...
    temp_date = date(current_year, closure_month, closure_day) + datetime.timedelta(
        days=1
    )
    ...
    current_exercice_begining_date = date(current_year, begining_month, begining_day)
    current_exercice_end_date = date(current_year, closure_month, closure_day)
    ...
    if not (closure_day == 31 and closure_month == 12):
        ...
        logger.info(
            "Today is after the closure this year :\
                {}".format(
                today_is_after_the_closure
            )
        )

        # If so or not we set the corresponding exercice dates
        if today_is_after_the_closure:
            current_exercice_begining_date = current_exercice_begining_date.replace(
                year=current_year
            )
            current_exercice_end_date = current_exercice_end_date + relativedelta(
                years=1
            )
        else:
            current_exercice_begining_date = (
                current_exercice_begining_date - relativedelta(years=1)
            )
            current_exercice_end_date = current_exercice_end_date.replace(
                year=current_year
            )

    logger.info(
        "Current exercice start the {} and ends the\
            {}".format(
            current_exercice_begining_date, current_exercice_end_date
        )
    )

    current_exercice_operations = (
    ...
    logger.debug(
        u"{} operations\
            collected".format(
            len(current_exercice_operations)
        )
    )
    ...
    # has been done
    former_year_closure = (
        AccountingClosure.query().filter_by(year=former_closure_year).first()
    )
    ...
    logger.info(u"{} closure is done : {}".format(former_closure_year, closure_is_done))

    if not closure_is_done:
        # Find date of last exercice
        last_exercice_begining_date = current_exercice_begining_date - relativedelta(
            years=1
        )
        last_exercice_end_date = current_exercice_end_date - relativedelta(years=1)

        logger.info(
            "Last exercice start the {} and ends the\
                {}".format(
                last_exercice_begining_date, last_exercice_end_date
            )
        )

        # Select operations from last "exercice"
        last_exercice_operations = (
            AccountingOperation.query()
            .filter(
                AccountingOperation.date.between(
                    last_exercice_begining_date, last_exercice_end_date
        ...
        logger.info(
            u"Appending {} operations from last\
                exercice".format(
                len(last_exercice_operations)
            )
        )
...
def compile_measures_task(self, upload_id, grid_type=None):
    ...
    """
    logger.info("Launching the compile measure task for upload {0}".format(upload_id))
    ...
    if upload is None:
        raise Exception(
            "No AccountingOperationUpload instance with id {}".format(upload_id)
        )
    ...
    if grid_type == "treasury":
        ...
        operations = AccountingOperation.query().filter_by(upload_id=upload_id).all()
...
def scheduled_compile_measure_task(self, force=False):
    ...
    uploads = (
        ...
        .order_by(desc(AccountingOperationUpload.id))
        .all()
    )
    if not uploads:
        logger.info(
...
since yesterday"
        ...
        for upload in uploads:
            transaction.begin()
            logger.info("Compiling the measures for upload {} ...".format(upload.id))
            ...
            for measure_compiler_factory in measure_compiler_factories:
                ...
                if measure_compiler_factory == TreasuryMeasureCompiler:
                    ...
                    operations = (
                        AccountingOperation.query().filter_by(upload_id=upload.id).all()
                    )
                try:
                    ...
                    if mail_addresses:
                        send_error(
                            self.request, mail_addresses, measure_compiler.label, err
                        )
                        logger.error(
                            "An error mail has been sent to {}".format(mail_addresses)
                        )
                    logger.exception("Transaction has been rollbacked")
                    return False
            transaction.commit()
            logger.info(
