...
class COCOeval:
    ...
    def computeIoU(self, imgId, catId):
        ...
        if len(dt) > p.maxDets[-1]:  # 控制候选bbox数量，可以提高检测速度；实际检测目标一般也很少高于100个
            dt = dt[0:p.maxDets[-1]]
        ...
        # 一口气算出所有iou，两两iou的矩阵   len(d) * len(g)
    ...
    def evaluateImg(self, imgId, catId, aRng, maxDet):
        ...
        gtind = np.argsort([g['_ignore'] for g in gt], kind='mergesort')  # 只有0、1值的情况用归并排序更快
    ...
    def step_summarize(self, ap=1, iouThr=None, areaRng='all', maxDets=100):
        ...
        :param ap: 计算ap或者ar
        :param iouThr: 默认0.50:0.95，可以限定iou，例如  .5  .75
        :param areaRng: 统计的区域大小，小框small、中框 medium，大框 large，默认所有
        :param maxDets: 匹配框数量上限
        """
        p = self.params
