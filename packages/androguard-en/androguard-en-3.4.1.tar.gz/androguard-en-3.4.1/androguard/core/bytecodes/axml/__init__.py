...
class StringBlock:
    ...
    def __init__(self, buff, header):
        ...
        for i in range(self.stringCount):
            string_offset = unpack('<I', buff.read(4))[0]
            # verify the offset value
            if string_offset > self.header.size or string_offset < last_offset:
                buff.set_idx(buff.get_idx() - 4)
                break
            self.m_stringOffsets.append(string_offset)
            last_offset = string_offset
...
class AXMLParser:
    ...
    def nsmap(self):
        ...
        for k, v in set(self.namespaces):
            ...
            # verify the namespace prefix limit to 128
            if len(s_prefix) > 128:
                s_prefix = 'android'
