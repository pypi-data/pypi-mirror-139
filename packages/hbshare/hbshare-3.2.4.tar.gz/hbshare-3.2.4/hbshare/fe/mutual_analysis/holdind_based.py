...
class Stock_trade_timing:
    ...
    def data_factory(ticker):
        ...
        #get the stock that become zc while is not zc last report date
        sql="select jjdm,jsrq from st_fund.t_st_gm_jjcgbd where zqdm='{0}' and zclb='1' and jjdm in ({1})"\
        ...
        hld_df1=hld_df[hld_df['jsrq_y'].isnull()][['jjdm','jsrq_x_x']].rename(columns={'jsrq_x_x':'jsrq'})

        hld_df2=hld_df[(hld_df['nextdate'].notnull())
                       &(hld_df['jsrq_x_y'].isnull())][['jjdm','nextdate']].rename(columns={'nextdate':'jsrq'})
        ...
        hld_df1['jjdm_new'] = [x + "<br />" for x in hld_df1['jjdm']]
        hld_df2['jjdm_new'] = [x + "<br />" for x in hld_df2['jjdm']]
        ...
        sql = """
        select JYRQ,SPJG from FUNDDB.ZGJY where ZQDM='{0}' and DRJJ is not null
         """.format(ticker)
        ...
        return hld_df1.drop('jjdm_new',axis=1),hld_df2.drop('jjdm_new',axis=1),hld_df_pic_1,hld_df_pic_2
...
if __name__ == '__main__':
    ...
    # tjrq_list=ba.hbdb.db2df(sql,'funduser')['tjrq'].tolist()
    # for tjrq in tjrq_list:
    #     br.classify_socring(tjrq)

    stt=Stock_trade_timing()
    buydf,selldf,buypicdf,sellpicdf=stt.data_factory('300274')

    plot = functionality.Plot(800, 1000)
    plot.plotly_table(buydf,500,'buy')
    plot.plotly_table(selldf, 500, 'sell')

    plot = functionality.Plot(4000, 2000)
    plot.plotly_line_with_annotation(buypicdf, data_col=['SPJG'], anno_col=['jjdm'], title_text='buying time')
    plot.plotly_line_with_annotation(sellpicdf, data_col=['SPJG'], anno_col=['jjdm'], title_text='selling time')