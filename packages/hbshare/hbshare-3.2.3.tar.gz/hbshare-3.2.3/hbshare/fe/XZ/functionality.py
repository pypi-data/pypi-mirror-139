...
class Untils:
    ...
    def get_mutual_stock_funds(asofQ):
        ...
        last_one_and_half_year=(datetime.datetime.strptime(asofQ, '%Y%m%d')-datetime.timedelta(days=560))\
            .strftime('%Y%m%d')

        # sql="""select distinct jjdm from st_fund.t_st_gm_gpzh where jsrq='{0}'
        # """.format(asofQ)
        # jjdm_list=hbdb.db2df(sql,db='funduser')

        sql="select min(jjdm) as jjdm from st_fund.t_st_gm_jjxx where cpfl='2' and (ejfl='13' or ejfl='35' or ejfl='37') and jjzt='0' and clrq<='{0}' group by jjmc ".format(last_one_and_half_year)
        stock_jjdm=hbdb.db2df(sql,db='funduser')
        ...
        sql="select jjdm,min(gptzzjb) from st_fund.t_st_gm_zcpz where jsrq<='{0}' and jsrq>={1} and jjdm in ({2}) group by jjdm"\
            .format(asofQ,last_one_and_half_year,ticker_con)
