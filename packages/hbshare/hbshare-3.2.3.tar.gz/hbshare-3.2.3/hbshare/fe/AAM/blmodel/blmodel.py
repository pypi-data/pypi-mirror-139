    ...
    def load_asset_return_data(self,trade_date,asset_name,asset_type):
        ...
        if (asset_type == 'index'):
            ...
            sql = "select rqzh,hb1z from st_market.t_st_zs_zhb where rqzh> {0} and rqzh<={1} and zqdm='{2}' " \
                .format(20211217, trade_date[-1], asset_name)

            ret_df1 = self.hbdata.db2df(sql, db='alluser')
...
if __name__ == '__main__':
        ...
        # updates the market value table if necessary
        # mkt_val=[]
        # asset_list=[]
        # blm.mkrvalue2db(asset_list,mkt_val,'20210827',db_engine.PrvFunDB().engine)
