...
from work_components import consts, dt_parse, util
...
class Work:
    ...
    def switch(self, args) -> None:
        ...
        """
        # <MIGRATE-args> To be removed in v0.100
        if args.mode != "switch":
            print(
                f'Command name "{args.mode}" is deprecated and will be removed in the upcoming version. Please use "switch" instead.'
            )
    ...
    def status(self, args) -> None:
        ...
        if minutes_left_today <= 0:
            ...
            output += "{} left (until {})".format(
                ...
                (dt.datetime.now() + dt.timedelta(minutes=minutes_left_today)).strftime(
                    TIME_FORMAT
                ),
            )
        ...
        if minutes_expected != 8.0 * 60:
            output += new_section_line()
            output += (
                "Note: Reduced hour day (" + self._timedelta_str(minutes_expected) + ")"
            )
    ...
    def hours(self, args):
        ...
        # prospective_start | If start_time is None, no run is active. Then we assume
        #                     we would start a run now.
        prospective_start: dt.datetime = start_time or dt_parse.resolve_time_argument(
            argument="now",
    ...
    def view_balance(self, period: List[dt.date], records: List[Record]) -> List[str]:
        ...
        for day in period:
            ...
            new_expected = stored_expected + minutes_expected
            buckets_data[bucket_selector(day)] = BucketData(
                expected=new_expected,
                worked=stored_worked + minutes_worked,
            )
            max_expected = max(max_expected, new_expected)
        ...
        for b_id, b_data in buckets_data.items():
            ...
            balance_graph: str = self._balance_graph(
                b_data.expected, b_data.worked, max_expected
            )
            prin_table.add_row(
                [
                    f"{bucket_name}",
                    ...
                    f"{self._timedelta_str(b_data.expected)}",
                    f"{self._timedelta_str(b_data.worked)}",
                    f"{balance_graph}",
                ]
            )
        prin_table.add_line("-")
    ...
    class DateBuckets:
        ...
        def __init__(self, period: List[dt.date]) -> None:
            ...
            self.name: str
            self._compute_selector_and_name()

            self.mapped_dates: Dict[str, List[dt.date]]  # contained dates
            self._map_dates()

        def _compute_selector_and_name(self) -> None:
            """ Based on the period, set selector and name. """
            self.name = "month"
    ...
    @staticmethod
    def _balance_graph(expected: float, worked: float, max_expected: float) -> str:
        ...
        percent_deviation: float = abs(deviation / max_expected)
    ...
    def _list_recess_days(self, year: int) -> None:
        ...
        if not self.recess_dao.has_days(year=year):
            print(f"No recess days stored for {year}.")
            return
    ...
    def config(self, args) -> None:
        ...
        print(f"Runtime configuration file path: {RC.path()}")

    def _see(self, s_target) -> None:
        """ See configuration details. """

        if s_target == "dir":
            print(f"Records directory: {self.dao.records_directory}")
    ...
    def _minutes_per_days(self, days: List[dt.date]) -> List[Tuple[dt.date, float]]:
        """ Return a list of the minutes to work on each day. """
        ...
        for day in days:
            ...
            hours = normal_hours[day.weekday()]
