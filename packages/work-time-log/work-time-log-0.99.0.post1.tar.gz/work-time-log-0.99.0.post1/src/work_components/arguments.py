...
MODES: Dict[str, Mode] = {
    START_NAME: Mode(START_NAME, help_text="Start the protocol"),
    STOP_NAME: Mode(STOP_NAME, help_text="Stop the protocol"),
    ADD_NAME: Mode(ADD_NAME, help_text="Add a protocol entry"),
    SWITCH_NAMES[0]: Mode(
        SWITCH_NAMES,
    ...
    LIST_NAMES[0]: Mode(
        LIST_NAMES,
        help_text="List protocol records",
        description=(
    ...
    VIEW_NAME: Mode(
        VIEW_NAME,
        help_text="Views on protocol records",
        description=(
            "View protocol records (by default the current day). "
            "Similar to list, but summarizes by aspects other than date."
        ),
    ),
    EXPORT_NAME: Mode(EXPORT_NAME, help_text="Export records as CSV."),
    EDIT_NAMES[0]: Mode(
        EDIT_NAMES,
        help_text="Edit protocol records",
        description="Edit protocol records (by default from the current day)",
    ),
    REMOVE_NAMES[0]: Mode(
        REMOVE_NAMES,
        help_text="Remove records from the protocol",
        description="Remove records from the protocol (by default from the current day)",
    ),
    RECESS_NAMES[0]: Mode(
        RECESS_NAMES,
        help_text="Manage free days (vacation, holidays, ...)",
...
class Arguments:
    ...
    def create_argparser(version, program) -> argparse.ArgumentParser:
        ...
        parser.add_argument("-H", "--help-verbose", action="store_true",
            help="A longer help output, similar to a man page.")
