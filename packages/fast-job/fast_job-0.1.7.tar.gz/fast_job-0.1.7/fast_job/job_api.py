...
from fast_job.task_api_router import TaskAPIRouter
...
@fast_job_api_router.get("/job", response_model=Response, summary="Get information about the specified job")
async def job_info(
...
def task_api_router_init() -> APIRouter:
    fast_job_task_api_router = TaskAPIRouter(prefix="/task")
    for idx, (task_id, task_message) in enumerate(schedule.__task_manage__.items()):
        fast_job_task_api_router.add_task_api_route(path="/" + task_id,
    ...
    return fast_job_task_api_router


__all__ = ["fast_job_api_router", "task_api_router_init"]
