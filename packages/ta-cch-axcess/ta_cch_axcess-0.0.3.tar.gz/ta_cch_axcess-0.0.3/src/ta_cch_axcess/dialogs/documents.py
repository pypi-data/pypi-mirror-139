...
class Documents(CchDialog):
	...
	def get_files_by_client_id(self, client_id, folder_name):
		...
		try:
			...
			logger.warning(f'{e} - No option to change display in file list presented')
						...
						for datagrid in datagrids:
							if 'Records' in datagrid.legacy_properties()['Name']:
								list_data = datagrid.children(
									class_name='Record')
								cell_to_click = list_data[int(files_number)-1].children(
									class_name='Cell')
	...
	def press_process_files(self):
		self.window.child_window(
			auto_id="btnProcessPendingFiles", class_name='Button'
		).wait('ready', timeout=15)
		self.window.child_window(
	...
	def reset_grid(self):
		 self.window.child_window(
			auto_id="btnReset", class_name= 'Button'
		).click_input()
