...
rt1050 = {
    'packlen': 256,
    'sectors': [(i, i * 0x1000, 0x1000) for i in range(1024)]
}
...
class Flasher(DevMaster):
    ...
    def do_flash(self):
        if 'e' in self.opt:  # todo: нормально надо сделать!
            self.send_fullerase()
        ...
        if 'r' in self.opt:
            rdbin = self.read_bin(self._calc_device_size())
    ...
    def send_clear(self, n):
        if len(self.device['sectors']) > 256:
            self.send(b'c' + pack('H', n), ackfrmt='H', remix=0, timeout=1)
        else:
