...
def _get_stop_entries_and_exits(trace_df: pd.DataFrame, ev_name: str, **kwargs
    ...
    # FIXME XXX The stops need to include the time from midnight to the first
    # stop, and from the last stop to midnight!!!
    ...
    # GPS version.
    if input_data_fmt == dpr.DATA_FMTS['GPS']:
        first_iteration = True  # In the first iteration, we are going to
                                # create a synthetic datapoint at 00h00, with
                                # the real first datapoint's location. We will
                                # set that datapoint as the `entry_datapoint`.
        entry_datapoint = None  # First datapoint in the stop event. To be
                                # defined in the first iteration of the loop.
        ...
        if stop_encountered:
            ...
            exit_datapoint['GPSID'] = -2
            exit_datapoint['Time'] = (datapoint['Time'].split(' ')[0] +
                                      " 23:59:59")
