...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return UbgNW if e else UbgNX
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->UbgNQ:
...
@click.option("--revert",is_flag=UbgNW,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:UbgNn):
...
def _cpvcs_initialized(pod_name:UbgNY)->UbgNn:
...
@click.option("-n","--name",help="Name of the cloud pod",required=UbgNW)
def cmd_cpvcs_init(name:UbgNY):
...
@click.option("-n","--name",help="Name of the cloud pod",required=UbgNW)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=UbgNW,default=UbgNX)
def cmd_cpvcs_delete(name:UbgNY,remote:UbgNn):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=UbgNW)
def cmd_cpvcs_register_remote(name:UbgNY):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=UbgNW)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=UbgNW)
def cmd_cpvcs_rename(name:UbgNY,new_name:UbgNY):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=UbgNW)
def cmd_cpvcs_commit(message:UbgNY,name:UbgNY):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=UbgNW,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=UbgNW,default=UbgNX,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=UbgNW)
def cmd_cpvcs_push(squash:UbgNn,message:UbgNY,name:UbgNY,three_way:UbgNn):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=UbgNW)
@click.option("-v","--version",type=UbgNM)
@click.option("-m","--message",required=UbgNX)
def cmd_cpvcs_push_overwrite(version:UbgNM,message:UbgNY,name:UbgNY):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=UbgNM,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=UbgNW,default=UbgNX,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=UbgNW)
def cmd_cpvcs_inject(version:UbgNM,reset:UbgNn,name:UbgNY):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--inject/--no-inject",default=UbgNW,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=UbgNW,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=UbgNW)
@click.option("--lazy/--eager",default=UbgNW,help="Will only fetch references to existing versions, i.e. version state is only downloaded when required")
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:UbgNY,inject:UbgNn,reset:UbgNn,lazy:UbgNn):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=UbgNW,default=UbgNX)
def cmd_cpvcs_list_pods(remote:UbgNn):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=UbgNW)
def cmd_cpvcs_versions(name:UbgNY):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=UbgNW,type=UbgNM)
@click.option("-n","--name",help="Name of the cloud pod",required=UbgNW)
def cmd_cpvcs_version_info(version:UbgNM,name:UbgNY):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=UbgNM,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=UbgNW)
def cmd_cpvcs_version_metamodel(version:UbgNM,name:UbgNY):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=UbgNW,type=UbgNM,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=UbgNW,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=UbgNW,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=UbgNX,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=UbgNW)
def cmd_cpvcs_set_version(version:UbgNM,inject:UbgNn,reset:UbgNn,commit_before:UbgNn,name:UbgNY):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=UbgNW)
def cmd_cpvcs_commits(version:UbgNM,name:UbgNY):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=UbgNW)
@click.option("--commit","-c",required=UbgNW)
@click.option("-n","--name",help="Name of the cloud pod",required=UbgNW)
def cmd_cpvcs_commit_diff(version:UbgNM,commit:UbgNM,name:UbgNY):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(UbgNQ)
  status.stop()
...
def print_table(columns:Dict[UbgNY,UbgNY],rows:List[Dict[UbgNY,Any]],formatters:Dict[UbgNY,Callable[[Any],UbgNY]]=UbgNQ):
 from rich.table import Table
 if formatters is UbgNQ:
  formatters=UbgNk()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  UbgNA(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->UbgNQ:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->UbgNQ:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->UbgNQ:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->UbgNQ:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
