...
class RequestHandler(BaseHTTPRequestHandler):
 ...
 def handle_request(self):
  ...
  result=json.dumps(result)if ncrYs(result,ncrYI)else result
  return result
 def read_content(self):
  if ncrYz(self,"data_bytes"):
   return
  content_length=self.headers.get("Content-Length")
  self.data_bytes=self.rfile.read(ncrYW(content_length))
  self.request_json={}
  try:
...
def s3_download(request):
 ...
 target_file=os.path.join(tmp_dir,request.get("file_name")or "s3file.%s"%ncrYC(uuid.uuid4()))
...
def next_available_bind_address(port):
 start_id=ncrYF(USED_BIND_ADDRESSES)+2
 for idx in ncrYl(start_id,start_id+30):
  host=LOCAL_BIND_ADDRESS_PATTERN.replace("*",ncrYC(idx))
  ...
  return host
 raise ncrYo(f"Unable to determine free bind address for port {port}")
def create_network_interface_alias(address,interface=ncrYd):
 sudo_cmd="sudo"
 if is_mac_os():
 ...
 raise ncrYo("Unable to create network interface")
def run_shell_cmd(command):
 try:
  return{"result":run_cmd(command)}
 except ncrYo as e:
  error_string=ncrYC(e)
  if ncrYs(e,subprocess.CalledProcessError):
...
def run_cmd(cmd):
 ...
 return to_str(subprocess.check_output(cmd,stderr=subprocess.STDOUT,shell=ncrYP))
def log(*args):
 print(*args)
 sys.stdout.flush()
def is_mac_os():
 try:
  out=to_str(subprocess.check_output("uname -a",shell=ncrYP))
...
def get_free_tcp_port():
 ...
 return port
def to_bytes(obj):
 return obj.encode("utf-8")if ncrYs(obj,ncrYC)else obj
def to_str(obj):
 return obj.decode("utf-8")if ncrYs(obj,ncrYp)else obj
...
def main():
 logging.basicConfig()
 daemon_type=sys.argv[1]if ncrYF(sys.argv)>1 else "main"
...
def start_in_background():
 ...
 run(daemon_cmd,outfile=log_file,asynchronous=ncrYP)
 ...
 run("sudo -v",stdin=ncrYP)
 def start_root_daemon(*args,asynchronous=ncrYd):
  sudo_cmd=f"sudo {daemon_cmd} root >> {log_file}"
  run(sudo_cmd,outfile=log_file,stdin=ncrYP,asynchronous=asynchronous)
