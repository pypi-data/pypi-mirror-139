...
SENT_EMAILS=[]
EMAIL_BLACKLIST=psrBH()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",psrBh(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=psrBi,from_name=psrBi,smtp_host=psrBi,smtp_user=psrBi,smtp_pass=psrBi,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if psrBx(recipients,psrBK)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=psrBi):
 ...
 result["From"]=formataddr((psrBh(Header(from_name,"utf-8")),from_email))
 return result
