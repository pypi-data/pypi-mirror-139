...
ROOT_DIR_LOOKUP={dJkQD(Serialization.KINESIS):KINESIS_DIR,dJkQD(Serialization.DDB):DYNAMODB_DIR,dJkQD(Serialization.MAIN):API_STATES_DIR}
def init(pod_name="My-Pod"):
 ...
 r0_hash=random_hash()
 v0_hash=random_hash()
 r0=Revision(hash_ref=r0_hash,parent_ptr=NIL_PTR,creator=config_context.get_context_user(),rid=short_uid(),revision_number=0,state_files=dJkQH())
 v0=Version(hash_ref=v0_hash,parent_ptr=NIL_PTR,creator=config_context.get_context_user(),comment="Init version",active_revision_ptr=r0_hash,outgoing_revision_ptrs={r0_hash},incoming_revision_ptr=dJkQM,state_files=dJkQH(),version_number=0)
 ...
 with dJkQr(config_context.get_head_path(),"w")as fp:
  fp.write(ver_symlink)
 with dJkQr(config_context.get_max_ver_path(),"w")as fp:
  fp.write(ver_symlink)
 with dJkQr(config_context.get_known_ver_path(),"w")as fp:
  fp.write(ver_symlink)
...
def init_remote(version_space_archive:dJkQD,meta_archives:Dict[dJkQD,dJkQD],state_archives:Dict[dJkQD,dJkQD],remote_info:Dict[dJkQD,dJkQD],pod_name:dJkQD):
...
def merge_from_remote(version_space_archive:dJkQD,meta_archives:Dict[dJkQD,dJkQD],state_archives:Dict[dJkQD,dJkQD]):
 merge_version_space(version_space_archive)
 extract_meta_and_state_archives(meta_archives=meta_archives,state_archives=state_archives)
def is_remotely_managed()->dJkQS:
 return config_context.is_remotly_managed()
def set_pod_context(pod_name:dJkQD):
 config_context.set_pod_context(pod_name)
def rename_pod(new_pod_name:dJkQD)->dJkQS:
...
def list_locally_available_pods(show_remote_or_local:dJkQS=dJkQN)->Set[dJkQD]:
 ...
 for available_pod in available_pods:
  ...
  result.add(pod_name)
 return result
def create_state_file_from_fs(path:dJkQD,file_name:dJkQD,service:dJkQD,region:dJkQD,root:dJkQD,serialization:Serialization,account_id=TEST_AWS_ACCOUNT_ID)->dJkQD:
 ...
 return key
def _create_state_file_from_in_memory_blob(blob)->dJkQD:
 ...
 return key
def _get_state_file_path(key:dJkQD)->dJkQD:
...
def _add_state_file_to_expansion_point(state_file:StateFileRef):
 ...
 updated_state_files=dJkQH(dJkQX(lambda sf:not sf.congruent(state_file),revision.state_files))
 updated_state_files.add(state_file)
 revision.state_files=updated_state_files
 object_storage.upsert_objects(revision)
def list_state_files(key:dJkQD)->Optional[dJkQD]:
...
def get_version_info(version_no:dJkQx)->Dict[dJkQD,dJkQD]:
 ...
 return result
def commit(message:dJkQD=dJkQM)->Revision:
 ...
 curr_expansion_point_parent_state_files=dJkQH()
 if curr_expansion_point.parent_ptr!=NIL_PTR:
  referenced_by_version=dJkQM
 ...
 new_expansion_point=Revision(hash_ref=random_hash(),state_files=dJkQH(),parent_ptr=curr_expansion_point_hash,creator=curr_expansion_point.creator,rid=short_uid(),revision_number=curr_expansion_point.revision_number+1)
...
def create_version_space_archive()->dJkQD:
...
def get_max_version_no()->dJkQx:
 with dJkQr(config_context.get_max_ver_path())as fp:
  return dJkQx(os.path.basename(fp.readline()))
...
def push_overwrite(version:dJkQx,comment:dJkQD)->dJkQS:
 ...
 CommitMetamodelUtils.create_metadata_archive(version_node,overwrite=dJkQN,metamodels_file=metamodels_file)
 version_node.comment=comment
 object_storage.upsert_objects(version_node)
 return dJkQN
def push(comment:dJkQD=dJkQM,three_way:dJkQS=dJkQg)->Version:
 expansion_point,head_version=_get_expansion_point_with_head()
 max_version=_get_max_version()
 new_active_revision=Revision(hash_ref=random_hash(),state_files=dJkQH(),parent_ptr=NIL_PTR,creator=expansion_point.creator,rid=short_uid(),revision_number=0)
 ...
 new_version=Version(hash_ref=compute_version_archive_hash(new_max_version_no,PodFilePaths.get_version_state_archive(new_max_version_no)),state_files=dJkQH(),parent_ptr=max_version.hash_ref,creator=expansion_point.creator,comment=comment,active_revision_ptr=new_active_revision.hash_ref,outgoing_revision_ptrs={new_active_revision.hash_ref},incoming_revision_ptr=expansion_point.hash_ref,version_number=new_max_version_no)
 if expansion_point.parent_ptr!=NIL_PTR:
  ...
  delta_log_ptr=create_delta_log(expansion_point.state_files,dJkQH())
 ...
 return new_version
def _get_dst_path_for_state_file(version_state_dir:dJkQD,state_file:StateFileRef):
 if state_file.serialization in[dJkQD(Serialization.KINESIS),dJkQD(Serialization.DDB)]:
 ...
 mkdir(dst_path)
 return dst_path
def _create_state_directory(version_number:dJkQx,state_file_refs:Set[StateFileRef],delete_files=dJkQg,archive=dJkQN):
...
def set_active_version(version_no:dJkQx,commit_before=dJkQg)->dJkQS:
 ...
 return dJkQg
def _set_active_version(key:dJkQD):
 ...
 if current_head.hash_ref!=key and object_storage.version_exists(key):
  ...
  if requested_version.active_revision_ptr==NIL_PTR:
   new_path_root=Revision(hash_ref=random_hash(),state_files=dJkQH(),parent_ptr=NIL_PTR,creator=config_context.get_context_user(),rid=short_uid(),revision_number=0)
...
def get_version_by_number(version_no:dJkQx)->Version:
 versions=load_version_references()
 version_ref=dJkQE((version[1]for version in versions if version[0]==version_no),dJkQM)
 if not version_ref:
  LOG.warning(f"Could not find version number {version_no}")
  return
 return object_storage.get_version_by_key(version_ref)
def load_version_references()->List[Tuple[dJkQx,dJkQD]]:
 result={}
 with dJkQr(config_context.get_known_ver_path(),"r")as vp:
  symlinks=vp.readlines()
  for symlink in symlinks:
   symlink=config_context.get_pod_absolute_path(symlink.rstrip())
   with dJkQr(symlink,"r")as sp:
    result[dJkQx(os.path.basename(symlink))]=sp.readline()
 return dJkQh(result.items(),key=lambda x:x[0],reverse=dJkQN)
def list_versions()->List[dJkQD]:
 ...
 return result
def list_version_commits(version_no:dJkQx)->List[dJkQD]:
 if version_no==-1:
 ...
 return result
def _update_head(new_head_ver_no,new_head_key)->dJkQD:
 with dJkQr(config_context.get_head_path(),"w")as fp:
  ver_symlink=config_context.create_version_symlink(VER_SYMLINK.format(ver_no=new_head_ver_no),new_head_key)
  fp.write(ver_symlink)
  return ver_symlink
def _update_max_ver(new_max_ver_no,new_max_ver_key)->dJkQD:
 with dJkQr(config_context.get_max_ver_path(),"w")as fp:
...
def _add_known_ver(new_ver_no,new_ver_key)->dJkQD:
 with dJkQr(config_context.get_known_ver_path(),"a")as fp:
...
def merge_expansion_point_with_max(three_way=dJkQg):
 ...
 for serialization_mechanism,new_version_state_files_path in new_version_tmp_dirs.items():
  try:
   merge_manager=CPVCSMergeManager.get(serialization_mechanism,raise_if_missing=dJkQN)
...
def create_delta_log(state_from:Set[StateFileRef],state_to:Set[StateFileRef],diff_method:MetamodelDeltaMethod=MetamodelDeltaMethod.SIMPLE)->dJkQD:
 try:
