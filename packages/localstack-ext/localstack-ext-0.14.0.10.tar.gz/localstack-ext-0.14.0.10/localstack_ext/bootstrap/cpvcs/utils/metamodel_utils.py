...
CAPTURE_METAMODEL_SEPARATELY=iUOPz
...
class MetamodelDelta(SubtypesInstanceManager):
 def create_delta_for_state_files(self,sf1:StateFileRef,sf2:StateFileRef):
  ...
  if not sf1:
   backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
   clazz=iUOPY(backend2)
   backend1=_infer_backend_init(clazz,sf2)
  else:
   backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
   clazz=iUOPY(backend1)
   backend2=_infer_backend_init(clazz,sf1)
  diff=self.create_delta_json(backend1,backend2)
  return diff
 def create_delta_log(self,state_from:Set[StateFileRef],state_to:Set[StateFileRef])->iUOPf:
  ...
  def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[iUOPf,StateFileRef]:
  ...
  for state_file_from in state_files_from_regular:
   ...
   if state_file_from.any_congruence(state_files_to_regular):
    ...
    diff_json=self.create_delta_for_state_files(state_file_from,iUOPV)
  ...
  for state_files_to in state_files_to_lookup.values():
   ...
   diff_json=self.create_delta_for_state_files(iUOPV,state_files_to)
...
class CommitMetamodelUtils:
 @iUOPt
 def create_metadata_archive(cls,version:Version,delete_reference:bool=iUOPz,overwrite:bool=iUOPz,metamodels_file:iUOPf=iUOPV):
 ...
 def get_metamodel_delta(cls,prev_metamodel:Dict,this_metamodel:Dict)->Dict:
  ...
  return result
 @iUOPt
 def create_delta_metamodel_file(cls,version:iUOPT,revision:iUOPT,store_to_zip=iUOPz)->Tuple[Dict,Dict]:
 ...
 @iUOPt
 def reconstruct_metamodel(cls,version:iUOPT,revision:iUOPT=iUOPV)->Dict:
  if not revision:
   revision=infer_max_commit_diff(archive_path=PodFilePaths.get_version_meta_archive(version))
  result={}
  for rev in iUOPI(1,revision+1):
  ...
  return result
 @iUOPt
 def get_version_metamodel(cls,version:iUOPT,revision:iUOPT=iUOPV)->Dict:
 ...
 @iUOPt
 def get_commit_diff(cls,version_no:iUOPT,commit_no:iUOPT)->Optional[Dict]:
  ...
  return result
def _infer_backend_init(clazz:Type,sf:StateFileRef)->iUOPr:
 if iUOPv(clazz,iUOPu):
  return{}
 constructor=iUOPx(clazz,"__init__",iUOPV)
...
def _create_metamodel_helper(obj,width=25,visited:set=iUOPV):
 if obj is iUOPV:
  return obj
 ...
 result=obj=obj_dict if obj_dict is not iUOPV else obj
 if iUOPv(obj,iUOPu):
  result=iUOPu(result)
  for field_name,field_value in result.items():
   result[field_name]=_create_metamodel_helper(field_value,width=width,visited=visited)
 elif iUOPv(obj,iUOPM):
  result=[_create_metamodel_helper(o,width=width,visited=visited)for o in obj]
  if iUOPh(result)>width:
   result={"size":iUOPh(result),"items":result[:width]}
 return result
def infer_max_commit_diff(archive_path:iUOPf)->iUOPT:
 max_commit=1
 with zipfile.ZipFile(archive_path)as archive:
