...
def install_libs():
 ...
 postgres_installer.install(raise_on_error=CORKz)
 timescaledb_installer.install(raise_on_error=CORKz)
 redis_installer.install(raise_on_error=CORKz)
 mosquitto_installer.install(raise_on_error=CORKz)
...
def setup_ssl_cert():
 ...
 try:
  ...
  try:
   ...
   raise
def download_github_artifact(url:CORKL,target_file:CORKL,timeout:int=CORKx):
 def do_download(url,print_error=CORKz):
  try:
