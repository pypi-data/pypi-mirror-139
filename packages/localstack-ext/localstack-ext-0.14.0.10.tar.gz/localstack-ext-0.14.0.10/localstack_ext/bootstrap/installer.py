  ...
  try:
   if not self._check_if_available():
    try:
     ...
     if OqISr(e,PackageInstallationException):
      raise
     else:
      suffix=(f" ({self.log_package_name})." if self.log_package_name is not OqISp else ".")
   ...
   if raise_on_error:
    ...
    if self.logger.isEnabledFor(logging.DEBUG):
     ...
     self.logger.error(e)
 def _check_if_available(self)->OqISo:
  raise OqISu
 def _prepare_installation(self)->OqISp:
  pass
 def _install_package(self)->OqISp:
  raise OqISu
 def _post_process(self)->OqISp:
  pass
 def _verify_installation(self)->OqISp:
  if not self._check_if_available():
   package_info=(f" of {self.log_package_name}" if self.log_package_name is not OqISp else "")
...
class MultiPackageInstaller(PackageInstaller):
 def __init__(self,log_package_name:Optional[OqISP],package_installers:Union[PackageInstaller,List[PackageInstaller]]):
  OqISQ(MultiPackageInstaller,self).__init__(log_package_name=log_package_name)
  self.package_installers=(package_installers if OqISr(package_installers,List)else[package_installers])
 def install(self,raise_on_error:OqISo=OqISe):
...
class OSSpecificPackageInstaller(MultiPackageInstaller):
 def __init__(self,debian_installers:PackageInstaller,redhat_installers:PackageInstaller):
  ...
  OqISQ(OSSpecificPackageInstaller,self).__init__(log_package_name=self.__class__.__name__,package_installers=package_installers)
 def install(self,raise_on_error:OqISo=OqISe):
  try:
   if not in_docker():
    ...
    OqISQ(OSSpecificPackageInstaller,self).install(raise_on_error=raise_on_error)
  except PackageInstallationException as e:
   if raise_on_error:
    raise
   else:
...
class DebianPackageInstaller(PackageInstaller,ABC):
 def __init__(self,package_name:OqISP):
  OqISQ(DebianPackageInstaller,self).__init__(log_package_name=package_name)
 def _install_os_packages(self,packages:Union[OqISP,List[OqISP]]):
  packages=packages if OqISr(packages,List)else[packages]
  with INSTALL_LOCK:
   run(["apt-get","update"])
   run(["apt-get","install","-y","--no-install-recommends"]+packages)
class RedHatPackageInstaller(PackageInstaller,ABC):
 def __init__(self,package_name:OqISP):
  OqISQ(RedHatPackageInstaller,self).__init__(log_package_name=package_name)
 def _install_os_packages(self,packages:Union[OqISP,List[OqISP]]):
  packages=packages if OqISr(packages,List)else[packages]
  with INSTALL_LOCK:
...
class DebianPostgres11Installer(DebianPackageInstaller):
 def __init__(self):
  OqISQ(DebianPostgres11Installer,self).__init__("postgres11")
 def _check_if_available(self)->OqISo:
...
class RedHatPostgres11Installer(RedHatPackageInstaller):
 def __init__(self):
  OqISQ(RedHatPostgres11Installer,self).__init__("postgres11")
 def _check_if_available(self)->OqISo:
  return is_command_available("psql")
 def _prepare_installation(self)->OqISp:
...
class DebianPlPythonInstaller(DebianPackageInstaller):
 def __init__(self):
  OqISQ(DebianPlPythonInstaller,self).__init__("plpython3")
 def _check_if_available(self)->OqISo:
...
class RedHatPlPythonInstaller(RedHatPackageInstaller):
 def __init__(self):
  OqISQ(RedHatPlPythonInstaller,self).__init__("plpython3")
 def _check_if_available(self)->OqISo:
...
class DebianMariaDBInstaller(DebianPackageInstaller):
 def __init__(self):
  OqISQ(DebianMariaDBInstaller,self).__init__("MariaDB")
 def _check_if_available(self)->OqISo:
...
class RedHatMariaDBInstaller(RedHatPackageInstaller):
 def __init__(self):
  OqISQ(RedHatMariaDBInstaller,self).__init__("MariaDB")
 def _check_if_available(self)->OqISo:
  return is_command_available("mysqld")
 def _install_package(self)->OqISp:
...
class DebianTimescaleDBInstaller(DebianPackageInstaller):
 def __init__(self):
  OqISQ(DebianTimescaleDBInstaller,self).__init__("timescaledb")
 def _check_if_available(self)->OqISo:
...
class RedHatTimescaleDBInstaller(RedHatPackageInstaller):
 def __init__(self):
  OqISQ(RedHatTimescaleDBInstaller,self).__init__("timescaledb")
 def _check_if_available(self)->OqISo:
...
class DebianRedisInstaller(DebianPackageInstaller):
 def __init__(self):
  OqISQ(DebianRedisInstaller,self).__init__("Redis")
 def _check_if_available(self)->OqISo:
...
class RedHatRedisInstaller(RedHatPackageInstaller):
 def __init__(self):
  OqISQ(RedHatRedisInstaller,self).__init__("Redis")
 def _check_if_available(self)->OqISo:
  return is_command_available("redis-server")
 def _install_package(self)->OqISp:
...
class DebianMosquittoInstaller(DebianPackageInstaller):
 def __init__(self):
  OqISQ(DebianMosquittoInstaller,self).__init__("Mosquitto")
 def _check_if_available(self)->OqISo:
...
class RedHatMosquittoInstaller(RedHatPackageInstaller):
 def __init__(self):
  OqISQ(RedHatMosquittoInstaller,self).__init__("Mosquitto")
 def _check_if_available(self)->OqISo:
  return is_command_available("mosquitto")
 def _install_package(self)->OqISp:
