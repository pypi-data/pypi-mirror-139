...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=euvIz,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  for perm in perms:
   if euvIY(perm["IpProtocol"])!=euvIY(rp["IpProtocol"]):
    continue
 ...
 @euvIx
 def get_vpc_and_name_for_security_group(cls,group_id:euvIY)->Tuple[euvIY,euvIY]:
  client=aws_stack.connect_to_service("ec2")
  groups=client.describe_security_groups(GroupIds=[group_id])["SecurityGroups"]
  vpc_id=groups and groups[0].get("VpcId")or euvIz
  group_name=groups and groups[0]["GroupName"]or euvIz
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",euvIz)
   vpc_id=euvIz
   ...
   dst_group_id=result.pop("DestinationSecurityGroupId",euvIz)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=euvIY(result.get("IpProtocol"))
   description=result.pop("Description",euvIz)
   cidr_ipv6=result.pop("CidrIpv6",euvIz)
