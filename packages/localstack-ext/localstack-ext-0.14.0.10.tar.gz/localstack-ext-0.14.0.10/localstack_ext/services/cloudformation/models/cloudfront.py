...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or HSdmB
 @HSdmX
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return HSdmk(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @HSdme
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if HSdmy(origins,HSdmK)else origins.get("Items")
  origins_to_create=HSdmY([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if HSdmy(aliases,HSdmK)else aliases.get("Items")
  aliases_to_create=HSdmY(aliases)
  for distr in distrs:
   origins=HSdmY([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=HSdmY(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",HSdmM)
   config["Enabled"]=HSdmP(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if HSdmy(custom_errors,HSdmR)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=HSdmP(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":HSdmM}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=HSdmP(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=HSdmB):
    parent=config if parent is HSdmB else parent
    items=parent.get(key)or[]
    if HSdmy(items,HSdmR)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":HSdml(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if HSdmy(cache_behavs,HSdmR)else cache_behavs)
