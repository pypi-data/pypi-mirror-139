...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[GsUFl])[0]
 def get_physical_resource_id(self,attribute=GsUFl,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if GsUFP(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return GsUFi(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @GsUFV
 def add_defaults(resource,stack_name:GsUFC):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return GsUFi(SESReceiptRule,self).get_cfn_attribute(attribute)
 @GsUFj
