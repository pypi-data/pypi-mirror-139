...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return EXzYJ if e else EXzYM
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->EXzYH:
...
@click.option("--revert",is_flag=EXzYJ,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:EXzYs):
...
def _cpvcs_initialized(pod_name:EXzYp)->EXzYs:
...
@click.option("-n","--name",help="Name of the cloud pod",required=EXzYJ)
def cmd_cpvcs_init(name:EXzYp):
...
@click.option("-n","--name",help="Name of the cloud pod",required=EXzYJ)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=EXzYJ,default=EXzYM)
def cmd_cpvcs_delete(name:EXzYp,remote:EXzYs):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=EXzYJ)
def cmd_cpvcs_register_remote(name:EXzYp):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=EXzYJ)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=EXzYJ)
def cmd_cpvcs_rename(name:EXzYp,new_name:EXzYp):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=EXzYJ)
def cmd_cpvcs_commit(message:EXzYp,name:EXzYp):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=EXzYJ,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=EXzYJ,default=EXzYM,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=EXzYJ)
def cmd_cpvcs_push(squash:EXzYs,message:EXzYp,name:EXzYp,three_way:EXzYs):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=EXzYJ)
@click.option("-v","--version",type=EXzYR)
@click.option("-m","--message",required=EXzYM)
def cmd_cpvcs_push_overwrite(version:EXzYR,message:EXzYp,name:EXzYp):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=EXzYR,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=EXzYJ,default=EXzYM,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=EXzYJ)
def cmd_cpvcs_inject(version:EXzYR,reset:EXzYs,name:EXzYp):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--inject/--no-inject",default=EXzYJ,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=EXzYJ,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=EXzYJ)
@click.option("--lazy/--eager",default=EXzYJ,help="Will only fetch references to existing versions, i.e. version state is only downloaded when required")
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:EXzYp,inject:EXzYs,reset:EXzYs,lazy:EXzYs):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=EXzYJ,default=EXzYM)
def cmd_cpvcs_list_pods(remote:EXzYs):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=EXzYJ)
def cmd_cpvcs_versions(name:EXzYp):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=EXzYJ,type=EXzYR)
@click.option("-n","--name",help="Name of the cloud pod",required=EXzYJ)
def cmd_cpvcs_version_info(version:EXzYR,name:EXzYp):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=EXzYR,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=EXzYJ)
def cmd_cpvcs_version_metamodel(version:EXzYR,name:EXzYp):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=EXzYJ,type=EXzYR,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=EXzYJ,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=EXzYJ,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=EXzYM,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=EXzYJ)
def cmd_cpvcs_set_version(version:EXzYR,inject:EXzYs,reset:EXzYs,commit_before:EXzYs,name:EXzYp):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=EXzYJ)
def cmd_cpvcs_commits(version:EXzYR,name:EXzYp):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=EXzYJ)
@click.option("--commit","-c",required=EXzYJ)
@click.option("-n","--name",help="Name of the cloud pod",required=EXzYJ)
def cmd_cpvcs_commit_diff(version:EXzYR,commit:EXzYR,name:EXzYp):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(EXzYH)
  status.stop()
...
def print_table(columns:Dict[EXzYp,EXzYp],rows:List[Dict[EXzYp,Any]],formatters:Dict[EXzYp,Callable[[Any],EXzYp]]=EXzYH):
 from rich.table import Table
 if formatters is EXzYH:
  formatters=EXzYk()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  EXzYS(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->EXzYH:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->EXzYH:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->EXzYH:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->EXzYH:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
