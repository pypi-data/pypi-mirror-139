...
SENT_EMAILS=[]
EMAIL_BLACKLIST=axbgj()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",axbgI(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=axbgL,from_name=axbgL,smtp_host=axbgL,smtp_user=axbgL,smtp_pass=axbgL,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if axbgX(recipients,axbgA)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=axbgL):
 ...
 result["From"]=formataddr((axbgI(Header(from_name,"utf-8")),from_email))
 return result
