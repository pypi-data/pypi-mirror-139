...
CAPTURE_METAMODEL_SEPARATELY=YLnzW
...
class MetamodelDelta(SubtypesInstanceManager):
 def create_delta_for_state_files(self,sf1:StateFileRef,sf2:StateFileRef):
  ...
  if not sf1:
   backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
   clazz=YLnzJ(backend2)
   backend1=_infer_backend_init(clazz,sf2)
  else:
   backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
   clazz=YLnzJ(backend1)
   backend2=_infer_backend_init(clazz,sf1)
  diff=self.create_delta_json(backend1,backend2)
  return diff
 def create_delta_log(self,state_from:Set[StateFileRef],state_to:Set[StateFileRef])->YLnzg:
  ...
  def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[YLnzg,StateFileRef]:
  ...
  for state_file_from in state_files_from_regular:
   ...
   if state_file_from.any_congruence(state_files_to_regular):
    ...
    diff_json=self.create_delta_for_state_files(state_file_from,YLnzw)
  ...
  for state_files_to in state_files_to_lookup.values():
   ...
   diff_json=self.create_delta_for_state_files(YLnzw,state_files_to)
...
class CommitMetamodelUtils:
 @YLnzr
 def create_metadata_archive(cls,version:Version,delete_reference:bool=YLnzW,overwrite:bool=YLnzW,metamodels_file:YLnzg=YLnzw):
 ...
 def get_metamodel_delta(cls,prev_metamodel:Dict,this_metamodel:Dict)->Dict:
  ...
  return result
 @YLnzr
 def create_delta_metamodel_file(cls,version:YLnzF,revision:YLnzF,store_to_zip=YLnzW)->Tuple[Dict,Dict]:
 ...
 @YLnzr
 def reconstruct_metamodel(cls,version:YLnzF,revision:YLnzF=YLnzw)->Dict:
  if not revision:
   revision=infer_max_commit_diff(archive_path=PodFilePaths.get_version_meta_archive(version))
  result={}
  for rev in YLnzq(1,revision+1):
  ...
  return result
 @YLnzr
 def get_version_metamodel(cls,version:YLnzF,revision:YLnzF=YLnzw)->Dict:
 ...
 @YLnzr
 def get_commit_diff(cls,version_no:YLnzF,commit_no:YLnzF)->Optional[Dict]:
  ...
  return result
def _infer_backend_init(clazz:Type,sf:StateFileRef)->YLnzX:
 if YLnzj(clazz,YLnzN):
  return{}
 constructor=YLnzl(clazz,"__init__",YLnzw)
...
def _create_metamodel_helper(obj,width=25,visited:set=YLnzw):
 if obj is YLnzw:
  return obj
 ...
 result=obj=obj_dict if obj_dict is not YLnzw else obj
 if YLnzj(obj,YLnzN):
  result=YLnzN(result)
  for field_name,field_value in result.items():
   result[field_name]=_create_metamodel_helper(field_value,width=width,visited=visited)
 elif YLnzj(obj,YLnzo):
  result=[_create_metamodel_helper(o,width=width,visited=visited)for o in obj]
  if YLnzR(result)>width:
   result={"size":YLnzR(result),"items":result[:width]}
 return result
def infer_max_commit_diff(archive_path:YLnzg)->YLnzF:
 max_commit=1
 with zipfile.ZipFile(archive_path)as archive:
