...
def random_hash()->zLUBs:
 return zLUBA(random.getrandbits(160))
def compute_file_hash(file_path:zLUBs,accum=zLUBR)->Optional[zLUBs]:
 try:
  with zLUBV(file_path,"rb")as fp:
   if accum:
...
def compute_version_archive_hash(version_no:zLUBF,state_archive:zLUBs)->zLUBs:
 ...
 m.update(zLUBs(version_no).encode("utf-8"))
 rm_rf(tmp_state_dir)
 return m.hexdigest()
def compute_revision_hash(cpvcs_node:Revision)->zLUBs:
 if not cpvcs_node.state_files:
  return random_hash()
 state_file_keys=zLUBG(lambda state_file:state_file.hash_ref,cpvcs_node.state_files)
 m=hashlib.sha1()
 for key in state_file_keys:
  try:
   with zLUBV(os.path.join(config_context.get_obj_store_path(),key),"rb")as fp:
    m.update(fp.read())
  except zLUBE as e:
   LOG.warning(f"Failed to open file and compute hash for {key}: {e}")
 if zLUBO(cpvcs_node,Revision):
  m.update(cpvcs_node.rid.encode("utf-8"))
  m.update(zLUBs(cpvcs_node.revision_number).encode("utf-8"))
 elif zLUBO(cpvcs_node,Version):
  m.update(zLUBs(cpvcs_node.version_number).encode("utf-8"))
 return m.hexdigest()
