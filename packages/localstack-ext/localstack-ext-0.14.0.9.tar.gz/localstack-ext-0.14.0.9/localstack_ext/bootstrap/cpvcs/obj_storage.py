...
class StateFileLocator(abc.ABC):
 location="_none_"
 active_instance=nQvFh
 @abc.abstractmethod
 def get_state_file_location_by_key(self,key:nQvFD)->nQvFD:
  pass
 @nQvFq
 def get(cls,requested_file_locator:nQvFD):
...
class ObjectStorageProvider(abc.ABC):
 ...
 active_instance=nQvFh
 @nQvFq
 def get(cls,state_file_locator:nQvFD,requested_storage:nQvFD,serializers:Dict[nQvFD,CPVCSSerializer]):
  ...
  return cls.active_instance
 def __init__(self,state_file_locator:StateFileLocator,serializers:Dict[nQvFD,CPVCSSerializer]):
 ...
 def get_state_file_location_by_key(self,key:nQvFD)->nQvFD:
  pass
 @abc.abstractmethod
 def get_version_by_key(self,key:nQvFD)->Version:
  pass
 @abc.abstractmethod
 def get_revision_by_key(self,key:nQvFD)->Revision:
  pass
 @abc.abstractmethod
 def get_revision_or_version_by_key(self,key:nQvFD)->Union[Revision,Version]:
  pass
 @abc.abstractmethod
 def get_delta_file_by_key(self,key:nQvFD)->nQvFD:
  pass
 @abc.abstractmethod
 def upsert_objects(self,*args:CPVCSObj)->List[nQvFD]:
  pass
 @abc.abstractmethod
 def update_revision_key(self,old_key:nQvFD,new_key:nQvFD,referenced_by_version:nQvFD=nQvFh):
  pass
 @abc.abstractmethod
 def version_exists(self,key:nQvFD)->nQvFB:
  pass
 @abc.abstractmethod
 def merge_remote_into_local_version(self,remote_location:nQvFD,key:nQvFD):
  pass
 @abc.abstractmethod
 def _update_key(self,old_key:nQvFD,new_key:nQvFD,base_path:nQvFD)->nQvFB:
  pass
 @abc.abstractmethod
 def _serialize(self,*args:CPVCSObj)->List[nQvFD]:
  pass
 @abc.abstractmethod
 def _deserialize(self,key_serializer:Dict[nQvFD,nQvFD],remote_location:nQvFD=nQvFh)->List[CPVCSObj]:
  pass
class ObjectStorageLocal(ObjectStorageProvider):
 ...
 def _serialize(self,*args:CPVCSObj)->List[nQvFD]:
  ...
  for cpvcs_obj in args:
   if nQvFe(cpvcs_obj,Version):
    hash_refs.append(self.serializers["version"].store_obj(cpvcs_obj))
   elif nQvFe(cpvcs_obj,Revision):
    hash_refs.append(self.serializers["revision"].store_obj(cpvcs_obj))
  return hash_refs
 def _deserialize(self,key_serializer:Dict[nQvFD,nQvFD],remote_location:nQvFD=nQvFh)->List[Union[Revision,Version]]:
