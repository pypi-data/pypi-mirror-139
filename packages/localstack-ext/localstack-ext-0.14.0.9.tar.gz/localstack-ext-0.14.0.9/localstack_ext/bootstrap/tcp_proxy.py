...
class AttrDict(KfqIO):
 def __init__(self,*args,**kwargs):
  KfqIk(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and KfqIi(socket,"IPV6_V6ONLY"):
...
class Half(KfqIy):
 def __init__(self,opt,sock,addr,KfqIm):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.KfqIm=KfqIm
  self.name="peer" if self.KfqIm else "client"
  self.queue=[]
  self.dest=KfqID
  self.err=KfqID
  self.ready=KfqIE
...
def server_loop(opt):
 ...
 proxy=start_thread(lambda*args,**kwargs:start_tcp_proxy(listen,target,handler=KfqID,**kwargs))
 return proxy.join()
def main():
 opt={"ip6":KfqIE,"bind_port":2223,"addr":"192.168.43.191","port":22}
 opt={"ip6":KfqIE,"bind_port":48429,"addr":"128.0.100.3","port":22}
 opt={"ip6":KfqIE,"port":54237,"bind_addr":"128.0.100.2","bind_port":22}
 opt={"ip6":KfqIE,"port":54237,"bind_addr":"192.168.100.3","bind_port":22}
 server_loop(opt)
