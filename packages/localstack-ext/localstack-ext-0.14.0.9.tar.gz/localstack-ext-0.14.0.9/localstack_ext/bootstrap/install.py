...
def install_libs():
 ...
 postgres_installer.install(raise_on_error=BQAEl)
 timescaledb_installer.install(raise_on_error=BQAEl)
 redis_installer.install(raise_on_error=BQAEl)
 mosquitto_installer.install(raise_on_error=BQAEl)
...
def setup_ssl_cert():
 ...
 try:
  ...
  try:
   ...
   raise
def download_github_artifact(url:BQAEk,target_file:BQAEk,timeout:int=BQAEt):
 def do_download(url,print_error=BQAEl):
  try:
