...
class RequestHandler(BaseHTTPRequestHandler):
 ...
 def handle_request(self):
  ...
  result=json.dumps(result)if pFugH(result,pFugM)else result
  return result
 def read_content(self):
  if pFugt(self,"data_bytes"):
   return
  content_length=self.headers.get("Content-Length")
  self.data_bytes=self.rfile.read(pFugS(content_length))
  self.request_json={}
  try:
...
def s3_download(request):
 ...
 target_file=os.path.join(tmp_dir,request.get("file_name")or "s3file.%s"%pFugr(uuid.uuid4()))
...
def next_available_bind_address(port):
 start_id=pFugQ(USED_BIND_ADDRESSES)+2
 for idx in pFuge(start_id,start_id+30):
  host=LOCAL_BIND_ADDRESS_PATTERN.replace("*",pFugr(idx))
  ...
  return host
 raise pFugL(f"Unable to determine free bind address for port {port}")
def create_network_interface_alias(address,interface=pFugb):
 sudo_cmd="sudo"
 if is_mac_os():
 ...
 raise pFugL("Unable to create network interface")
def run_shell_cmd(command):
 try:
  return{"result":run_cmd(command)}
 except pFugL as e:
  error_string=pFugr(e)
  if pFugH(e,subprocess.CalledProcessError):
...
def run_cmd(cmd):
 ...
 return to_str(subprocess.check_output(cmd,stderr=subprocess.STDOUT,shell=pFugj))
def log(*args):
 print(*args)
 sys.stdout.flush()
def is_mac_os():
 try:
  out=to_str(subprocess.check_output("uname -a",shell=pFugj))
...
def get_free_tcp_port():
 ...
 return port
def to_bytes(obj):
 return obj.encode("utf-8")if pFugH(obj,pFugr)else obj
def to_str(obj):
 return obj.decode("utf-8")if pFugH(obj,pFugD)else obj
...
def main():
 logging.basicConfig()
 daemon_type=sys.argv[1]if pFugQ(sys.argv)>1 else "main"
...
def start_in_background():
 ...
 run(daemon_cmd,outfile=log_file,asynchronous=pFugj)
 ...
 run("sudo -v",stdin=pFugj)
 def start_root_daemon(*args,asynchronous=pFugb):
  sudo_cmd=f"sudo {daemon_cmd} root >> {log_file}"
  run(sudo_cmd,outfile=log_file,stdin=pFugj,asynchronous=asynchronous)
