...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[JwGuv])[0]
 def get_physical_resource_id(self,attribute=JwGuv,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if JwGur(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return JwGum(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @JwGul
 def add_defaults(resource,stack_name:JwGuC):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return JwGum(SESReceiptRule,self).get_cfn_attribute(attribute)
 @JwGuO
