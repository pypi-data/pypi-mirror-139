...
class RDSDBSubnetGroup(GenericBaseModel):
 ...
 def get_physical_resource_id(self,attribute=KqvCQ,**kwargs):
...
class RDSDBCluster(GenericBaseModel):
 ...
 @KqvCI
 def add_defaults(resource:Dict,stack_name:KqvCG):
 ...
 def get_cfn_attribute(self,attribute):
  ...
  return KqvCB(RDSDBCluster,self).get_cfn_attribute(attribute)
 @KqvCI
 def get_deploy_templates():
  ...
  def _params(params,**kwargs):
   ...
   if scaling_config:
    if "MinCapacity" in scaling_config:
     scaling_config["MinCapacity"]=KqvCb(scaling_config["MinCapacity"])
    if "MaxCapacity" in scaling_config:
     scaling_config["MaxCapacity"]=KqvCb(scaling_config["MaxCapacity"])
   return params
  result={"create":{"function":"create_db_cluster","parameters":_params,"types":{"BackupRetentionPeriod":KqvCb,"Port":KqvCb}},"delete":{"function":"delete_db_cluster","parameters":["DBClusterIdentifier"]}}
  return result
...
class RDSDBInstance(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  ...
  return KqvCB(RDSDBInstance,self).get_cfn_attribute(attribute)
 def fetch_state(self,stack_name,resources):
 ...
 def get_deploy_templates():
  ...
  return{"create":{"function":"create_db_instance","parameters":attrs+[{"VpcSecurityGroupIds":"VPCSecurityGroups"}],"types":{"AllocatedStorage":KqvCb,"Port":KqvCb}},"delete":{"function":"delete_db_instance","parameters":["DBInstanceIdentifier"]}}
