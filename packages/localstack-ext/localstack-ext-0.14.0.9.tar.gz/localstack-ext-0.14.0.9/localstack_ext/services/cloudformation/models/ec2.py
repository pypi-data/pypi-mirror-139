...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=RKobw,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  for perm in perms:
   if RKobl(perm["IpProtocol"])!=RKobl(rp["IpProtocol"]):
    continue
 ...
 @RKobA
 def get_vpc_and_name_for_security_group(cls,group_id:RKobl)->Tuple[RKobl,RKobl]:
  client=aws_stack.connect_to_service("ec2")
  groups=client.describe_security_groups(GroupIds=[group_id])["SecurityGroups"]
  vpc_id=groups and groups[0].get("VpcId")or RKobw
  group_name=groups and groups[0]["GroupName"]or RKobw
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",RKobw)
   vpc_id=RKobw
   ...
   dst_group_id=result.pop("DestinationSecurityGroupId",RKobw)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=RKobl(result.get("IpProtocol"))
   description=result.pop("Description",RKobw)
   cidr_ipv6=result.pop("CidrIpv6",RKobw)
