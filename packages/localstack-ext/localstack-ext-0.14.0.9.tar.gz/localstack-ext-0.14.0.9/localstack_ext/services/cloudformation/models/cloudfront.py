...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or yIEMU
 @yIEMs
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return yIEMw(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @yIEMb
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if yIEMk(origins,yIEMV)else origins.get("Items")
  origins_to_create=yIEMu([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if yIEMk(aliases,yIEMV)else aliases.get("Items")
  aliases_to_create=yIEMu(aliases)
  for distr in distrs:
   origins=yIEMu([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=yIEMu(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",yIEMW)
   config["Enabled"]=yIEMq(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if yIEMk(custom_errors,yIEMp)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=yIEMq(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":yIEMW}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=yIEMq(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=yIEMU):
    parent=config if parent is yIEMU else parent
    items=parent.get(key)or[]
    if yIEMk(items,yIEMp)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":yIEMj(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if yIEMk(cache_behavs,yIEMp)else cache_behavs)
