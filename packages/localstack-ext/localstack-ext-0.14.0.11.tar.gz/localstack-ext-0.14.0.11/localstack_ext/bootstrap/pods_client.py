...
class PodInfo:
 def __init__(self,name=OcQdE,pod_size=0):
  self.name=name
  self.pod_size=pod_size
  self.pod_size_compressed=0
  self.persisted_resource_names=[]
class CloudPodManager(OcQdg):
 BACKEND="_none_"
 def __init__(self,pod_name=OcQdE,config=OcQdE):
  self.pod_name=pod_name
  self._pod_config=config
 def init(self):
  raise OcQdJ("Not implemented")
 def delete(self,remote:OcQdu)->OcQdu:
  raise OcQdJ("Not implemented")
 def push(self,comment:OcQdA=OcQdE,three_way:OcQdu=OcQda)->PodInfo:
  raise OcQdJ("Not implemented")
 def push_overwrite(self,version:OcQds,comment:OcQdA=OcQdE):
  pass
 def pull(self,inject_version_state:OcQdu=OcQda,reset_state_before:OcQdu=OcQda,lazy:OcQdu=OcQda):
  raise OcQdJ("Not implemented")
 def commit(self,message:OcQdA=OcQdE):
  raise OcQdJ("Not implemented")
 def inject(self,version:OcQds,reset_state:OcQdu)->OcQdu:
  raise OcQdJ("Not implemented")
 def list_versions(self)->List[OcQdA]:
  raise OcQdJ("Not implemented")
 def version_info(self,version:OcQds):
  raise OcQdJ("Not implemented")
 def version_metamodel(self,version:OcQds)->Dict:
  raise OcQdJ("Not implemented")
 def set_version(self,version:OcQds,inject_version_state:OcQdu,reset_state:OcQdu,commit_before:OcQdu):
  raise OcQdJ("Not implemented")
 def list_version_commits(self,version:OcQds)->List[OcQdA]:
  raise OcQdJ("Not implemented")
 def get_commit_diff(self,version:OcQds,commit:OcQds)->Dict:
  raise OcQdJ("Not implemented")
 def register_remote(self,pod_name:OcQdA,ci_pod:OcQdu)->OcQdu:
  raise OcQdJ("Not implemented")
 def rename_pod(self,current_pod_name,new_pod_name)->OcQdu:
  raise OcQdJ("Not implemented")
 def list_pods(self,fetch_remote:OcQdu)->Set[OcQdA]:
  raise OcQdJ("Not implemented")
 ...
 def get(cls,pod_name,pre_config=OcQdE):
  ...
  raise OcQdJ('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 def deploy_pod_into_instance(self,pod_path):
  delete_pod_zip=OcQda
  if os.path.isdir(pod_path):
   ...
   for folder in PERSISTED_FOLDERS:
    ...
    cp_r(src_folder,tgt_folder,rm_dest_on_conflict=OcQdG)
   pod_path=create_zip_file(tmpdir)
   rm_rf(tmpdir)
   delete_pod_zip=OcQdG
  zip_content=load_file(pod_path,mode="rb")
  url=get_pods_endpoint()
  result=requests.post(url,data=zip_content)
  if result.status_code>=400:
   raise OcQdJ("Unable to restore pod state via local pods management API %s (code %s): %s"%(url,result.status_code,result.content))
  if delete_pod_zip:
   rm_rf(pod_path)
  else:
   return pod_path
 @OcQdT
 def get_state_zip_from_instance(get_content=OcQda):
  url=f"{get_pods_endpoint()}/state"
  result=requests.get(url)
  if result.status_code>=400:
   raise OcQdJ("Unable to get local pod state via management API %s (code %s): %s"%(url,result.status_code,result.content))
  if get_content:
   return result.content
  zip_file=f"{new_tmp_file()}.zip"
  save_file(zip_file,result.content)
  return zip_file
 def get_pod_info(self,pod_data_dir:OcQdA=OcQdE):
...
class CloudPodManagerCPVCS(CloudPodManager):
 BACKEND="cpvcs"
 @OcQdT
 def parse_pod_name_from_qualifying_name(qualifying_name:OcQdA)->OcQdA:
 ...
 @OcQdT
 def _get_max_version_for_pod_from_platform(pod_name:OcQdA,auth_headers):
  ...
  remote_max_ver=OcQds(content["max_ver"])
 ...
 def _add_state_files_func(**kwargs):
  ...
  cpvcs_api.create_state_file_from_fs(path=dir_name,file_name=file_name,service=service_name,region=region,account_id=account_id,root=API_STATES_DIR,serialization=Serialization.MAIN)
 def _upload_version_and_product_space(self,presigned_urls):
  ...
  with OcQdt(version_space_archive,"rb")as version_space_content:
  ...
  for version_no,urls in presigned_meta_state_urls.items():
   ...
   with OcQdt(meta_archive,"rb")as meta_archive_content:
   ...
   with OcQdt(state_archive,"rb")as state_archive_content:
    self.upload_content(state_presigned_url,state_archive_content)
 @OcQdT
 def _add_state_files_from_directory(service:OcQdA,path:OcQdA,root_dir:OcQdA,serialization:Serialization):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def _add_state_to_cpvcs_store(self):
  ...
  with ZipFile(zip_file,"r")as state_zip:
   ...
   api_states_traverse(api_states_path=api_states_path,side_effect=CloudPodManagerCPVCS._add_state_files_func,mutables=OcQdE)
  ...
  rm_rf(zip_file)
  rm_rf(tmp_dir)
 def _pull_versions(self,auth_headers,required_versions:OcQdA):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def _clone_pod(self,auth_headers,lazy:OcQdu=OcQda):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def delete(self,remote:OcQdu)->OcQdu:
  ...
  if remote:
   pass
  return OcQda
 def _push_to_remote(self,url:OcQdA):
 ...
 def pull(self,inject_version_state:OcQdu=OcQda,reset_state_before:OcQdu=OcQda,lazy:OcQdu=OcQda):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
  auth_headers=get_auth_headers()
  if self.pod_name in cpvcs_api.list_locally_available_pods(show_remote_or_local=OcQda):
   cpvcs_api.set_pod_context(self.pod_name)
   ...
   if remote_max_ver==current_max_ver:
    ...
    return
   if not lazy:
    required_versions=",".join(OcQdX(lambda ver:OcQdA(ver),OcQdV(current_max_ver+1,remote_max_ver+1)))
   else:
 ...
 def _download_version_product(self,version:OcQds,retain:OcQdu=OcQda)->Dict[OcQdA,OcQdA]:
 ...
 def _get_presigned_url_for_version_product(self,version:OcQds)->Dict:
 ...
 def _inject_from_remote(self,version:OcQds,retain:OcQdu=OcQda)->OcQdu:
  from localstack_ext.bootstrap.cpvcs.utils.remote_utils import(extract_meta_and_state_archives)
  content=self._get_presigned_url_for_version_product(version=version)
  state_url=content.get("presigned_url_state")
  tmp_state_archive=new_tmp_file()
 ...
 def inject(self,version:OcQds,reset_state:OcQdu)->OcQdu:
  ...
  if not cpvcs_api.config_context.pod_exists_locally(self.pod_name):
   ...
   if not state_archive_path and cpvcs_api.is_remotely_managed():
    ...
    product_archive_path=self._download_version_product(version=version,retain=OcQdG)
    if not product_archive_path:
     return OcQda
    state_archive_path=cpvcs_api.PodFilePaths.get_version_state_archive(version)
  if reset_state:
   reset_local_state(reset_data_dir=OcQdG)
 ...
 def version_metamodel(self,version:OcQds)->Dict:
  ...
  if not result and cpvcs_api.is_remotely_managed():
   self._download_version_product(version=version,retain=OcQdG)
 ...
 def get_commit_diff(self,version:OcQds,commit:OcQds)->Dict:
  ...
  return commit_diff
 def register_remote(self,pod_name:OcQdA,ci_pod:OcQdu=OcQda)->OcQdu:
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
  cpvcs_api.set_pod_context(self.pod_name)
 ...
 def list_pods(self,fetch_remote:OcQdu)->Set[OcQdA]:
  ...
  return result
 @OcQdU
 def upload_content(cls,presigned_url:OcQdA,zip_data_content):
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise OcQdJ("Unable to upload pod state to S3 (code %s): %s"%(res.status_code,res.content))
  return res
 @OcQdT
 def create_platform_url(request:OcQdA)->OcQdA:
...
class CloudPodManagerManaged(CloudPodManager):
 BACKEND="managed"
 def push(self,comment:OcQdA=OcQdE,three_way:OcQdu=OcQda)->PodInfo:
  zip_data_content=self.get_state_zip_from_instance(get_content=OcQdG)
  print('Pushing state of cloud pod "%s" to backend server (%s KB)'%(self.pod_name,format_number(OcQdY(zip_data_content)/1000.0)))
  self.push_content(self.pod_name,zip_data_content)
  print("Done.")
  result=self.get_pod_info()
  result.pod_size_compressed=OcQdY(zip_data_content)
  return result
 def pull(self,inject_version_state:OcQdu=OcQda,reset_state_before:OcQdu=OcQda,lazy:OcQdu=OcQda):
  ...
  rm_rf(zip_path)
 @OcQdT
 def presigned_url(pod_name:OcQdA,mode:OcQdA)->OcQdA:
  ...
  if response.status_code>=400:
   raise OcQdJ("Unable to get cloud pod presigned URL (code %s): %s"%(response.status_code,content))
 ...
 def push_content(cls,pod_name,zip_data_content):
  ...
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise OcQdJ("Unable to push pod state to API (code %s): %s"%(res.status_code,res.content))
  return res
class CloudPodManagerGit(CloudPodManager):
 BACKEND="git"
 def push(self,comment:OcQdA=OcQdE,three_way:OcQdu=OcQda):
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=OcQdG)
 ...
 def pull(self,inject_version_state:OcQdu=OcQda,reset_state_before:OcQdu=OcQda,lazy:OcQdu=OcQda):
  ...
  if remote_ref not in remote_refs:
   raise OcQdJ('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get("url")
  self.switch_branch(branch)
 ...
 def client(self):
  ...
  return client,path
 def local_repo(self):
  self.clone_dir=OcQdP(self,"clone_dir",OcQdE)
...
class PodConfigManagerMeta(OcQdl):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=OcQdE
   for manager in cls.CHAIN:
    try:
     tmp=OcQdP(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif OcQdq(tmp,OcQdj)and OcQdq(result,OcQdj):
       result.extend(tmp)
    except OcQdJ:
     if LOG.isEnabledFor(logging.DEBUG):
      LOG.exception("error during PodConfigManager call chain")
   if result is not OcQdE:
    return result
   raise OcQdJ('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(OcQdg,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @OcQdU
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise OcQdJ('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(OcQdg):
 CONFIG_FILE=".localstack.yml"
 def list_pods(self):
  local_pods=self._load_config(safe=OcQdG).get("pods",{})
  local_pods=[{"pod_name":k,"state":"Local Only",**v}for k,v in local_pods.items()]
  existing_names=OcQdv([pod["pod_name"]for pod in local_pods])
  result=[pod for pod in local_pods if pod["pod_name"]not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=OcQda):
  try:
   return yaml.safe_load(to_str(load_file(self.CONFIG_FILE)))
  except OcQdJ:
   if safe:
    return{}
   raise OcQdJ('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(OcQdg):
 def list_pods(self):
  result=[]
  auth_headers=get_auth_headers()
  ...
  content=response.content
  if response.status_code>=400:
   raise OcQdJ("Unable to fetch list of pods from API (code %s): %s"%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise OcQdJ("Unable to store pod metadata in API (code %s): %s"%(response.status_code,content))
...
def init_cpvcs(pod_name:OcQdA,pre_config:Dict[OcQdA,OcQdA],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.init()
def delete_pod(pod_name:OcQdA,remote:OcQdu,pre_config:Dict[OcQdA,OcQdA])->OcQdu:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.delete(remote=remote)
 return result
def register_remote(pod_name:OcQdA,pre_config:Dict[OcQdA,OcQdA],**kwargs)->OcQdu:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.register_remote(pod_name=pod_name,ci_pod=pre_config.get("ci_pod",OcQda))
 return result
def rename_pod(current_pod_name:OcQdA,new_pod_name:OcQdA,pre_config:Dict[OcQdA,OcQdA],**kwargs):
 ...
 return result
def list_pods_cpvcs(remote:OcQdu,pre_config:Dict[OcQdA,OcQdA],**kwargs)->List[OcQdA]:
 backend=CloudPodManager.get(pod_name="",pre_config=pre_config)
 result=backend.list_pods(fetch_remote=remote)
 return result
def commit_state(pod_name:OcQdA,pre_config:Dict[OcQdA,OcQdA],message:OcQdA=OcQdE,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.commit(message=message)
def inject_state(pod_name:OcQdA,version:OcQds,reset_state:OcQdu,pre_config:Dict[OcQdA,OcQdA],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.inject(version=version,reset_state=reset_state)
 return result
def list_versions(pod_name:OcQdA,pre_config:Dict[OcQdA,OcQdA],**kwargs)->List[OcQdA]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 versions=backend.list_versions()
 return versions
def get_version_info(version:OcQds,pod_name:OcQdA,pre_config:Dict[OcQdA,OcQdA],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 info=backend.version_info(version=version)
 return info
def get_version_metamodel(version:OcQds,pod_name:OcQdA,pre_config:Dict[OcQdA,OcQdA],**kwargs)->Dict:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 metamodel=backend.version_metamodel(version=version)
 return metamodel
def set_version(version:OcQds,inject_version_state:OcQdu,reset_state:OcQdu,commit_before:OcQdu,pod_name:OcQdA,pre_config:Dict[OcQdA,OcQdA],**kwargs)->OcQdu:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 ...
 return success
def list_version_commits(version:OcQds,pod_name:OcQdA,pre_config:Dict[OcQdA,OcQdA])->List[OcQdA]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commits=backend.list_version_commits(version=version)
 return commits
def get_commit_diff(version:OcQds,commit:OcQds,pod_name:OcQdA,pre_config:Dict[OcQdA,OcQdA])->Dict:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commit_diff=backend.get_commit_diff(version=version,commit=commit)
 return commit_diff
def push_overwrite(version:OcQds,pod_name:OcQdA,comment:OcQdA,pre_config:Dict[OcQdA,OcQdA]):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.push_overwrite(version=version,comment=comment)
def push_state(pod_name,pre_config=OcQdE,squash_commits=OcQda,comment=OcQdE,three_way=OcQda,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
...
def pull_state(pod_name,inject_version_state=OcQda,reset_state_before=OcQda,lazy=OcQda,**kwargs):
 pre_config=kwargs.get("pre_config",OcQdE)
 if not pod_name:
  raise OcQdJ("Need to specify a pod name")
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.pull(inject_version_state=inject_version_state,reset_state_before=reset_state_before,lazy=lazy)
 print("Done.")
def reset_local_state(reset_data_dir=OcQda,exclude_from_reset:List[OcQdA]=OcQdE):
 url=f"{get_pods_endpoint()}/state"
 if reset_data_dir:
  url+="/datadir"
 ...
 if result.status_code>=400:
  raise OcQdJ("Unable to reset service state via local management API %s (code %s): %s"%(url,result.status_code,result.content))
 print("Done.")
def list_pods(args):
 return PodConfigManager.list_pods()
def get_data_dir_from_container()->OcQdA:
 try:
...
def get_persisted_resource_names(data_dir)->List[OcQdA]:
 names=[]
 ...
 with os.scandir(os.path.join(data_dir,"api_states"))as entries:
  for entry in entries:
   if entry.is_dir()and OcQdY(os.listdir(entry.path))>0:
    names.append(entry.name)
