...
CAPTURE_METAMODEL_SEPARATELY=UQxAh
...
class MetamodelDelta(SubtypesInstanceManager):
 def create_delta_for_state_files(self,sf1:StateFileRef,sf2:StateFileRef):
  ...
  if not sf1:
   backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
   clazz=UQxAg(backend2)
   backend1=_infer_backend_init(clazz,sf2)
  else:
   backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
   clazz=UQxAg(backend1)
   backend2=_infer_backend_init(clazz,sf1)
  diff=self.create_delta_json(backend1,backend2)
  return diff
 def create_delta_log(self,state_from:Set[StateFileRef],state_to:Set[StateFileRef])->UQxAu:
  ...
  def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[UQxAu,StateFileRef]:
  ...
  for state_file_from in state_files_from_regular:
   ...
   if state_file_from.any_congruence(state_files_to_regular):
    ...
    diff_json=self.create_delta_for_state_files(state_file_from,UQxAY)
  ...
  for state_files_to in state_files_to_lookup.values():
   ...
   diff_json=self.create_delta_for_state_files(UQxAY,state_files_to)
...
class CommitMetamodelUtils:
 @UQxAI
 def create_metadata_archive(cls,version:Version,delete_reference:bool=UQxAh,overwrite:bool=UQxAh,metamodels_file:UQxAu=UQxAY):
 ...
 def get_metamodel_delta(cls,prev_metamodel:Dict,this_metamodel:Dict)->Dict:
  ...
  return result
 @UQxAI
 def create_delta_metamodel_file(cls,version:UQxAB,revision:UQxAB,store_to_zip=UQxAh)->Tuple[Dict,Dict]:
 ...
 @UQxAI
 def reconstruct_metamodel(cls,version:UQxAB,revision:UQxAB=UQxAY)->Dict:
  if not revision:
   revision=infer_max_commit_diff(archive_path=PodFilePaths.get_version_meta_archive(version))
  result={}
  for rev in UQxAW(1,revision+1):
  ...
  return result
 @UQxAI
 def get_version_metamodel(cls,version:UQxAB,revision:UQxAB=UQxAY)->Dict:
 ...
 @UQxAI
 def get_commit_diff(cls,version_no:UQxAB,commit_no:UQxAB)->Optional[Dict]:
  ...
  return result
def _infer_backend_init(clazz:Type,sf:StateFileRef)->UQxAE:
 if UQxAo(clazz,UQxAv):
  return{}
 constructor=UQxAm(clazz,"__init__",UQxAY)
...
def _create_metamodel_helper(obj,width=25,visited:set=UQxAY):
 if obj is UQxAY:
  return obj
 ...
 result=obj=obj_dict if obj_dict is not UQxAY else obj
 if UQxAo(obj,UQxAv):
  result=UQxAv(result)
  for field_name,field_value in result.items():
   result[field_name]=_create_metamodel_helper(field_value,width=width,visited=visited)
 elif UQxAo(obj,UQxAz):
  result=[_create_metamodel_helper(o,width=width,visited=visited)for o in obj]
  if UQxAX(result)>width:
   result={"size":UQxAX(result),"items":result[:width]}
 return result
def infer_max_commit_diff(archive_path:UQxAu)->UQxAB:
 max_commit=1
 with zipfile.ZipFile(archive_path)as archive:
