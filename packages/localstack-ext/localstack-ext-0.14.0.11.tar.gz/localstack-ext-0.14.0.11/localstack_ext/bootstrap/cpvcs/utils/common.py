...
class CPVCSConfigContext:
 ...
 def get_version_state_archive_path(self,version,with_format:dwPjg=dwPjC)->dwPjX:
 ...
 def update_ver_log(self,author:dwPjX,ver_no:dwPjq,rev_id:dwPjX,rev_no:dwPjq):
  with dwPjL(self.get_ver_log_path(),"a")as fp:
   fp.write(f"{VER_LOG_STRUCTURE.format(author=author, ver_no=ver_no, rev_rid_no=f'{rev_id}_{rev_no}')}\n")
 def create_version_symlink(self,name:dwPjX,key:dwPjX=dwPjJ)->dwPjX:
  return self._create_symlink(name,key,self.get_ver_refs_path())
 def create_revision_symlink(self,name:dwPjX,key:dwPjX=dwPjJ)->dwPjX:
  return self._create_symlink(name,key,self.get_rev_refs_path())
 def is_initialized(self)->dwPjg:
  return self.pod_root_dir and os.path.isdir(self.pod_root_dir)
 def _create_symlink(self,name:dwPjX,key:dwPjX,path:dwPjX)->dwPjX:
...
def zip_directories(zip_dest:dwPjX,directories:List[dwPjX])->dwPjX:
 ...
 return zip_dest
def add_file_to_archive(archive:dwPjX,entry_name:dwPjX,content:Union[dwPjX,dwPjv]):
 with zipfile.ZipFile(archive,"a")as zip_file:
  zip_file.writestr(entry_name,content)
def read_file_from_archive(archive_path:dwPjX,file_name:dwPjX)->dwPjX:
 try:
