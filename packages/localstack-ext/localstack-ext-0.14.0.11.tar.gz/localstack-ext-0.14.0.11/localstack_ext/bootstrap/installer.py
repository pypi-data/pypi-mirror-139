  ...
  try:
   if not self._check_if_available():
    try:
     ...
     if CKopQ(e,PackageInstallationException):
      raise
     else:
      suffix=(f" ({self.log_package_name})." if self.log_package_name is not CKopg else ".")
   ...
   if raise_on_error:
    ...
    if self.logger.isEnabledFor(logging.DEBUG):
     ...
     self.logger.error(e)
 def _check_if_available(self)->CKopH:
  raise CKoph
 def _prepare_installation(self)->CKopg:
  pass
 def _install_package(self)->CKopg:
  raise CKoph
 def _post_process(self)->CKopg:
  pass
 def _verify_installation(self)->CKopg:
  if not self._check_if_available():
   package_info=(f" of {self.log_package_name}" if self.log_package_name is not CKopg else "")
...
class MultiPackageInstaller(PackageInstaller):
 def __init__(self,log_package_name:Optional[CKopu],package_installers:Union[PackageInstaller,List[PackageInstaller]]):
  CKopU(MultiPackageInstaller,self).__init__(log_package_name=log_package_name)
  self.package_installers=(package_installers if CKopQ(package_installers,List)else[package_installers])
 def install(self,raise_on_error:CKopH=CKopT):
...
class OSSpecificPackageInstaller(MultiPackageInstaller):
 def __init__(self,debian_installers:PackageInstaller,redhat_installers:PackageInstaller):
  ...
  CKopU(OSSpecificPackageInstaller,self).__init__(log_package_name=self.__class__.__name__,package_installers=package_installers)
 def install(self,raise_on_error:CKopH=CKopT):
  try:
   if not in_docker():
    ...
    CKopU(OSSpecificPackageInstaller,self).install(raise_on_error=raise_on_error)
  except PackageInstallationException as e:
   if raise_on_error:
    raise
   else:
...
class DebianPackageInstaller(PackageInstaller,ABC):
 def __init__(self,package_name:CKopu):
  CKopU(DebianPackageInstaller,self).__init__(log_package_name=package_name)
 def _install_os_packages(self,packages:Union[CKopu,List[CKopu]]):
  packages=packages if CKopQ(packages,List)else[packages]
  with INSTALL_LOCK:
   run(["apt-get","update"])
   run(["apt-get","install","-y","--no-install-recommends"]+packages)
class RedHatPackageInstaller(PackageInstaller,ABC):
 def __init__(self,package_name:CKopu):
  CKopU(RedHatPackageInstaller,self).__init__(log_package_name=package_name)
 def _install_os_packages(self,packages:Union[CKopu,List[CKopu]]):
  packages=packages if CKopQ(packages,List)else[packages]
  with INSTALL_LOCK:
...
class DebianPostgres11Installer(DebianPackageInstaller):
 def __init__(self):
  CKopU(DebianPostgres11Installer,self).__init__("postgres11")
 def _check_if_available(self)->CKopH:
...
class RedHatPostgres11Installer(RedHatPackageInstaller):
 def __init__(self):
  CKopU(RedHatPostgres11Installer,self).__init__("postgres11")
 def _check_if_available(self)->CKopH:
  return is_command_available("psql")
 def _prepare_installation(self)->CKopg:
...
class DebianPlPythonInstaller(DebianPackageInstaller):
 def __init__(self):
  CKopU(DebianPlPythonInstaller,self).__init__("plpython3")
 def _check_if_available(self)->CKopH:
...
class RedHatPlPythonInstaller(RedHatPackageInstaller):
 def __init__(self):
  CKopU(RedHatPlPythonInstaller,self).__init__("plpython3")
 def _check_if_available(self)->CKopH:
...
class DebianMariaDBInstaller(DebianPackageInstaller):
 def __init__(self):
  CKopU(DebianMariaDBInstaller,self).__init__("MariaDB")
 def _check_if_available(self)->CKopH:
...
class RedHatMariaDBInstaller(RedHatPackageInstaller):
 def __init__(self):
  CKopU(RedHatMariaDBInstaller,self).__init__("MariaDB")
 def _check_if_available(self)->CKopH:
  return is_command_available("mysqld")
 def _install_package(self)->CKopg:
...
class DebianTimescaleDBInstaller(DebianPackageInstaller):
 def __init__(self):
  CKopU(DebianTimescaleDBInstaller,self).__init__("timescaledb")
 def _check_if_available(self)->CKopH:
...
class RedHatTimescaleDBInstaller(RedHatPackageInstaller):
 def __init__(self):
  CKopU(RedHatTimescaleDBInstaller,self).__init__("timescaledb")
 def _check_if_available(self)->CKopH:
...
class DebianRedisInstaller(DebianPackageInstaller):
 def __init__(self):
  CKopU(DebianRedisInstaller,self).__init__("Redis")
 def _check_if_available(self)->CKopH:
...
class RedHatRedisInstaller(RedHatPackageInstaller):
 def __init__(self):
  CKopU(RedHatRedisInstaller,self).__init__("Redis")
 def _check_if_available(self)->CKopH:
  return is_command_available("redis-server")
 def _install_package(self)->CKopg:
...
class DebianMosquittoInstaller(DebianPackageInstaller):
 def __init__(self):
  CKopU(DebianMosquittoInstaller,self).__init__("Mosquitto")
 def _check_if_available(self)->CKopH:
...
class RedHatMosquittoInstaller(RedHatPackageInstaller):
 def __init__(self):
  CKopU(RedHatMosquittoInstaller,self).__init__("Mosquitto")
 def _check_if_available(self)->CKopH:
  return is_command_available("mosquitto")
 def _install_package(self)->CKopg:
