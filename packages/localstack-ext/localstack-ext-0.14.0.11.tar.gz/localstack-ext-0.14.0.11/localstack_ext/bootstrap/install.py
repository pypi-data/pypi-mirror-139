...
def install_libs():
 ...
 postgres_installer.install(raise_on_error=WQBzy)
 timescaledb_installer.install(raise_on_error=WQBzy)
 redis_installer.install(raise_on_error=WQBzy)
 mosquitto_installer.install(raise_on_error=WQBzy)
...
def install_stepfunctions():
 if not os.path.exists(INSTALL_PATH_STEPFUNCTIONS_JAR):
  LOG.warning("StepFunctions JAR not available - skipping installation")
  return
...
def setup_ssl_cert():
 ...
 try:
  ...
  try:
   ...
   raise
def download_github_artifact(url:WQBzg,target_file:WQBzg,timeout:int=WQBzh):
 def do_download(url,print_error=WQBzy):
  try:
