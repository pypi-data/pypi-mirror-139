...
class DecryptionHandler:
 decryption_key:pvgYL
 def __init__(self,decryption_key:pvgYL):
  self.decryption_key=decryption_key
 def decrypt(self,content)->pvgYL:
...
class EncryptedFileFinder(MetaPathFinder):
 ...
 def find_spec(self,fullname,path,target=pvgYl):
  if path and not pvgYS(path,pvgYB):
   path=pvgYB(pvgYO(path,"_path",[]))
  if not path:
   return pvgYl
...
def patch_traceback_lines():
 if pvgYO(traceback.FrameSummary,"_ls_patch_applied",pvgYl):
  return
 @pvgYd
 def line(self):
  try:
 ...
 pvgYn(traceback.FrameSummary,"line",line)
 traceback.FrameSummary._ls_patch_applied=pvgYs
def patch_inspect_findsource():
 if pvgYO(inspect,"_ls_patch_applied",pvgYl):
  return
 ...
 inspect._ls_patch_applied=pvgYs
