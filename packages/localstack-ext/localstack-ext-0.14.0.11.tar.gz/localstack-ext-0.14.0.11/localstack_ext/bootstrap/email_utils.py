...
SENT_EMAILS=[]
EMAIL_BLACKLIST=cuGjP()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",cuGja(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=cuGjT,from_name=cuGjT,smtp_host=cuGjT,smtp_user=cuGjT,smtp_pass=cuGjT,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if cuGji(recipients,cuGjr)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=cuGjT):
 ...
 result["From"]=formataddr((cuGja(Header(from_name,"utf-8")),from_email))
 return result
