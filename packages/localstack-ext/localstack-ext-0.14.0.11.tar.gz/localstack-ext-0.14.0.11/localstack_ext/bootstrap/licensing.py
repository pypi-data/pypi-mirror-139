...
class KeyActivationError(RQgfj):
 pass
...
class InvalidDecryptionKeyError(KeyActivationError):
 pass
def is_enterprise():
 return read_api_key(raise_if_missing=RQgfs)=="enterprise"
def read_api_key(raise_if_missing=RQgfE):
 key=(os.environ.get(ENV_LOCALSTACK_API_KEY)or "").strip()
 if not key and raise_if_missing:
  raise RQgfj("Unable to retrieve API key. Please configure $%s in your environment"%ENV_LOCALSTACK_API_KEY)
 return key
def truncate_api_key(api_key:RQgft):
 return '"%s..."(%s)'%(api_key[:3],RQgfl(api_key))
def fetch_key()->RQgfF:
 ...
 if api_key=="test":
  ...
  key_base64=load_cached_key(api_key=api_key,check_timeout=RQgfs)
  decoded_key=base64.b64decode(key_base64)
  return decoded_key
 proxies=get_proxies()or RQgfK
 data={"api_key":api_key,"version":__version__}
 try:
  logging.getLogger("py.warnings").setLevel(logging.ERROR)
  result=requests.post("%s/activate"%config.API_URL,json.dumps(data),verify=RQgfs,proxies=proxies)
  ...
  if log_license_issues():
   api_key=RQgft(api_key_configured()or "")
...
def cache_key_locally(api_key,key_b64):
 timestamp=RQgft(RQgfS(now_utc()))
 key_raw=to_str(base64.b64decode(key_b64))
 for i in RQgfh(RQgfl(timestamp)):
  key_raw=str_insert(key_raw,i*2,timestamp[i])
 key_b64=to_str(base64.b64encode(to_bytes(key_raw)))
 cache=get_key_cache()
 cache.update({"timestamp":RQgfS(timestamp),"key_hash":md5(api_key),"key":key_b64})
 cache.save()
def load_cached_key(api_key:RQgft,check_timeout:bool=RQgfE)->RQgft:
 cache=get_key_cache()
 ...
 timestamp=RQgft(cache["timestamp"])
 ...
 key_b64=to_str(base64.b64encode(to_bytes(key_raw)))
 return key_b64
def enable_file_decryption(key:RQgfF):
 ...
 try:
  ...
  if "import" not in to_str(file_content):
   raise RQgfY("Decryption resulted in invalid python file!")
 except RQgfj:
...
def prepare_environment():
 ...
 if not ENV_PREPARED.get("finalized"):
  try:
   key=fetch_key()
   if not key:
    raise RQgfj("Unable to fetch and validate API key from environment")
   if to_str(key)!="test":
...
def api_key_configured():
 return read_api_key(raise_if_missing=RQgfs)
def is_logged_in():
 return RQgfE if get_auth_cache().get("token")else RQgfs
def get_auth_headers()->Dict:
 ...
 api_key=read_api_key(raise_if_missing=RQgfs)
 if api_key:
  return{"ls-api-key":api_key,"ls-version":__version__}
 raise RQgfj("Please log in first")
