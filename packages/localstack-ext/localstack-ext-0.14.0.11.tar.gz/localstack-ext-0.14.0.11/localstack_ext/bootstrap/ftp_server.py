...
def apply_patches():
 ...
 extended_proto_cmds.update({"SITE ADDUSER":xnDiT(perm="m",auth=xnDiR,arg=xnDiR,help="Syntax: SITE <SP> ADDUSER USERNAME PASSWORD HOME PRIVS <SP>.")})
...
def start_ftp(port):
 if USE_SUBPROCESS:
  ...
  thread=ShellCommandThread(cmd,outfile=subprocess.PIPE,env_vars=env_vars,quiet=xnDiX)
  thread.start()
  time.sleep(2)
 else:
  thread=do_start_ftp(port,asynchronous=xnDiR)
 time.sleep(2)
 wait_for_port_open(port,retries=10,sleep_time=1.5)
 TMP_THREADS.append(thread)
 return thread
def do_start_ftp(port,asynchronous=xnDiR):
...
def get_directory_mapping_file():
 return DIRECTORY_MAPPING_FILE.replace("<data_dir>",localstack_config.dirs.tmp)
def main():
 do_start_ftp(xnDib(sys.argv[1]),asynchronous=xnDiX)
