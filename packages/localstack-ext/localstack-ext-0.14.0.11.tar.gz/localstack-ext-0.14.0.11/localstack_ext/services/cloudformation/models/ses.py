...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[LIEJG])[0]
 def get_physical_resource_id(self,attribute=LIEJG,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if LIEJh(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return LIEJe(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @LIEJm
 def add_defaults(resource,stack_name:LIEJY):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return LIEJe(SESReceiptRule,self).get_cfn_attribute(attribute)
 @LIEJB
