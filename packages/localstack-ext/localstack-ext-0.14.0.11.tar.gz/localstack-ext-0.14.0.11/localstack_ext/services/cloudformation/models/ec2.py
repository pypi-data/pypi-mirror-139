...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=XNLVg,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  for perm in perms:
   if XNLVY(perm["IpProtocol"])!=XNLVY(rp["IpProtocol"]):
    continue
 ...
 @XNLVj
 def get_vpc_and_name_for_security_group(cls,group_id:XNLVY)->Tuple[XNLVY,XNLVY]:
  client=aws_stack.connect_to_service("ec2")
  groups=client.describe_security_groups(GroupIds=[group_id])["SecurityGroups"]
  vpc_id=groups and groups[0].get("VpcId")or XNLVg
  group_name=groups and groups[0]["GroupName"]or XNLVg
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",XNLVg)
   vpc_id=XNLVg
   ...
   dst_group_id=result.pop("DestinationSecurityGroupId",XNLVg)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=XNLVY(result.get("IpProtocol"))
   description=result.pop("Description",XNLVg)
   cidr_ipv6=result.pop("CidrIpv6",XNLVg)
