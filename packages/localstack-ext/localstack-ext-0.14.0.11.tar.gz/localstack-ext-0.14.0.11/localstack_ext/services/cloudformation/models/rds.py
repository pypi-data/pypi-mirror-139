...
class RDSDBSubnetGroup(GenericBaseModel):
 ...
 def get_physical_resource_id(self,attribute=CeMdn,**kwargs):
...
class RDSDBCluster(GenericBaseModel):
 ...
 @CeMdb
 def add_defaults(resource:Dict,stack_name:CeMdH):
 ...
 def get_cfn_attribute(self,attribute):
  ...
  return CeMdi(RDSDBCluster,self).get_cfn_attribute(attribute)
 @CeMdb
 def get_deploy_templates():
  ...
  def _params(params,**kwargs):
   ...
   if scaling_config:
    if "MinCapacity" in scaling_config:
     scaling_config["MinCapacity"]=CeMdp(scaling_config["MinCapacity"])
    if "MaxCapacity" in scaling_config:
     scaling_config["MaxCapacity"]=CeMdp(scaling_config["MaxCapacity"])
   return params
  result={"create":{"function":"create_db_cluster","parameters":_params,"types":{"BackupRetentionPeriod":CeMdp,"Port":CeMdp}},"delete":{"function":"delete_db_cluster","parameters":["DBClusterIdentifier"]}}
  return result
...
class RDSDBInstance(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  ...
  return CeMdi(RDSDBInstance,self).get_cfn_attribute(attribute)
 def fetch_state(self,stack_name,resources):
 ...
 def get_deploy_templates():
  ...
  return{"create":{"function":"create_db_instance","parameters":attrs+[{"VpcSecurityGroupIds":"VPCSecurityGroups"}],"types":{"AllocatedStorage":CeMdp,"Port":CeMdp}},"delete":{"function":"delete_db_instance","parameters":["DBInstanceIdentifier"]}}
