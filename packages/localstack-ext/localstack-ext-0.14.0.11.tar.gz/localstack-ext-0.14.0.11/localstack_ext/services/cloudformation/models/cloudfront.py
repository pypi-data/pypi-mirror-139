...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or hSoaC
 @hSoav
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return hSoaY(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @hSoal
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if hSoaK(origins,hSoaI)else origins.get("Items")
  origins_to_create=hSoaL([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if hSoaK(aliases,hSoaI)else aliases.get("Items")
  aliases_to_create=hSoaL(aliases)
  for distr in distrs:
   origins=hSoaL([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=hSoaL(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",hSoaP)
   config["Enabled"]=hSoau(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if hSoaK(custom_errors,hSoaJ)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=hSoau(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":hSoaP}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=hSoau(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=hSoaC):
    parent=config if parent is hSoaC else parent
    items=parent.get(key)or[]
    if hSoaK(items,hSoaJ)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":hSoag(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if hSoaK(cache_behavs,hSoaJ)else cache_behavs)
