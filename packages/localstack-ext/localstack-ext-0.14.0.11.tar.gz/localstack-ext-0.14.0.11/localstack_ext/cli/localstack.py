...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return klXJB if e else klXJR
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->klXJa:
...
@click.option("--revert",is_flag=klXJB,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:klXJU):
...
def _cpvcs_initialized(pod_name:klXJg)->klXJU:
...
@click.option("-n","--name",help="Name of the cloud pod",required=klXJB)
def cmd_cpvcs_init(name:klXJg):
...
@click.option("-n","--name",help="Name of the cloud pod",required=klXJB)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=klXJB,default=klXJR)
def cmd_cpvcs_delete(name:klXJg,remote:klXJU):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=klXJB)
def cmd_cpvcs_register_remote(name:klXJg):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=klXJB)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=klXJB)
def cmd_cpvcs_rename(name:klXJg,new_name:klXJg):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=klXJB)
def cmd_cpvcs_commit(message:klXJg,name:klXJg):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=klXJB,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=klXJB,default=klXJR,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=klXJB)
def cmd_cpvcs_push(squash:klXJU,message:klXJg,name:klXJg,three_way:klXJU):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=klXJB)
@click.option("-v","--version",type=klXJA)
@click.option("-m","--message",required=klXJR)
def cmd_cpvcs_push_overwrite(version:klXJA,message:klXJg,name:klXJg):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=klXJA,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=klXJB,default=klXJR,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=klXJB)
def cmd_cpvcs_inject(version:klXJA,reset:klXJU,name:klXJg):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--inject/--no-inject",default=klXJB,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=klXJB,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=klXJB)
@click.option("--lazy/--eager",default=klXJB,help="Will only fetch references to existing versions, i.e. version state is only downloaded when required")
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:klXJg,inject:klXJU,reset:klXJU,lazy:klXJU):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=klXJB,default=klXJR)
def cmd_cpvcs_list_pods(remote:klXJU):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=klXJB)
def cmd_cpvcs_versions(name:klXJg):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=klXJB,type=klXJA)
@click.option("-n","--name",help="Name of the cloud pod",required=klXJB)
def cmd_cpvcs_version_info(version:klXJA,name:klXJg):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=klXJA,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=klXJB)
def cmd_cpvcs_version_metamodel(version:klXJA,name:klXJg):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=klXJB,type=klXJA,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=klXJB,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=klXJB,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=klXJR,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=klXJB)
def cmd_cpvcs_set_version(version:klXJA,inject:klXJU,reset:klXJU,commit_before:klXJU,name:klXJg):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=klXJB)
def cmd_cpvcs_commits(version:klXJA,name:klXJg):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=klXJB)
@click.option("--commit","-c",required=klXJB)
@click.option("-n","--name",help="Name of the cloud pod",required=klXJB)
def cmd_cpvcs_commit_diff(version:klXJA,commit:klXJA,name:klXJg):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(klXJa)
  status.stop()
...
def print_table(columns:Dict[klXJg,klXJg],rows:List[Dict[klXJg,Any]],formatters:Dict[klXJg,Callable[[Any],klXJg]]=klXJa):
 from rich.table import Table
 if formatters is klXJa:
  formatters=klXJv()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  klXJM(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->klXJa:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->klXJa:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->klXJa:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->klXJa:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
