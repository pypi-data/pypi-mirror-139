...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return WpqJD if e else WpqJt
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->WpqJa:
...
@click.option("--revert",is_flag=WpqJD,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:WpqJN):
...
def _cpvcs_initialized(pod_name:WpqJI)->WpqJN:
...
@click.option("-n","--name",help="Name of the cloud pod",required=WpqJD)
def cmd_cpvcs_init(name:WpqJI):
...
@click.option("-n","--name",help="Name of the cloud pod",required=WpqJD)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=WpqJD,default=WpqJt)
def cmd_cpvcs_delete(name:WpqJI,remote:WpqJN):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=WpqJD)
def cmd_cpvcs_register_remote(name:WpqJI):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=WpqJD)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=WpqJD)
def cmd_cpvcs_rename(name:WpqJI,new_name:WpqJI):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=WpqJD)
def cmd_cpvcs_commit(message:WpqJI,name:WpqJI):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=WpqJD,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=WpqJD,default=WpqJt,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=WpqJD)
def cmd_cpvcs_push(squash:WpqJN,message:WpqJI,name:WpqJI,three_way:WpqJN):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=WpqJD)
@click.option("-v","--version",type=WpqJx)
@click.option("-m","--message",required=WpqJt)
def cmd_cpvcs_push_overwrite(version:WpqJx,message:WpqJI,name:WpqJI):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=WpqJx,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=WpqJD,default=WpqJt,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=WpqJD)
def cmd_cpvcs_inject(version:WpqJx,reset:WpqJN,name:WpqJI):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--inject/--no-inject",default=WpqJD,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=WpqJD,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=WpqJD)
@click.option("--lazy/--eager",default=WpqJD,help="Will only fetch references to existing versions, i.e. version state is only downloaded when required")
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:WpqJI,inject:WpqJN,reset:WpqJN,lazy:WpqJN):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=WpqJD,default=WpqJt)
def cmd_cpvcs_list_pods(remote:WpqJN):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=WpqJD)
def cmd_cpvcs_versions(name:WpqJI):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=WpqJD,type=WpqJx)
@click.option("-n","--name",help="Name of the cloud pod",required=WpqJD)
def cmd_cpvcs_version_info(version:WpqJx,name:WpqJI):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=WpqJx,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=WpqJD)
def cmd_cpvcs_version_metamodel(version:WpqJx,name:WpqJI):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=WpqJD,type=WpqJx,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=WpqJD,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=WpqJD,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=WpqJt,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=WpqJD)
def cmd_cpvcs_set_version(version:WpqJx,inject:WpqJN,reset:WpqJN,commit_before:WpqJN,name:WpqJI):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=WpqJD)
def cmd_cpvcs_commits(version:WpqJx,name:WpqJI):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=WpqJD)
@click.option("--commit","-c",required=WpqJD)
@click.option("-n","--name",help="Name of the cloud pod",required=WpqJD)
def cmd_cpvcs_commit_diff(version:WpqJx,commit:WpqJx,name:WpqJI):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(WpqJa)
  status.stop()
...
def print_table(columns:Dict[WpqJI,WpqJI],rows:List[Dict[WpqJI,Any]],formatters:Dict[WpqJI,Callable[[Any],WpqJI]]=WpqJa):
 from rich.table import Table
 if formatters is WpqJa:
  formatters=WpqJd()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  WpqJi(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->WpqJa:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->WpqJa:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->WpqJa:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->WpqJa:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
