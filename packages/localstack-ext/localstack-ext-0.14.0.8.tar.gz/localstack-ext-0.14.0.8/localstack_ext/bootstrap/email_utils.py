...
SENT_EMAILS=[]
EMAIL_BLACKLIST=Eikle()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",Eiklu(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=EiklS,from_name=EiklS,smtp_host=EiklS,smtp_user=EiklS,smtp_pass=EiklS,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if EiklA(recipients,Eiklt)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=EiklS):
 ...
 result["From"]=formataddr((Eiklu(Header(from_name,"utf-8")),from_email))
 return result
