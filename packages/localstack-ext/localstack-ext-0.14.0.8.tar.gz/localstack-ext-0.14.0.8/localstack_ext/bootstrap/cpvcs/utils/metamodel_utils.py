...
CAPTURE_METAMODEL_SEPARATELY=dFWCn
...
class MetamodelDelta(SubtypesInstanceManager):
 def create_delta_for_state_files(self,sf1:StateFileRef,sf2:StateFileRef):
  ...
  if not sf1:
   backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
   clazz=dFWCI(backend2)
   backend1=_infer_backend_init(clazz,sf2)
  else:
   backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
   clazz=dFWCI(backend1)
   backend2=_infer_backend_init(clazz,sf1)
  diff=self.create_delta_json(backend1,backend2)
  return diff
 def create_delta_log(self,state_from:Set[StateFileRef],state_to:Set[StateFileRef])->dFWCX:
  ...
  def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[dFWCX,StateFileRef]:
  ...
  for state_file_from in state_files_from_regular:
   ...
   if state_file_from.any_congruence(state_files_to_regular):
    ...
    diff_json=self.create_delta_for_state_files(state_file_from,dFWCN)
  ...
  for state_files_to in state_files_to_lookup.values():
   ...
   diff_json=self.create_delta_for_state_files(dFWCN,state_files_to)
...
class CommitMetamodelUtils:
 @dFWCl
 def create_metadata_archive(cls,version:Version,delete_reference:bool=dFWCn,overwrite:bool=dFWCn,metamodels_file:dFWCX=dFWCN):
 ...
 def get_metamodel_delta(cls,prev_metamodel:Dict,this_metamodel:Dict)->Dict:
  ...
  return result
 @dFWCl
 def create_delta_metamodel_file(cls,version:dFWCq,revision:dFWCq,store_to_zip=dFWCn)->Tuple[Dict,Dict]:
 ...
 @dFWCl
 def reconstruct_metamodel(cls,version:dFWCq,revision:dFWCq=dFWCN)->Dict:
  if not revision:
   revision=infer_max_commit_diff(archive_path=PodFilePaths.get_version_meta_archive(version))
  result={}
  for rev in dFWCw(1,revision+1):
  ...
  return result
 @dFWCl
 def get_version_metamodel(cls,version:dFWCq,revision:dFWCq=dFWCN)->Dict:
 ...
 @dFWCl
 def get_commit_diff(cls,version_no:dFWCq,commit_no:dFWCq)->Optional[Dict]:
  ...
  return result
def _infer_backend_init(clazz:Type,sf:StateFileRef)->dFWCi:
 if dFWCD(clazz,dFWCY):
  return{}
 constructor=dFWCr(clazz,"__init__",dFWCN)
...
def _create_metamodel_helper(obj,width=25,visited:set=dFWCN):
 if obj is dFWCN:
  return obj
 ...
 result=obj=obj_dict if obj_dict is not dFWCN else obj
 if dFWCD(obj,dFWCY):
  result=dFWCY(result)
  for field_name,field_value in result.items():
   result[field_name]=_create_metamodel_helper(field_value,width=width,visited=visited)
 elif dFWCD(obj,dFWCm):
  result=[_create_metamodel_helper(o,width=width,visited=visited)for o in obj]
  if dFWCH(result)>width:
   result={"size":dFWCH(result),"items":result[:width]}
 return result
def infer_max_commit_diff(archive_path:dFWCX)->dFWCq:
 max_commit=1
 with zipfile.ZipFile(archive_path)as archive:
