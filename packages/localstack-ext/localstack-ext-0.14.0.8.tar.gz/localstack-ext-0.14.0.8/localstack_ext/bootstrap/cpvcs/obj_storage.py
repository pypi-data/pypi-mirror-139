...
class StateFileLocator(abc.ABC):
 location="_none_"
 active_instance=LhzRN
 @abc.abstractmethod
 def get_state_file_location_by_key(self,key:LhzRB)->LhzRB:
  pass
 @LhzRT
 def get(cls,requested_file_locator:LhzRB):
...
class ObjectStorageProvider(abc.ABC):
 ...
 active_instance=LhzRN
 @LhzRT
 def get(cls,state_file_locator:LhzRB,requested_storage:LhzRB,serializers:Dict[LhzRB,CPVCSSerializer]):
  ...
  return cls.active_instance
 def __init__(self,state_file_locator:StateFileLocator,serializers:Dict[LhzRB,CPVCSSerializer]):
 ...
 def get_state_file_location_by_key(self,key:LhzRB)->LhzRB:
  pass
 @abc.abstractmethod
 def get_version_by_key(self,key:LhzRB)->Version:
  pass
 @abc.abstractmethod
 def get_revision_by_key(self,key:LhzRB)->Revision:
  pass
 @abc.abstractmethod
 def get_revision_or_version_by_key(self,key:LhzRB)->Union[Revision,Version]:
  pass
 @abc.abstractmethod
 def get_delta_file_by_key(self,key:LhzRB)->LhzRB:
  pass
 @abc.abstractmethod
 def upsert_objects(self,*args:CPVCSObj)->List[LhzRB]:
  pass
 @abc.abstractmethod
 def update_revision_key(self,old_key:LhzRB,new_key:LhzRB,referenced_by_version:LhzRB=LhzRN):
  pass
 @abc.abstractmethod
 def version_exists(self,key:LhzRB)->LhzRI:
  pass
 @abc.abstractmethod
 def merge_remote_into_local_version(self,remote_location:LhzRB,key:LhzRB):
  pass
 @abc.abstractmethod
 def _update_key(self,old_key:LhzRB,new_key:LhzRB,base_path:LhzRB)->LhzRI:
  pass
 @abc.abstractmethod
 def _serialize(self,*args:CPVCSObj)->List[LhzRB]:
  pass
 @abc.abstractmethod
 def _deserialize(self,key_serializer:Dict[LhzRB,LhzRB],remote_location:LhzRB=LhzRN)->List[CPVCSObj]:
  pass
class ObjectStorageLocal(ObjectStorageProvider):
 ...
 def _serialize(self,*args:CPVCSObj)->List[LhzRB]:
  ...
  for cpvcs_obj in args:
   if LhzRl(cpvcs_obj,Version):
    hash_refs.append(self.serializers["version"].store_obj(cpvcs_obj))
   elif LhzRl(cpvcs_obj,Revision):
    hash_refs.append(self.serializers["revision"].store_obj(cpvcs_obj))
  return hash_refs
 def _deserialize(self,key_serializer:Dict[LhzRB,LhzRB],remote_location:LhzRB=LhzRN)->List[Union[Revision,Version]]:
