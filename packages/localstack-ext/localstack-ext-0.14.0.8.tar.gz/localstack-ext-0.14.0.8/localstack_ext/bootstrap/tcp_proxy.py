...
class AttrDict(rWapQ):
 def __init__(self,*args,**kwargs):
  rWapd(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and rWapH(socket,"IPV6_V6ONLY"):
...
class Half(rWapy):
 def __init__(self,opt,sock,addr,rWapN):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.rWapN=rWapN
  self.name="peer" if self.rWapN else "client"
  self.queue=[]
  self.dest=rWapq
  self.err=rWapq
  self.ready=rWapk
...
def server_loop(opt):
 ...
 proxy=start_thread(lambda*args,**kwargs:start_tcp_proxy(listen,target,handler=rWapq,**kwargs))
 return proxy.join()
def main():
 opt={"ip6":rWapk,"bind_port":2223,"addr":"192.168.43.191","port":22}
 opt={"ip6":rWapk,"bind_port":48429,"addr":"128.0.100.3","port":22}
 opt={"ip6":rWapk,"port":54237,"bind_addr":"128.0.100.2","bind_port":22}
 opt={"ip6":rWapk,"port":54237,"bind_addr":"192.168.100.3","bind_port":22}
 server_loop(opt)
