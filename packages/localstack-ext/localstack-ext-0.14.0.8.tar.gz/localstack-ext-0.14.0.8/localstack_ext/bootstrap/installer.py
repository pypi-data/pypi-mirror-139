  ...
  try:
   if not self._check_if_available():
    try:
     ...
     if QztfY(e,PackageInstallationException):
      raise
     else:
      suffix=(f" ({self.log_package_name})." if self.log_package_name is not Qztfh else ".")
   ...
   if raise_on_error:
    ...
    if self.logger.isEnabledFor(logging.DEBUG):
     ...
     self.logger.error(e)
 def _check_if_available(self)->QztfG:
  raise QztfS
 def _prepare_installation(self)->Qztfh:
  pass
 def _install_package(self)->Qztfh:
  raise QztfS
 def _post_process(self)->Qztfh:
  pass
 def _verify_installation(self)->Qztfh:
  if not self._check_if_available():
   package_info=(f" of {self.log_package_name}" if self.log_package_name is not Qztfh else "")
...
class MultiPackageInstaller(PackageInstaller):
 def __init__(self,log_package_name:Optional[QztfT],package_installers:Union[PackageInstaller,List[PackageInstaller]]):
  QztfE(MultiPackageInstaller,self).__init__(log_package_name=log_package_name)
  self.package_installers=(package_installers if QztfY(package_installers,List)else[package_installers])
 def install(self,raise_on_error:QztfG=Qztfl):
...
class OSSpecificPackageInstaller(MultiPackageInstaller):
 def __init__(self,debian_installers:PackageInstaller,redhat_installers:PackageInstaller):
  ...
  QztfE(OSSpecificPackageInstaller,self).__init__(log_package_name=self.__class__.__name__,package_installers=package_installers)
 def install(self,raise_on_error:QztfG=Qztfl):
  try:
   if not in_docker():
    ...
    QztfE(OSSpecificPackageInstaller,self).install(raise_on_error=raise_on_error)
  except PackageInstallationException as e:
   if raise_on_error:
    raise
   else:
...
class DebianPackageInstaller(PackageInstaller,ABC):
 def __init__(self,package_name:QztfT):
  QztfE(DebianPackageInstaller,self).__init__(log_package_name=package_name)
 def _install_os_packages(self,packages:Union[QztfT,List[QztfT]]):
  packages=packages if QztfY(packages,List)else[packages]
  with INSTALL_LOCK:
   run(["apt-get","update"])
   run(["apt-get","install","-y","--no-install-recommends"]+packages)
class RedHatPackageInstaller(PackageInstaller,ABC):
 def __init__(self,package_name:QztfT):
  QztfE(RedHatPackageInstaller,self).__init__(log_package_name=package_name)
 def _install_os_packages(self,packages:Union[QztfT,List[QztfT]]):
  packages=packages if QztfY(packages,List)else[packages]
  with INSTALL_LOCK:
...
class DebianPostgres11Installer(DebianPackageInstaller):
 def __init__(self):
  QztfE(DebianPostgres11Installer,self).__init__("postgres11")
 def _check_if_available(self)->QztfG:
...
class RedHatPostgres11Installer(RedHatPackageInstaller):
 def __init__(self):
  QztfE(RedHatPostgres11Installer,self).__init__("postgres11")
 def _check_if_available(self)->QztfG:
  return is_command_available("psql")
 def _prepare_installation(self)->Qztfh:
...
class DebianPlPythonInstaller(DebianPackageInstaller):
 def __init__(self):
  QztfE(DebianPlPythonInstaller,self).__init__("plpython3")
 def _check_if_available(self)->QztfG:
...
class RedHatPlPythonInstaller(RedHatPackageInstaller):
 def __init__(self):
  QztfE(RedHatPlPythonInstaller,self).__init__("plpython3")
 def _check_if_available(self)->QztfG:
...
class DebianMariaDBInstaller(DebianPackageInstaller):
 def __init__(self):
  QztfE(DebianMariaDBInstaller,self).__init__("MariaDB")
 def _check_if_available(self)->QztfG:
...
class RedHatMariaDBInstaller(RedHatPackageInstaller):
 def __init__(self):
  QztfE(RedHatMariaDBInstaller,self).__init__("MariaDB")
 def _check_if_available(self)->QztfG:
  return is_command_available("mysqld")
 def _install_package(self)->Qztfh:
...
class DebianTimescaleDBInstaller(DebianPackageInstaller):
 def __init__(self):
  QztfE(DebianTimescaleDBInstaller,self).__init__("timescaledb")
 def _check_if_available(self)->QztfG:
...
class RedHatTimescaleDBInstaller(RedHatPackageInstaller):
 def __init__(self):
  QztfE(RedHatTimescaleDBInstaller,self).__init__("timescaledb")
 def _check_if_available(self)->QztfG:
...
class DebianRedisInstaller(DebianPackageInstaller):
 def __init__(self):
  QztfE(DebianRedisInstaller,self).__init__("Redis")
 def _check_if_available(self)->QztfG:
...
class RedHatRedisInstaller(RedHatPackageInstaller):
 def __init__(self):
  QztfE(RedHatRedisInstaller,self).__init__("Redis")
 def _check_if_available(self)->QztfG:
  return is_command_available("redis-server")
 def _install_package(self)->Qztfh:
...
class DebianMosquittoInstaller(DebianPackageInstaller):
 def __init__(self):
  QztfE(DebianMosquittoInstaller,self).__init__("Mosquitto")
 def _check_if_available(self)->QztfG:
...
class RedHatMosquittoInstaller(RedHatPackageInstaller):
 def __init__(self):
  QztfE(RedHatMosquittoInstaller,self).__init__("Mosquitto")
 def _check_if_available(self)->QztfG:
  return is_command_available("mosquitto")
 def _install_package(self)->Qztfh:
