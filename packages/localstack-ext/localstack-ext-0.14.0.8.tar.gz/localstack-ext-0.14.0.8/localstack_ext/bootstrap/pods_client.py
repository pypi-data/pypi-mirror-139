...
class PodInfo:
 def __init__(self,name=brBHF,pod_size=0):
  self.name=name
  self.pod_size=pod_size
  self.pod_size_compressed=0
  self.persisted_resource_names=[]
class CloudPodManager(brBHR):
 BACKEND="_none_"
 def __init__(self,pod_name=brBHF,config=brBHF):
  self.pod_name=pod_name
  self._pod_config=config
 def init(self):
  raise brBHG("Not implemented")
 def delete(self,remote:brBHa)->brBHa:
  raise brBHG("Not implemented")
 def push(self,comment:brBHT=brBHF,three_way:brBHa=brBHx)->PodInfo:
  raise brBHG("Not implemented")
 def push_overwrite(self,version:brBHu,comment:brBHT=brBHF):
  pass
 def pull(self,inject_version_state:brBHa=brBHx,reset_state_before:brBHa=brBHx,lazy:brBHa=brBHx):
  raise brBHG("Not implemented")
 def commit(self,message:brBHT=brBHF):
  raise brBHG("Not implemented")
 def inject(self,version:brBHu,reset_state:brBHa)->brBHa:
  raise brBHG("Not implemented")
 def list_versions(self)->List[brBHT]:
  raise brBHG("Not implemented")
 def version_info(self,version:brBHu):
  raise brBHG("Not implemented")
 def version_metamodel(self,version:brBHu)->Dict:
  raise brBHG("Not implemented")
 def set_version(self,version:brBHu,inject_version_state:brBHa,reset_state:brBHa,commit_before:brBHa):
  raise brBHG("Not implemented")
 def list_version_commits(self,version:brBHu)->List[brBHT]:
  raise brBHG("Not implemented")
 def get_commit_diff(self,version:brBHu,commit:brBHu)->Dict:
  raise brBHG("Not implemented")
 def register_remote(self,pod_name:brBHT,ci_pod:brBHa)->brBHa:
  raise brBHG("Not implemented")
 def rename_pod(self,current_pod_name,new_pod_name)->brBHa:
  raise brBHG("Not implemented")
 def list_pods(self,fetch_remote:brBHa)->Set[brBHT]:
  raise brBHG("Not implemented")
 ...
 def get(cls,pod_name,pre_config=brBHF):
  ...
  raise brBHG('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 def deploy_pod_into_instance(self,pod_path):
  delete_pod_zip=brBHx
  if os.path.isdir(pod_path):
   ...
   for folder in PERSISTED_FOLDERS:
    ...
    cp_r(src_folder,tgt_folder,rm_dest_on_conflict=brBHp)
   pod_path=create_zip_file(tmpdir)
   rm_rf(tmpdir)
   delete_pod_zip=brBHp
  zip_content=load_file(pod_path,mode="rb")
  url=get_pods_endpoint()
  result=requests.post(url,data=zip_content)
  if result.status_code>=400:
   raise brBHG("Unable to restore pod state via local pods management API %s (code %s): %s"%(url,result.status_code,result.content))
  if delete_pod_zip:
   rm_rf(pod_path)
  else:
   return pod_path
 @brBHs
 def get_state_zip_from_instance(get_content=brBHx):
  url=f"{get_pods_endpoint()}/state"
  result=requests.get(url)
  if result.status_code>=400:
   raise brBHG("Unable to get local pod state via management API %s (code %s): %s"%(url,result.status_code,result.content))
  if get_content:
   return result.content
  zip_file=f"{new_tmp_file()}.zip"
  save_file(zip_file,result.content)
  return zip_file
 def get_pod_info(self,pod_data_dir:brBHT=brBHF):
...
class CloudPodManagerCPVCS(CloudPodManager):
 BACKEND="cpvcs"
 @brBHs
 def parse_pod_name_from_qualifying_name(qualifying_name:brBHT)->brBHT:
 ...
 @brBHs
 def _get_max_version_for_pod_from_platform(pod_name:brBHT,auth_headers):
  ...
  remote_max_ver=brBHu(content["max_ver"])
 ...
 def _upload_version_and_product_space(self,presigned_urls):
  ...
  with brBHY(version_space_archive,"rb")as version_space_content:
  ...
  for version_no,urls in presigned_meta_state_urls.items():
   ...
   with brBHY(meta_archive,"rb")as meta_archive_content:
   ...
   with brBHY(state_archive,"rb")as state_archive_content:
    self.upload_content(state_presigned_url,state_archive_content)
 @brBHs
 def _add_state_files_from_directory(service:brBHT,path:brBHT,root_dir:brBHT,serialization:Serialization):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def _add_state_to_cpvcs_store(self):
  ...
  with ZipFile(zip_file,"r")as state_zip:
   ...
   api_states_traverse(api_states_path=api_states_path,side_effect=CloudPodManagerCPVCS._add_state_files_func,mutables=brBHF)
  ...
  rm_rf(zip_file)
  rm_rf(tmp_dir)
 def _pull_versions(self,auth_headers,required_versions:brBHT):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def _clone_pod(self,auth_headers,lazy:brBHa=brBHx):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def delete(self,remote:brBHa)->brBHa:
  ...
  if remote:
   pass
  return brBHx
 def _push_to_remote(self,url:brBHT):
 ...
 def pull(self,inject_version_state:brBHa=brBHx,reset_state_before:brBHa=brBHx,lazy:brBHa=brBHx):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
  auth_headers=get_auth_headers()
  if self.pod_name in cpvcs_api.list_locally_available_pods(show_remote_or_local=brBHx):
   cpvcs_api.set_pod_context(self.pod_name)
   ...
   if remote_max_ver==current_max_ver:
    ...
    return
   if not lazy:
    required_versions=",".join(brBHc(lambda ver:brBHT(ver),brBHw(current_max_ver+1,remote_max_ver+1)))
   else:
 ...
 def _download_version_product(self,version:brBHu,retain:brBHa=brBHx)->Dict[brBHT,brBHT]:
 ...
 def _get_presigned_url_for_version_product(self,version:brBHu)->Dict:
 ...
 def _inject_from_remote(self,version:brBHu,retain:brBHa=brBHx)->brBHa:
  from localstack_ext.bootstrap.cpvcs.utils.remote_utils import(extract_meta_and_state_archives)
  content=self._get_presigned_url_for_version_product(version=version)
  state_url=content.get("presigned_url_state")
  tmp_state_archive=new_tmp_file()
 ...
 def inject(self,version:brBHu,reset_state:brBHa)->brBHa:
  ...
  if not cpvcs_api.config_context.pod_exists_locally(self.pod_name):
   ...
   if not state_archive_path and cpvcs_api.is_remotely_managed():
    ...
    product_archive_path=self._download_version_product(version=version,retain=brBHp)
    if not product_archive_path:
     return brBHx
    state_archive_path=cpvcs_api.PodFilePaths.get_version_state_archive(version)
  if reset_state:
   reset_local_state(reset_data_dir=brBHp)
 ...
 def version_metamodel(self,version:brBHu)->Dict:
  ...
  if not result and cpvcs_api.is_remotely_managed():
   self._download_version_product(version=version,retain=brBHp)
 ...
 def get_commit_diff(self,version:brBHu,commit:brBHu)->Dict:
  ...
  return commit_diff
 def register_remote(self,pod_name:brBHT,ci_pod:brBHa=brBHx)->brBHa:
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
  cpvcs_api.set_pod_context(self.pod_name)
 ...
 def list_pods(self,fetch_remote:brBHa)->Set[brBHT]:
  ...
  return result
 @brBHX
 def upload_content(cls,presigned_url:brBHT,zip_data_content):
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise brBHG("Unable to upload pod state to S3 (code %s): %s"%(res.status_code,res.content))
  return res
 @brBHs
 def create_platform_url(request:brBHT)->brBHT:
...
class CloudPodManagerManaged(CloudPodManager):
 BACKEND="managed"
 def push(self,comment:brBHT=brBHF,three_way:brBHa=brBHx)->PodInfo:
  zip_data_content=self.get_state_zip_from_instance(get_content=brBHp)
  print('Pushing state of cloud pod "%s" to backend server (%s KB)'%(self.pod_name,format_number(brBHW(zip_data_content)/1000.0)))
  self.push_content(self.pod_name,zip_data_content)
  print("Done.")
  result=self.get_pod_info()
  result.pod_size_compressed=brBHW(zip_data_content)
  return result
 def pull(self,inject_version_state:brBHa=brBHx,reset_state_before:brBHa=brBHx,lazy:brBHa=brBHx):
  ...
  rm_rf(zip_path)
 @brBHs
 def presigned_url(pod_name:brBHT,mode:brBHT)->brBHT:
  ...
  if response.status_code>=400:
   raise brBHG("Unable to get cloud pod presigned URL (code %s): %s"%(response.status_code,content))
 ...
 def push_content(cls,pod_name,zip_data_content):
  ...
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise brBHG("Unable to push pod state to API (code %s): %s"%(res.status_code,res.content))
  return res
class CloudPodManagerGit(CloudPodManager):
 BACKEND="git"
 def push(self,comment:brBHT=brBHF,three_way:brBHa=brBHx):
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=brBHp)
 ...
 def pull(self,inject_version_state:brBHa=brBHx,reset_state_before:brBHa=brBHx,lazy:brBHa=brBHx):
  ...
  if remote_ref not in remote_refs:
   raise brBHG('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get("url")
  self.switch_branch(branch)
 ...
 def client(self):
  ...
  return client,path
 def local_repo(self):
  self.clone_dir=brBHn(self,"clone_dir",brBHF)
...
class PodConfigManagerMeta(brBHD):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=brBHF
   for manager in cls.CHAIN:
    try:
     tmp=brBHn(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif brBHU(tmp,brBHI)and brBHU(result,brBHI):
       result.extend(tmp)
    except brBHG:
     if LOG.isEnabledFor(logging.DEBUG):
      LOG.exception("error during PodConfigManager call chain")
   if result is not brBHF:
    return result
   raise brBHG('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(brBHR,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @brBHX
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise brBHG('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(brBHR):
 CONFIG_FILE=".localstack.yml"
 def list_pods(self):
  local_pods=self._load_config(safe=brBHp).get("pods",{})
  local_pods=[{"pod_name":k,"state":"Local Only",**v}for k,v in local_pods.items()]
  existing_names=brBHd([pod["pod_name"]for pod in local_pods])
  result=[pod for pod in local_pods if pod["pod_name"]not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=brBHx):
  try:
   return yaml.safe_load(to_str(load_file(self.CONFIG_FILE)))
  except brBHG:
   if safe:
    return{}
   raise brBHG('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(brBHR):
 def list_pods(self):
  result=[]
  auth_headers=get_auth_headers()
  ...
  content=response.content
  if response.status_code>=400:
   raise brBHG("Unable to fetch list of pods from API (code %s): %s"%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise brBHG("Unable to store pod metadata in API (code %s): %s"%(response.status_code,content))
...
def init_cpvcs(pod_name:brBHT,pre_config:Dict[brBHT,brBHT],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.init()
def delete_pod(pod_name:brBHT,remote:brBHa,pre_config:Dict[brBHT,brBHT])->brBHa:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.delete(remote=remote)
 return result
def register_remote(pod_name:brBHT,pre_config:Dict[brBHT,brBHT],**kwargs)->brBHa:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.register_remote(pod_name=pod_name,ci_pod=pre_config.get("ci_pod",brBHx))
 return result
def rename_pod(current_pod_name:brBHT,new_pod_name:brBHT,pre_config:Dict[brBHT,brBHT],**kwargs):
 ...
 return result
def list_pods_cpvcs(remote:brBHa,pre_config:Dict[brBHT,brBHT],**kwargs)->List[brBHT]:
 backend=CloudPodManager.get(pod_name="",pre_config=pre_config)
 result=backend.list_pods(fetch_remote=remote)
 return result
def commit_state(pod_name:brBHT,pre_config:Dict[brBHT,brBHT],message:brBHT=brBHF,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.commit(message=message)
def inject_state(pod_name:brBHT,version:brBHu,reset_state:brBHa,pre_config:Dict[brBHT,brBHT],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.inject(version=version,reset_state=reset_state)
 return result
def list_versions(pod_name:brBHT,pre_config:Dict[brBHT,brBHT],**kwargs)->List[brBHT]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 versions=backend.list_versions()
 return versions
def get_version_info(version:brBHu,pod_name:brBHT,pre_config:Dict[brBHT,brBHT],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 info=backend.version_info(version=version)
 return info
def get_version_metamodel(version:brBHu,pod_name:brBHT,pre_config:Dict[brBHT,brBHT],**kwargs)->Dict:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 metamodel=backend.version_metamodel(version=version)
 return metamodel
def set_version(version:brBHu,inject_version_state:brBHa,reset_state:brBHa,commit_before:brBHa,pod_name:brBHT,pre_config:Dict[brBHT,brBHT],**kwargs)->brBHa:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 ...
 return success
def list_version_commits(version:brBHu,pod_name:brBHT,pre_config:Dict[brBHT,brBHT])->List[brBHT]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commits=backend.list_version_commits(version=version)
 return commits
def get_commit_diff(version:brBHu,commit:brBHu,pod_name:brBHT,pre_config:Dict[brBHT,brBHT])->Dict:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commit_diff=backend.get_commit_diff(version=version,commit=commit)
 return commit_diff
def push_overwrite(version:brBHu,pod_name:brBHT,comment:brBHT,pre_config:Dict[brBHT,brBHT]):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.push_overwrite(version=version,comment=comment)
def push_state(pod_name,pre_config=brBHF,squash_commits=brBHx,comment=brBHF,three_way=brBHx,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
...
def pull_state(pod_name,inject_version_state=brBHx,reset_state_before=brBHx,lazy=brBHx,**kwargs):
 pre_config=kwargs.get("pre_config",brBHF)
 if not pod_name:
  raise brBHG("Need to specify a pod name")
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.pull(inject_version_state=inject_version_state,reset_state_before=reset_state_before,lazy=lazy)
 print("Done.")
def reset_local_state(reset_data_dir=brBHx,exclude_from_reset:List[brBHT]=brBHF):
 url=f"{get_pods_endpoint()}/state"
 if reset_data_dir:
  url+="/datadir"
 ...
 if result.status_code>=400:
  raise brBHG("Unable to reset service state via local management API %s (code %s): %s"%(url,result.status_code,result.content))
 print("Done.")
def list_pods(args):
 return PodConfigManager.list_pods()
def get_data_dir_from_container()->brBHT:
 try:
...
def get_persisted_resource_names(data_dir)->List[brBHT]:
 names=[]
 ...
 with os.scandir(os.path.join(data_dir,"api_states"))as entries:
  for entry in entries:
   if entry.is_dir()and brBHW(os.listdir(entry.path))>0:
    names.append(entry.name)
