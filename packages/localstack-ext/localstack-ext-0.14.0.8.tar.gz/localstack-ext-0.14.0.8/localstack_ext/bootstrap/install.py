...
def install_libs():
 ...
 postgres_installer.install(raise_on_error=xIcCu)
 timescaledb_installer.install(raise_on_error=xIcCu)
 redis_installer.install(raise_on_error=xIcCu)
 mosquitto_installer.install(raise_on_error=xIcCu)
...
def setup_ssl_cert():
 ...
 try:
  ...
  try:
   ...
   raise
def download_github_artifact(url:xIcCK,target_file:xIcCK,timeout:int=xIcCt):
 def do_download(url,print_error=xIcCu):
  try:
