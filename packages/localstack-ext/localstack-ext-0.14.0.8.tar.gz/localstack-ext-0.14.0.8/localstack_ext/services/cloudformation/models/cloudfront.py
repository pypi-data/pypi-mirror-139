...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or YlsAQ
 @YlsAm
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return YlsAn(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @YlsAJ
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if YlsAW(origins,YlsAj)else origins.get("Items")
  origins_to_create=YlsAr([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if YlsAW(aliases,YlsAj)else aliases.get("Items")
  aliases_to_create=YlsAr(aliases)
  for distr in distrs:
   origins=YlsAr([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=YlsAr(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",YlsAi)
   config["Enabled"]=YlsAC(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if YlsAW(custom_errors,YlsAM)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=YlsAC(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":YlsAi}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=YlsAC(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=YlsAQ):
    parent=config if parent is YlsAQ else parent
    items=parent.get(key)or[]
    if YlsAW(items,YlsAM)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":YlsAS(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if YlsAW(cache_behavs,YlsAM)else cache_behavs)
