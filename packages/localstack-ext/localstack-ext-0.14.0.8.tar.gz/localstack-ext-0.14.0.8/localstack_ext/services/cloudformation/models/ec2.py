...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=uBDxy,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  for perm in perms:
   if uBDxb(perm["IpProtocol"])!=uBDxb(rp["IpProtocol"]):
    continue
 ...
 @uBDxg
 def get_vpc_and_name_for_security_group(cls,group_id:uBDxb)->Tuple[uBDxb,uBDxb]:
  client=aws_stack.connect_to_service("ec2")
  groups=client.describe_security_groups(GroupIds=[group_id])["SecurityGroups"]
  vpc_id=groups and groups[0].get("VpcId")or uBDxy
  group_name=groups and groups[0]["GroupName"]or uBDxy
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",uBDxy)
   vpc_id=uBDxy
   ...
   dst_group_id=result.pop("DestinationSecurityGroupId",uBDxy)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=uBDxb(result.get("IpProtocol"))
   description=result.pop("Description",uBDxy)
   cidr_ipv6=result.pop("CidrIpv6",uBDxy)
