...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[AXWmp])[0]
 def get_physical_resource_id(self,attribute=AXWmp,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if AXWmh(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return AXWmC(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @AXWmU
 def add_defaults(resource,stack_name:AXWmP):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return AXWmC(SESReceiptRule,self).get_cfn_attribute(attribute)
 @AXWmK
