...
def get_sensor_cols(sensors='12', N_SENSORS=None):
    if sensors=='24':
        return SENSORS_ALL
    elif sensors=='12':
...
def get_train_features(df, sensor='12', stage='pause', use_average = False, X_y=False):

    SENSORS_ALL = df.columns.tolist()[3:27]
    STAGES = df['measurement_stage'].unique().tolist()
    # ALL_MODELS = models().index.tolist()
    TARGET_COL = 'result'
    ...
    col_to_use = get_sensor_cols(sensor, N_SENSORS)


    if use_average:
        data = df.groupby(['measurement_stage', 'unique_id', 'result']).mean().reset_index()[col_to_use+[TARGET_COL]]
        if X_y:
          return get_X_y(data, TARGET_COL), col_to_use, stages_to_use
        else:
