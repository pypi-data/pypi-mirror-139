...
class BinaryField(fields.Field):
    def __init__(self, base64: bool = True, encoding: str = "utf-8", **kwargs):
        ...
        :param base64: True or False, setting this will
                        encode/decode the binary in base64. Default: True
...
class BinarySetField(fields.Field):
    def __init__(self, base64: bool = True, encoding: str = "utf-8", **kwargs):
        """Bytestring field.
    ...
    def _deserialize(self, value, attr, data, **kwargs):
        if not value:
            return None
        if self.base64:
            return {b64decode(v.encode(self.encoding)) for v in value}
        else:
            return {v.encode(self.encoding) for v in value}

    def _serialize(self, value, attr, data, **kwargs):
        if not value:
            return None
        if self.base64:
            return [b64encode(v).decode(self.encoding) for v in value]
        else:
