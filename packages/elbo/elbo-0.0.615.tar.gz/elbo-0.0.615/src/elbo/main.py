...
def prompt_user(compute_options):
    ...
    for k, v in compute_options.items():
        ...
        if v.get('spot') is True:
            ...
            suffix = ""
        option = f" ${v['cost']:7.4f}/hour {k:>36} {v['num_cpu']:>3} vcpus {v['mem']:>5}Gb mem " \
                 f"{v['gpu_mem']:>4}Gb gpu-mem {suffix}"
        options.append(option)
        mapping[option] = v

    if len(options) > 1:
        logger.info(f"number of compute choices - {len(options)}")
        logger.info(f"Please choose:")
...
def process_compute_provisioned_response(response_json, keep_alive=False, ssh_only=False):
    ...
    if not ssh_only:
        ...
        logger.info(f"elbo show {task_id}")
    ...
    if keep_alive:
        ...
        logger.info(f"elbo kill {task_id}")
...
@click.option('--open-port',
              ...
              type=int,
              multiple=True,
              help='The port that should be opened on the instance. da')
