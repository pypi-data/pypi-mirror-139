...
class DbMysqlHelper:
    def __init__(self,
        ...
        self._db = dbmysql(host, port, user, password, "", autocommit, loop)
        self._db.database = database
        self.database = database
        if not self.has_db(database):
            self._db.post(f"create database `{database}`")
            self._db.conn.select_db(database)
            self._db.database = database
            if not autocommit:
                self._db.commit()
        else:
    ...
    def has_db(self, db_name: str) -> bool:
        dbs = self._db.get("show databases")
        for db in dbs:
