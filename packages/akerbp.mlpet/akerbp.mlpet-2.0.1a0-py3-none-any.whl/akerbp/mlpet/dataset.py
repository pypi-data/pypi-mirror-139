...
class Dataset(DataLoader):
    ...
    Args:
        ...
        settings: dict or path to a yaml file. If a path is provided it must
            be provided as an absolute path. The possible keys for the settings:
                ...
                - depth_column (required): name of the measured depth column, e.g. "DEPTH"
    ...
    def __standardize_curves(self) -> None:
        ...
        curves = {"id_column": self.id_column, "depth_column": self.depth_column}
    ...
    def __init__(
        ...
        required = ["id_column", "depth_column"]
        for r in required:
        ...
        # Standardize curve names and create all_curves attribute
    ...
    def preprocess(self, df: DataFrame = None, verbose=True, **kwargs) -> DataFrame:
        ...
        if not kwargs:
            ...
            # instantiation via the settings file
            if self.preprocessing_pipeline is not None:
                kwargs = self.preprocessing_pipeline
            else:
                raise ValueError(
