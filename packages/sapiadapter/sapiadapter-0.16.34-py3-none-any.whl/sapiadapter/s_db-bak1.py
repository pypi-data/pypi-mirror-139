...
def db_con_setup( ) :
    ...
    thread_id = str( s_util.thread_get_id( ) )

    if thread_id in db_cons:
        if( db_cons[thread_id] != False ) : return

    db_filepath_setup( )

    dbfp = db_filepath_get( )
    db_cons[thread_id] = sqlite3.connect( dbfp ,  check_same_thread = False )
    #db_con.row_factory = sqlite3.Row
    db_cons[thread_id].row_factory = lambda C , R : { c[ 0 ] : R[ i ] for i , c in enumerate( C.description ) }

def db_con_cursor_get( ) :
    thread_id = str( s_util.thread_get_id( ) )
    # FIXME TODO do other blocks require setup logic like below?
    if not thread_id in db_cons :
        setup( )
    return( db_cons[thread_id].cursor( ) )

def db_con_commit( ) :
    thread_id = str( s_util.thread_get_id( ) )
    db_cons[thread_id].commit( )

def db_con_close( ) :
    thread_id = str( s_util.thread_get_id( ) )
    db_cons[thread_id].close( )
...
def db_commitclose( ) :
    ...
    thread_id = str( s_util.thread_get_id( ) )
    if thread_id in db_cons:
        if( db_cons[thread_id] != False ) :
            db_con_commit( )
            db_con_close( )
            db_cons[thread_id] = False 
...
def db_filepath_get( ) :
    global db_filepaths
    thread_id = str( s_util.thread_get_id( ) )
    #s_log.write(db_filepaths)
    if thread_id in db_filepaths:
        return( db_filepaths[thread_id] )
    return(False)

def db_filepath_set( dbfp ) :
    global db_filepaths

    thread_id = str( s_util.thread_get_id( ) )
    db_filepaths[thread_id] = dbfp
    #s_log.write(db_filepaths)
...
def db_filepath_setup( ) :
    ...
    db_fn = s_config.get_key( "/config/session" ) + "-" + thread_id + ".db"

    s_log.write( db_fn )

    db_filepath_set( dbworkspacedir + db_fn )
...
def db_filepath_delete_all( ) :

    global db_cons

    for tid in list(db_cons.keys()) :
        if db_cons[tid]!=False:
            db_cons[tid].commit()
            db_cons[tid].close()
            db_cons[tid]=False
    ...
    for tid in list(db_filepaths.keys()):
        if( os.path.isfile( db_filepaths[tid] ) ) :
            os.remove( db_filepaths[tid] )
...
def setup( ) :
    ...
    # FIXME TODO check if db conn is False...?
    if( dbfp != False ) :
        flush( )
        return( True )

    #s_log.write( "db setup" )
    ...
    # FIXME TODO check if db file was created? - but requires getting dbfp again
