...
class Auth:
    ...
    def authenticate(self) -> Optional[str]:
        ...
        if JWTToken.is_expired(self.jwt_token):
        ...
        async def save_token(request: Request,
            ...
            if token:
                auth.save(token,
                          ...
                          user_id=user_id,
                          api_key=key)
                from lightning_cloud.rest_client import GridRestClient

                user = GridRestClient().auth_service_get_user()
                logger.info("Authentication Successful")
                logger.info(
                    f"You are logged in as {user.first_name or user.username}")
