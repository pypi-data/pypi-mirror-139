...
from asyncio import get_event_loop, create_task, wait, wait_for, run_coroutine_threadsafe, run
...
class Client(object):
    ...
    def __init__(self, endpoint: str, host: str = 'localhost', port: str = 1234, key: str = None, debug = False):
        ...
        self._handler = create_task(self._handler())
        if self._debug:
    ...
    async def _handler(self):
        async for ws in connect(self.uri, open_timeout=1):
            ...
            self._conn = Connection(ws, self._debug)
            self.connected.set()
            async def rx():
                global rxed
                if self._debug:
                    print("* starting RX task...")
                while True:
                    try:
                        msg = await self._conn.recv()
                        if msg.reference and msg.reference in self._replies:
                            if self._debug:
                                print(f"Setting future {self._replies[msg.reference]} result {msg}")
                            ...
                            for filter, callback in self._callbacks[msg.channel].items():
                                if fnmatch(msg.subject, filter):
                                    if iscoroutinefunction(callback):
                                        if self._debug:
                                            print(f"Scheduling coroutine {callback} for {msg}")
                                        create_task(callback(self, msg))
                                    elif isfunction(callback):
                                        if self._debug:
                                            print(f"Scheduling executor {callback} for {msg}")
...
class ClientThread(threading.Thread):
    ...
    def __init__(self, endpoint: str, host: str = 'localhost', port: int = 1234, key: str = None, debug=False):
    ...
    async def main(self):
        self._stop = Event()
        self._client = Client(self.endpoint, self.host, self.port, self.key, self.debug)
