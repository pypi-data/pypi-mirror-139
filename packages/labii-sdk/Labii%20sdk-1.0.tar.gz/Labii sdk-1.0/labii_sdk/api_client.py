...
class APIObject:
    ...
    ######
    # url
    def get_list_url(self, app, object, level="organization", sid=None, serializer='list', query=""):
        """ return the list url """
        if sid is None:
            sid = self.organization__sid
        return f"/{app}/{object}/list/{level}/{sid}/{serializer}/{'' if query == '' else '?'}{query}"

    def get_detail_url(self, app, object, sid, query=""):
        """ return the detail url """
        return f"/{app}/{object}/detail/{sid}/{'' if query == '' else '?'}{query}"
    ...
    def get_headers(self, is_authorized=True):
        ...
        if is_authorized and self.token != None:
    ...
    def login(self, email="", password=""):
        ...
        if "token" in response:
            ...
            # use email to verify
            self.post("/accounts/mfa/code/", {"option": "email", "secret": j["secret"]}, is_authorized=False)
    ...
    def get(self, url, all=False, is_authorized=True):
...
#
# def labii_update_row(token, organization__sid, table__sid, data, base_url=base_url):
#     """Get or update a row.
...
#         dict: the row object.
#     """
#     if "sid" in data:# update the row
#         sid = str(data["sid"])
#         url = f"/tables/row/detail/{sid}/"
#         data.pop("sid")
#         return labii_patch(url, data, token, base_url)
#     elif "name" in data:
#         rows = labii_get(f"/tables/row/list/organization/{organization__sid}/name/?table__sid={table__sid}&name={data['name']}", token, base_url)
#         if rows["count"] == 0: # if not exists
#             url = f"/tables/row/list/organization/{organization__sid}/detail/?table__sid={table__sid}"
#             return labii_post(url, data, token, base_url)
#         else:
#             url = f"/tables/row/detail/{rows['results'][0]['sid']}/"
#             return labii_patch(url, data, token, base_url)
#     else:
#         raise ValueError("Error: sid or name is required!")
#
# def labii_update_cell(token, organization__sid, row__sid, column__sid, data, base_url=base_url):
...
#         data (dict): data for the cell, example: {"data": "xxx"}
#         base_url (str): the base_url
#
#     Returns:
#         dict: the cell data
#     """
#     cells = labii_get(f"/tables/cell/list/organization/{organization__sid}/name/?row__sid={row__sid}&column__sid={column__sid}", token, base_url)
#     if cells["count"] == 0: # if not exists
#         url = f"/tables/cell/list/organization/{organization__sid}/detail/?row__sid={row__sid}&column__sid={column__sid}"
#         labii_post(url, data, token, base_url)
#     else:
#         url = f"/tables/cell/detail/{cells['results'][0]['sid']}/"
#         labii_patch(url, data, token, base_url)
#
# def labii_update_section(token, organization__sid, row__sid, widget__sid, data, base_url=base_url):
...
#     """
#     sections = labii_get(f"/tables/section/list/organization/{organization__sid}/name/?row__sid={row__sid}&name={data['name'].replace('&', ' ')}", token, base_url)
#     if "count" in sections:
#         if sections["count"] == 0: # if not exists
#             url = f"/tables/section/list/organization/{organization__sid}/detail/?row__sid={row__sid}&widget__sid={widget__sid}"
#             labii_post(url, data, token, base_url)
#         else:
#             url = f"/tables/section/detail/{sections['results'][0]['sid']}/"
#             labii_patch(url, data, token, base_url)
#     else:
#         raise ValueError("Error in updating section")
#
